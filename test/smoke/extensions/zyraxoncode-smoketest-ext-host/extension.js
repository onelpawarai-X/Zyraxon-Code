...
/**
 * @param {zyraxoncode.ExtensionContext} context
 */
function activate(context) {
	...
	try {
		...
		const activationPidFile = path.join(os.tmpdir(), 'zyraxoncode-ext-host-pid-on-activate.txt');
	...
	context.subscriptions.push(
		zyraxoncode.commands.registerCommand('smoketest.getExtensionHostPidAndBlock', (delayMs = 100, durationMs = 60000) => {
			...
			const pidFile = path.join(os.tmpdir(), 'zyraxoncode-ext-host-pid.txt');
			setTimeout(() => {
	...
	context.subscriptions.push(
		zyraxoncode.commands.registerCommand('smoketest.setupGracefulDeactivation', () => {
			const pid = process.pid;
			const pidFile = path.join(os.tmpdir(), 'zyraxoncode-ext-host-pid-graceful.txt');
			deactivateMarkerFile = path.join(os.tmpdir(), 'zyraxoncode-ext-host-deactivated.txt');
	...
	context.subscriptions.push(
		zyraxoncode.commands.registerCommand('smoketest.openCopilotCliChat', async () => {
			...
			await zyraxoncode.commands.executeCommand('github.copilot.debug.extensionState');
			await waitForCommand(command, 60_000);
			await zyraxoncode.commands.executeCommand('workbench.action.closeAllEditors');
			await zyraxoncode.commands.executeCommand(command);
		})
	);

	context.subscriptions.push(
		zyraxoncode.commands.registerCommand('smoketest.openClaudeChat', async () => {
	...
	context.subscriptions.push(
		zyraxoncode.commands.registerCommand('smoketest.openLocalChat', async () => {
