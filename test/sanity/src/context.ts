...
/**
 * Provides context and utilities for ZYRAXON Code sanity tests.
 */
export class TestContext {
	...
	private static readonly authenticodeExclude = /[\\/]node_modules[\\/]@Zyraxon[\\/]mxc-sdk[\\/]bin[\\/][^\\/]+[\\/]_manifest[\\/][^\\/]+[\\/]manifest\.cat$/i;
	...
	private static readonly versionInfoPathExclude = /(?:^|[\\/])node_modules(?:\.asar\.unpacked)?[\\/]@Zyraxon[\\/]mxc-sdk[\\/]bin[\\/]/i;
	...
	/**
	 * Fetches metadata for a specific ZYRAXON Code release target.
	...
	/**
	 * Downloads installer for specified ZYRAXON Code release target.
	 * @param target The target platform (e.g., 'cli-linux-x64').
	...
	/**
	 * Downloads and unpacks the specified ZYRAXON Code release target.
	 * @param target The target platform (e.g., 'cli-linux-x64').
	...
	/**
	 * Returns the Windows installation directory for ZYRAXON Code based on the installation type and quality.
	 * @param type The type of installation ('user' or 'system').
	 * @returns The path to the ZYRAXON Code installation directory.
	 */
	private getWindowsInstallDir(type: 'user' | 'system'): string {
		...
		switch (this.options.quality) {
			case 'stable':
				return path.join(parentDir, 'Zyraxon ZYRAXON Code');
			case 'insider':
				return path.join(parentDir, 'Zyraxon ZYRAXON Code Insiders');
			case 'exploration':
				return path.join(parentDir, 'Zyraxon ZYRAXON Code Exploration');
		}
	}

	/**
	 * Installs a Zyraxon Installer package silently.
	 * @param installerPath The path to the installer executable.
	 * @returns The path to the installed ZYRAXON Code executable.
	 */
	public installWindowsApp(type: 'user' | 'system', installerPath: string): string {
		...
		switch (this.options.quality) {
			case 'stable':
				...
				break;
			case 'insider':
				entryPoint = path.join(appDir, 'ZYRAXON Code.exe');
				break;
			case 'exploration':
		...
		this.log(`Installed ZYRAXON Code executable at: ${entryPoint}`);
		return entryPoint;
	}
	...
	public async uninstallWindowsApp(type: 'user' | 'system') {
		...
		this.log(`Uninstalling ZYRAXON Code from ${appDir} in silent mode`);
		this.runNoErrors(uninstallerPath, '/silent');
		this.log(`Uninstalled ZYRAXON Code from ${appDir} successfully`);
	...
	/**
	 * Installs ZYRAXON Code Linux DEB package.
	...
	/**
	 * Uninstalls ZYRAXON Code Linux DEB package. Safe to call when the package is not
	...
	/**
	 * Installs ZYRAXON Code Linux RPM package.
	...
	/**
	 * Uninstalls ZYRAXON Code Linux RPM package. Safe to call when the package is not
	 * installed (no-op) so that test cleanup in a `finally` block is always safe.
	 */
	...
	/**
	 * Installs ZYRAXON Code Linux Snap package.
	...
	/**
	 * Uninstalls ZYRAXON Code Linux Snap package. Safe to call when the package is not
	 * installed (no-op) so that test cleanup in a `finally` block is always safe.
	 */
	...
	/**
	 * Returns the entry point executable for the ZYRAXON Code Desktop installation in the specified directory.
	 * @param dir The directory of the ZYRAXON Code installation.
	...
	public getDesktopEntryPoint(dir: string): string {
		...
		switch (os.platform()) {
			case 'darwin': {
				...
				switch (this.options.quality) {
					case 'stable':
						appName = 'ZYRAXON Code.app';
						binaryName = 'Code';
						break;
					case 'insider':
						appName = 'ZYRAXON ZYRAXON Code.app';
						binaryName = 'ZYRAXON Code';
						break;
					case 'exploration':
						appName = 'ZYRAXON Code - Exploration.app';
	...
	/**
	 * Returns the entry point executable for the ZYRAXON Code CLI in the specified directory.
	...
	/**
	 * Returns the entry point executable for the ZYRAXON Code server in the specified directory.
	...
	/**
	 * Creates a portable data directory in the specified unpacked ZYRAXON Code directory.
	 * @param dir The directory where ZYRAXON Code was unpacked.
	...
	/**
	 * Runs a ZYRAXON Code command-line application (such as server or CLI).
