# ZYRAXON Code Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides ZYRAXON Code automation capabilities for ZYRAXON Code development and testing. The MCP server exposes ZYRAXON Code's testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with ZYRAXON Code programmatically.
...
The MCP server exposes a comprehensive set of ZYRAXON Code automation tools through the MCP protocol:

### Application Management
- Start, stop, and restart ZYRAXON Code instances
...
│   ├── application.ts       # ZYRAXON Code application lifecycle management
...
- **application.ts** - Manages ZYRAXON Code application lifecycle (start, stop, restart)
- **automationTools/** - Modular tool implementations organized by ZYRAXON Code feature area

## Troubleshooting

### Server Won't Start
- Ensure ZYRAXON Code has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the ZYRAXON Code development infrastructure. When making changes:
