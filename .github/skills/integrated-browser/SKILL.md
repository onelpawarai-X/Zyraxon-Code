...
- **Focus & keyboard.** Focus is bridged explicitly. A **preload script** (injected into every page in an isolated world) decides which keystrokes the page keeps vs forwards to ZYRAXON Code keybindings. Treat it as a **trust boundary**: assume a hostile page, keep it minimal and side-effect-free.
...
- **Extension API tests** under `extensions/zyraxoncode-api-tests/src/singlefolder-tests/browser*.test.ts` are the preferred integration layer for browser APIs, CDP behavior, browser tools, extension-host wiring, and cross-process contracts exposed to extensions.
