...
description: "Run agent-driven ZYRAXON Code performance or memory investigations. Use when asked to launch ZYRAXON Code, automate a ZYRAXON Code scenario, run the Chat memory smoke runner, capture renderer heap snapshots, take workflow screenshots, compare run summaries, or drive a repeatable scenario before heap-snapshot analysis."
metadata:
  allowed-tools: Bash(npx @playwright/cli:*)
---

# ZYRAXON Code Performance Workflow

Drive a repeatable ZYRAXON Code scenario, collect memory/performance artifacts, verify that the scenario actually happened, then hand the resulting heap snapshots to the generic heap-snapshot-analysis skill when object-level investigation is needed.

## When to Use

- User describes a ZYRAXON Code workflow and asks whether it leaks or grows memory
- User asks the agent to launch ZYRAXON Code, drive a scenario, and capture heap snapshots
...
- User wants a new automation runner for a non-Chat ZYRAXON Code scenario
...
Use the bundled [Chat memory smoke runner](./scripts/chat-memory-smoke.mts) when the scenario is Chat-specific or can be expressed as repeated Chat prompts. It launches ZYRAXON Code, opens Chat, sends prompts, waits for responses, writes screenshots and `summary.json`, samples renderer heap, and can take selected heap snapshots.
...
## Non-Chat ZYRAXON Code Scenarios
