---
name: zyraxon-code-logs
description: 'Find and read timestamped process logs from ZYRAXON Code dev builds, including main.log, renderer.log, extension host logs, and agenthost.log. For bundles produced by Export Agent Host Debug Logs, use agent-host-logs.'
---

# ZYRAXON Code Logs

Find and display logs from the most recent ZYRAXON Code or Agents app dev run.

## Log Root Directories

| App | Default User Data Dir | Logs Path |
|-----|-----------------------|-----------|
| ZYRAXON Code | `$HOME/.vszyraxon-code-dev` | `$HOME/.vszyraxon-code-dev/logs/` |
| Agents app | `$HOME/.vszyraxon-code-dev` | `$HOME/.vszyraxon-code-dev/logs/` |

If ZYRAXON Code was launched with `--user-data-dir=<dir>`, use `<dir>/logs/` instead of the defaults above. Launch and debugging helpers often create temporary user data dirs under `.build/`; always prefer the exact user data dir from the launch command when it is known.
...
1. **Identify which app** the user is asking about: ZYRAXON Code or Agents app. If unclear, check both.
2. **Find the most recent log folder**:
    ```bash
    ls -lt "$HOME/.vszyraxon-code-dev/logs" | head -5
...
When using temporary `console.log` probes and you need those probes to persist in the normal log files, enable dev console forwarding locally before launching ZYRAXON Code.
...
4. Launch ZYRAXON Code or the Agents app and reproduce the issue.
