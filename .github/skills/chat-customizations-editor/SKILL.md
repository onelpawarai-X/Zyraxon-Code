...
MyNewTab: defineComponentFixture({
    ...
    render: ctx => renderEditor(ctx, {
        harness: CustomizationHarness.zyraxon,
...
Component fixtures use mock data and a fixed container size. Layout bugs caused by reflow timing, real data shapes, or narrow window sizes often **don't reproduce in fixtures**. When a user reports a broken layout, debug in the live ZYRAXON Code product.

For launching ZYRAXON Code with CDP and connecting `@playwright/cli`, see the **`launch` skill**. Use `--user-data-dir /tmp/zyraxon-code-debug` to avoid colliding with an already-running instance from another worktree.
