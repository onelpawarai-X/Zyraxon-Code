...
Copilot configuration (delivered to ZYRAXON Code via native MDM or the GitHub server) flows
into ZYRAXON Code's policy stack and locks a setting. It is a companion to `SKILL.md` — read
...
## The big idea: one canonical bag, three delivery channels (in ZYRAXON Code)
...
ZYRAXON Code implements **three** channels feeding the bag, matching the external schema's
...
All three ZYRAXON Code channels converge in `AccountPolicyService.getPolicyData()`.
...
the ZYRAXON Code bag is **flattened** to dot-paths — e.g. the schema's nested
...
> API source of truth, and keep the ZYRAXON Code `managedSettings` declarations aligned with
> what the server actually projects into the bag):
> - `extraKnownMarketplaces`: the schema permits source kinds `github` / `git` /
>   `directory`, but the ZYRAXON Code normalizer only accepts `github` and `git` —
...
enterprise/org/user. Inside ZYRAXON Code the bag has already been collapsed to a single
...
3. ZYRAXON Code policy projection from `projectManagedSettings`.

It then shows per-key channel precedence, the merged normalized bag, and the final bag delivered to
ZYRAXON Code policy callbacks. Runtime-owned settings can therefore remain visible in a raw source even
when ZYRAXON Code has no corresponding policy declaration and the projected bag is empty.
...
active session. This runtime snapshot is not treated as another ZYRAXON Code delivery channel because
