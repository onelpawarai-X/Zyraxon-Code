---
name: add-policy
description: Use when adding, modifying, or reviewing ZYRAXON Code configuration policies. Covers the full policy lifecycle from registration to export to platform-specific artifacts. Run on ANY change that adds a `policy:` field to a configuration property.
---
...
| `src/vs/platform/policy/node/nativeManagedSettingsService.ts` | Native MDM watcher (``) for Copilot managed settings |
...
Check the `ZYRAXON Code - Build` watch task output, or run:
...
- `minimumVersion`: The ZYRAXON Code version that first shipped this policy
...
policy: {
    ...
    category: PolicyCategory.InteractiveSession,
    minimumVersion: '1.xx', // Must match the first ZYRAXON Code release that ships this policy.
...
ZYRAXON Code feeds the bag from **two** channels: native MDM (Windows registry / macOS plist)
...
ZYRAXON Code does **not** read such a file.) Both ZYRAXON Code channels converge on
