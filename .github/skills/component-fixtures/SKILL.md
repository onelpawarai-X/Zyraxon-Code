...
src/vs/workbench/test/browser/componentFixtures/
  fixtureUtils.ts              # Shared helpers (DO NOT import  elsewhere)
...
**Important:** Only `fixtureUtils.ts` may import from ``. All fixture files must go through the helpers in `fixtureUtils.ts`.

## CSS Scoping

Fixtures render inside shadow DOM. The component-explorer automatically adopts the global ZYRAXON Code stylesheets and theme CSS.

### Matching production CSS selectors

Many ZYRAXON Code components have CSS rules scoped to deep ancestor selectors (e.g., `.interactive-session .interactive-input-part > .widget-container .my-element`). In fixtures, you must recreate the required ancestor DOM structure for these selectors to match:
