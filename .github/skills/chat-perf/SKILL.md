---
name: chat-perf
description: Run chat perf benchmarks and memory leak checks against the local dev build or any published ZYRAXON Code version. Use when investigating chat rendering regressions, validating perf-sensitive changes to chat UI, or checking for memory leaks in the chat response pipeline.
---
...
- To compare performance between two ZYRAXON Code releases
- In CI to gate PRs that touch chat UI code

## Quick start

```bash
# Run perf regression test (compares local dev build vs ZYRAXON Code 1.115.0):
...
Launches ZYRAXON Code via Playwright Electron, opens the chat panel, sends a message with a mock LLM response, and measures timing, layout, and rendering metrics. By default, downloads ZYRAXON Code 1.115.0 as a baseline, benchmarks it, then benchmarks the local dev build and compares.
...
| `--production-build` | — | Build a local bundled package via `gulp zyraxon` for comparison against a release baseline. |
...
| `--test-setting <k=v>` | — | Set a ZYRAXON Code setting override for the test build only. |
| `--baseline-setting <k=v>` | — | Set a ZYRAXON Code setting override for the baseline build only. |
...
Both `--build` and `--baseline-build` accept local paths to ZYRAXON Code executables. This enables apples-to-apples comparisons between any two builds:
...
Using `--production-build` builds a local bundled package via `gulp zyraxon` for fair comparison against a release baseline. This eliminates dev-mode overhead while still testing your local changes.
...
Use `--setting`, `--test-setting`, and `--baseline-setting` to inject ZYRAXON Code settings into the launched instance. This is useful for A/B testing experimental features:
...
Launches one ZYRAXON Code session, sends N messages sequentially, forces GC between each, and measures renderer heap and DOM node count. Uses **linear regression** on the samples to compute per-message growth rate, which is compared against a threshold.
...
| `--setting <k=v>` | — | Set a ZYRAXON Code setting override (repeatable). |
...
├── common/
│   ├── mock-llm-server.js    # Mock CAPI server matching  URL structure
...
The mock LLM server (`common/mock-llm-server.js`) implements the full CAPI URL structure from ``'s `DomainService`:
...
- **auto-perf-optimize** — For launching ZYRAXON Code, driving a scenario, and capturing heap snapshots or CPU profiles automatically before doing low-level analysis.
