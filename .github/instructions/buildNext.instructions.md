...
**The `ZYRAXON Code - Build` task is NOT needed to validate changes in the `build/` folder!**

Build scripts in `build/` are TypeScript files that run directly with `node` (e.g., `node build/next/index.ts`). They are not compiled by the main ZYRAXON Code build.
...
The default `ZYRAXON Code - Build` task now runs three parallel watchers:
