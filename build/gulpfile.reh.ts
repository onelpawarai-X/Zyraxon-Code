...
import { vscodeWebResourceIncludes, createZyraxonCodeWebFileContentMapper } from './gulpfile.vscode.web.ts';
		...
		optimize.bundleTask(
			{
				...
				esm: {
					...
					fileContentMapper: createZyraxonCodeWebFileContentMapper('.build/extensions', type === 'reh-web' ? tweakProductForServerWeb(product) : product)
				}
			}
		)
	));
