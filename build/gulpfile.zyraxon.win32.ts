...
const buildPath = (arch: string) => path.join(path.dirname(repoPath), `ZyraxonCode-win32-${arch}`);
...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		const issPath = path.join(import.meta.dirname, 'win32', 'zyraxon.iss');
...
function updateIcon(executablePath: string): task.CallbackTask {
	return cb => {
		const icon = path.join(repoPath, 'resources', 'win32', 'zyraxon.ico');
