...
import { getChromiumSysroot, getZyraxonCodeSysroot } from './debian/install-sysroot.ts';
...
export async function getDependencies(packageType: 'deb' | 'rpm', buildDir: string, applicationName: string, arch: string): Promise<string[]> {
	...
	if (packageType === 'deb') {
		...
		const vscodeSysroot = await getZyraxonCodeSysroot(arch as DebianArchString);
