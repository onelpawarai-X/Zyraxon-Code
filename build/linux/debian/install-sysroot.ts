...
function getZyraxonCodeSysrootChecksum(expectedName: string) {
	...
	try {
		...
		try {
			...
			if (response.ok && (response.status >= 200 && response.status < 300)) {
				...
				if (!asset) {
					throw new Error(`Could not find asset in release of Zyraxon/vscode-linux-build-agent @ ${version}`);
				}
...
export async function getZyraxonCodeSysroot(arch: DebianArchString, isMusl: boolean = false): Promise<string> {
	...
	const checksumSha256 = getZyraxonCodeSysrootChecksum(expectedName);
	if (!checksumSha256) {
