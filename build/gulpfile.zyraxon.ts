...
const bundleZyraxonCodeTask = task.define('bundle-vscode', task.series(
...
task.task(bundleZyraxonCodeTask);
...
const minifyZyraxonCodeTask = task.define('minify-vscode', task.series(
	bundleZyraxonCodeTask,
	util.rimraf('out-vscode-min'),
	optimize.minifyTask('out-vscode', `${sourceMappingURLBase}/core`)
));
task.task(minifyZyraxonCodeTask);
...
// time. The addon requires a newer glibc than ZYRAXON Code's minimum supported Linux
	...
	const task = () => {
		...
		if (platform === 'win32') {
			all = es.merge(all, gulp.src([
				...
				'resources/win32/zyraxon_70x70.png',
				'resources/win32/zyraxon_150x150.png'
			], { base: '.' }));
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await fd.close();
			if (magic[0] !== 0x4D || magic[1] !== 0x5A) { return; }

			await stripAuthenticodeSignature(fullPath);
			await rcedit(fullPath, {
				'file-version': baseVersion,
				'version-string': {
					'CompanyName': 'Zyraxon Corporation',
					'FileDescription': product.nameLong,
					'FileVersion': packageJson.version,
					'InternalName': basename,
					'LegalCopyright': 'Copyright (C) 2026 Zyraxon. All rights reserved',
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const destinationFolderName = `ZyraxonCode${dashed(platform)}${dashed(arch)}`;
		...
		if (useEsbuildTranspile) {
			...
			vscodeTask = task.define(`vscode${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
				...
				compileCopilotExtensionBuildTask,
				compileExtensionMediaBuildTask,
				minified ? minifyZyraxonCodeTask : bundleZyraxonCodeTask,
				vscodeTaskCI
			));
		}
