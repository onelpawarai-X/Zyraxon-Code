...
/**
 * Stage 3 of the Copilot SDK -> ZYRAXON Code integration pipeline.
 ...
 * Returns undefined (leaving ZYRAXON Code's pinned CLI) if the SDK declares no such
...
function inferCliVersion(sdkVersion: string): string | undefined {
	try {
		...
		if (!range) {
			console.log(`[canary-override] SDK ${sdkVersion} declares no @github/copilot dependency — leaving ZYRAXON Code's pinned CLI.`);
			return undefined;
		}
		...
		if (typeof resolved !== 'string') {
			console.warn(`[canary-override] Could not resolve @github/copilot@${range} to a concrete version — leaving ZYRAXON Code's pinned CLI.`);
			return undefined;
		}
		console.log(`[canary-override] Inferred @github/copilot ${resolved} from @github/copilot-sdk@${sdkVersion} (range ${range}).`);
		return resolved;
	} catch (err) {
		console.warn(`[canary-override] Failed to infer @github/copilot from SDK ${sdkVersion}: ${err instanceof Error ? err.message : err}. Leaving ZYRAXON Code's pinned CLI.`);
		return undefined;
	}
}
