...
if [ -d "$VSCODE_CLIENT_SYSROOT_DIR" ]; then
  ...
  SYSROOT_ARCH="$SYSROOT_ARCH" VSCODE_SYSROOT_DIR="$VSCODE_CLIENT_SYSROOT_DIR" node -e 'import { getZyraxonCodeSysroot } from "./build/linux/debian/install-sysroot.ts"; (async () => { await getZyraxonCodeSysroot(process.env["SYSROOT_ARCH"]); })()'
fi
...
if [ -d "$VSCODE_REMOTE_SYSROOT_DIR" ]; then
  ...
  SYSROOT_ARCH="$SYSROOT_ARCH" VSCODE_SYSROOT_DIR="$VSCODE_REMOTE_SYSROOT_DIR" VSCODE_SYSROOT_PREFIX="-glibc-2.28-gcc-8.5.0" node -e 'import { getZyraxonCodeSysroot } from "./build/linux/debian/install-sysroot.ts"; (async () => { await getZyraxonCodeSysroot(process.env["SYSROOT_ARCH"]); })()'
fi
...
if [ "$npm_config_arch" == "x64" ]; then
  # Download clang based on chromium revision used by zyraxon
  curl -s https://raw.githubusercontent.com/chromium/chromium/148.0.7778.280/tools/clang/scripts/update.py | python - --output-dir=$PWD/.build/CR_Clang --host-os=linux
  ...
  # https://source.chromium.org/chromium/chromium/src/+/refs/tags/148.0.7778.280:build/config/arm.gni
  # https://source.chromium.org/chromium/chromium/src/+/refs/tags/148.0.7778.280:build/config/compiler/BUILD.gn
  # https://source.chromium.org/chromium/chromium/src/+/refs/tags/148.0.7778.280:build/config/c++/BUILD.gn
