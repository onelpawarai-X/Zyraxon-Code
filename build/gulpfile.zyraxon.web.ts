 ...
 */
export const createZyraxonCodeWebFileContentMapper = (extensionsRoot: string, product: typeof import('../product.json')) => {
...
const bundleZyraxonCodeWebTask = task.define('bundle-vscode-web-OLD', task.series(
	...
	optimize.bundleTask(
		{
			...
			esm: {
				...
				fileContentMapper: createZyraxonCodeWebFileContentMapper('.build/web/extensions', product)
			}
		}
	)
));

const minifyZyraxonCodeWebTask = task.define('minify-vscode-web-OLD', task.series(
	bundleZyraxonCodeWebTask,
	util.rimraf('out-vscode-web-min'),
	optimize.minifyTask('out-vscode-web', `https://main.vscode-cdn.net/sourcemaps/${commit}/core`)
));
task.task(minifyZyraxonCodeWebTask);

// esbuild-based tasks (new)
const sourceMappingURLBase = `https://main.vscode-cdn.net/sourcemaps/${commit}`;
const esbuildBundleZyraxonCodeWebTask = task.define('esbuild-vscode-web', () => runEsbuildBundle('out-vscode-web', false, true));
const esbuildBundleZyraxonCodeWebMinTask = task.define('esbuild-vscode-web-min', () => runEsbuildBundle('out-vscode-web-min', true, true, `${sourceMappingURLBase}/core`));
...
['', 'min'].forEach(minified => {
	...
	const vscodeWebTaskCI = task.define(`vscode-web${dashed(minified)}-ci`, task.series(
		...
		minified ? esbuildBundleZyraxonCodeWebMinTask : esbuildBundleZyraxonCodeWebTask,
