...
async function main(buildDir?: string): Promise<void> {
	...
	const appRoot = path.join(buildDir, `ZyraxonCode-darwin-${arch}`);
	...
	if (arch !== 'universal') {
		await spawn('plutil', [
			...
			'-string',
			'An application in ZYRAXON Code wants to use AppleScript.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in ZYRAXON Code wants to use the Microphone.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in ZYRAXON Code wants to use the Camera.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in ZYRAXON Code wants to use Audio Capture.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-insert',
