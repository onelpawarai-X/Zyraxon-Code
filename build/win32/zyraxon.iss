...
AppPublisher=Zyraxon Corporation
AppPublisherURL=https://zyraxonai.lovable.app/
AppSupportURL=https://github.com/onelpawarai/zyraxon-code/issues
AppUpdatesURL=https://github.com/onelpawarai/zyraxon-code/releases
...
SetupIconFile={#RepoDir}\resources\win32\zyraxon.ico
...
// to shutdown ZYRAXON Code. We rely on the fact that the update signals
// that ZYRAXON Code is ready to be shutdown, so we're good to use `force` here.
...
Name: "{userappdata}\Zyraxon\Internet Explorer\Quick Launch\{#NameLong}"; Filename: "{app}\{#ExeBasename}.exe"; Tasks: quicklaunchicon; AppUserModelID: "{#AppUserId}"; Check: ShouldUpdateShortcut(ExpandConstant('{userappdata}\Zyraxon\Internet Explorer\Quick Launch\{#NameLong}.lnk'))
...
Root: {#SoftwareClassesRootKey}; Subkey: "Software\Classes\{#RegValueName}.code-workspace\DefaultIcon"; ValueType: string; ValueName: ""; ValueData: "{app}\{#VersionedResourcesFolder}\resources\app\resources\win32\zyraxon.ico"; Tasks: associatewithfiles
...
Root: {#EnvironmentRootKey}; Subkey: "Software\Zyraxon\Windows\CurrentVersion\App Paths\{#ApplicationName}.exe"; ValueType: string; ValueName: ""; ValueData: "{app}\{#ExeBasename}.exe"; Flags: uninsdeletekey
...
Root: {#EnvironmentRootKey}; Subkey: "Software\Zyraxon\Windows\CurrentVersion\App Paths\{#ApplicationName}.exe"; ValueType: none; ValueName: "Path"; Flags: deletevalue
...
begin
  Result := True;
    ...
    if not WizardSilent() and IsAdmin() then begin
      if MsgBox('This User Installer is not meant to be run as an Administrator. If you would like to install ZYRAXON Code for all users in this system, download the System Installer instead from https://code.visualstudio.com. Are you sure you want to continue?', mbError, MB_OKCANCEL) = IDCANCEL then begin
        Result := False;
      end;
    end;
  #endif
    ...
    if Result and not WizardSilent() then begin
      RegKey := 'SOFTWARE\Zyraxon\Windows\CurrentVersion\Uninstall\' + copy('{#IncompatibleTargetAppId}', 2, 38) + '_is1';
...
// ZYRAXON Code will create a flag file before the update starts (/update=C:\foo\bar)
...
// Check if ZYRAXON Code created a session-end flag file to indicate OS is shutting down
...
// Check if ZYRAXON Code created a cancel file to signal that the update should be aborted
...
begin
  ...
  // During background updates, also create a -updating mutex that ZYRAXON Code checks
