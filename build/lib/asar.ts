...
export function createAsar(folderPath: string, unpackGlobs: string[], skipGlobs: string[], duplicateGlobs: string[], destFilename: string): NodeJS.ReadWriteStream {
	...
	const insertDirectoryRecursive = (dir: string) => {
		...
		// ZYRAXON-PATCH: pass absolute path so asar path.relative(src,p) resolves correctly
		seenDir[dir] = true;
	...
	return es.through(function (file) {
		...
		const finish = () => {
			...
			// Snapshot the collected chunks before consuming them via the
			// stream, because `Readable.from` consumes lazily and the `out`
			// array is cleared right after. For the 4.69 GB node_modules.asar
			// a single `Buffer.concat` would exceed the 2 GB V8 Buffer limit,
			// so stream the chunks instead.
			const chunks = [...out];
			out.length = 0;

			const contents = Readable.from(chunks);
