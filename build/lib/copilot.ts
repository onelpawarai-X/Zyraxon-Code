...
/**
 * Converts ZYRAXON Code build platform/arch to the values that Node.js reports
...
function toNodePlatformArch(platform: string, arch: string): { nodePlatform: string; nodeArch: string } {
	...
	if (arch === 'armhf') {
		// ZYRAXON Code build uses 'armhf'; Node reports process.arch === 'arm'
		nodeArch = 'arm';
	} else if (arch === 'alpine') {
 ...
 * ZYRAXON Code packaging can cross-build targets such as darwin-x64 on arm64 hosts.
 */
 ...
 * `node-pty` from the embedder (ZYRAXON Code) via `hostRequire` and falls back to
