...
/**
 * Derive the ZYRAXON Code repo root from an Electron executable path.
...
/**
 * Returns true if the string looks like a ZYRAXON Code version or commit hash
...
/**
 * Get the built-in extensions directory for a ZYRAXON Code executable.
...
/**
 * Pre-seed the ZYRAXON Code storage database to prevent the
...
/**
 * Build the environment variables for launching ZYRAXON Code with the mock server.
...
/**
 * Build the default ZYRAXON Code launch args.
 * @param {string} userDataDir
 * @param {string} extDir
...
/**
 * Write ZYRAXON Code settings that point the copilot extension at the mock server.
 * @param {string} userDataDir
 * @param {{ url: string }} mockServer
...
/**
 * Wait until ZYRAXON Code exposes its CDP endpoint.
 * @param {number} port
...
/**
 * Launch ZYRAXON Code via child_process and connect via CDP.
 * Works with dev builds, insiders, and stable releases.
 *
 * @param {string} executable - Path to the ZYRAXON Code executable (Electron binary or CLI)
...
async function launchVSCode(executable, launchArgs, env, opts = {}) {
	...
	child.once('exit', (code, signal) => {
		if (!exitError) {
			exitError = new Error(`ZYRAXON Code exited before CDP connected (code=${code} signal=${signal})`);
		}
	});
