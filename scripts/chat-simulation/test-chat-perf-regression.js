		...
		switch (args[i]) {
			...
			case '--help': case '-h':
				console.log([
					...
					'  --build <path|ver>  Path to ZYRAXON Code build, or a version to download',
					...
					'  --baseline-setting <k=v> Set a ZYRAXON Code setting override for baseline build only',
 ...
 * @param {Record<string, any>} [settingsOverrides] - custom ZYRAXON Code settings
...
async function runOnce(electronPath, scenario, mockServer, verbose, runIndex, runDir, role, settingsOverrides, runOpts) {
	...
	try {
		...
		// Trace file may not exist if ZYRAXON Code crashed before shutdown
	}
