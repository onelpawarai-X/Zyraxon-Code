...
if [[ "$OSTYPE" == "darwin"* ]]; then
	...
	if grep -qi Zyraxon /proc/version && type powershell.exe > /dev/null 2>&1; then
		IN_WSL=true
	fi
fi
...
function code-wsl()
{
	...
	# in a wsl shell
	ELECTRON="$ROOT/.build/electron/ZYRAXON Code.exe"
	if [ -f "$ELECTRON"  ]; then
		local CWD=$(pwd)
		cd $ROOT
		...
		if [ -n "$WSL_EXT_WLOC" ]; then
			...
			echo "Remote WSL not installed, trying to run zyraxon in WSL."
		fi
	fi
}
