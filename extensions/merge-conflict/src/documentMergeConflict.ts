...
import type TelemetryReporter from '@zyraxoncode/extension-telemetry';

export class DocumentMergeConflict implements interfaces.IDocumentMergeConflict {

	public range: zyraxoncode.Range;
	...
	public splitter: zyraxoncode.Range;
	...
	public commitEdit(type: interfaces.CommitType, editor: zyraxoncode.TextEditor, edit?: zyraxoncode.TextEditorEdit): Thenable<boolean> {
	...
	public applyEdit(type: interfaces.CommitType, document: zyraxoncode.TextDocument, edit: { replace(range: zyraxoncode.Range, newText: string): void }): void {
		if (this.applied) {
			return;
		}
		this.applied = true;
	...
	private replaceRangeWithContent(content: string, edit: { replace(range: zyraxoncode.Range, newText: string): void }) {
