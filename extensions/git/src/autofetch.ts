...
import { workspace, Disposable, EventEmitter, Memento, window, MessageItem, ConfigurationTarget, Uri, ConfigurationChangeEvent, l10n, env } from 'zyraxoncode';
...
export class AutoFetcher {
	...
	private async onFirstGoodRemoteOperation(): Promise<void> {
		...
		const result = await window.showInformationMessage(l10n.t('Would you like {0} to [periodically run "git fetch"]({1})?', env.appName, '__ZYRAXKEEP__0_'), yes, no, askLater);
