...
class CommandCenter {
    ...
    getSCMResource(uri) {
        uri = uri ?? vscode_1.window.activeNotebookEditor?.notebook.uri ?? vscode_1.window.activeTextEditor?.document.uri;
        // A notebook diff editor is neither the active text nor notebook editor, so
        // fall back to its modified resource.
        if (!uri && vscode_1.window.tabGroups.activeTabGroup.activeTab?.input instanceof vscode_1.TabInputNotebookDiff) {
            uri = vscode_1.window.tabGroups.activeTabGroup.activeTab.input.modified;
        }
