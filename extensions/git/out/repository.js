...
class ProgressManager {
    ...
    updateEnablement() {
        ...
        const showProgress = config.get('showProgress') === true && !vscode_1.workspace.isAgentSessionsWorkspace;
        if (showProgress) {
            this.enable();
        }
        else {
...
class Repository {
    ...
    _isUsingVirtualFileSystem = undefined;
    get isUsingVirtualFileSystem() {
    ...
    constructor(repository, repositoryResolver, pushErrorHandlerRegistry, remoteSourcePublisherRegistry, postCommitCommandsProviderRegistry, branchProtectionProviderRegistry, historyItemDetailProviderRegistry, globalState, logger, telemetryReporter, repositoryCache) {
        ...
        this.disposables.push(this.branchProtectionProviderRegistry.registerBranchProtectionProvider(root, new branchProtection_1.GitBranchProtectionProvider(root, this.logger)));
    ...
    async appendAICoAuthorTrailer(message, indexResources, workingGroupResources) {
        ...
        const chatConfig = vscode_1.workspace.getConfiguration('chat');
        if (chatConfig.get('disableAIFeatures', false)) {
            return message;
        }
        const config = vscode_1.workspace.getConfiguration('git', vscode_1.Uri.file(this.root));
            ...
            try {
                ...
                if (err.gitErrorCode === git_constants_1.GitErrorCodes.WorktreeContainsChanges) {
                    ...
                    const message = options?.label
                        ? vscode_1.l10n.t('The worktree for session "{0}" contains modified or untracked files. Do you want to force delete?', options.label)
                        : vscode_1.l10n.t('The worktree contains modified or untracked files. Do you want to force delete?');
                    const choice = await vscode_1.window.showWarningMessage(message, { modal: true, detail: vscode_1.l10n.t('Worktree: {0}', path) }, forceDelete);
    ...
    async initIsUsingVirtualFileSystem() {
