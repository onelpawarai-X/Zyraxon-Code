...
exports.ensureAskpassScripts = ensureAskpassScripts;
exports.getAskpassPaths = getAskpassPaths;
 ...
 * like GIT_ASKPASS point to scripts inside the ZYRAXON Code installation directory, which
 ...
 */
/**
 * Checks if the current ZYRAXON Code installation is a Windows user or system setup.
 * Returns false for archive, portable, or non-Windows installations.
 */
function isWindowsUserOrSystemSetup() {
    if (process.platform !== 'win32') {
        return false;
    }
    try {
        const productJsonPath = path.join(vscode_1.env.appRoot, 'product.json');
        const productJson = JSON.parse(fs.readFileSync(productJsonPath, 'utf8'));
        const target = productJson.target;
 ...
 */
function computeContentHash(sourcePaths) {
 ...
 */
async function setWindowsPermissions(filePath, logger) {
 ...
 */
async function copyFileSecure(source, dest, logger) {
 ...
 */
async function updateDirectoryMtime(dirPath, logger) {
    try {
        const now = new Date();
 ...
 * This prevents accumulation of old versions when ZYRAXON Code updates.
 */
async function garbageCollectOldDirectories(askpassBaseDir, currentHash, logger) {
    try {
 ...
 */
async function ensureAskpassScripts(sourceDir, storageDir, logger) {
    const sourcePaths = {
 ...
 */
async function getAskpassPaths(sourceDir, storagePath, logger) {
