...
import { Range as LSRange } from 'zyraxoncode-languageserver-textdocument';
import { getRootNode } from './parseDocument';

export class DefaultCompletionItemProvider implements zyraxoncode.CompletionItemProvider {

	private lastCompletionType: string | undefined;

	public provideCompletionItems(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, _: zyraxoncode.CancellationToken, context: zyraxoncode.CompletionContext): Thenable<zyraxoncode.CompletionList | undefined> | undefined {
	...
	private provideCompletionItemsInternal(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: zyraxoncode.CompletionContext): Thenable<zyraxoncode.CompletionList | undefined> | undefined {
		...
		const lineRange = new zyraxoncode.Range(position.line, 0, position.line, position.character);
		...
		if (syntax === 'html') {
			if (context.triggerKind === zyraxoncode.CompletionTriggerKind.TriggerForIncompleteCompletions) {
		...
		if (isStyleSheet(document.languageId) && context.triggerKind !== zyraxoncode.CompletionTriggerKind.TriggerForIncompleteCompletions) {
		...
		if (!isStyleSheet(document.languageId) && isStyleSheet(syntax) && context.triggerKind !== zyraxoncode.CompletionTriggerKind.TriggerForIncompleteCompletions) {
			validateLocation = true;
		...
		if (!isStyleSheet(syntax) && (document.languageId === 'javascript' || document.languageId === 'javascriptreact' || document.languageId === 'typescript' || document.languageId === 'typescriptreact')) {
			...
			if (abbreviation.startsWith('this.') || /\[[^\]=]*\]/.test(abbreviation)) {
				...
				isNoisePromise = zyraxoncode.commands.executeCommand<zyraxoncode.SymbolInformation[] | undefined>('zyraxoncode.executeDocumentSymbolProvider', document.uri).then(symbols => {
					return !!symbols && symbols.some(x => abbreviation === x.name || (abbreviation.startsWith(x.name + '.') && !/>|\*|\+/.test(abbreviation)));
				});
			}
		}

		return isNoisePromise.then((isNoise): zyraxoncode.CompletionList | undefined => {
			if (isNoise) {
				return undefined;
			}
			...
			const newItems: zyraxoncode.CompletionItem[] = [];
			if (result && result.items) {
				result.items.forEach((item: any) => {
					const newItem = new zyraxoncode.CompletionItem(item.label);
					newItem.documentation = item.documentation;
					newItem.detail = item.detail;
					newItem.insertText = new zyraxoncode.SnippetString(item.textEdit.newText);
					const oldrange = item.textEdit.range;
					newItem.range = new zyraxoncode.Range(oldrange.start.line, oldrange.start.character, oldrange.end.line, oldrange.end.character);

					newItem.filterText = item.filterText;
					newItem.sortText = item.sortText;

					if (emmetConfig['showSuggestionsAsSnippets'] === true) {
						newItem.kind = zyraxoncode.CompletionItemKind.Snippet;
					}
					newItems.push(newItem);
				});
			}

			return new zyraxoncode.CompletionList(newItems, true);
		});
	}
}

function toRange(lsRange: LSRange) {
	return new zyraxoncode.Range(lsRange.start.line, lsRange.start.character, lsRange.end.line, lsRange.end.character);
}
