...
zyraxoncode.workspace.registerFileSystemProvider(testFs.scheme, testFs, { isCaseSensitive: testFs.isCaseSensitive });

export async function createRandomFile(contents: string | Uint8Array = '', dir: zyraxoncode.Uri | undefined = undefined, ext = ''): Promise<zyraxoncode.Uri> {
	let fakeFile: zyraxoncode.Uri;
	if (dir) {
		assert.strictEqual(dir.scheme, testFs.scheme);
		fakeFile = dir.with({ path: dir.path + '/' + rndName() + ext });
	} else {
		fakeFile = zyraxoncode.Uri.parse(`${testFs.scheme}:/${rndName() + ext}`);
	}
	testFs.writeFile(fakeFile, typeof contents === 'string' ? Buffer.from(contents) : Buffer.from(contents), { create: true, overwrite: true });
	return fakeFile;
}

export async function deleteFile(file: zyraxoncode.Uri): Promise<boolean> {
	try {
		testFs.delete(file);
		return true;
	} catch {
...
export function closeAllEditors(): Thenable<any> {
	return zyraxoncode.commands.executeCommand('workbench.action.closeAllEditors');
}

export function saveAllEditors(): Thenable<any> {
	return zyraxoncode.commands.executeCommand('workbench.action.files.saveAll');
}

export async function revertAllDirty(): Promise<void> {
	return zyraxoncode.commands.executeCommand('_workbench.revertAllDirty');
}

export function disposeAll(disposables: zyraxoncode.Disposable[]) {
	zyraxoncode.Disposable.from(...disposables).dispose();
}
...
function withLogLevel(level: string, runnable: () => Promise<any>): () => Promise<void> {
	return async (): Promise<void> => {
		const logLevel = await zyraxoncode.commands.executeCommand('_extensionTests.getLogLevel');
		await zyraxoncode.commands.executeCommand('_extensionTests.setLogLevel', level);

		try {
			await runnable();
		} finally {
			await zyraxoncode.commands.executeCommand('_extensionTests.setLogLevel', logLevel);
		}
	};
}
...
export function assertNoRpc() {
	assertNoRpcFromEntry([zyraxoncode, 'zyraxoncode']);
}
...
export async function asPromise<T>(event: zyraxoncode.Event<T>, timeout = zyraxoncode.env.uiKind === zyraxoncode.UIKind.Desktop ? 5000 : 15000): Promise<T> {
