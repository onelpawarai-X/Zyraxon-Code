...
const generatorInstalledPackages = {
    script: ['pnpm', 'ls'],
    postProcess: function (out) {
        /**
         * out
...
// SUBCOMMANDS
const SUBCOMMANDS_MANAGE_DEPENDENCIES = [
    {
        name: 'add',
...
If you want to disable this behavior, set the recursive-install setting to false`,
        async generateSpec(tokens) {
            ...
            const hasArgs = tokens.filter((token) => token.trim() !== '' && !token.startsWith('-'))
                .length > 2;
            return {
                name: 'install',
                options: [
...
When used without arguments, updates all dependencies. You can use patterns to update specific dependencies`,
        args: {
            ...
            generators: yarn_1.dependenciesGenerator,
            isVariadic: true,
        },
        options: [
            {
...
const completionSpec = {
    ...
    generateSpec: async (tokens, executeShellCommand) => {
        const { script, postProcess } = yarn_1.dependenciesGenerator;
        if (postProcess === undefined) {
            return undefined;
        }
        const packages = postProcess((await executeShellCommand({
            command: script[0],
            args: script.slice(1),
        })).stdout, tokens)
            ?.filter((e) => e !== null)
        ...
        const subcommands = packages
            ?.filter((name) => yarn_1.nodeClis.has(name))
            .map((name) => ({
            name,
            loadSpec: name,
            icon: 'fig://icon?type=package',
