...
async function startClientWithParticipants(_context, languageParticipants, newLanguageClient, runtime) {
    ...
    let schemaAssociationRefreshTrigger;
    ...
    const registryWatchers = new Map();
    const updateRegistryWatchers = () => {
        ...
        for (const uri of registryUris) {
            if (!registryWatchers.has(uri)) {
                const registryUri = vscode_1.Uri.parse(uri);
                ...
                const watcher = vscode_1.workspace.createFileSystemWatcher(new vscode_1.RelativePattern(parentUri, fileName));
                registryWatchers.set(uri, vscode_1.Disposable.from(watcher, watcher.onDidCreate(refreshSchemaAssociations), watcher.onDidChange(refreshSchemaAssociations), watcher.onDidDelete(refreshSchemaAssociations)));
            }
        }
    };
    updateRegistryWatchers();
    toDispose.push(new vscode_1.Disposable(() => registryWatchers.forEach(watcher => watcher.dispose())));
    toDispose.push(vscode_1.extensions.onDidChange(() => {
        updateRegistryWatchers();
...
function resolveExtensionResource(extensionUri, resource) {
    return resource.startsWith('./') ? vscode_1.Uri.joinPath(extensionUri, resource) : vscode_1.Uri.parse(resource);
}
...
function getSchemaRegistryUris() {
    const result = [];
...
async function getSchemaRegistryAssociations() {
    const result = [];
    for (const registryUri of getSchemaRegistryUris()) {
        try {
            const data = await vscode_1.workspace.fs.readFile(registryUri);
            const rawStr = new TextDecoder().decode(data);
            const registry = JSON.parse(rawStr);
