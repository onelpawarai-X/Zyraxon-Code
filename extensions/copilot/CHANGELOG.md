...
- **Ask questions tool improvements** — The `askQuestions` tool moved into ZYRAXON Code core for improved reliability. You can now send steering messages without dismissing pending questions first.
- **Prevent auto-suspend during chat** — ZYRAXON Code asks the OS not to suspend the machine while a chat request is running.
...
- **Claude compatibility** — ZYRAXON Code reads Claude configuration files directly: `CLAUDE.md` instructions, `.claude/agents`, `.claude/skills`, `.claude/settings.json` hooks.
...
- **Agent tooling reorganization** — Renamed tool references for better compatibility with GitHub custom agents across ZYRAXON Code and GitHub environments.
...
- **Reuse Claude skills (Experimental)** — ZYRAXON Code can discover and use Claude Code skills from `~/.claude/skills/` and workspace `.claude/skills/` folders.
...
- **Python: Localized Copilot Hover Summaries** — Respects ZYRAXON Code display language.
...
This iteration, we're introducing auto model selection in chat. When you choose the **Auto** model in the model picker, ZYRAXON Code automatically selects a model to ensure that you get the optimal performance and avoid rate limits.
...
We continued to expand on ways to delegate local tasks in ZYRAXON Code to a Copilot coding agent:
...
ZYRAXON Code now reads MCP server instructions and will include them in its base prompt.
...
The `chat.mcp.enabled` setting that previously controlled whether MCP servers could run in ZYRAXON Code has been migrated to a new `chat.mcp.access` setting with more descriptive options:
...
In this release of ZYRAXON Code, we have enabled an experimental tool-calling mode for when the number of tools exceeds the maximum limit. Tools are automatically grouped and the model is given the ability to activate and call groups of tools.
...
Previously, when you added or updated an MCP server configuration, ZYRAXON Code would show a blue "refresh" icon in the Chat view, enabling you to manually refresh the list of tools. In the milestone, you can now configure the auto-start behavior for MCP servers, so you no longer have to manually restart the MCP server.
...
The ideal flow for a remote MCP server that wants to support authentication is to use an auth provider that supports Dynamic Client Registration (DCR). This enables the client (ZYRAXON Code) to register itself with that auth provider, so the auth flow is seamless.
...
ZYRAXON Code now fully supports the latest MCP specification, version `2025-06-18`, with support for `resource_link`s and structured output in tool results.
...
* **Contribute directly** to the development of AI-powered chat experiences in ZYRAXON Code
...
* **Uninstall**: Remove the server from your ZYRAXON Code instance
...
With MCP servers being first-class resources and the associated change to their configuration, ZYRAXON Code provides comprehensive migration support for users upgrading from the previous MCP server configuration format:
...
* **Real-time migration**: When you add MCP servers to user settings, ZYRAXON Code immediately migrates them with a helpful notification explaining the change
...
ZYRAXON Code's Model Context Protocol support now includes resource support, which includes support for resource templates. It is available in several places:
...
ZYRAXON Code now supports MCP servers that require authentication, allowing you to interact with an MCP server that operates on behalf of your user account for that service.
...
We have also introduced the command `Authentication: Remove Dynamic Authentication Providers` that allows you to clean up any of these dynamic client registrations. This will throw away the client id issued to ZYRAXON Code and all data associated with this authentication provider.
...
We're continuously working to improve the chat user experience in ZYRAXON Code based on your feedback. One such feedback was that it can be difficult to distinguish between user messages and AI responses in the chat. To address this, we've made the appearance of user messages more distinct.
...
> **Note**: The feature is work in progress, but please try it out! Please follow the latest progress in ZYRAXON Code Insiders and let us know what's not working or is missing.
...
To see the toggle, enable the setting and reload ZYRAXON Code. We are also in the process of identifying and fixing some of the less accurate settings search results, and we welcome feedback on when a natural language query did not find an expected setting.
...
With Copilot coding agent, GitHub Copilot can work independently in the background to complete tasks, just like a human developer. We have expanded the GitHub Pull Requests extension to make it easier to assign and track tasks for the agent from within ZYRAXON Code.

We have added the following features to the extension:

* **Assign to Copilot**: assign a pull request or issue to Copilot from the issue or PR view in ZYRAXON Code
...
You can tailor your AI experience in ZYRAXON Code to your specific coding practices and technology stack by using Markdown-based instructions and prompt files. We've aligned the implementation and usage of these two related concepts, however they each have distinct purposes.
...
We've implemented support for OpenAI's apply patch editing format when using GPT 4.1 and o4-mini in agent mode. This means that you benefit from significantly faster edits, especially in large files. The tool is enabled by default in ZYRAXON Code Insiders and will be progressively rolled out in ZYRAXON Code Stable.

#### Use GPT 4.1 as the base model

When you're using chat in ZYRAXON Code, the base model is now updated to GPT-4.1. You can still use the model switcher in the Chat view to change to another model.
...
### Configure ZYRAXON Code

#### Prevent installation of Copilot Chat pre-release versions in ZYRAXON Code stable

ZYRAXON Code now prevents the installation of the pre-release version of the Copilot Chat extension in ZYRAXON Code Stable. This helps avoid situations where you inadvertently install the Copilot Chat pre-release version and get stuck in a broken state. This means that you can only install the Copilot Chat extension pre-release version in the Insiders build of ZYRAXON Code.
...
ZYRAXON Code now provides auditory signals when you keep or undo AI-generated edits. These signals are configurable via `accessibility.signals.editsKept` and `accessibility.signals.editsUndone`.
...
#### Agent mode is available in ZYRAXON Code Stable
...
This setting will be enabled gradually for users in ZYRAXON Code Stable.
...
- **Move chat to editor or window**: Select **Open Chat in New Editor/New Window** to pop out your chat conversation from the side bar into a new editor tab or separate ZYRAXON Code window. Chat has supported this for a long time, but now you can run your edit/agent sessions from an editor pane or a separate window as well.
...
ZYRAXON Code now provides a dedicated tool for creating new Jupyter notebooks directly from chat. This tool plans and creates a new notebook based on your query.
...
However, for private GitHub repositories, ZYRAXON Code needs to have permission to interact with your repositories on GitHub. For a while, this was presented with our usual ZYRAXON Code authentication flow, where a modal dialog showed up when you invoked certain functionality (for example, asking `@workspace` or `@github` a question, or using the `#codebase` tool).
...
* Attach a screenshot of the ZYRAXON Code window (select the **paperclip 📎 button** > **Screenshot Window**)
