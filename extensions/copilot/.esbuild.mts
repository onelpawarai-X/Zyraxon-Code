...
const testBundlePlugin: esbuild.Plugin = {
	...
	setup(build) {
		...
		build.onLoad({ filter: /[\/\\]test-extension\.ts$/ }, async args => {
			...
			// ZYRAXON-PATCH: emit empty test bundle instead of throwing when no test sources exist.
			if (files.length === 0) {
...
async function main() {
	...
	if (isWatch) {
		...
		// ZYRAXON-PATCH: skip when simulation test options are null
		...
		await Promise.all([
			...
			nodeExtHostSimulationTestOptions ? esbuild.build(nodeExtHostSimulationTestOptions) : Promise.resolve(),
