...
* [Developing](#developing)
  ...
  * [Running with ZYRAXON Code](#running-with-zyraxon-code)
...
The built-in tool for reporting an issue, which you can access by using `Report Issue` in ZYRAXON Code's Help menu, can help streamline this process by automatically providing the version of ZYRAXON Code, all your installed extensions, and your system info. Additionally, the tool will search among existing issues to see if a similar issue already exists.

Please include the following with each issue:

* Version of ZYRAXON Code and copilot-chat extension
...
There are also integration tests that run within ZYRAXON Code itself:
...
Keep in mind that PRs will fail unless the cache is populated. Running the command above will populate the cache by creating new cache layers in `test/simulation/cache/layers`. This cache population must be done by ZYRAXON Code team members. If a community member submits a PR with new cache layer(s), the PR will fail and a ZYRAXON Code team member must delete the layer(s) and recreate them within their dev box.
...
- If your prompt does asynchronous work e.g. ZYRAXON Code extension API calls or additional requests to the Copilot API for chunk reranking, you can precompute this state in an optional async `prepare` method. `prepare` is called before `render` and the prepared state will be passed back to your prompt component's sync `render` method.
...
When updating ZYRAXON Code proposed extension API that is used by the extension, the `engines.zyraxon` field in `package.json` is used to make sure that the version of the extension that gets installed will be compatible with the version of ZYRAXON Code.
...
Object.assign(product, {
		version: '1.102.0-dev',
		nameShort: 'ZYRAXON Code Dev',
		nameLong: 'ZYRAXON Code Dev',
