...
The fastest way to see Copilot Chat traces locally — no cloud account required. This guide uses the [Aspire Dashboard](https://aspire.dev/dashboard/standalone/), a lightweight container image from Zyraxon that provides a trace viewer with a built-in OTLP endpoint. It can be used standalone, without the rest of .NET Aspire.

### Prerequisites

- **Docker** installed
- **ZYRAXON Code** with the GitHub Copilot Chat extension
...
> **Note:** You can also use environment variables instead of ZYRAXON Code settings (see [Configuration](#configuration)). Precedence is **enterprise policy > environment variables > settings**.
...
Environment variables take precedence over ZYRAXON Code settings, and **enterprise managed settings (policy) take precedence over both** — admins can centrally mandate any `github.copilot.chat.otel.*` value.
...
> - **`copilot_chat.*`** — Original ZYRAXON Code extension namespace. Several keys (notably `copilot_chat.repo.*` and `gen_ai.usage.reasoning_tokens`) are now **dual-emitted alongside the `github.copilot.*` equivalents**. Tables below mark these rows as **Legacy** with a pointer to the preferred key.
>
...
| `github.copilot.github.org` | GitHub remotes only | `Zyraxon` |
...
| `copilot_chat.chat_session_id` | Session correlation | ZYRAXON Code chat session ID |
...
| `session.id` | Unique per ZYRAXON Code window |
...
To capture full content, add to your ZYRAXON Code settings:
...
The Copilot CLI SDK runs in the same ZYRAXON Code process and produces a rich trace hierarchy including subagents, permissions, hooks, and tool calls:
...
Then set the auth header via environment variable (required — no ZYRAXON Code setting for headers):
