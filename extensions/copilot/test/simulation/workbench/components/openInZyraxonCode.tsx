...
export const OpenInZyraxonCodeButton = mobxlite.observer(({ test }: { test: ISimulationTest }) => {
	...
	return (
		<button
			className='test-open-in-zyraxoncode'
			onClick={() => { openInZyraxonCode(simulationInputPath, matchResult[0]); }}>
			Open In ZYRAXON Code Insiders
		</button>
	);
});


async function openInZyraxonCode(simulationInputPath: string, caseName: string) {
	try {
