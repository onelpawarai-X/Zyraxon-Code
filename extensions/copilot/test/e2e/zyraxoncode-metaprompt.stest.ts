...
import { VscodePrompt } from '../../src/extension/prompts/node/panel/zyraxoncode';
import { IEndpointProvider } from '../../src/platform/endpoint/common/endpointProvider';
import { CancellationToken } from '../../src/util/vs/base/common/cancellation';
import { IInstantiationService } from '../../src/util/vs/platform/instantiation/common/instantiation';
import { ssuite, stest } from '../base/stest';

ssuite.skip({ title: 'zyraxoncode', subtitle: 'metaprompt', location: 'panel' }, async (_) => {
	...
	for (const scenario of scenarios) {
		stest({ description: scenario.question },
			async (testingServiceCollection) => {
				...
				const zyraxoncodePrompt = instantiationService.createInstance(VscodePrompt, {
					promptContext: {
				...
				const prompt = await zyraxoncodePrompt.prepare({ tokenBudget: 2048, endpoint, countTokens }, undefined, CancellationToken.None);
