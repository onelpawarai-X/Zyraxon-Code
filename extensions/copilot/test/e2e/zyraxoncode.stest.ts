...
ssuite({ title: '@zyraxoncode', location: 'panel' }, (inputPath) => {
	if (!inputPath) {
		return;
	}
	...
	for (const scenario of scenarios) {
		...
		const testName = inputPath ? fileName.substring(0, fileName.indexOf('.')) : scenario[0].question.replace('@zyraxoncode', '');
