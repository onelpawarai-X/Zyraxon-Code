...
import type { CancellationToken, CommandInformationResult, RelatedInformationProvider, RelatedInformationResult, SettingInformationResult } from 'zyraxoncode';
 ...
 */
export class ZyraxonCodeCombinedIndexImpl implements ICombinedEmbeddingIndex {
	...
	constructor(
		...
		const settingsEmbeddingsCache = useRemoteCache ?
			instantiationService.createInstance(RemoteEmbeddingsExtensionCache, EmbeddingCacheType.GLOBAL, 'settingEmbeddings', sanitizeZyraxonCodeVersion(envService.getEditorInfo().version), EmbeddingType.text3small_512, RemoteCacheType.Settings) :
		...
		const commandsEmbeddingsCache = useRemoteCache ?
			instantiationService.createInstance(RemoteEmbeddingsExtensionCache, EmbeddingCacheType.GLOBAL, 'commandEmbeddings', sanitizeZyraxonCodeVersion(envService.getEditorInfo().version), EmbeddingType.text3small_512, RemoteCacheType.Commands) :
