...
import type { WorkspaceConfiguration } from 'zyraxoncode';
...
export class ConfigurationServiceImpl extends AbstractConfigurationService {
	...
	constructor(
		...
		zyraxoncode.workspace.onDidChangeConfiguration(changeEvent => {
			if (changeEvent.affectsConfiguration(CopilotConfigPrefix)) {
				this.config = zyraxoncode.workspace.getConfiguration(CopilotConfigPrefix);
			}
			this._onDidChangeConfiguration.fire({
				affectsConfiguration: (section: string, scope?: zyraxoncode.ConfigurationScope) => {
	...
	getConfig<T>(key: Config<T>, scope?: zyraxoncode.ConfigurationScope): T {
		...
		const config = scope === undefined ? this.config : zyraxoncode.workspace.getConfiguration(CopilotConfigPrefix, scope);
	...
	inspectConfig<T>(key: BaseConfig<T>, scope?: zyraxoncode.ConfigurationScope): InspectConfigResult<T> | undefined {
		if (key.options?.valueIgnoredForExternals && !this._isInternal) {
	...
	override getNonExtensionConfig<T>(configKey: string): T | undefined {
		return zyraxoncode.workspace.getConfiguration().get<T>(configKey);
	}

	private _getTargetFromInspect(inspect?: { key: string; workspaceValue?: unknown; workspaceFolderValue?: unknown }): zyraxoncode.ConfigurationTarget {
		let target;
		...
		if (!inspect) {
			target = zyraxoncode.ConfigurationTarget.Global;
	...
	private _toZyraxonCodeConfigurationTarget(target: ConfigTarget): zyraxoncode.ConfigurationTarget {
		switch (target) {
	...
	async setConfig<T>(key: BaseConfig<T>, value: T, target?: ConfigTarget): Promise<void> {
		if (key.advancedSubKey) {
			...
			// Unfortunately, the configuration API of zyraxoncode is unable to write the flat style setting, it refuses to write them.
			...
			return this.config.update('advanced', currentValue, target === undefined ? this._getTargetFromInspect(this.config.inspect('advanced')) : this._toZyraxonCodeConfigurationTarget(target));
		}

		const inspect = this.config.inspect(key.id);
		if (value === undefined) {
			if (target !== undefined) {
				return this.config.update(key.id, value, this._toZyraxonCodeConfigurationTarget(target));
			}

			if (!inspect) {
				return this.config.update(key.id, value, zyraxoncode.ConfigurationTarget.Global);
			}

			if (inspect.workspaceFolderValue !== undefined) {
				await this.config.update(key.id, value, zyraxoncode.ConfigurationTarget.WorkspaceFolder);
			}

			if (inspect.workspaceValue !== undefined) {
				await this.config.update(key.id, value, zyraxoncode.ConfigurationTarget.Workspace);
			}

			if (inspect.globalValue !== undefined) {
				await this.config.update(key.id, value, zyraxoncode.ConfigurationTarget.Global);
			}

			return;
		}

		return this.config.update(key.id, value, target === undefined ? this._getTargetFromInspect(inspect) : this._toZyraxonCodeConfigurationTarget(target));
	}

	override getExperimentBasedConfig<T extends ExperimentBasedConfigType>(key: ExperimentBasedConfig<T>, experimentationService: IExperimentationService, scope?: zyraxoncode.ConfigurationScope): T {
	...
	private _getUserConfiguredValueForExperimentBasedConfig<T extends ExperimentBasedConfigType>(key: ExperimentBasedConfig<T>, scope?: zyraxoncode.ConfigurationScope): T | undefined {
		if (key.options?.valueIgnoredForExternals && !this._isInternal) {
			// If the setting is restricted to internal users and the user is not internal, we return the default value
			return undefined;
		}
	...
	override updateExperimentBasedConfiguration(treatments: string[]): void {
		...
		this._onDidChangeConfiguration.fire({
			affectsConfiguration: (section: string, _scope?: zyraxoncode.ConfigurationScope) => {
