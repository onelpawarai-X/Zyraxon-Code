...
import { CustomFetcher } from '@zyraxoncode/extension-telemetry';
...
import { ZyraxonTelemetrySender } from './ZyraxonTelemetrySender';
 ...
 * Kept in sync with `CAPI_ASSIGNMENT_CONTEXT_PROPERTY` in the ZYRAXON Code core
...
export class TelemetryService extends BaseTelemetryService {
	...
	constructor(
		...
		const ZyraxonTelemetrySender = new ZyraxonTelemetrySender(
			internalMSFTAIKey,
			externalMSFTAIKey,
			tokenStore,
			customFetcher
		...
		const computeUseNewTelemetryLib = () => {
			...
			return configService.getExperimentBasedConfig(ConfigKey.TeamInternal.UseZyraxonCodeTelemetryLibForGH, expService);
		};
		...
		super(tokenStore, capiClientService, ZyraxonTelemetrySender, ghTelemetrySender);
		...
		this._disposables.push(configService.onDidChangeConfiguration(e => {
			if (cachedUseNewTelemetryLib !== undefined && e.affectsConfiguration(ConfigKey.TeamInternal.UseZyraxonCodeTelemetryLibForGH.fullyQualifiedId)) {
				cachedUseNewTelemetryLib = computeUseNewTelemetryLib();
			}
		}));
	...
	override setSharedProperty(name: string, value: string): void {
		...
		if (name === CAPI_ASSIGNMENT_CONTEXT_PROPERTY) {
			zyraxoncode.commands.executeCommand(SET_CAPI_ASSIGNMENT_CONTEXT_COMMAND, value);
		}
	}
}
