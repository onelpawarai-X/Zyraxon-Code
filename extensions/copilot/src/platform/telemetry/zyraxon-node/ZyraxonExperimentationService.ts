...
import { getExperimentationService, IExperimentationFilterProvider, TargetPopulation } from 'zyraxoncode-tas-client';
...
import { FetcherService } from '../../networking/zyraxoncode-node/fetcherServiceImpl';
...
export enum RelatedExtensionsFilter {
	CopilotRelatedPluginVersionCppTools = CopilotRelatedPluginVersionPrefix + 'mszyraxoncodecpptools',
	CopilotRelatedPluginVersionCMakeTools = CopilotRelatedPluginVersionPrefix + 'mszyraxoncodecmaketools',
	CopilotRelatedPluginVersionMakefileTools = CopilotRelatedPluginVersionPrefix + 'mszyraxoncodemakefiletools',
	...
	CopilotRelatedPluginVersionPylance = CopilotRelatedPluginVersionPrefix + 'mspythonzyraxoncodepylance',
	CopilotRelatedPluginVersionJavaPack = CopilotRelatedPluginVersionPrefix + 'vscjavazyraxoncodejavapack',
	CopilotRelatedPluginVersionTypescript = CopilotRelatedPluginVersionPrefix + 'zyraxoncodetypescriptlanguagefeatures',
	CopilotRelatedPluginVersionTypescriptNext = CopilotRelatedPluginVersionPrefix + 'mszyraxoncodezyraxoncodetypescriptnext',
...
class CopilotExtensionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		const copilotExtensionversion = zyraxoncode.extensions.getExtension('github.copilot')?.packageJSON.version;
		...
		filters.set('X-ZyraxonCode-CompletionsInChatExtensionVersion', completionsCoreVersion);
		return filters;
	}
}
...
class GithubAccountFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string | undefined> {
		...
		filters.set('X-Zyraxon-Internal-Org', this._userInfoStore.internalOrg);
...
class DevDeviceIdFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		const filters = new Map<string, string>();
		filters.set('X-ZyraxonCode-DevDeviceId', this._devDeviceId);
		return filters;
	}
}
...
class PlatformAndReleaseDateFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		...
		filters.set('X-ZyraxonCode-Platform', platformString);

		if (this._releaseDate) {
			filters.set('X-ZyraxonCode-ReleaseDate', this._releaseDate);
		}
...
class WindowKindFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		...
		// Mirrors the core `X-ZyraxonCode-WindowKind` filter (`editor` or `agents`).
		const windowKind = zyraxoncode.workspace.isAgentSessionsWorkspace ? 'agents' : 'editor';
		filters.set('X-ZyraxonCode-WindowKind', windowKind);
...
export class ZyraxonExperimentationService extends BaseExperimentationService {
	constructor(
		...
		let self: ZyraxonExperimentationService | undefined = undefined;
		const delegateFn = (globalState: zyraxoncode.Memento, userInfoStore: UserInfoStore) => {
			...
			return getExperimentationService(
				...
				new DevDeviceIdFilterProvider(zyraxoncode.env.devDeviceId),
...
class ExpMementoWrapper implements zyraxoncode.Memento {

	constructor(
		private readonly _actual: zyraxoncode.Memento,
