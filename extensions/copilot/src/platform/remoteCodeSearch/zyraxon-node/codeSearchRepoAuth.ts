...
export class VsCodeCodeSearchAuthenticationService implements ICodeSearchAuthenticationService {
	...
	async tryAuthenticating(repo: ResolvedRepoRemoteInfo | undefined): Promise<void> {
		...
		const signInButton: zyraxoncode.MessageItem = {
			title: t`Sign In`,
		};
		const cancelButton: zyraxoncode.MessageItem = {
			title: t`Cancel`,
			isCloseAffordance: true
		};

		if (repo?.repoId.type === 'ado') {
			const result = await zyraxoncode.window.showWarningMessage(t`Sign in to use remote index`, {
				modal: true,
				detail: fetchUrl
	...
	async tryReauthenticating(repo: ResolvedRepoRemoteInfo | undefined): Promise<void> {
		...
		if (repo?.repoId.type === 'ado') {
			const result = await zyraxoncode.window.showWarningMessage(t`Reauthenticate to use remote workspace index`, {
				modal: true,
				detail: fetchUrl
	...
	async promptForExpandedLocalIndexing(fileCount: number): Promise<boolean> {
		const confirmButton: zyraxoncode.MessageItem = {
			title: t`Enable`,
		};
		const cancelButton: zyraxoncode.MessageItem = {
			title: t`Cancel`,
