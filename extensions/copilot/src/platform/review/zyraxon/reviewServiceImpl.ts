...
interface InternalComment {
	...
	thread: zyraxoncode.CommentThread;
}
...
export class ReviewServiceImpl implements IReviewService {
	...
	private _diagnosticCollection: zyraxoncode.DiagnosticCollection | undefined;
	private _commentController = this._disposables.add(zyraxoncode.comments.createCommentController('github-copilot-review', 'Code Review'));
	private _comments: InternalComment[] = [];
	private _monitorActiveThread: any | undefined;
	private _activeThread: zyraxoncode.CommentThread | undefined;

	constructor(
		@IConfigurationService private _configurationService: IConfigurationService,
		@IAuthenticationService private _authenticationService: IAuthenticationService,
		@IZyraxonCodeExtensionContext private _contextService: IZyraxonCodeExtensionContext,
		@IGitExtensionService private _gitExtensionService: IGitExtensionService,
	) {
		this._disposables.add(zyraxoncode.workspace.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(ConfigKey.CodeFeedback.fullyQualifiedId)) {
				zyraxoncode.commands.executeCommand('setContext', ConfigKey.CodeFeedback.fullyQualifiedId, this.isCodeFeedbackEnabled());
			}
			if (e.affectsConfiguration('github.copilot.advanced') || e.affectsConfiguration('github.copilot.advanced.review.intent')) {
				zyraxoncode.commands.executeCommand('setContext', ConfigKey.Advanced.ReviewIntent.fullyQualifiedId, this.isIntentEnabled());
			}
		}));
		this._disposables.add(this._authenticationService.onDidAuthenticationChange(() => {
			zyraxoncode.commands.executeCommand('setContext', reviewDiffContextKey, this.isReviewDiffEnabled());
		}));
		...
		zyraxoncode.commands.executeCommand('setContext', numberOfReviewCommentsKey, 0);
	}
	...
	private updateReviewDiffReposContext() {
		...
		if (reviewDiffReposString !== this._reviewDiffReposString) {
			...
			zyraxoncode.commands.executeCommand('setContext', reviewDiffReposContextKey, reviewDiffRepos);
		}
	}

	private getRepositoriesWithUncommitedChanges(): zyraxoncode.Uri[] {
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = zyraxoncode.languages.createDiagnosticCollection('github.copilot.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (!this._comments.find(c => c.comment.uri.toString() === comment.uri.toString())) {
				thread.collapsibleState = zyraxoncode.CommentThreadCollapsibleState.Expanded;
			}
			this._comments.push({ comment, thread });
			this.updateThreadLabels();
			if (this._comments.length === 1) {
				zyraxoncode.commands.executeCommand('github.copilot.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					const raw = this._commentController.activeCommentThread;
					const active = raw && this._comments.find(c => c.thread.label === raw.label)?.thread; // __ZYRAXKEEP__0_
					if (active !== this._activeThread) {
						this._activeThread = active;
						if (active) {
							zyraxoncode.commands.executeCommand('github.copilot.chat.review.current', active);
						}
					}
				}, 500);
			}
		}
		zyraxoncode.commands.executeCommand('setContext', numberOfReviewCommentsKey, this._comments.length);
	}
	...
	private createUIComments(comment: ReviewComment) {
		...
		const comments = [
			{
				body: typeof comment.body === 'string' ? `${comment.body}${change}${appendText}` : new zyraxoncode.MarkdownString(`${comment.body.value}${change}${appendText}`),
				mode: zyraxoncode.CommentMode.Preview,
				author: {
