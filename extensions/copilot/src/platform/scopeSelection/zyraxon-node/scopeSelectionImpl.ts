...
import { IParserService, treeSitterOffsetRangeToZyraxonCodeRange, zyraxoncodeToTreeSitterOffsetRange } from '../../parser/node/parserService';
...
export class ScopeSelectorImpl implements IScopeSelector {
	...
	private async findEnclosingBlocks(document: TextDocumentSnapshot, range: Selection): Promise<IScope[] | undefined> {
		...
		return fineScopes?.map((scope) => {
			const range = treeSitterOffsetRangeToZyraxonCodeRange(document, scope);
