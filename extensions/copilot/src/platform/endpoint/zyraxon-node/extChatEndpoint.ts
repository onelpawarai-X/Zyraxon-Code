...
export class ExtensionContributedChatEndpoint implements IChatEndpoint {
	...
	constructor(
		private readonly languageModel: zyraxoncode.LanguageModelChat,
	...
	get maxOutputTokens(): number {
		// The ZYRAXON Code API doesn't expose max output tokens, use a reasonable default
		return 8192;
	}
	...
	public acquireTokenizer(): ITokenizer {
		// Use the extension-contributed tokenizer that leverages the ZYRAXON Code language model API
	...
	async makeChatRequest2({
		...
		const zyraxoncodeMessages = convertToApiChatMessage(messages, {
			ignoreStatefulMarker,
			summarizedAtRoundId,
		...
		const zyraxoncodeOptions: zyraxoncode.LanguageModelChatRequestOptions = {
		...
		try {
			const response = await this.languageModel.sendRequest(zyraxoncodeMessages, zyraxoncodeOptions, token);
			let text = '';
			...
			for await (const chunk of response.stream) {
				...
				} else if (chunk instanceof zyraxoncode.LanguageModelToolCallPart) {
					if (streamRecorder.callback) {
				...
				} else if (chunk instanceof zyraxoncode.LanguageModelDataPart) {
				...
				} else if (chunk instanceof zyraxoncode.LanguageModelThinkingPart) {
					if (streamRecorder.callback) {
...
export function convertToApiChatMessage(messages: Raw.ChatMessage[], options: ConvertToApiChatMessageOptions = {}): Array<zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2> {
	const apiMessages: Array<zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2> = [];
	for (const message of messages) {
		const apiContent: Array<zyraxoncode.LanguageModelTextPart | zyraxoncode.LanguageModelToolResultPart2 | zyraxoncode.LanguageModelToolCallPart | zyraxoncode.LanguageModelDataPart | zyraxoncode.LanguageModelThinkingPart> = [];
		...
		for (const contentPart of message.content) {
			if (contentPart.type === Raw.ChatCompletionContentPartKind.Text) {
				apiContent.push(new zyraxoncode.LanguageModelTextPart(contentPart.text));
				...
				if (contentPart.imageUrl.url.startsWith('data:')) {
					...
					if (match) {
						...
						apiContent.push(new zyraxoncode.LanguageModelDataPart(Buffer.from(base64Data, 'base64'), mimeType as ChatImageMimeType));
					}
				} else {
					// Not a base64 image
					continue;
				...
				if (options.emitCacheBreakpoints) {
					apiContent.push(new zyraxoncode.LanguageModelDataPart(new TextEncoder().encode('ephemeral'), CustomDataPartMimeTypes.CacheControl));
				}
				...
				if (statefulMarker
					...
					apiContent.push(new zyraxoncode.LanguageModelDataPart(encodeStatefulMarker(statefulMarker.modelId, statefulMarker.marker), CustomDataPartMimeTypes.StatefulMarker));
				}
				const thinkingData = rawPartAsThinkingData(contentPart);
				if (thinkingData) {
					apiContent.push(new zyraxoncode.LanguageModelThinkingPart(thinkingData.text, thinkingData.id, thinkingData.metadata));
				}
			}
		}
		...
		if (message.role === Raw.ChatRole.System || message.role === Raw.ChatRole.User) {
			apiMessages.push({
				role: message.role === Raw.ChatRole.System ? zyraxoncode.LanguageModelChatMessageRole.System : zyraxoncode.LanguageModelChatMessageRole.User,
				name: message.name,
				content: apiContent
			});
			...
			if (message.toolCalls) {
				for (const toolCall of message.toolCalls) {
					apiContent.push(new zyraxoncode.LanguageModelToolCallPart(toolCall.id, toolCall.function.name, JSON.parse(toolCall.function.arguments)));
				}
			}
			apiMessages.push({
				role: zyraxoncode.LanguageModelChatMessageRole.Assistant,
				name: message.name,
				content: apiContent
			});
		} else if (message.role === Raw.ChatRole.Tool) {
			const toolResultPart: zyraxoncode.LanguageModelToolResultPart2 = new zyraxoncode.LanguageModelToolResultPart2(
				message.toolCallId ?? '',
				apiContent
			);
			apiMessages.push({
				role: zyraxoncode.LanguageModelChatMessageRole.User,
				name: '',
