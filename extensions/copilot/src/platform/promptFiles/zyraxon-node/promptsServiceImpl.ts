...
import type { ChatCustomAgent, ChatHook, ChatInstruction, ChatPlugin, ChatSkill } from 'zyraxoncode';
...
export class PromptsServiceImpl extends Disposable implements IPromptsService {
	...
	constructor(
		...
		this._register(zyraxoncode.chat.onDidChangeCustomAgents(() => this._onDidChangeCustomAgents.fire()));
		this._register(zyraxoncode.chat.onDidChangeInstructions(() => this._onDidChangeInstructions.fire()));
		this._register(zyraxoncode.chat.onDidChangeSkills(() => this._onDidChangeSkills.fire()));
		this._register(zyraxoncode.chat.onDidChangeHooks(() => this._onDidChangeHooks.fire()));
		this._register(zyraxoncode.chat.onDidChangePlugins(() => this._onDidChangePlugins.fire()));
	}

	getCustomAgents(token: CancellationToken): Promise<readonly ChatCustomAgent[]> {
		return Promise.resolve(zyraxoncode.chat.getCustomAgents(token));
	}

	getSlashCommands(token: CancellationToken): Promise<readonly zyraxoncode.ChatSlashCommand[]> {
		return Promise.resolve(zyraxoncode.chat.getSlashCommands(token));
	}

	getInstructions(token: CancellationToken): Promise<readonly ChatInstruction[]> {
		return Promise.resolve(zyraxoncode.chat.getInstructions(token));
	}

	getSkills(token: CancellationToken): Promise<readonly ChatSkill[]> {
		return Promise.resolve(zyraxoncode.chat.getSkills(token));
	}

	getHooks(token: CancellationToken): Promise<readonly ChatHook[]> {
		return Promise.resolve(zyraxoncode.chat.getHooks(token));
	}

	getPlugins(token: CancellationToken): Promise<readonly ChatPlugin[]> {
		return Promise.resolve(zyraxoncode.chat.getPlugins(token));
	}
