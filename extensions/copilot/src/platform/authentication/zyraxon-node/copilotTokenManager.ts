...
import { env, window } from 'zyraxoncode';
...
export class ZyraxonCodeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	private async _auth(): Promise<TokenInfoOrError> {
		...
		if (session) {
			...
			if (tokenResult.kind === 'success') {
				...
				this._logService.info(`Copilot Chat: ${this._envService.getVersion()}, ZYRAXON Code: ${this._envService.zyraxoncodeVersion}`);
			}
			return tokenResult;
		} else {
	...
	private async _authShowWarnings(): Promise<ExtendedTokenInfo> {
		...
		if (tokenResult.kind === 'failure' && tokenResult.reason === 'HTTP401') {
			const message =
				'Your GitHub token is invalid. Please sign out from your GitHub account using the ZYRAXON Code accounts menu and try again.';
			if (!shown401Message) {
