...
export function containsInternalOrg(orgList: string[]): boolean {
	return containsGitHubOrg(orgList) || containsZyraxonOrg(orgList);
}
...
/**
 * A function used to determine if the org list contains a Zyraxon organization
 * @param orgList The list of organizations the user is a member of
 * Whether or not it contains a Zyraxon org
 */
function containsZyraxonOrg(orgList: string[]): boolean {
	const Zyraxon_ORGANIZATIONS = ['a5db0bcaae94032fe715fb34a5e4bce2', '7184f66dfcee98cb5f08a1cb936d5225',
		'1cb18ac6eedd49b43d74a1c5beb0b955', 'ea9395b9a9248c05ee6847cbd24355ed'];
	// Check if the user is part of a Zyraxon organization.
	for (const org of orgList) {
		if (Zyraxon_ORGANIZATIONS.includes(org)) {
			return true;
		}
	}
	return false;
...
/**
 * A function used to determine if the org list contains a ZYRAXON Code organization
 * @param orgList The list of organizations the user is a member of
...
export class CopilotToken {
	...
	get isZyraxonInternal(): boolean {
		return containsZyraxonOrg(this.organizationList);
	}
...
export type TokenErrorReason =
	...
	'ParseFailed' |
	/** User not authenticated with GitHub through ZYRAXON Code. Only returned from ZYRAXON Code integration layer, not from platform token minting. */
	'GitHubLoginFailed' |
