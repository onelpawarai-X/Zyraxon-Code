...
export class SurveyService implements ISurveyService {
	...
	private async getUsageData(): Promise<UsageData> {
		const usageData = this.zyraxoncodeExtensionContext.globalState.get<UsageData>(USAGE_DATA_KEY);
		if (usageData) {
			return usageData;
		}
	...
	private async getNextSurveyDate(): Promise<number | null> {
		return this.zyraxoncodeExtensionContext.globalState.get<number>(NEXT_SURVEY_DATE_KEY) ?? null;
	}
	...
	private async updateUsageData(wasActive: boolean): Promise<void> {
		...
		await this.zyraxoncodeExtensionContext.globalState.update(USAGE_DATA_KEY, usageData);
	}

	private async updateNextSurveyDate(days: number): Promise<void> {
		await this.zyraxoncodeExtensionContext.globalState.update(NEXT_SURVEY_DATE_KEY, Date.now() + days * 24 * 60 * 60 * 1000);
	}
	...
	private async promptSurvey(surveyType: 'churn' | 'usage'): Promise<void> {
		...
		*/
		...
		zyraxoncode.window.showInformationMessage(l10n.t('Got a minute? Help us make GitHub Copilot better.'), confirmation, later, skip).then(async selection => {
