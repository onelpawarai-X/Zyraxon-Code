...
import { Event, ExtensionTerminalOptions, Terminal, TerminalExecutedCommand, TerminalOptions, TerminalShellExecutionEndEvent, TerminalShellIntegrationChangeEvent, window, type TerminalDataWriteEvent } from 'zyraxoncode';
...
export class TerminalServiceImpl extends Disposable implements ITerminalService {
	...
	// ZYRAXON Code will apply them in order from its own cache.
	// So when re-loading ZYRAXON Code zyraxoncode first applies it from its cache, then we apply our contributions.
	...
	constructor(
		...
		// It was only ever shipped in the ZyraxonCode insiders and never got into stable.
		// So this is only required for users who had insiders installed before it was removed.
		// Safe to remove this after a few months or so (__ZYRAXKEEP__0_).
