...
export class NotebookService implements INotebookService {
	...
	async getVariables(notebook: Uri): Promise<VariablesResult[]> {
		if (!this._hasJupyterExtension()) {
			try {
				const results = await commands.executeCommand<Variable | VariablesResult>('zyraxoncode.executeNotebookVariableProvider', notebook);
				...
				this._logger.error(`Failed to get notebook variables (zyraxoncode.executeNotebookVariableProvider) for ${notebook.toString()}: ${_ex}`);
				return [];
			}
		}
				...
				if (cellChange.executionSummary) {
					...
					if (executionSummary.success) {
						...
						commands.executeCommand<DocumentSymbol[]>(
							'zyraxoncode.executeDocumentSymbolProvider',
