	...
	if (options.retryFallbacks && availableFetchers.length > 1) {
		...
		for (const fetcher of availableFetchers) {
			...
			if (fetcher !== availableFetchers[0]) {
				...
				*/
				telemetryService?.sendTelemetryEvent('fetcherFallback', { github: true, Zyraxon: true }, {
	...
	try {
		...
		if (fetcher.isNetworkProcessCrashedError(err)) {
			...
			try {
				...
				*/
				telemetryService?.sendTelemetryEvent('fetcherCrashRetry', { github: true, Zyraxon: true }, {
					fetcher: fetcherId,
					outcome: 'recovered',
