...
export class ZyraxonCodeImageServiceImpl extends ImageServiceImpl {
	constructor(
	...
	override async resizeImage(data: Uint8Array, mimeType: string): Promise<{ data: Uint8Array; mimeType: string }> {
		try {
			const result = await zyraxoncode.commands.executeCommand('_chat.resizeImage', data, mimeType);
