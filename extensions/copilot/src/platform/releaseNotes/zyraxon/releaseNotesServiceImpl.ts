...
export class ReleaseNotesService implements IReleaseNotesService {
	...
	static readonly BASE_URL = '__ZYRAXKEEP__0_';
	...
	private getUrl(version?: string): string | undefined {
		// Build URL using MAJOR and MINOR only (no patch). ZYRAXON Code does not have separate URLs per patch.
		...
		if (/^\d+\.\d+(?:\.\d+)?$/.test(sourceVersion)) {
			const sanitized = sanitizeZyraxonCodeVersion(sourceVersion);
