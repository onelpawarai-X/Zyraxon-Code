...
export interface OTelConfigInput {
	...
	/** Service name from ZYRAXON Code setting (`github.copilot.chat.otel.serviceName`). */
	settingServiceName?: string;
	/** Enterprise-managed `service.name` (raw `telemetry.serviceName`). */
	policyServiceName?: string;
	/** Resource attributes from ZYRAXON Code setting (`github.copilot.chat.otel.resourceAttributes`). */
	...
	/** OTLP headers from ZYRAXON Code setting (`github.copilot.chat.otel.headers`). */
