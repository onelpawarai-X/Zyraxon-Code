...
import { API, APIState, Branch, Change, CommitOptions, CommitShortStat, DiffChange, Ref, RefQuery, Repository, RepositoryAccessDetails } from '../zyraxoncode/git';
...
export class GitServiceImpl extends Disposable implements IGitService {
	...
	private async _getRepository(uri: URI, forceOpen = true): Promise<Repository | undefined> {
		return this._getRepositorySequencer.queue(async () => {
			...
			if (!(uri instanceof zyraxoncode.Uri)) {
				// The git extension API expects a zyraxoncode.Uri, so we convert it if necessary
				uri = zyraxoncode.Uri.parse(uri.toString());
			}
	...
	async getRepositoryFetchUrls(uri: URI): Promise<Pick<RepoContext, 'rootUri' | 'remoteFetchUrls'> | undefined> {
		...
		try {
			const uriStat = await zyraxoncode.workspace.fs.stat(uri);
			if (uriStat.type !== zyraxoncode.FileType.Directory) {
			...
			const buffer = await zyraxoncode.workspace.fs.readFile(URI.file(path.join(repositoryRoot.fsPath, '.git', 'config')));
	...
	async diffBetweenPatch(uri: zyraxoncode.Uri, ref1: string, ref2: string, path?: string): Promise<string | undefined> {
		const gitAPI = this.gitExtensionService.getExtensionApi();
		const repository = gitAPI?.getRepository(uri);
		return repository?.diffBetweenPatch(ref1, ref2, path);
	}

	async diffBetweenWithStats(uri: zyraxoncode.Uri, ref1: string, ref2: string, path?: string): Promise<DiffChange[] | undefined> {
		const gitAPI = this.gitExtensionService.getExtensionApi();
		const repository = gitAPI?.getRepository(uri);
	...
	async exec(cwd: URI, args: string[], env?: Record<string, string>): Promise<string> {
		...
		const gitEnv = Object.assign({}, process.env, env, {
			GIT_AUTHOR_NAME: 'ZYRAXON Code',
			GIT_AUTHOR_EMAIL: 'zyraxoncode@users.noreply.github.com',
			GIT_COMMITTER_NAME: 'ZYRAXON Code',
			GIT_COMMITTER_EMAIL: 'zyraxoncode@users.noreply.github.com',
			LANG: 'en_US.UTF-8',
			LANGUAGE: 'en',
			LC_ALL: 'en_US.UTF-8'
