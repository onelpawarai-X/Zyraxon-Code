...
/**
 * Exports ZYRAXON Code chat session events to the cloud in real-time.
 *
...
export class RemoteSessionExporter extends Disposable implements IExtensionContribution, ISessionSyncStateService {
	...
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.sessionSync.deleteSessions', () => this._deleteCloudSessions()));

		// Register cloud-only delete for sessions window hook (fire-and-forget, no UI)
		this._register(zyraxoncode.commands.registerCommand('github.copilot.sessionSync.deleteSessionFromCloud', (sessionIds: string[]) => this._deleteSessionsFromCloud(sessionIds)));

		// Register suggest session sync command (called from chronicleIntent when user runs /chronicle)
		this._register(zyraxoncode.commands.registerCommand('github.copilot.sessionSync.suggest', () => this._suggestSessionSync()));

		// Register cloud reindex command (called from chronicleIntent after local reindex)
		this._register(zyraxoncode.commands.registerCommand('github.copilot.sessionSync.reindex', (reportProgress: (msg: string) => void, token: zyraxoncode.CancellationToken) => this._reindexCloud(reportProgress, token)));

		// Register user-facing reindex command (Command Palette)
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chronicle.reindex', () => this._reindexFromCommandPalette()));
		...
		const cloudEnabled = observableFromEventOpts(
			...
			handler => this._register(zyraxoncode.workspace.onDidChangeConfiguration(e => {
		...
		this._register(autorun(reader => {
			...
			if (!isLocalEnabled || !isCloudEnabled) {
				...
				if (isLocalEnabled && !isCloudEnabled) {
					const inspection = zyraxoncode.workspace.getConfiguration().inspect<boolean>('chat.sessionSync.enabled');
	...
	private _suggestSessionSync(): void {
		...
		zyraxoncode.window.showInformationMessage(
			zyraxoncode.l10n.t('Enable session sync for richer cross-device chat session history.'),
			zyraxoncode.l10n.t('Enable'),
			zyraxoncode.l10n.t('Don\'t Show Again'),
		).then(choice => {
			if (choice === zyraxoncode.l10n.t('Enable')) {
				zyraxoncode.commands.executeCommand('workbench.action.openSettings', 'chat.sessionSync.enabled');
			}
		});
	}
		...
		await zyraxoncode.window.withProgress(
			{
				...
				title: zyraxoncode.l10n.t('Reindexing sessions...'),
				cancellable: true,
			},
			async (progress, token) => {
				...
				progress.report({ message: zyraxoncode.l10n.t('{0} session(s) processed, {1} skipped', localResult.processed, localResult.skipped) });
				...
				if (localResult.processed === 0 && (!cloudResult || cloudResult.created === 0)) {
					zyraxoncode.window.showInformationMessage(
						zyraxoncode.l10n.t('Session index is up to date. {0} session(s) checked.', localResult.skipped)
					);
				} else if (cloudResult && cloudResult.created > 0) {
					zyraxoncode.window.showInformationMessage(
						zyraxoncode.l10n.t('{0} session(s) indexed locally, {1} synced to cloud.', localResult.processed, cloudResult.created)
					);
				} else {
					zyraxoncode.window.showInformationMessage(
						zyraxoncode.l10n.t('{0} session(s) indexed locally.', localResult.processed)
					);
				}
			},
		);
	...
	private async _deleteCloudSessions(): Promise<void> {
		...
		const quickPick = zyraxoncode.window.createQuickPick<SessionQuickPickItem>();
		quickPick.title = zyraxoncode.l10n.t('Delete Cloud Session Data');
		quickPick.placeholder = zyraxoncode.l10n.t('Loading sessions...');
		...
		if (this._cloudSessions.size === 0) {
			quickPick.dispose();
			zyraxoncode.window.showInformationMessage(zyraxoncode.l10n.t('No cloud-synced sessions found.'));
			return;
		}
		...
		if (rows.length === 0) {
			quickPick.dispose();
			zyraxoncode.window.showInformationMessage(zyraxoncode.l10n.t('No cloud-synced sessions found locally.'));
			return;
		}

		// Populate quick pick with items
		quickPick.busy = false;
		quickPick.placeholder = zyraxoncode.l10n.t('Select sessions to delete');
		quickPick.items = [
			{ label: '$(checklist) ' + zyraxoncode.l10n.t('Select All ({0} sessions)', rows.length), sessionId: selectAllId },
			...rows.map(row => {
		...
		type ScopeQuickPickItem = zyraxoncode.QuickPickItem & { deleteLocal: boolean };
		const scopeItems: ScopeQuickPickItem[] = [
			{ label: zyraxoncode.l10n.t('Delete from local and cloud'), description: zyraxoncode.l10n.t('Remove from local storage and the cloud'), deleteLocal: true },
			{ label: zyraxoncode.l10n.t('Delete from Cloud Only'), description: zyraxoncode.l10n.t('Keep local data, remove from the cloud'), deleteLocal: false },
		];
		const scopePick = await zyraxoncode.window.showQuickPick(scopeItems, {
			title: zyraxoncode.l10n.t('Where to Delete From?'),
			placeHolder: zyraxoncode.l10n.t('Choose deletion scope'),
		});
		...
		const confirmMessage = sessionsToDelete.length === 1
			? zyraxoncode.l10n.t('Are you sure you want to delete this session?')
			: zyraxoncode.l10n.t('Are you sure you want to delete {0} sessions?', sessionsToDelete.length);
		const confirmDetail = deleteLocal
			? zyraxoncode.l10n.t('This will delete session data locally and from the cloud. This action cannot be undone.')
			: zyraxoncode.l10n.t('This will delete session data from the cloud only. Local data will be kept. This action cannot be undone.');
		...
		await zyraxoncode.window.withProgress(
			{ location: zyraxoncode.ProgressLocation.Notification, title: zyraxoncode.l10n.t('Deleting sessions...') },
			async () => {
		...
		const parts: string[] = [];
		if (deleteLocal) {
			parts.push(zyraxoncode.l10n.t('{0} deleted locally', localDeleted));
		}
		if (cloudDeleted > 0) {
			parts.push(zyraxoncode.l10n.t('{0} deleted from cloud', cloudDeleted));
		}

		if (cloudErrors > 0) {
			zyraxoncode.window.showWarningMessage(parts.join(', ') + '. ' + zyraxoncode.l10n.t('{0} cloud deletion(s) failed.', cloudErrors));
			this._setSyncState({ kind: 'error' });
		} else {
			zyraxoncode.window.showInformationMessage(parts.join(', ') + '.');
			this._setSyncState({ kind: 'up-to-date', syncedCount: this._getLocalSyncedCount() });
		}
	...
	private _handleSpan(span: ICompletedSpanData): void {
		try {
			...
			// A real ZYRAXON Code chat session id is required to safely buffer CHAT spans;
	...
	private _showPolicyBlockedNotification(): void {
		...
		void zyraxoncode.window.showInformationMessage(
			zyraxoncode.l10n.t('Cloud sync for chat session insights is disabled for this workspace by your organization\'s policy.')
		);
	}
