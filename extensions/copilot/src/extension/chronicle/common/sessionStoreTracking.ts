...
const FILE_TRACKING_TOOLS = new Set([
	// ZYRAXON Code model-facing tool names (from ToolName enum)
 ...
 * Handles both CLI-style (edit/create with `path`) and ZYRAXON Code-style tools
...
export function extractFilePath(toolName: string, toolArgs: unknown): string | undefined {
	...
	// ZYRAXON Code tools use 'filePath', CLI tools use 'path', list_dir uses 'path',
 ...
 * Matches both ZYRAXON Code-style `mcp_github_*` and CLI-style `github-mcp-server-*` prefixes.
 */
