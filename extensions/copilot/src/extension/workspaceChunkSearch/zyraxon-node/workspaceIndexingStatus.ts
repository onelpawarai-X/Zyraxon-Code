...
const codebaseSemanticSearchDocsLink = '__ZYRAXKEEP__0_';
const externalIngestPolicyLink = '__ZYRAXKEEP__1_';
...
export class ChatStatusWorkspaceIndexingStatus extends Disposable {
	...
	private readonly _statusItem: zyraxoncode.ChatStatusItem;
	...
	constructor(
		...
		this._statusItem = this._register(zyraxoncode.window.createChatStatusItem('copilot.workspaceIndexStatus'));
	...
	private registerCommands(): IDisposable {
		...
		disposables.add(zyraxoncode.commands.registerCommand(reauthenticateCommandId, async (repo: ResolvedRepoRemoteInfo | undefined) => {
			if (!repo) {
				return;
			}
