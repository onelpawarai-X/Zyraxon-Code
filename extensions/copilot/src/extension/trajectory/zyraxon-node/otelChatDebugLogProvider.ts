...
export class OTelChatDebugLogProviderContribution extends Disposable implements IExtensionContribution {
	...
	/** Currently active ZYRAXON Code session ID */
	private _activeSessionId: string | undefined;

	/** Active progress callback for streaming events */
	private _activeProgress: zyraxoncode.Progress<zyraxoncode.ChatDebugEvent> | undefined;
	...
	constructor(
		...
		if (typeof zyraxoncode.chat?.registerChatDebugLogProvider !== 'function') {
			this._logService.info('[OTelDebug] Chat debug API not available, skipping registration');
			return;
		}

		try {
			this._register(zyraxoncode.chat.registerChatDebugLogProvider({
	...
	private _streamEvent(evt: zyraxoncode.ChatDebugEvent, dedupKey?: string): void {
	 ...
	 */
	private _prefixChildEventId(evt: zyraxoncode.ChatDebugEvent, entry: IDebugLogEntry): void {
	...
	private async _provideChatDebugLog(
		...
		progress: zyraxoncode.Progress<zyraxoncode.ChatDebugEvent>,
		token: zyraxoncode.CancellationToken,
	): Promise<zyraxoncode.ChatDebugEvent[]> {
	 ...
	 */
	private async _processEntries(entries: readonly IDebugLogEntry[], startTime: number): Promise<zyraxoncode.ChatDebugEvent[]> {
	 ...
	 */
	private _streamOlderEntries(sessionId: string, token: zyraxoncode.CancellationToken): void {
	...
	): zyraxoncode.ProviderResult<zyraxoncode.ChatDebugResolvedEventContent> {
	...
	private _resolveEntry(entry: IDebugLogEntry): zyraxoncode.ProviderResult<zyraxoncode.ChatDebugResolvedEventContent> {
	...
	private async _provideChatDebugLogExport(
		sessionResource: zyraxoncode.Uri,
		options: zyraxoncode.ChatDebugLogExportOptions,
		_token: zyraxoncode.CancellationToken,
		...
		const coreEntries = options.coreEvents
			.filter(e => e instanceof zyraxoncode.ChatDebugGenericEvent)
			.filter(e => {
				const id = (e as zyraxoncode.ChatDebugGenericEvent).id;
				return !id || !existingSpanIds.has(id);
			})
			.map((e): IDebugLogEntry => {
				const ge = e as zyraxoncode.ChatDebugGenericEvent;
				return {
					...
					status: ge.level === zyraxoncode.ChatDebugLogLevel.Error ? 'error' : 'ok',
					attrs: {
	...
	private _resolveChatDebugLogImport(
		data: Uint8Array,
		_token: zyraxoncode.CancellationToken,
	): zyraxoncode.ProviderResult<zyraxoncode.ChatDebugLogImportResult> {
		try {
			...
			const uri = zyraxoncode.Uri.parse(`__ZYRAXKEEP__0_{encoded}`);
	...
	private async _getAvailableDebugSessionResources(
		token: zyraxoncode.CancellationToken,
	): Promise<{ uri: zyraxoncode.Uri; title?: string }[]> {
		...
		try {
			...
			const settled = await Promise.allSettled(toRead.map(async (id): Promise<{ uri: zyraxoncode.Uri; title?: string } | undefined> => {
				if (token.isCancellationRequested) {
					return undefined;
				}
				const encoded = Buffer.from(id).toString('base64url');
				const uri = zyraxoncode.Uri.parse(`__ZYRAXKEEP__1_{encoded}`);
			...
			const results: { uri: zyraxoncode.Uri; title?: string }[] = [];
