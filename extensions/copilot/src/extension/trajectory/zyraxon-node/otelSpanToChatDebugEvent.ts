 ...
 */
export function completedSpanToDebugEvent(span: ICompletedSpanData): zyraxoncode.ChatDebugEvent | undefined {
 ...
 */
export function extractConversationEvents(spans: readonly ICompletedSpanData[]): zyraxoncode.ChatDebugEvent[] {
	const events: zyraxoncode.ChatDebugEvent[] = [];
	for (const span of spans) {
		...
		if (outputMessages) {
			...
			if (hasTextContent && summary) {
				const evt = new zyraxoncode.ChatDebugAgentResponseEvent(
 ...
 */
export function spanEventToUserMessage(event: ISpanEventData): zyraxoncode.ChatDebugUserMessageEvent | undefined {
	if (event.eventName !== 'user_message') {
		return undefined;
	}
	const content = asString(event.attributes.content as string) ?? '';
	const evt = new zyraxoncode.ChatDebugUserMessageEvent(
 ...
 */
export function resolveSpanToContent(span: ICompletedSpanData): zyraxoncode.ChatDebugResolvedEventContent | undefined {
	const opName = asString(span.attributes[GenAiAttr.OPERATION_NAME]);
 ...
 */
export function resolveUserMessageFromSpan(span: ICompletedSpanData): zyraxoncode.ChatDebugUserMessageEvent {
	const sections: zyraxoncode.ChatDebugMessageSection[] = [];

	// Build sections from dedicated attributes
	const systemInstr = asString(span.attributes[GenAiAttr.SYSTEM_INSTRUCTIONS]);
	if (systemInstr) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('System', systemInstr));
	}
	const promptContext = asString(span.attributes[CopilotChatAttr.PROMPT_CONTEXT]);
	if (promptContext) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Context', promptContext));
	}
	const promptInstructions = asString(span.attributes[CopilotChatAttr.PROMPT_INSTRUCTIONS]);
	if (promptInstructions) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Instructions', promptInstructions));
	}
	const userRequest = asString(span.attributes[CopilotChatAttr.USER_REQUEST]);
	if (userRequest) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('User Request', userRequest));
	}
			...
			try {
				...
				for (const msg of parsed) {
					...
					const textContent = msg.parts
						...
						.join('\n');
					if (textContent) {
						sections.push(new zyraxoncode.ChatDebugMessageSection(
	...
	if (sections.length === 0) {
		const inputMessages = asString(span.attributes[GenAiAttr.INPUT_MESSAGES]);
		if (inputMessages) {
			sections.push(new zyraxoncode.ChatDebugMessageSection('Input Messages', inputMessages));
		}
	}

	const summary = userRequest ?? 'User Message';
	const evt = new zyraxoncode.ChatDebugUserMessageEvent(truncate(summary, 200), new Date(span.startTime));
 ...
 */
export function resolveAgentResponseFromSpan(span: ICompletedSpanData): zyraxoncode.ChatDebugAgentResponseEvent {
	const sections: zyraxoncode.ChatDebugMessageSection[] = [];
		...
		try {
			...
			for (const msg of parsed) {
				...
				const textContent = msg.parts
					...
					.join('\n');
				if (textContent) {
					sections.push(new zyraxoncode.ChatDebugMessageSection('Response', textContent));
				}
				...
				if (toolCalls.length > 0) {
					...
					sections.push(new zyraxoncode.ChatDebugMessageSection('Tool Calls', toolSection));
				}
			}
		} catch { /* invalid JSON, skip */ }
	}
	...
	if (reasoning) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Reasoning', reasoning));
	}

	const agentName = asString(span.attributes[GenAiAttr.AGENT_NAME])
		?? asString(span.attributes[GenAiAttr.RESPONSE_MODEL]);
	const summary = extractAgentResponseSummary(outputMessages ?? '', agentName);
	const evt = new zyraxoncode.ChatDebugAgentResponseEvent(truncate(summary, 200), new Date(span.endTime));
	evt.id = agentMsgId(span.spanId);
	evt.sections = sections;
	return evt;
}
...
function spanToToolCallEvent(span: ICompletedSpanData): zyraxoncode.ChatDebugToolCallEvent {
	...
	const evt = new zyraxoncode.ChatDebugToolCallEvent(displayName, new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
	...
	evt.result = span.status.code === SpanStatusCode.OK
		? zyraxoncode.ChatDebugToolCallResult.Success
		: span.status.code === SpanStatusCode.ERROR
			? zyraxoncode.ChatDebugToolCallResult.Error
			: undefined;
	evt.durationInMillis = span.endTime - span.startTime;
	return evt;
}

function spanToModelTurnEvent(span: ICompletedSpanData): zyraxoncode.ChatDebugModelTurnEvent {
	const evt = new zyraxoncode.ChatDebugModelTurnEvent(new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
...
function spanToSubagentEvent(span: ICompletedSpanData): zyraxoncode.ChatDebugSubagentInvocationEvent {
	...
	const evt = new zyraxoncode.ChatDebugSubagentInvocationEvent(displayName, new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
	evt.durationInMillis = span.endTime - span.startTime;
	...
	evt.status = span.status.code === SpanStatusCode.OK
		? zyraxoncode.ChatDebugSubagentStatus.Completed
		: span.status.code === SpanStatusCode.ERROR
			? zyraxoncode.ChatDebugSubagentStatus.Failed
			: zyraxoncode.ChatDebugSubagentStatus.Running;
	const turnCount = asNumber(span.attributes[CopilotChatAttr.TURN_COUNT]);
	evt.modelTurnCount = turnCount;
	return evt;
}

function resolveHookExecutionContent(span: ICompletedSpanData): zyraxoncode.ChatDebugEventHookContent {
	const hookType = asString(span.attributes[CopilotChatAttr.HOOK_TYPE]) ?? 'unknown';
	const content = new zyraxoncode.ChatDebugEventHookContent(hookType);
	...
	content.result = resultKind === 'success'
		? zyraxoncode.ChatDebugHookResult.Success
		: resultKind === 'error'
			? zyraxoncode.ChatDebugHookResult.Error
			: resultKind === 'non_blocking_error'
				? zyraxoncode.ChatDebugHookResult.NonBlockingError
				: undefined;
...
function spanToHookExecutionEvent(span: ICompletedSpanData): zyraxoncode.ChatDebugGenericEvent {
	const hookType = asString(span.attributes[CopilotChatAttr.HOOK_TYPE]) ?? 'unknown';
	...
	const level = resultKind === 'error'
		? zyraxoncode.ChatDebugLogLevel.Error
		: resultKind === 'non_blocking_error'
			? zyraxoncode.ChatDebugLogLevel.Warning
			: zyraxoncode.ChatDebugLogLevel.Info;
	const evt = new zyraxoncode.ChatDebugGenericEvent(name, level, new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
 ...
 */
function spanToSdkHookEvent(span: ICompletedSpanData): zyraxoncode.ChatDebugGenericEvent {
	...
	const level = isError ? zyraxoncode.ChatDebugLogLevel.Error : zyraxoncode.ChatDebugLogLevel.Info;
	const evt = new zyraxoncode.ChatDebugGenericEvent(`Hook: ${hookType}`, level, new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
...
function spanToGenericEvent(span: ICompletedSpanData): zyraxoncode.ChatDebugGenericEvent {
	const name = asString(span.attributes[CopilotChatAttr.DEBUG_NAME]) ?? span.name;
	const evt = new zyraxoncode.ChatDebugGenericEvent(name, zyraxoncode.ChatDebugLogLevel.Info, new Date(span.startTime));
	evt.id = span.spanId;
	evt.parentEventId = span.parentSpanId;
...
function resolveToolCallContent(span: ICompletedSpanData): zyraxoncode.ChatDebugEventToolCallContent {
	const toolName = asString(span.attributes[GenAiAttr.TOOL_NAME]) ?? 'unknown';
	const content = new zyraxoncode.ChatDebugEventToolCallContent(toolName);
...
function resolveModelTurnContent(span: ICompletedSpanData): zyraxoncode.ChatDebugEventModelTurnContent {
	const requestName = asString(span.attributes[CopilotChatAttr.DEBUG_NAME])
		?? asString(span.attributes[GenAiAttr.AGENT_NAME])
		?? span.name;
	const content = new zyraxoncode.ChatDebugEventModelTurnContent(requestName);
	...
	if (requestShape) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Request Shape', requestShape));
	}
	const toolDefinitions = asString(span.attributes[GenAiAttr.TOOL_DEFINITIONS]);
	if (toolDefinitions) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Tools', toolDefinitions));
	}
	const outputMessages = asString(span.attributes[GenAiAttr.OUTPUT_MESSAGES]);
	if (outputMessages) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Output Messages', outputMessages));
	}
	if (sections.length > 0) {
 ...
 *   entries because ZYRAXON Code core is already displaying them for live sessions.
 *   Set to false for historical sessions where core won't fire those events.
 */
export function debugLogEntryToDebugEvent(entry: IDebugLogEntry, skipCoreEvents = true): zyraxoncode.ChatDebugEvent | undefined {
	switch (entry.type) {
		case 'tool_call':
		...
		case 'discovery':
			...
			// by ZYRAXON Code core via onDidReceiveChatDebugEvent for live sessions.
		...
		case 'generic':
			...
			// by ZYRAXON Code core directly for live sessions.
 ...
 * spanId collisions from OTel counter resets across ZYRAXON Code restarts.
 */
export function entryDedupKey(entry: IDebugLogEntry): string {
	return `${entry.type}:${entry.spanId}:${entry.ts}`;
}

function entryToToolCallEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugToolCallEvent {
	...
	const evt = new zyraxoncode.ChatDebugToolCallEvent(displayName, new Date(entry.ts));
	evt.id = entry.spanId;
...
function entryToModelTurnEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugModelTurnEvent {
	const evt = new zyraxoncode.ChatDebugModelTurnEvent(new Date(entry.ts));
	evt.id = entry.spanId;
	evt.parentEventId = entry.parentSpanId;
...
function entryToUserMessageEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugUserMessageEvent {
...
function entryToAgentResponseEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugAgentResponseEvent {
...
function entryToSubagentEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugSubagentInvocationEvent {
	const agentName = (entry.attrs.agentName as string) ?? entry.name;
	const evt = new zyraxoncode.ChatDebugSubagentInvocationEvent(entry.name, new Date(entry.ts));
	evt.id = entry.spanId;
	evt.parentEventId = entry.parentSpanId;
	evt.durationInMillis = entry.dur;
...
function entryToHookEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugGenericEvent {
	...
	const level = resultKind === 'error'
		...
		: resultKind === 'non_blocking_error'
			? zyraxoncode.ChatDebugLogLevel.Warning
			: zyraxoncode.ChatDebugLogLevel.Info;
	const evt = new zyraxoncode.ChatDebugGenericEvent(`Hook: ${entry.name}`, level, new Date(entry.ts));
	evt.id = entry.spanId;
	evt.parentEventId = entry.parentSpanId;
...
function entryToChildSessionRefEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugGenericEvent | undefined {
	...
	const evt = new zyraxoncode.ChatDebugGenericEvent(label, zyraxoncode.ChatDebugLogLevel.Info, new Date(entry.ts));
	evt.id = entry.spanId;
	evt.parentEventId = entry.parentSpanId;
...
function entryToGenericEvent(entry: IDebugLogEntry): zyraxoncode.ChatDebugGenericEvent {
	const level = entry.status === 'error'
		? zyraxoncode.ChatDebugLogLevel.Error
		: zyraxoncode.ChatDebugLogLevel.Info;
	const evt = new zyraxoncode.ChatDebugGenericEvent(entry.name, level, new Date(entry.ts));
	evt.id = entry.spanId;
	evt.parentEventId = entry.parentSpanId;
...
): Promise<zyraxoncode.ChatDebugResolvedEventContent | undefined> {
	switch (entry.type) {
		case 'tool_call':
...
function resolveToolCallEntry(entry: IDebugLogEntry): zyraxoncode.ChatDebugEventToolCallContent {
	const content = new zyraxoncode.ChatDebugEventToolCallContent(entry.name);
...
async function resolveModelTurnEntry(
	entry: IDebugLogEntry,
	readCompanionFile?: (fileName: string) => Promise<string | undefined>,
): Promise<zyraxoncode.ChatDebugEventModelTurnContent> {
	const content = new zyraxoncode.ChatDebugEventModelTurnContent(entry.name);
	...
	if (systemPromptFile && readCompanionFile) {
		...
		if (systemPrompt) {
			sections.push(new zyraxoncode.ChatDebugMessageSection('System', systemPrompt));
		}
	}
	...
	if (toolsFile && readCompanionFile) {
		...
		if (tools) {
			sections.push(new zyraxoncode.ChatDebugMessageSection('Tools', tools));
		}
	}
...
function resolveUserMessageEntry(entry: IDebugLogEntry): zyraxoncode.ChatDebugUserMessageEvent {
	const content = (entry.attrs.content as string) ?? '';
	const evt = new zyraxoncode.ChatDebugUserMessageEvent(truncate(content, 200), new Date(entry.ts));
	evt.id = entry.spanId;
	if (content) {
		evt.sections = [new zyraxoncode.ChatDebugMessageSection('User Request', content)];
	}
	return evt;
}

function resolveAgentResponseEntry(entry: IDebugLogEntry): zyraxoncode.ChatDebugAgentResponseEvent {
	const response = (entry.attrs.response as string) ?? '';
	const reasoning = entry.attrs.reasoning as string | undefined;
	const sections: zyraxoncode.ChatDebugMessageSection[] = [];

	if (response) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Response', response));
	}
	if (reasoning) {
		sections.push(new zyraxoncode.ChatDebugMessageSection('Reasoning', reasoning));
	}

	const evt = new zyraxoncode.ChatDebugAgentResponseEvent(truncate(response, 200), new Date(entry.ts));
	evt.id = entry.spanId;
	evt.sections = sections;
	return evt;
}

function resolveHookEntry(entry: IDebugLogEntry): zyraxoncode.ChatDebugEventHookContent {
