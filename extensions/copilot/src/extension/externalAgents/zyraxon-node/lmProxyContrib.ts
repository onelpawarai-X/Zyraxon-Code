...
export class LanguageModelProxyContrib extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		const providerDisposable = this._register(new MutableDisposable<zyraxoncode.Disposable>());
		const updateRegistration = () => {
			...
			if (!providerDisposable.value && enableProxy) {
				providerDisposable.value = zyraxoncode.lm.registerLanguageModelProxyProvider(instantiationService.createInstance(LanguageModelProxyProvider));
