...
/**
 * Strip the internal `__zyraxoncode-<number>` suffix that is appended to tool-call
 * IDs for uniqueness inside ZYRAXON Code.  The transcript should contain only the
 * original model-generated ID.
 */
function stripInternalToolCallId(id: string): string {
	return id.split('__zyraxoncode-')[0];
}
