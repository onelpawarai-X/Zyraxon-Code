 ...
 */
const compatibleHookEventNames: ReadonlyMap<zyraxoncode.ChatHookType, zyraxoncode.ChatHookType> = new Map([
	['Stop', 'SubagentStop'],
	['SessionStart', 'SubagentStart'],
]);

export function isCompatibleHookEventName(hookEventName: string, hookType: string): boolean {
	return hookEventName === hookType || compatibleHookEventNames.get(hookEventName as zyraxoncode.ChatHookType) === hookType;
}
...
export class ChatHookService implements IChatHookService {
	...
	async executeHook(hookType: zyraxoncode.ChatHookType, hooks: zyraxoncode.ChatRequestHooks | undefined, input: unknown, sessionId?: string, token?: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatHookResult[]> {
		if (!hooks) {
			return [];
		}
		...
		try {
			...
			let transcriptPath: zyraxoncode.Uri | undefined;
			if (sessionId) {
			...
			const results: zyraxoncode.ChatHookResult[] = [];
	...
	private _toHookResult(hookType: string, commandResult: IHookCommandResult): zyraxoncode.ChatHookResult {
	...
	async executePreToolUseHook(toolName: string, toolInput: unknown, toolCallId: string, hooks: zyraxoncode.ChatRequestHooks | undefined, sessionId?: string, token?: zyraxoncode.CancellationToken, outputStream?: zyraxoncode.ChatResponseStream): Promise<IPreToolUseHookResult | undefined> {
	...
	async executePostToolUseHook(toolName: string, toolInput: unknown, toolResponseText: string, toolCallId: string, hooks: zyraxoncode.ChatRequestHooks | undefined, sessionId?: string, token?: zyraxoncode.CancellationToken, outputStream?: zyraxoncode.ChatResponseStream): Promise<IPostToolUseHookResult | undefined> {
