...
interface IActiveLogSession {
	...
	/** Run index: 0 for the first run, incremented on each ZYRAXON Code restart that resumes this session */
	runIndex: number;
}
...
export class ChatDebugFileLoggerService extends Disposable implements IChatDebugFileLoggerService {
	...
	constructor(
		...
		if (typeof zyraxoncode.chat?.onDidReceiveChatDebugEvent === 'function') {
			this._register(zyraxoncode.chat.onDidReceiveChatDebugEvent(event => {
		...
		if (childInfo) {
			...
			this._bufferEntry(sessionId, {
				...
				attrs: {
					...
					zyraxoncodeVersion: this._envService.zyraxoncodeVersion,
					parentSessionId: childInfo.parentSessionId,
					label: childInfo.label,
				},
			});
	...
	private _onSpanCompleted(span: ICompletedSpanData): void {
		...
		// ZYRAXON Code restart where title/categorization won't re-fire.
		const session = this._activeSessions.get(sessionId);
	...
	private _onCoreDebugEvent(event: zyraxoncode.ChatDebugEvent): void {
		...
		const entry: IDebugLogEntry = {
			...
			status: event.level === zyraxoncode.ChatDebugLogLevel.Error ? 'error' : 'ok',
			attrs: {
