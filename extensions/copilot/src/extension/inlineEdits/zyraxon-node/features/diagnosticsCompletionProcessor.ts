...
import { IZyraxonCodeObservableDocument, ZyraxonCodeWorkspace } from '../parts/zyraxoncodeWorkspace';
...
export class DiagnosticsCompletionProcessor extends Disposable {
	...
	static get documentSelector(): zyraxoncode.DocumentSelector {
	...
	constructor(
		private readonly _workspace: ZyraxonCodeWorkspace,
		git: ObservableGit,
		...
		const isValidEditor = (editor: zyraxoncode.TextEditor | undefined): editor is zyraxoncode.TextEditor => {
		...
		this._register(zyraxoncode.window.onDidChangeTextEditorSelection(async e => {
			const activeEditor = this._tabsAndEditorsService.activeTextEditor;
	...
	private async _updateState(): Promise<void> {
		...
		const range = new zyraxoncode.Range(activeTextEditor.selection.active, activeTextEditor.selection.active);
	...
	private _getDiagnostics(workspaceDocument: IZyraxonCodeObservableDocument, cursor: Position, logContext: DiagnosticInlineEditRequestLogContext): Diagnostic[] {
	...
	private async _runCompletionHandler(workspaceDocument: IZyraxonCodeObservableDocument, diagnosticsSorted: Diagnostic[], cursor: Position, log: DiagnosticInlineEditRequestLogContext, token: CancellationToken): Promise<IDiagnosticsCompletionState> {
	...
	private async _getCompletionFromDiagnostics(workspaceDocument: IZyraxonCodeObservableDocument, diagnosticsSorted: Diagnostic[], pos: Position, logContext: DiagnosticInlineEditRequestLogContext, token: CancellationToken, tb: DiagnosticsCompletionHandlerTelemetry): Promise<DiagnosticCompletionItem | null> {
	...
	private async _fetchDiagnosticsBasedCompletions(workspaceDocument: IZyraxonCodeObservableDocument, sortedDiagnostics: Diagnostic[], pos: Position, logContext: DiagnosticInlineEditRequestLogContext, token: CancellationToken): Promise<DiagnosticCompletionItem[]> {
		const providers = this._diagnosticsCompletionProviders.get();
	...
	public handleEndOfLifetime(completionItem: DiagnosticCompletionItem, reason: zyraxoncode.InlineCompletionEndOfLifeReason): void {
		...
		if (reason.kind === zyraxoncode.InlineCompletionEndOfLifeReasonKind.Rejected) {
			this._rejectDiagnosticCompletion(provider, completionItem);
		} else if (reason.kind === zyraxoncode.InlineCompletionEndOfLifeReasonKind.Accepted) {
	...
	private _filterDiagnosticsByRecentEditNearby(diagnostics: Diagnostic[], document: IZyraxonCodeObservableDocument): Diagnostic[] {
...
function isEditorFromEditorGrid(editor: zyraxoncode.TextEditor): boolean {
