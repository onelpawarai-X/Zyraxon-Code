...
import { IZyraxonCodeObservableDocument } from '../../parts/zyraxoncodeWorkspace';
...
export class ImportDiagnosticCompletionItem extends DiagnosticCompletionItem {
	...
	constructor(
		...
		workspaceDocument: IZyraxonCodeObservableDocument,
	...
	async updateNodeModules(): Promise<void> {
		...
		const workspaceNodeModules = await Promise.all(workspaceFolders.map(async folder => {
			try {
				...
				if (resourceStat.type !== zyraxoncode.FileType.Directory) {
					return new Set<string>();
				}

				// Read all node_modules directories
				const entries = await this._fileService.readDirectory(nodeModulesFolder);
				const directories = entries.filter(([_, type]) => type === zyraxoncode.FileType.Directory);
...
export class ImportDiagnosticCompletionProvider implements IDiagnosticCompletionProvider<ImportDiagnosticCompletionItem> {
	...
	public providesCompletionsForDiagnostic(workspaceDocument: IZyraxonCodeObservableDocument, diagnostic: Diagnostic, language: LanguageId, pos: Position): boolean {
	...
	async provideDiagnosticCompletionItem(workspaceDocument: IZyraxonCodeObservableDocument, sortedDiagnostics: Diagnostic[], pos: Position, logContext: DiagnosticInlineEditRequestLogContext, token: CancellationToken): Promise<ImportDiagnosticCompletionItem | null> {
	...
	private _getImportCodeActions(codeActions: CodeActionData[], workspaceDocument: IZyraxonCodeObservableDocument, diagnostic: Diagnostic, workspaceInfo: WorkspaceInformation): ImportCodeAction[] {
