...
import { InlineCompletionModelInfo, InlineCompletionProviderOption } from 'zyraxoncode';
...
import { registerUnificationCommands } from '../../completions-core/zyraxoncode-node/completionsServiceBridges';
import { GhostTextCompletionItem, GhostTextCompletionList } from '../../completions-core/zyraxoncode-node/extension/src/ghostText/ghostTextProvider';
import { CopilotInlineCompletionItemProvider } from '../../completions-core/zyraxoncode-node/extension/src/zyraxoncodeInlineCompletionItemProvider';
import { ICopilotInlineCompletionItemProviderService } from '../../completions/common/copilotInlineCompletionItemProviderService';
import { CompletionsCoreContribution } from '../../completions/zyraxoncode-node/completionsCoreContribution';
import { unificationStateObservable } from '../../completions/zyraxoncode-node/completionsUnificationContribution';
...
import { ZyraxonCodeWorkspace } from './parts/zyraxoncodeWorkspace';
...
export class JointCompletionsProviderContribution extends Disposable implements IExtensionContribution {
	...
	private readonly _workspace = derivedDisposable(this, _reader => {
		return this._instantiationService.createInstance(ZyraxonCodeWorkspace);
	});
		...
		this._register(autorun((reader) => { // FX
			...
			reader.store.add(autorun((reader) => {
				...
				if (!excludes.includes(JointCompletionsProviderContribution.NES_GROUP_ID) && this.inlineEditsEnabled.read(reader)) {
					...
					const recordingDirPath = join(this._zyraxoncodeExtensionContext.globalStorageUri.fsPath, 'logContextRecordings');
					...
					reader.store.add(zyraxoncode.commands.registerCommand(learnMoreCommandId, () => {
						this._envService.openExternal(URI.parse(learnMoreLink));
					}));

					reader.store.add(zyraxoncode.commands.registerCommand(clearCacheCommandId, () => {
						model.nextEditProvider.clearCache();
					}));

					reader.store.add(zyraxoncode.commands.registerCommand(reportNotebookNESIssueCommandId, () => {
						const activeNotebook = zyraxoncode.window.activeNotebookEditor;
						...
						void zyraxoncode.commands.executeCommand(reportFeedbackCommandId, { logContext });
					}));

					// Register expected edit capture commands
					reader.store.add(zyraxoncode.commands.registerCommand(captureExpectedStartCommandId, () => {
						void expectedEditCaptureController.startCapture('manual');
					}));

					reader.store.add(zyraxoncode.commands.registerCommand(captureExpectedConfirmCommandId, () => {
						void expectedEditCaptureController.confirmCapture();
					}));

					reader.store.add(zyraxoncode.commands.registerCommand(captureExpectedAbortCommandId, () => {
						void expectedEditCaptureController.abortCapture();
					}));

					reader.store.add(zyraxoncode.commands.registerCommand(captureExpectedSubmitCommandId, () => {
				...
				let completionsProvider: CopilotInlineCompletionItemProvider | undefined;
				{
					...
					void zyraxoncode.commands.executeCommand('setContext', 'github.copilot.extensionUnification.activated', extensionUnification);
				...
				reader.store.add(zyraxoncode.languages.registerInlineCompletionItemProvider(
					'*',
					singularProvider,
					{
...
type LastNesSuggestion = {
	docUri: zyraxoncode.Uri;
	docVersionId: number;
	docWithNesEditApplied: StringText;
	completionItem: NesCompletionItem;
};

class JointCompletionsProvider extends Disposable implements zyraxoncode.InlineCompletionItemProvider {

	private readonly _onDidChangeEmitter: zyraxoncode.EventEmitter<NesChangeHint> | undefined;
	public readonly onDidChange?: zyraxoncode.Event<NesChangeHint> | undefined;
	...
	constructor(
		...
		if (this._inlineEditProvider?.onDidChange) {
			this._onDidChangeEmitter = this._register(new zyraxoncode.EventEmitter<NesChangeHint>());
	...
	public async provideInlineCompletionItems(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: zyraxoncode.InlineCompletionContext, token: zyraxoncode.CancellationToken): Promise<SingularCompletionList | undefined> {
	...
	private async provideInlineCompletionItemsCursorEndOfLine(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: zyraxoncode.InlineCompletionContext, token: zyraxoncode.CancellationToken, logger: ILogger): Promise<SingularCompletionList | undefined> {
	...
	private async provideInlineCompletionItemsRegular(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: zyraxoncode.InlineCompletionContext, token: zyraxoncode.CancellationToken, logger: ILogger): Promise<SingularCompletionList | undefined> {
	...
	private async _provideInlineCompletionItemsRegular(
		{ document, docSnapshot }: { document: zyraxoncode.TextDocument; docSnapshot: StringText },
		position: zyraxoncode.Position,
		lastNesSuggestion: null | LastNesSuggestion,
		context: zyraxoncode.InlineCompletionContext,
		logger: ILogger,
		...
		// scoping the variables
		{
			...
			if (tokens.coreToken.isCancellationRequested) {
				...
				void setEndOfLifeReason(this._completionsProvider, undefined, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.TokenCancellation });
				void setEndOfLifeReason(this._inlineEditProvider, nesP, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.TokenCancellation });
		...
		if (suggestionsList === undefined) {
			logger.trace(`suggestions request was cancelled`);
			void setEndOfLifeReason(this._completionsProvider, completionsP, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.TokenCancellation });
			void setEndOfLifeReason(this._inlineEditProvider, nesP, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.TokenCancellation });
			tokens.completionsCts.cancel();
			tokens.nesCts.cancel();
			return undefined;
		...
		if (suggestionsList.type === 'nes' && suggestionsList.res && this.doesNesSuggestionAgree(docSnapshot, lastNesSuggestion.docWithNesEditApplied, (suggestionsList.res.items as NesCompletionItem[]).at(0))) {
			logger.trace('last NES suggestion agrees with the current suggestion, using NES');
			return this._returnNES(suggestionsList.res, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.NotTaken }, completionsP, sw, logger, tokens);
		}
	...
	private _invokeNESProvider(logger: ILogger, document: zyraxoncode.TextDocument, position: zyraxoncode.Position, enforceCacheDelay: boolean, context: zyraxoncode.InlineCompletionContext, ct: CancellationToken, sw: StopWatch) {
	...
	private _invokeCompletionsProvider(logger: ILogger, document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: zyraxoncode.InlineCompletionContext, ct: CancellationToken, sw: StopWatch) {
	...
	private async _returnCompletionsOrOtherwiseNES(
		...
		if (completionsR && completionsR.items.length > 0) {
			...
			if (filteredCompletionR.items.length === 0) {
				...
				return this._returnCompletions(filteredCompletionR, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.LostRace }, nesP, sw, logger, tokens);
			}
		}
		...
		if (nesR && nesR.items.length > 0) {
			...
			if (filteredNesR.items.length === 0) {
				...
				return this._returnNES(filteredNesR, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.NotTaken }, completionsP, sw, logger, tokens);
			}
		}

		// return empty completions
		return this._returnCompletions(completionsR, { kind: zyraxoncode.InlineCompletionsDisposeReasonKind.NotTaken }, nesP, sw, logger, tokens);
	}

	private _returnCompletions(
		completionsR: GhostTextCompletionList | undefined,
		nesDisposeReason: zyraxoncode.InlineCompletionsDisposeReason,
		nesP: Promise<NesCompletionList | undefined> | undefined,
		sw: StopWatch,
		logger: ILogger,
		tokens: { coreToken: CancellationToken; completionsCts: CancellationTokenSource; nesCts: CancellationTokenSource },
	...
	private _returnNES(
		...
		completionsDisposeReason: zyraxoncode.InlineCompletionsDisposeReason,
		completionsP: Promise<GhostTextCompletionList | undefined> | undefined,
		sw: StopWatch,
		logger: ILogger,
		tokens: { coreToken: CancellationToken; completionsCts: CancellationTokenSource; nesCts: CancellationTokenSource },
	...
	private static retainOnlyMeaningfulEdits<T extends zyraxoncode.InlineCompletionList>(docSnapshot: StringText, list: T): T {
		// meaningful = not noop
	...
	public handleDidPartiallyAcceptCompletionItem?(completionItem: SingularCompletionItem, acceptedLength: number & zyraxoncode.PartialAcceptInfo): void {
		switch (completionItem.source) {
			case 'completions':
	...
	public handleEndOfLifetime?(completionItem: SingularCompletionItem, reason: zyraxoncode.InlineCompletionEndOfLifeReason): void {
		switch (completionItem.source) {
			case 'completions':
	...
	public handleListEndOfLifetime?(list: SingularCompletionList, reason: zyraxoncode.InlineCompletionsDisposeReason): void {
		switch (list.source) {
			case 'completions':
...
function applyTextEdit(doc: StringText, range: zyraxoncode.Range, insertText: string): string {
...
async function setEndOfLifeReason(provider: zyraxoncode.InlineCompletionItemProvider | undefined, promise: Promise<zyraxoncode.InlineCompletionList | undefined> | undefined, reason: zyraxoncode.InlineCompletionsDisposeReason) {
	...
	for (const item of result.items) {
		provider?.handleEndOfLifetime?.(item, { kind: zyraxoncode.InlineCompletionEndOfLifeReasonKind.Ignored, userTypingDisagreed: false });
	}
