...
export class InlineCompletionProviderImpl extends Disposable implements InlineCompletionItemProvider {
	...
	// copied from `zyraxoncodeWorkspace.ts` `DocumentFilter#_enabledLanguages`
	...
	private async _provideInlineCompletionItems(
		...
		try {
			...
			// re-surface in any other form. Suppress here without evicting the cache entry â€”
	...
	private createCompletionItem(
		doc: IZyraxonCodeObservableDocument,
		document: TextDocument,
		range: Range,
	...
	private async _trackSurvivalRate(item: LlmCompletionInfo) {
		...
		this._instantiationService.createInstance(
			...
			res => {
				...
				*/
				this._telemetryService.sendTelemetryEvent('reportInlineEditSurvivalRate', { Zyraxon: true, github: { eventNamePrefix: 'copilot-nes/' } },
					{
...
function hasNotebookCellMarker(document: TextDocument, newText: string) {
	return isNotebookCell(document.uri) && newText.includes('%% zyraxoncode.cell [id=');
}
...
function addNotebookTelemetry(document: TextDocument, position: Position, newText: string, documents: [TextDocument, Range][], telemetryBuilder: NextEditProviderTelemetryBuilder) {
	...
	const cellMarkerCount = newText.match(/%% zyraxoncode.cell \[id=/g)?.length || 0;
	const cellMarkerIndex = newText.indexOf('#%% zyraxoncode.cell [id=');
