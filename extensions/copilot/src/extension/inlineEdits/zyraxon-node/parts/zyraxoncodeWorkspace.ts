...
export class ZyraxonCodeWorkspace extends ObservableWorkspace implements IDisposable {
	private readonly _openDocuments = observableValue<readonly IZyraxonCodeObservableDocument[], { added: readonly IZyraxonCodeObservableDocument[]; removed: readonly IZyraxonCodeObservableDocument[] }>(this, []);
	...
	constructor(
		...
		let lastDocs: Map<DocumentId, IZyraxonCodeObservableDocument> = new Map();
		this._store.add(autorun(reader => {
		...
		this._store.add(workspace.onDidChangeTextDocument(e => {
			...
			if (doc instanceof ZyraxonCodeObservableTextDocument) {
		...
		this._store.add(workspace.onDidChangeNotebookDocument(e => {
			...
			if (!doc || !e.contentChanges.length || doc instanceof ZyraxonCodeObservableTextDocument) {
				return;
			}
		...
		this._store.add(window.onDidChangeTextEditorSelection(e => {
			...
			const selections = doc instanceof ZyraxonCodeObservableTextDocument ?
		...
		this._store.add(window.onDidChangeTextEditorVisibleRanges(e => {
			...
			const visibleRanges = doc instanceof ZyraxonCodeObservableTextDocument ?
		...
		this._store.add(languages.onDidChangeDiagnostics(e => {
			e.uris.forEach(uri => {
				...
				const diagnostics = document instanceof ZyraxonCodeObservableTextDocument ?
	...
	private readonly _obsDocsByDocId = derived(this, reader => {
		...
		const map: Map<DocumentId, ZyraxonCodeObservableTextDocument | ZyraxonCodeObservableNotebookDocument> = new Map(obsDocs.map(d => [d.id, d]));
	...
	private readonly _zyraxoncodeTextDocuments: IObservable<readonly TextDocument[]> = getObservableTextDocumentList(this._useAlternativeNotebookFormat, this._markdownNotebookCells.value);
	private readonly _textDocsWithShouldTrackFlag = mapObservableArrayCached(this, this._zyraxoncodeTextDocuments, (doc, store) => {
		...
		const obsDoc = derived(this, reader => {
			...
			const document = new ZyraxonCodeObservableTextDocument(documentId, stringValueFromDoc(doc), doc.version, [], [], LanguageId.create(doc.languageId), [], doc);
	...
	private readonly _zyraxoncodeNotebookDocuments: IObservable<readonly NotebookDocument[]> = this.getNotebookDocuments();
	...
	private readonly _notebookDocsWithShouldTrackFlag = mapObservableArrayCached(this, this._zyraxoncodeNotebookDocuments, (doc, store) => {
		const shouldTrack = observableValue<boolean>(this, false);
		const updateShouldTrack = () => {
			// @ulugbekna: not sure if invoking `isCopilotIgnored` on every textDocument-edit event is a good idea
			// 	also not sure if we should be enforcing local copilot-ignore rules (vs only remote-exclusion rules)
		...
		const obsDoc = derived(this, reader => {
			...
			const document = new ZyraxonCodeObservableNotebookDocument(documentId, stringValueFromDoc(altNotebook), doc.version, selections ?? [], visibleRanges ?? [], LanguageId.create(language), diagnostics, doc, altNotebook);
			return document;
		}).recomputeInitiallyAndOnChange(store);
	...
	private _getDocumentByDocumentAndUpdateShouldTrack(uri: URI): ZyraxonCodeObservableTextDocument | ZyraxonCodeObservableNotebookDocument | undefined {
	...
	private _createTextDocumentDiagnosticData(document: ZyraxonCodeObservableTextDocument) {
		return languages.getDiagnostics(document.textDocument.uri).map(d => this._createDiagnosticData(d, document)).filter(isDefined);
	}

	private _createDiagnosticData(diagnostic: Diagnostic, doc: ZyraxonCodeObservableTextDocument): DiagnosticData | undefined {
	...
	private readonly _obsDocsWithUpdateIgnored = derived(this, reader => {
		...
		const map: Map<string, {
			...
			obsDoc: IObservable<ZyraxonCodeObservableTextDocument | ZyraxonCodeObservableNotebookDocument | undefined>;
	...
	*/
	public getDocumentByTextDocument(doc: TextDocument, reader?: IReader): IZyraxonCodeObservableDocument | undefined {
...
export interface IZyraxonCodeObservableDocument extends IObservableDocument {
	/**
...
abstract class AbstractZyraxonCodeObservableDocument {
...
class ZyraxonCodeObservableTextDocument extends AbstractZyraxonCodeObservableDocument implements IZyraxonCodeObservableDocument {
	constructor(
		id: DocumentId,
		value: StringText,
		versionId: number,
...
class ZyraxonCodeObservableNotebookDocument extends AbstractZyraxonCodeObservableDocument implements IZyraxonCodeObservableDocument {
	constructor(
		id: DocumentId,
		value: StringText,
		versionId: number,
...
function getObservableTextDocumentList(excludeNotebookCells: boolean, markdownCellUris: ResourceSet): IObservable<readonly TextDocument[]> {
	...
	}, () => excludeNotebookCells ? workspace.textDocuments.filter(doc => doc.uri.scheme !== Schemas.zyraxoncodeNotebookCell || markdownCellUris.has(doc.uri)) : workspace.textDocuments);
}
...
function toCodeActionData(codeAction: CodeAction, workspaceDocument: IZyraxonCodeObservableDocument, translateRange: (range: Range) => OffsetRange | undefined): CodeActionData {
...
function getCommandDiagnostics(codeAction: CodeAction, uri: URI, translateRange: (range: Range) => OffsetRange | undefined): DiagnosticData[] {
	...
	return coalesce(commandArgs.map(arg => {
		if (arg && typeof arg === 'object' && 'diagnostic' in arg) {
			...
			// @benibenj Can we not check `diagnostic instanceOf zyraxoncode.Diagnostic?
...
function getDocumentEdits(codeAction: CodeAction, workspaceDocument: IZyraxonCodeObservableDocument): TextReplacement[] | undefined {
	const edit = codeAction.edit;
	if (!edit) {
		return undefined;
	}

	if (workspaceDocument instanceof ZyraxonCodeObservableTextDocument) {
		return edit.get(workspaceDocument.id.toUri()).map(e => toInternalTextEdit(e.range, e.newText));
	} else if (workspaceDocument instanceof ZyraxonCodeObservableNotebookDocument) {
...
async function getQuickFixCodeActions(uri: Uri, range: Range, itemResolveCount: number): Promise<CodeAction[]> {
	return asPromise(
		() => commands.executeCommand<CodeAction[]>(
			'zyraxoncode.executeCodeActionProvider',
			uri,
			range,
