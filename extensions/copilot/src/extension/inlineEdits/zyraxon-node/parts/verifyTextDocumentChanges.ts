...
/**
 * Verifies that ZYRAXON Code content change API reports consistent document edits.
...
export class VerifyTextDocumentChanges extends Disposable {
	...
	constructor(
		...
		const allowedSchemes = new Set([
			'file',
			'zyraxoncode-notebook-cell',
			'untitled',
			// "zyraxoncode-local",
			// "zyraxoncode-chat-code-block",
			// "chat-editing-text-model",
			// "embedded-html",
			// "zyraxoncode-userdata",
			// "zyraxoncode-remote",
			// "git",
		]);
	...
	private _verifyDocumentStateConsistency(e: TextDocumentChangeEvent): void {
		...
		if (previousValue === undefined) {
			/* __GDPR__
				"zyraxoncode.contentChangeForUnknownDocument" : {
					"owner": "hediet",
					"comment": "Telemetry for verifying ZyraxonCode content change API consistency"
				}
			*/
			this._telemetryService.sendMSFTTelemetryEvent('zyraxoncode.contentChangeForUnknownDocument', {}, {});
			return;
		}
		...
		if (expectedText !== currentText) {
			/* __GDPR__
				"zyraxoncode.contentChangeInconsistencyDetected" : {
					"owner": "hediet",
					"comment": "Telemetry for verifying ZyraxonCode content change API consistency",
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent('zyraxoncode.contentChangeInconsistencyDetected', {
