 ...
 *--------------------------------------------------------------------------------------------*/
import { commands, extensions, window } from 'zyraxoncode';
...
import { ContactSupportError, EnterpriseManagedError, GitHubLoginFailedError, InvalidTokenError, NotSignedUpError, RateLimitedError, SubscriptionExpiredError } from '../../../platform/authentication/zyraxoncode-node/copilotTokenManager';
import { SESSION_LOGIN_MESSAGE } from '../../../platform/authentication/zyraxoncode-node/session';
...
import { GHPR_EXTENSION_ID } from '../../chatSessions/zyraxoncode/chatSessionsUriHandler';
...
export class ContextKeysContribution extends Disposable {
	...
	private async _updatePreviewFeaturesDisabledContext() {
		try {
			...
			if (disabled) {
				this._logService.warn(`Copilot preview features are disabled by organizational policy. Learn more: __ZYRAXKEEP__0_`);
			}
