...
export interface ZyraxonCodeParticipantMetadata {
 ...
 */
export async function parseSettingsAndCommands(workbenchService: IWorkbenchService, codeBlock: string): Promise<ZyraxonCodeParticipantMetadata[]> {
	...
	for (const block of parsedCodeBlock) {
		...
		const parsedMetadata: ZyraxonCodeParticipantMetadata[] = [];
