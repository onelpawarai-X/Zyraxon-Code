...
export class GitHubOrgCustomAgentProvider extends Disposable implements zyraxoncode.ChatCustomAgentProvider {
	...
	async provideCustomAgents(_context: unknown, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatResource[]> {
		try {
	...
	private async pollAgents(orgId: string): Promise<void> {
		try {
			// Convert ZYRAXON Code API options to internal options
