...
export class PromptFileContribution extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		if ('registerCustomAgentProvider' in zyraxoncode.chat) {
			...
			if (configurationService.getConfig(ConfigKey.EnableOrganizationCustomAgents)) {
				const githubOrgAgentProvider: zyraxoncode.ChatCustomAgentProvider = instantiationService.createInstance(new SyncDescriptor(GitHubOrgCustomAgentProvider));
				this._register(zyraxoncode.chat.registerCustomAgentProvider(githubOrgAgentProvider));
			}
			...
			this._register(zyraxoncode.chat.registerCustomAgentProvider(planProvider));

			// Register Ask agent provider for read-only Q&A mode
			const askProvider = instantiationService.createInstance(AskAgentProvider);
			this._register(zyraxoncode.chat.registerCustomAgentProvider(askProvider));

			// Register Explore agent provider for code research subagent
			const exploreProviderRegistration = this._register(new MutableDisposable<zyraxoncode.Disposable>());
			const updateExploreProvider = () => {
				...
				if (isEnabled) {
					if (!exploreProviderRegistration.value) {
						...
						const registration = zyraxoncode.chat.registerCustomAgentProvider(provider);
		...
		if ('registerInstructionsProvider' in zyraxoncode.chat) {
			// Only register the provider if the setting is enabled
			if (configurationService.getConfig(ConfigKey.EnableOrganizationInstructions)) {
				const githubOrgInstructionsProvider: zyraxoncode.ChatInstructionsProvider = instantiationService.createInstance(new SyncDescriptor(GitHubOrgInstructionsProvider));
				this._register(zyraxoncode.chat.registerInstructionsProvider(githubOrgInstructionsProvider));
			}
		}
	}
}
