...
export interface IGitHubOrgChatResourcesService extends IDisposable {
	 ...
	 */
	listCachedFiles(type: PromptsType, orgName: string): Promise<zyraxoncode.ChatResource[]>;
}
...
export class GitHubOrgChatResourcesService extends Disposable implements IGitHubOrgChatResourcesService {
	...
	private getCacheDir(orgName: string, type: PromptsType): zyraxoncode.Uri {
		const sanitizedOrg = this.sanitizeFilename(orgName);
		const subdirectory = getCacheSubdirectory(type);
		return zyraxoncode.Uri.joinPath(
	...
	private getCacheFileUri(orgName: string, type: PromptsType, filename: string): zyraxoncode.Uri {
		return zyraxoncode.Uri.joinPath(this.getCacheDir(orgName, type), filename);
	}
		...
		try {
			...
			for (const [filename, fileType] of files) {
				if (fileType === FileType.File && isValidFile(type, filename) && !exclude?.has(filename)) {
					await this.fileSystem.delete(zyraxoncode.Uri.joinPath(cacheDir, filename));
	...
	async listCachedFiles(type: PromptsType, orgName: string): Promise<zyraxoncode.ChatResource[]> {
		const resources: zyraxoncode.ChatResource[] = [];
		const cacheDir = this.getCacheDir(orgName, type);
		...
		try {
			...
			for (const [filename, fileType] of files) {
				if (fileType === FileType.File && isValidFile(type, filename)) {
					const fileUri = zyraxoncode.Uri.joinPath(cacheDir, filename);
