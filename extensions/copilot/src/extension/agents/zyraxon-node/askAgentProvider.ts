...
const BASE_ASK_AGENT_CONFIG: AgentConfig = {
	...
	tools: [
		...DEFAULT_READ_TOOLS,
		'zyraxoncode.mermaid-markdown-features/renderMermaidDiagram',
	],
 ...
 */
export class AskAgentProvider extends Disposable implements zyraxoncode.ChatCustomAgentProvider {
	readonly label = zyraxoncode.l10n.t('Ask Agent');
	...
	private async _writeCacheFile(content: string): Promise<zyraxoncode.Uri> {
		...
		const fileUri = zyraxoncode.Uri.joinPath(cacheDir, AskAgentProvider.AGENT_FILENAME);
...
- Use #tool:zyraxoncode/askQuestions to clarify ambiguous questions before researching
