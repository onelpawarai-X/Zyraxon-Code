...
import { commands, env, ExtensionContext, ExtensionMode, l10n as zyraxoncodeL10n } from 'zyraxoncode';
...
import { CopilotExtensionApi } from '../../api/zyraxoncode/extensionApi';
...
export async function baseActivate(configuration: IExtensionActivationConfiguration) {
	...
	// Check if the extension is running in a pre-release version of ZYRAXON Code
	...
	if (context.extension.packageJSON.isPreRelease && isStableVsCode) {
		// Prevent activation of the extension if the user is using a pre-release version in stable ZYRAXON Code
	...
	if (zyraxoncodeL10n.bundle) {
		l10n.config({ contents: zyraxoncodeL10n.bundle });
	}
