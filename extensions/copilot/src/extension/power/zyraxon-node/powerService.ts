...
export class PowerService extends Disposable implements IPowerService {
	...
	private _blocker: (zyraxoncode.Disposable & { readonly id: number }) | undefined;
	...
	constructor(
		...
		if (typeof zyraxoncode.env.power?.onDidSuspend === 'function') {
			this._register(zyraxoncode.env.power.onDidSuspend(() => this._onDidSuspend.fire()));
			this._register(zyraxoncode.env.power.onDidResume(() => this._onDidResume.fire()));
		}
	}
	...
	private async _startBlocker(): Promise<void> {
		...
		try {
			...
			this._blocker = await zyraxoncode.env.power.startPowerSaveBlocker('prevent-app-suspension');
