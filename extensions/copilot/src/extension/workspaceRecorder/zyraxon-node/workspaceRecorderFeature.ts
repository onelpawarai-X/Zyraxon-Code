...
import * as gitExt from '../../../platform/git/zyraxoncode/git';
...
import { editFromTextDocumentContentChangeEvents } from '../../inlineEdits/zyraxoncode-node/parts/common';
import { VirtualTextDocumentProvider } from '../../inlineEdits/zyraxoncode-node/utils/virtualTextDocumentProvider';
...
export class WorkspaceRecorderFeature extends Disposable {
	...
	async init(store: DisposableStore) {
		...
		const recordingDirPath = join(this._zyraxoncodeExtensionContext.globalStorageUri.fsPath, 'workspaceRecordings');
...
class InitializedWorkspaceRecorderFeature extends Disposable {
	...
	constructor(
		...
		const commandIdOpenRecordingFolder = 'zyraxoncodeCopilot.openRecordingFolder';
		this._register(zyraxoncode.commands.registerCommand(commandIdOpenRecordingFolder, () => {
			zyraxoncode.env.openExternal(zyraxoncode.Uri.file(recordingDirPath));
		}));

		const commandIdAddBookmark = 'zyraxoncodeCopilot.addRecordingBookmark';
		this._register(zyraxoncode.commands.registerCommand(commandIdAddBookmark, () => {
			for (const r of this.recorders.values()) {
				r.addBookmark();
			}
			zyraxoncode.window.showInformationMessage('Bookmark added to recording.');
		}));

		const doc = this._logProvider.createDocument('', 'current.recording.w.json');

		const commandIdViewRecording = 'zyraxoncodeCopilot.viewRecording';
		this._register(zyraxoncode.commands.registerCommand(commandIdViewRecording, async () => {
			const first = this.recorders.values().next().value;
			if (!first) {
				zyraxoncode.window.showInformationMessage('No recording found.');
				return;
			}
			...
			await zyraxoncode.commands.executeCommand('zyraxoncode.open', doc.uri);
		}));

		this._register(autorunWithStore((reader, store) => {
			if (!this.hasWorkspace.read(reader)) { return; }

			const item = store.add(zyraxoncode.window.createStatusBarItem(zyraxoncode.StatusBarAlignment.Right, 10000));
			...
			const md = new zyraxoncode.MarkdownString(lines.join('\n\n'));
			md.isTrusted = true;
		...
		this._register(zyraxoncode.workspace.onDidOpenTextDocument(e => {
		...
		this._register(zyraxoncode.workspace.onDidOpenNotebookDocument(async e => {
		...
		this._register(zyraxoncode.workspace.onDidCloseTextDocument(e => {
			const docUri = documentUriFromTextDocument(e);
			this.getWorkspaceRecorder(docUri)?.handleOnDidCloseTextDocument(docUri);
		}));

		for (const doc of zyraxoncode.workspace.textDocuments) {
...
function documentUriFromTextDocument(textDocument: zyraxoncode.TextDocument): SerializedDocumentId {
	if (textDocument.uri.scheme === Schemas.zyraxoncodeNotebookCell) {
		const notebookDocument = findNotebook(textDocument.uri, zyraxoncode.workspace.notebookDocuments);
		if (!notebookDocument) {
