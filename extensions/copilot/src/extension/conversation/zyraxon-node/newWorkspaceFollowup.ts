...
import { NewWorkspacePreviewFileSystemProvider } from '../../intents/zyraxoncode-node/newWorkspacePreviewFileSystemProvider';
import { NewWorkspaceTextDocumentProvider } from '../../intents/zyraxoncode-node/newWorkspaceTextDocumentProvider';
...
async function createWorkspace(logService: ILogService, workspaceRoot: Uri | undefined, parentFolder: Uri, fileTreePart: ChatResponseFileTreePart) {
	...
	try {
		...
		if (result === open) {
			...
			commands.executeCommand('zyraxoncode.openFolder', workspaceUri);
		} else if (result === openNewWindow) {
			commands.executeCommand('zyraxoncode.openFolder', workspaceUri, true);
		}
	}
	catch (error) {
...
function getAiGeneratedWorkspacesFile(): Uri | undefined {
	const zyraxoncodeFolderName = 'ZYRAXON Code';
	...
	switch (process.platform) {
		case 'darwin':
			return Uri.joinPath(
				...
				'Library',
				'Application Support',
				zyraxoncodeFolderName,
				'User',
				'workspaceStorage',
				'aiGeneratedWorkspaces.json'
			);
		case 'linux':
			return Uri.joinPath(homeDir, '.config', zyraxoncodeFolderName, 'User', 'workspaceStorage', 'aiGeneratedWorkspaces.json');
		case 'win32':
			return process.env.APPDATA
				? Uri.joinPath(Uri.file(process.env.APPDATA), zyraxoncodeFolderName, 'User', 'workspaceStorage', 'aiGeneratedWorkspaces.json')
				: undefined;
		default:
			return;
	}
