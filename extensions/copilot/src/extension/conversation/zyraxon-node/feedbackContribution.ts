...
export class FeedbackCommandContribution extends Disposable {
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.report', async (title: string = '') => {
			...
			await zyraxoncode.commands.executeCommand('workbench.action.openIssueReporter', {
				issueTitle: title,
				issueSource: 'zyraxoncode',
				data: output.join('\n'),
				privateUri: isTeamMember ? zyraxoncode.Uri.parse('__ZYRAXKEEP__0_') : undefined,
			});
		}));
	}
}
