...
export interface IUserFeedbackService {
	...
	handleUserAction(e: zyraxoncode.ChatUserActionEvent, participantId: string): void;
	handleFeedback(e: zyraxoncode.ChatResultFeedback, participantId: string): void;
}
...
export class UserFeedbackService implements IUserFeedbackService {
	...
	handleUserAction(e: zyraxoncode.ChatUserActionEvent, agentId: string): void {
		const document = zyraxoncode.window.activeTextEditor?.document;
		const selection = zyraxoncode.window.activeTextEditor?.selection;
		...
		switch (e.action.kind) {
			...
			case 'bug':
				if (conversation) {
					...
					zyraxoncode.window.showInformationMessage('Conversation not found, is it restored? Please try again.');
				}
				break;
			case 'chatEditingSessionAction':
				if (conversation instanceof Conversation) {
					...
					if (editCodeStep && (e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Accepted || e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Rejected)
					) {
						editCodeStep.setWorkingSetEntryState(e.action.uri, {
							accepted: e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Accepted,
					...
					*/
					this.telemetryService.sendMSFTTelemetryEvent('panel.edit.feedback', {
						...
						isNotebookCell: e.action.uri.scheme === Schemas.zyraxoncodeNotebookCell ? 1 : 0
					});
...

					{
						...
						if (e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Accepted
							|| e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Rejected) {
					...
					if (result.metadata?.responseId
						&& (e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Accepted
							|| e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Rejected)
					) {
						const outcome = e.action.outcome === zyraxoncode.ChatEditingSessionActionOutcome.Accepted ? 'accept' : 'reject';
		...
		if (e.action.kind === 'copy' || e.action.kind === 'insert') {
			...
			sendUserActionTelemetry(
				this.telemetryService,
				zyraxoncode.window.activeTextEditor?.document,
				{
	...
	private handleApplyAction(e: zyraxoncode.ChatApplyAction, agentId: string, result: ICopilotChatResultIn): void {
		sendUserActionTelemetry(
			this.telemetryService,
			zyraxoncode.window.activeTextEditor?.document,
			{
	...
	handleFeedback(e: zyraxoncode.ChatResultFeedback, agentId: string): void {
		const document = zyraxoncode.window.activeTextEditor?.document;
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('panel.action.vote', {
			...
			direction: e.kind === zyraxoncode.ChatResultFeedbackKind.Helpful ? 1 : 2, // map to previous enum values
		});
		...
		sendUserActionTelemetry(
			...
			document,
			{
				rating: e.kind === zyraxoncode.ChatResultFeedbackKind.Helpful ? 'positive' : 'negative',
				messageId: result.metadata?.modelMessageId ?? '',
				headerRequestId: result.metadata?.responseId ?? '',
			},
			{},
		...
		const otelRating = e.kind === zyraxoncode.ChatResultFeedbackKind.Helpful ? 'positive' : 'negative';
	...
	private _handleChatUserAction(sessionId: string | undefined, conversation: Conversation, event: zyraxoncode.ChatUserActionEvent) {
...
const outcomes = new Map<zyraxoncode.ChatEditingSessionActionOutcome, EditOutcome>([
	[zyraxoncode.ChatEditingSessionActionOutcome.Accepted, 'accepted'],
	[zyraxoncode.ChatEditingSessionActionOutcome.Rejected, 'rejected'],
	[zyraxoncode.ChatEditingSessionActionOutcome.Saved, 'saved']
]);
