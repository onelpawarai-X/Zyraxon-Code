...
import { ICombinedEmbeddingIndex } from '../../../platform/embeddings/common/zyraxoncodeIndex';
...
import { zyraxoncodeNodeChatContributions } from '../../extension/zyraxoncode-node/contributions';
import { IMergeConflictService } from '../../git/common/mergeConflictService';
import { registerInlineChatCommands } from '../../inlineChat/zyraxoncode-node/inlineChatCommands';
...
import { registerLinkCommands } from '../../linkify/zyraxoncode-node/commands';
import { InlineCodeSymbolLinkifier } from '../../linkify/zyraxoncode-node/inlineCodeSymbolLinkifier';
import { NotebookCellLinkifier } from '../../linkify/zyraxoncode-node/notebookCellLinkifier';
import { SymbolLinkifier } from '../../linkify/zyraxoncode-node/symbolLinkifier';
...
export class ConversationFeature implements IExtensionContribution {
	...
	constructor(
		...
		const refreshHasByokModels = async () => {
			try {
				const models = await zyraxoncode.lm.selectChatModels({});
		...
		this._disposables.add(zyraxoncode.lm.onDidChangeChatModels(() => void refreshHasByokModels()));
	...
	set enabled(value: boolean) {
		...
		zyraxoncode.commands.executeCommand('setContext', 'github.copilot.interactiveSession.disabled', !value);
	}
	...
	set activated(value: boolean) {
		...
		if (!value) {
			...
			this._activatedDisposables.add(this.instantiationService.createInstance(ContributionCollection, zyraxoncodeNodeChatContributions));
		}
	}
	...
	private registerSearchProvider(): IDisposable | undefined {
		if (this._searchProviderRegistered) {
			...
			return combinedDisposable(
				provider,
				zyraxoncode.workspace.registerAITextSearchProvider('file', provider),
			);
		}
	}
	...
	private registerSettingsSearchProvider(): IDisposable | undefined {
		...
		return zyraxoncode.ai.registerSettingsSearchProvider(this.settingsEditorSearchService);
	}
	...
	private registerParticipantDetectionProvider() {
		...
		if ('registerChatParticipantDetectionProvider' in zyraxoncode.chat) {
			const provider = this.instantiationService.createInstance(IntentDetector);
			return zyraxoncode.chat.registerChatParticipantDetectionProvider(provider);
		}
	}
	...
	private registerCommands(options: IConversationOptions): IDisposable {
		...
		[
			zyraxoncode.commands.registerCommand('github.copilot.interactiveSession.feedback', () => zyraxoncode.commands.executeCommand('github.copilot.report', 'Copilot chat feedback')),
			zyraxoncode.commands.registerCommand('github.copilot.chat.compact', () => zyraxoncode.commands.executeCommand('workbench.action.chat.open', { query: '/compact', preserveInput: true })),
			zyraxoncode.commands.registerCommand('github.copilot.terminal.explainTerminalLastCommand', async () => this.triggerTerminalChat({ query: `/${TerminalExplainIntent.intentName} #terminalLastCommand` })),
			zyraxoncode.commands.registerCommand('github.copilot.terminal.fixTerminalLastCommand', async () => generateTerminalFixes(this.instantiationService)),
			zyraxoncode.commands.registerCommand('github.copilot.terminal.generateCommitMessage', async () => {
				const workspaceFolders = zyraxoncode.workspace.workspaceFolders;

				if (!workspaceFolders?.length) {
					return;
				}
				const uri = workspaceFolders.length === 1 ? workspaceFolders[0].uri : await zyraxoncode.window.showWorkspaceFolderPick().then(folder => folder?.uri);
				if (!uri) {
					return;
				}
				...
				if (commitMessage) {
					...
					zyraxoncode.window.activeTerminal?.sendText(message, false);
				}
			}),
			zyraxoncode.commands.registerCommand('github.copilot.git.generateCommitMessage', async (rootUri: zyraxoncode.Uri | undefined, _: unknown, cancellationToken: zyraxoncode.CancellationToken | undefined) => {
			...
			zyraxoncode.commands.registerCommand('github.copilot.git.resolveMergeConflicts', async (...resourceStates: (zyraxoncode.Uri | zyraxoncode.SourceControlResourceState)[]) => {
			...
			zyraxoncode.commands.registerCommand('github.copilot.devcontainer.generateDevContainerConfig', async (args: DevContainerConfigGeneratorArguments, cancellationToken?: zyraxoncode.CancellationToken) => {
			...
			zyraxoncode.commands.registerCommand('github.copilot.chat.openUserPreferences', async () => {
	...
	private async triggerTerminalChat(options: { query: string; isPartialQuery?: boolean }) {
		...
		await zyraxoncode.commands.executeCommand(commandId, options);
	}

	private registerRelatedInformationProviders(): IDisposable {
		const disposables = new DisposableStore();
		[
			zyraxoncode.ai.registerRelatedInformationProvider(
				zyraxoncode.RelatedInformationType.CommandInformation,
				this.embeddingIndex.commandIdIndex
			),
			zyraxoncode.ai.registerRelatedInformationProvider(
				zyraxoncode.RelatedInformationType.SettingInformation,
	...
	private registerTerminalQuickFixProviders() {
		...
		return combinedDisposable(
			zyraxoncode.window.registerTerminalQuickFixProvider('copilot-chat.fixWithCopilot', {
					...
					return [
						{
							...
							title: zyraxoncode.l10n.t('Fix using Copilot')
						},
						{
							command: 'github.copilot.terminal.explainTerminalLastCommand',
							title: zyraxoncode.l10n.t('Explain using Copilot')
						}
					];
				}
			}),
			zyraxoncode.window.registerTerminalQuickFixProvider('copilot-chat.generateCommitMessage', {
				provideTerminalQuickFixes: (commandMatchResult, token) => {
					return this.enabled ? [{
						command: 'github.copilot.terminal.generateCommitMessage',
						title: zyraxoncode.l10n.t('Generate Commit Message')
					}] : [];
				},
			})
		);
...
function registerSearchIntentCommand(): IDisposable {
	return zyraxoncode.commands.registerCommand('github.copilot.executeSearch', async (arg: FindInFilesArgs) => {
		const show = arg.filesToExclude.length > 0 || arg.filesToInclude.length > 0;
		zyraxoncode.commands.executeCommand('workbench.view.search.focus').then(() =>
			zyraxoncode.commands.executeCommand('workbench.action.search.toggleQueryDetails', { show })
		);
		zyraxoncode.commands.executeCommand('workbench.action.findInFiles', arg);
	});
}
