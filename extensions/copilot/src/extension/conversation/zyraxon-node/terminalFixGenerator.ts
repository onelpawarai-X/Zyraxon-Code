...
export function setLastCommandMatchResult(value: zyraxoncode.TerminalCommandMatchResult) { lastCommandMatchResult = value; }
export let lastCommandMatchResult: zyraxoncode.TerminalCommandMatchResult | undefined;
...
export async function generateTerminalFixes(instantiationService: IInstantiationService) {
	...
	type CommandPick = zyraxoncode.QuickPickItem & { suggestion: ICommandSuggestion };
	const picksPromise: Promise<(CommandPick | zyraxoncode.QuickPickItem)[]> = new Promise(r => {
		instantiationService.createInstance(TerminalQuickFixGenerator).generateTerminalQuickFix(commandMatchResult, CancellationToken.None).then(fixes => {
			const picks: (CommandPick | zyraxoncode.QuickPickItem)[] = (fixes ?? []).sort((a, b) => b.relevance - a.relevance).map(e => ({
				label: e.command,
			...
			for (let i = 0; i < picks.length; i++) {
				...
				if (
					...
					picks.splice(i++, 0, { label: relevanceToString(currentRelevance), kind: zyraxoncode.QuickPickItemKind.Separator });
				}
			}
			r(picks);
		}, err => {
	...
	picksPromise.then(picks => {
		if (picks.length === 0) {
			zyraxoncode.window.showInformationMessage('No fixes found');
		}
	});
	const pick = zyraxoncode.window.createQuickPick<(zyraxoncode.QuickPickItem | CommandPick)>();
	...
	if (item && 'suggestion' in item) {
		...
		zyraxoncode.window.activeTerminal?.sendText(item.suggestion.command, shouldExecute);
	}
...
class TerminalQuickFixGenerator {
	...
	async generateTerminalQuickFix(commandMatchResult: zyraxoncode.TerminalCommandMatchResult, token: CancellationToken): Promise<ICommandSuggestion[] | undefined> {
		...
		for (const uri of unverifiedContextUris) {
			try {
				const exists = await zyraxoncode.workspace.fs.stat(uri);
				// This does not support binary files
				if (exists.type === zyraxoncode.FileType.File || exists.type === zyraxoncode.FileType.SymbolicLink) {
					verifiedContextUris.push(uri);
				} else if (exists.type === zyraxoncode.FileType.Directory) {
		...
		if (fetchResult.type !== 'success') {
			throw new Error(zyraxoncode.l10n.t('Encountered an error while determining terminal quick fixes: {0}', fetchResult.type));
		}
	...
	private async _generateTerminalQuickFixFileContext(commandMatchResult: zyraxoncode.TerminalCommandMatchResult, token: CancellationToken) {
		const endpoint = await this._endpointProvider.getChatEndpoint('copilot-utility-small');
		...
		if (fetchResult.type !== 'success') {
			throw new Error(zyraxoncode.l10n.t('Encountered an error while fetching quick fix file context: {0}', fetchResult.type));
		}
