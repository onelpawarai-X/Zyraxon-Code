...
export function startFeedbackCollection(accessor: ServicesAccessor) {
	...
	disposables.add(zyraxoncode.workspace.onDidChangeTextDocument(async event => {
		if (event.document.uri.scheme === 'file' && event.contentChanges.length && event.document === zyraxoncode.window.activeTextEditor?.document) {
			try {
				...
				if (result.type === 'success') {
					const diagnostics = result.comments.map(comment => new zyraxoncode.Diagnostic(comment.range, typeof comment.body === 'string' ? comment.body : comment.body.value, zyraxoncode.DiagnosticSeverity.Information));
	...
	disposables.add(zyraxoncode.workspace.onDidCloseTextDocument(doc => {
