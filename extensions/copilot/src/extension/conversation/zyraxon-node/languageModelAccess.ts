...
import { ChatLocation as ApiChatLocation, ExtensionMode } from '../../../zyraxoncodeTypes';
...
function getContextSizeOptions(endpoint: IChatEndpoint, preferLongContext: boolean): { value: number; description: string; isDefault: boolean }[] | undefined {
	...
	// When both tiers cost the same and the user prefers long context, show only the full window as a non-switchable indicator. See zyraxon/zyraxoncode#322950, zyraxon/zyraxoncode#323116.
	if (preferLongContext && !hasLongContextSurcharge) {
		return [
			{ value: fullMax, description: zyraxoncode.l10n.t('Longer sessions'), isDefault: true },
		];
	}

	return [
		{ value: defaultMax, description: zyraxoncode.l10n.t('Default recommended context size'), isDefault: true },
		{
			value: fullMax,
			description: zyraxoncode.l10n.t('Longer sessions'),
			isDefault: false,
		},
	];
}
...
function buildConfigurationSchema(endpoint: IChatEndpoint, preferLongContext: boolean): { configurationSchema?: zyraxoncode.LanguageModelConfigurationSchema } {
	...
	if (contextSizeOptions) {
		...
		properties.contextSize = {
			type: 'number',
			title: zyraxoncode.l10n.t('Context Size'),
...
/**
 * Builds the {@link zyraxoncode.LanguageModelChatInformation} entry that publishes a
...
export function buildUtilityAliasModelInfo(
	...
	models: readonly zyraxoncode.LanguageModelChatInformation[],
	baseCount: number,
	requiresAuthorization: zyraxoncode.LanguageModelChatInformation['requiresAuthorization'],
): { info: zyraxoncode.LanguageModelChatInformation; synthesized: boolean } {
...
export class LanguageModelAccess extends Disposable implements IExtensionContribution {
	...
	private _currentModels: zyraxoncode.LanguageModelChatInformation[] = []; // Store current models for reference
	...
	constructor(
		...
		@IZyraxonCodeExtensionContext private readonly _vsCodeExtensionContext: IZyraxonCodeExtensionContext,
	...
	get currentModels(): zyraxoncode.LanguageModelChatInformation[] {
	...
	private async _registerChatProvider(): Promise<void> {
		...
		this._register(zyraxoncode.lm.registerLanguageModelChatProvider('copilot', provider));
	...
	private async _provideLanguageModelChatInfo(options: { silent: boolean }, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.LanguageModelChatInformation[]> {
		...
		const models: zyraxoncode.LanguageModelChatInformation[] = [];
		...
		for (const endpoint of chatEndpoints) {
			...
			if (endpoint.degradationReason) {
				...
				if (showExperimentalHint) {
					modelTooltip = `${modelTooltip} ${zyraxoncode.l10n.t('This model may be experimental or in evaluation.')}`;
				}
			} else {
			...
			if (endpoint.customModel) {
				...
				modelTooltip = zyraxoncode.l10n.t('{0} is contributed by {1} using {2}.', sanitizedModelName, customModel.owner_name, customModel.key_name);
			}

			const session = this._authenticationService.anyGitHubSession;
			const isDefault = endpoint === defaultChatEndpoint;

			const model: zyraxoncode.LanguageModelChatInformation = {
				...
				detail: modelDetail,
				statusIcon: endpoint.degradationReason ? new zyraxoncode.ThemeIcon('warning') : undefined,
				version: endpoint.version,
				maxInputTokens: endpoint.modelMaxPromptTokens - baseCount - BaseTokensPerCompletion,
				maxOutputTokens: endpoint.maxOutputTokens,
	...
	private _registerUtilityAliasModels(
		models: zyraxoncode.LanguageModelChatInformation[],
	): void {
	...
	private async _getEndpointForModel(model: zyraxoncode.LanguageModelChatInformation) {
	...
	private async _provideLanguageModelChatResponse(
		model: zyraxoncode.LanguageModelChatInformation,
		messages: Array<zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2>,
		options: zyraxoncode.ProvideLanguageModelChatResponseOptions,
		progress: zyraxoncode.Progress<zyraxoncode.LanguageModelResponsePart2>,
	...
	private async _provideTokenCount(
		model: zyraxoncode.LanguageModelChatInformation,
		text: string | zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2,
		token: zyraxoncode.CancellationToken
	): Promise<number> {
	...
	private async _registerEmbeddings(): Promise<void> {
		...
		const update = async () => {
			...
			dispo.clear();
			dispo.value = zyraxoncode.lm.registerEmbeddingsProvider(`copilot.${model}`, new class implements zyraxoncode.EmbeddingsProvider {
				async provideEmbeddings(input: string[], token: zyraxoncode.CancellationToken): Promise<zyraxoncode.Embedding[]> {
...
export class CopilotLanguageModelWrapper extends Disposable {
	...
	private async _provideLanguageModelResponse(_endpoint: IChatEndpoint, _messages: Array<zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2>, _options: zyraxoncode.ProvideLanguageModelChatResponseOptions, extensionId: string | undefined, callback: FinishedCallback, token: zyraxoncode.CancellationToken): Promise<APIUsage | undefined> {
		if (extensionId === 'core') {
			extensionId = undefined;
		}

		const extensionInfo = !extensionId ? { packageJSON: { version: this._envService.zyraxoncodeVersion } } : zyraxoncode.extensions.getExtension(extensionId, true);
		...
		const blockedExtensionMessage = zyraxoncode.l10n.t('The extension has been temporarily blocked due to making too many requests. Please try again later.');
		if (extensionId && this._blockedExtensionService.isExtensionBlocked(extensionId)) {
			throw zyraxoncode.LanguageModelError.Blocked(blockedExtensionMessage);
		}
		...
		*/
		...
		if (_options.toolMode === zyraxoncode.LanguageModelChatToolMode.Required && _options.tools?.length && _options.tools.length > 1) {
			throw new Error('LanguageModelChatToolMode.Required is not supported with more than one tool');
		}

		options.tool_choice = _options.toolMode === zyraxoncode.LanguageModelChatToolMode.Required && _options.tools?.length ?
		...
		// when crossing the ZYRAXON Code IPC boundary.
		...
		if (result.type !== ChatFetchResponseType.Success) {
			if (result.type === ChatFetchResponseType.ExtensionBlocked) {
				...
				const err = new zyraxoncode.LanguageModelError(details.message);
	...
	async provideLanguageModelResponse(endpoint: IChatEndpoint, messages: Array<zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2>, options: zyraxoncode.ProvideLanguageModelChatResponseOptions, extensionId: string | undefined, progress: zyraxoncode.Progress<LMResponsePart>, token: zyraxoncode.CancellationToken): Promise<void> {
		...
		const finishCallback: FinishedCallback = async (_text, index, delta): Promise<undefined> => {
			if (delta.thinking) {
				if (isEncryptedThinkingDelta(delta.thinking)) {
					if (options.includeEncryptedThinking) {
						progress.report(new zyraxoncode.LanguageModelThinkingPart(
							delta.thinking.text ?? '',
							delta.thinking.id,
					...
					progress.report(new zyraxoncode.LanguageModelThinkingPart(text, delta.thinking.id, delta.thinking.metadata));
					thinkingActive = true;
				}
			} else if (thinkingActive) {
				progress.report(new zyraxoncode.LanguageModelThinkingPart('', '', { zyraxoncode_reasoning_done: true }));
				thinkingActive = false;
			}
			if (delta.text) {
				progress.report(new zyraxoncode.LanguageModelTextPart(delta.text));
			}
			if (delta.copilotToolCalls) {
				for (const call of delta.copilotToolCalls) {
					...
					progress.report(new zyraxoncode.LanguageModelToolCallPart(call.id, call.name, parameters));
				}
			}

			if (delta.statefulMarker) {
				progress.report(
					new zyraxoncode.LanguageModelDataPart(encodeStatefulMarker(endpoint.model, delta.statefulMarker), CustomDataPartMimeTypes.StatefulMarker)
				);
			}
		...
		if (usage) {
			progress.report(new zyraxoncode.LanguageModelDataPart(new TextEncoder().encode(JSON.stringify(usage)), CustomDataPartMimeTypes.Usage));
		}
	}

	async provideTokenCount(endpoint: IEndpoint, message: string | zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2): Promise<number> {
		if (typeof message === 'string') {
			...
			const content = message.content.map((part): Raw.ChatCompletionContentPart | undefined => {
				if (part instanceof zyraxoncode.LanguageModelTextPart) {
					return { type: Raw.ChatCompletionContentPartKind.Text, text: part.value };
				} else if (part instanceof zyraxoncode.LanguageModelDataPart && part.mimeType === 'application/pdf') {
			...
			switch (message.role) {
				case zyraxoncode.LanguageModelChatMessageRole.User:
					raw = { role: Raw.ChatRole.User, content, name: message.name };
					break;
				case zyraxoncode.LanguageModelChatMessageRole.System:
					raw = { role: Raw.ChatRole.Assistant, content, name: message.name };
					break;
				case zyraxoncode.LanguageModelChatMessageRole.Assistant:
					raw = {
						role: Raw.ChatRole.Assistant,
						content,
						name: message.name,
						toolCalls: message.content
							.filter(part => part instanceof zyraxoncode.LanguageModelToolCallPart)
							.map(part => part as zyraxoncode.LanguageModelToolCallPart)
	...
	private validateTools(tools: readonly zyraxoncode.LanguageModelChatTool[]): void {
	...
	private async countToolTokens(endpoint: IChatEndpoint, tools: readonly zyraxoncode.LanguageModelChatTool[]): Promise<number> {
		return await endpoint.acquireTokenizer().countToolTokens(tools);
	}

	private validateRequest(_messages: Array<zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2>): void {
		...
		_messages.forEach((message, i) => {
			if (message.role === zyraxoncode.LanguageModelChatMessageRole.Assistant) {
				// Filter out DataPart since it does not share the same value type and does not have callId, function, etc.
				const filteredContent = message.content.filter(part => part instanceof zyraxoncode.LanguageModelDataPart);
				...
				while (toolCallIds.size > 0) {
					...
					if (!nextMessage || nextMessage.role !== zyraxoncode.LanguageModelChatMessageRole.User) {
						throw new Error(errMsg);
					}

					nextMessage.content.forEach(part => {
						if (!(part instanceof zyraxoncode.LanguageModelToolResultPart2 || part instanceof zyraxoncode.LanguageModelToolResultPart)) {
							throw new Error(errMsg);
						}
