...
export class LogWorkspaceStateContribution extends Disposable implements IExtensionContribution {
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.debug.workbenchState', async () => {
			const symbolQueries = await zyraxoncode.window.showInputBox({
			...
			const fileName = await zyraxoncode.window.showInputBox({
			...
			// Write the file
			const fileUri = zyraxoncode.Uri.joinPath(workspaceRoot, `${fileName}.state.json`);
			...
			zyraxoncode.workspace.fs.writeFile(fileUri, Buffer.from(serializedState));
		}));
	}
}
...
function serializeRelatedInformation(r: zyraxoncode.DiagnosticRelatedInformation): ISerializedDiagnosticRelatedInformation {
	return {
