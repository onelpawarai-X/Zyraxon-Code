...
import { ChatCompletionItem, ChatContext, ChatPromptReference, ChatRequest, ChatRequestTurn, ChatResponseMarkdownPart, ChatResponseReferencePart, ChatResponseTurn, ChatResponseWarningPart, ChatVariableLevel, Disposable, DynamicChatParticipantProps, Location, MarkdownString, Position, Progress, Range, TextDocument, TextEditor, ThemeIcon, chat, commands, l10n } from 'zyraxoncode';
...
import { ChatResponseReferencePart2, Uri } from '../../../zyraxoncodeTypes';
...
interface IGitHubRepositoryReference {
	...
	id: string; // e.g. "zyraxon/zyraxoncode"
}
...
export class RemoteAgentContribution implements IDisposable {
	...
	private checkAuthorized(agent: string) {
		...
		return this.zyraxoncodeExtensionContext.globalState.get<boolean>(key, false) || this.zyraxoncodeExtensionContext.workspaceState.get<boolean>(key, false);
	}

	private async setAuthorized(agent: string, isGlobal = false) {
		const memento = isGlobal ? this.zyraxoncodeExtensionContext.globalState : this.zyraxoncodeExtensionContext.workspaceState;
		...
		const agent = store.add(chat.createDynamicChatParticipant(participantId, dynamicProps, async (request, context, responseStream, token): Promise<ICopilotChatResult> => {
			...
			if (request.acceptedConfirmationData) {
				for (const data of request.acceptedConfirmationData) {
					if (data?.url) {
						...
						await commands.executeCommand('zyraxoncode.open', Uri.parse(data.url));
			...
			try {
				...
				try {
					...
					for (const ref of result.zyraxoncodeReferences) {
					...
					if (response.reason.includes('400 no docs found')) {
						...
						responseStream.confirmation(
							...
							l10n.t({
								...
								args: [slug, url.hostname, '__ZYRAXKEEP__0_'],
								comment: [`{Locked=']({'}`]
							}),
	...
	private async prepareClientPlatformReferences(variables: ChatPromptReference[], slug: string) {
		...
		const addFileReference = async (document: TextDocument, variableName?: string, isImplicit?: boolean) => {
			...
			zyraxoncodeReferences.push(variableName
		...
		if (this.checkAuthorized(slug)) {
			...
			if (activeTextEditor && variables.find(v => v.id.startsWith('zyraxoncode.implicit'))) {
		...
		return { clientReferences, zyraxoncodeReferences, hasIgnoredFiles };
	}
