...
import { AssistantMessage, PromptElement, PromptElementProps, SystemMessage, ToolMessage, UserMessage } from '@zyraxoncode/prompt-tsx';
...
function groupThinkingParts(parts: readonly zyraxoncode.LanguageModelThinkingPart[]): IThinkingGroup[] {
...
export class LanguageModelAccessPrompt extends PromptElement<Props> {
	async render() {
		...
		for (const message of this.props.messages) {
			if (message.role === zyraxoncode.LanguageModelChatMessageRole.System) {
				...
				systemMessages.push(filteredContent
					.filter(part => part instanceof zyraxoncode.LanguageModelTextPart)
					.map(part => part.value).join(''));

			} else if (message.role === zyraxoncode.LanguageModelChatMessageRole.Assistant) {
				const statefulMarkerPart = message.content.find(part => part instanceof zyraxoncode.LanguageModelDataPart && part.mimeType === CustomDataPartMimeTypes.StatefulMarker) as zyraxoncode.LanguageModelDataPart | undefined;
				...
				const toolCalls = filteredContent.filter(part => part instanceof zyraxoncode.LanguageModelToolCallPart);
				const thinkingParts = filteredContent.filter(part => part instanceof zyraxoncode.LanguageModelThinkingPart);
			...
			} else if (message.role === zyraxoncode.LanguageModelChatMessageRole.User) {
				for (const part of message.content) {
					...
					} else if (part instanceof zyraxoncode.LanguageModelTextPart) {
