		...
		const doRequest = async () => {
			...
			if (webSearchEnabled && !tools.some(tool => 'name' in tool && tool.name === 'web_search')) {
				...
				if (hasAllowed && hasBlocked) {
					webSearchDomainConflictError = zyraxoncode.l10n.t('The settings `github.copilot.chat.anthropic.tools.websearch.allowedDomains` and `github.copilot.chat.anthropic.tools.websearch.blockedDomains` cannot be used together. Please configure only one.');
				} else if (hasAllowed) {
				...
				if (chunk.delta.type === 'text_delta') {
					...
					if ('citation' in chunk.delta) {
						...
						if (citation.type === 'web_search_result_location') {
							...
							const referenceText = `\n> "${citation.cited_text}" â€” [${zyraxoncode.l10n.t('Source')}](${citation.url})\n\n`;
