...
import { CancellationToken, LanguageModelChatInformation, LanguageModelChatMessage, LanguageModelChatMessage2, LanguageModelDataPart, LanguageModelResponsePart2, LanguageModelTextPart, LanguageModelThinkingPart, LanguageModelToolCallPart, Progress, ProvideLanguageModelChatResponseOptions } from 'zyraxoncode';
...
export class GeminiNativeBYOKLMProvider extends AbstractLanguageModelChatProvider {
	...
	async provideLanguageModelChatResponse(model: ExtendedLanguageModelChatInformation<LanguageModelChatConfiguration>, messages: Array<LanguageModelChatMessage | LanguageModelChatMessage2>, options: ProvideLanguageModelChatResponseOptions, progress: Progress<LanguageModelResponsePart2>, token: CancellationToken): Promise<any> {
		...
		// This handles the case where AsyncLocalStorage context was lost crossing ZYRAXON Code IPC.
		...
		const doRequest = async () => {
			...
			// Convert ZYRAXON Code tools to Gemini function declarations
			...
			// Bridge ZYRAXON Code cancellation token to Gemini abortSignal for early network termination
			const abortController = new AbortController();
			const cancelSub = token.onCancellationRequested(() => {
				abortController.abort();
				this._logService.trace('Gemini request aborted via ZYRAXON Code cancellation token');
			});
			...
			try {
				...
				/* __GDPR__
					"response.success" : {
						...
						"parentRequestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "For a subagent: the ZYRAXON Code chat request id of the parent turn that invoked this subagent (matches panel.request.parentRequestId)." },
				...
				*/
				this._telemetryService.sendTelemetryEvent('response.success', { github: true, Zyraxon: true }, {
					source: 'byok.gemini',
					model: model.id,
					requestId,
				}, {
