 ...
 *--------------------------------------------------------------------------------------------*/
import { LanguageModelChatInformation, LanguageModelChatProvider, lm } from 'zyraxoncode';
...
export class BYOKContrib extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		@IZyraxonCodeExtensionContext extensionContext: IZyraxonCodeExtensionContext,
	...
	private async _fetchKnownModelList(fetcherService: IFetcherService): Promise<Record<string, BYOKKnownModels>> {
		...
		const data = await (await fetcherService.fetch('__ZYRAXKEEP__0_', { method: 'GET', callSite: 'byok-known-models' })).json();
		// Use this for testing with changes from a local file. Don't check in
		// const data = JSON.parse((await this._fileSystemService.readFile(URI.file('/Users/roblou/code/zyraxoncode-engineering/chat/copilotChat.json'))).toString());
		if (data.version !== 1) {
