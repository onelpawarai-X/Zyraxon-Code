...
export class McpPickRef {
	public _inner?: { type: 'pick'; value: zyraxoncode.QuickPick<zyraxoncode.QuickPickItem> } | { type: 'input'; value: zyraxoncode.InputBox };
	...
	private getInput() {
		if (this._inner?.type !== 'input') {
			...
			const input = zyraxoncode.window.createInputBox();
	...
	private getPick() {
		if (this._inner?.type !== 'pick') {
			...
			const pick = zyraxoncode.window.createQuickPick();
...
export class QuickInputTool {
	...
	public static async invoke(ref: McpPickRef, args: IQuickInputToolArgs): Promise<zyraxoncode.LanguageModelToolResult> {
		...
		return new zyraxoncode.LanguageModelToolResult([new zyraxoncode.LanguageModelTextPart(`${args.title}: ${result}`)]);
	}
}
...
export class QuickPickTool {
	...
	public static async invoke(ref: McpPickRef, args: IQuickPickToolArgs): Promise<zyraxoncode.LanguageModelToolResult> {
