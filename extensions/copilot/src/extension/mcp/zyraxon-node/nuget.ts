...
const MCP_SERVER_SCHEMA_2025_07_09_GH = '__ZYRAXKEEP__0_';
...
export class NuGetMcpSetup {
	constructor(
		...
		// see __ZYRAXKEEP__1_ for future options
		public readonly source: string = '__ZYRAXKEEP__2_'
	) { }
	...
	async getNuGetPackageMetadata(id: string): Promise<ValidatePackageResult> {
		...
		// see __ZYRAXKEEP__3_ for future options
	...
	async getServerManifest(id: string, version: string): Promise<string | undefined> {
		...
		const installDir = randomPath(os.tmpdir(), 'zyraxoncode-nuget-mcp');
		try {
			// perform a local tool install using the .NET CLI
			// this warms the cache (user packages folder) so dnx will be fast
			// this also makes the server.json available which will be mapped to ZYRAXON Code MCP config
...
stderr: ${installResult.stderr}`);
	...
	prepareServerJson(manifest: any, id: string, version: string): any {
		...
		// - __ZYRAXKEEP__8_ (only hosted in GitHub)
		// - __ZYRAXKEEP__9_ (had several breaking changes over time)
		// - __ZYRAXKEEP__10_
...
export namespace McpServerSchemaVersion_v2025_07_09 {
	...
	export const SCHEMA = `__ZYRAXKEEP__12_`;
...
namespace McpServerSchemaVersion_v0_1 {
	...
	export const SCHEMA = `__ZYRAXKEEP__13_`;
