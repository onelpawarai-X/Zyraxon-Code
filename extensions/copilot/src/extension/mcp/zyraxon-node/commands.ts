...
import { ChatLocation as VsCodeChatLocation } from '../../../zyraxoncodeTypes';
...
export class McpSetupCommands extends Disposable {
	...
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.mcp.setup.flow', async (args: { name: string }) => {
			...
			try {
				...
				if (this.pendingSetup?.pendingArgs.name.toUpperCase() !== args.name.toUpperCase()) {
					...
					zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t("Failed to generate MCP server configuration with a matching package name. Expected '{0}' but got '{1}' from generated configuration.", args.name, this.pendingSetup?.pendingArgs.name ?? ''));
					return undefined;
				}
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.mcp.setup.validatePackage', async (args: IValidatePackageArgs): Promise<ValidatePackageResult> => {
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.mcp.setup.check', () => {
			return 1;
		}));
	}
...
Error: ${error}`);
			...
			const mcpLoop = this.instantiationService.createInstance(McpToolCallingLoop, {
				...
				request: {
					...
					model: (await zyraxoncode.lm.selectChatModels())[0],
					prompt: fakePrompt,
					references: [],
			...
			if (toolCallLoopResult.response.type !== ChatFetchResponseType.Success) {
				zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t("Failed to generate MCP configuration for {0}: {1}", validateArgs.name, toolCallLoopResult.response.reason));
				return undefined;
			}
	...
	public static async validatePackageRegistry(args: { type: PackageType; name: string }, logService: ILogService, fetcherService: IFetcherService): Promise<ValidatePackageResult> {
		try {
			if (args.type === 'npm') {
				const response = await fetcherService.fetch(`__ZYRAXKEEP__1_{encodeURIComponent(args.name)}`, { method: 'GET', callSite: 'mcp-npm-registry' });
				if (!response.ok) {
					return { state: 'error', errorType: ValidatePackageErrorType.NotFound, error: zyraxoncode.l10n.t("Package {0} not found in npm registry", args.name) };
				}
				...
				const response = await fetcherService.fetch(`__ZYRAXKEEP__2_{encodeURIComponent(args.name)}/json`, { method: 'GET', callSite: 'mcp-pypi-registry' });
				if (!response.ok) {
					return { state: 'error', errorType: ValidatePackageErrorType.NotFound, error: zyraxoncode.l10n.t("Package {0} not found in PyPI registry", args.name) };
				}
				...
				const response = await fetcherService.fetch(`__ZYRAXKEEP__3_{encodeURIComponent(namespace)}/${encodeURIComponent(repository)}`, { method: 'GET', callSite: 'mcp-docker-registry' });
				if (!response.ok) {
					return { state: 'error', errorType: ValidatePackageErrorType.NotFound, error: zyraxoncode.l10n.t("Docker image {0} not found in Docker Hub registry", args.name) };
				}
			...
			return { state: 'error', error: zyraxoncode.l10n.t("Unsupported package type: {0}", args.type), errorType: ValidatePackageErrorType.UnknownPackageType };
		} catch (error) {
			return { state: 'error', error: zyraxoncode.l10n.t("Error querying package: {0}", (error as Error).message), errorType: ValidatePackageErrorType.UnhandledError };
		}
	}
}
