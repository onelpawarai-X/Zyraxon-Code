...
import { parseSettingsAndCommands } from '../../context/node/resolvers/zyraxoncodeContext';
...
import { VscodePrompt } from '../../prompts/node/panel/zyraxoncode';
import { ToolName } from '../../tools/common/toolNames';
import { IToolsService } from '../../tools/common/toolsService';


class ZyraxonCodeIntentInvocation extends RendererIntentInvocation implements IIntentInvocation {
	...
	async createRenderer(promptContext: IBuildPromptContext, endpoint: IChatEndpoint, progress: zyraxoncode.Progress<zyraxoncode.ChatResponseProgressPart | zyraxoncode.ChatResponseReferencePart>, token: zyraxoncode.CancellationToken) {
	...
	processResponse(context: IResponseProcessorContext, inputStream: AsyncIterable<IResponsePart>, outputStream: zyraxoncode.ChatResponseStream, token: CancellationToken): Promise<void> {
		const responseProcessor = new ZyraxonCodeResponseProcessor(this.workbenchService);
	...
	getAvailableTools(): zyraxoncode.LanguageModelToolInformation[] | Promise<zyraxoncode.LanguageModelToolInformation[]> | undefined {
		return this.toolsService.getEnabledTools(this.request, this.endpoint, tool =>
			tool.name === 'zyraxoncode_searchExtensions_internal' ||
			tool.name === ToolName.ZyraxonCodeAPI
		);
	}
}

class ZyraxonCodeResponseProcessor {
	...
	private async processNonReporting(codeBlock: string, progress: zyraxoncode.ChatResponseStream) {
	...
	private async applyDelta(textDelta: string, progress: zyraxoncode.ChatResponseStream) {
...
export class VscodeIntent implements IIntent {
	...
	static readonly ID = Intent.ZyraxonCode;
	readonly id: string = VscodeIntent.ID;
	readonly locations = [ChatLocation.Panel];
	readonly description: string = l10n.t('Ask questions about ZYRAXON Code');
	...
	async invoke(invocationContext: IIntentInvocationContext): Promise<IIntentInvocation> {
		...
		return this.instantiationService.createInstance(ZyraxonCodeIntentInvocation, this, location, endpoint, invocationContext.request);
	}
}
