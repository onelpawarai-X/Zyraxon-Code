...
type FixCommandArgs = { message: zyraxoncode.TestMessage; test: zyraxoncode.TestItem | zyraxoncode.TestResultSnapshot; source?: 'sparkles' | 'testResultsPanel' | 'retry' };

function registerTestMessageSparkles(store: DisposableStore, telemetryService: ITelemetryService, testProvider: ITestProvider) {
	function getLastFailureForItemOrChildren(item: zyraxoncode.TestItem): ITestFailure | undefined {
	...
	store.add(zyraxoncode.commands.registerCommand('github.copilot.tests.fixTestFailure.fromInline', (item: zyraxoncode.TestItem) => {
	...
	store.add(zyraxoncode.commands.registerCommand('github.copilot.tests.fixTestFailure', openFixChat));
	...
	async function openFixChat(args: FixCommandArgs) {
		...
		*/
		...
		const doc = await zyraxoncode.workspace.openTextDocument(args.test.uri);
		await zyraxoncode.window.showTextDocument(doc, {
			...
			preview: true,
			selection: args.test.range ? new zyraxoncode.Range(args.test.range.start, args.test.range.start) : undefined
		});
...
function registerTestFailureCodeAction(testProvider: ITestProvider, configurationService: IConfigurationService, store: DisposableStore) {
	store.add(zyraxoncode.languages.registerCodeActionsProvider('*', {
		provideCodeActions(document, range, context, token) {
			...
			const ca = new zyraxoncode.CodeAction(l10n.t('Fix test failure'), zyraxoncode.CodeActionKind.QuickFix);
			ca.isAI = true;
			ca.command = {
