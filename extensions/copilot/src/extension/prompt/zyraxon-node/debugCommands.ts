...
import { generateSTest } from '../../conversation/zyraxoncode-node/feedbackReporter';
...
export class DebugCommandsContribution extends Disposable {
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.debug.generateSTest', async () => {
			...
			if (lastTurn) {
				...
				if (sTestValue) {
					zyraxoncode.env.clipboard.writeText(sTestValue.join('\n'));
					zyraxoncode.window.showInformationMessage('STest copied to clipboard');
				}
			}
		}));
		const ensureTask = async (workspaceFolder: URI | undefined, config: IStartDebuggingParsedResponse) => {
			const wf = workspaceFolder || zyraxoncode.workspace.workspaceFolders?.[0].uri;
			if (!wf) {
				zyraxoncode.window.showErrorMessage(needsWorkspaceFolderForTaskError());
				return;
			}
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.createLaunchJsonFileWithContents', async (launchConfig: IStartDebuggingParsedResponse) => {
			// Define the path for the .zyraxoncode/launch.json file
			const workspaceFolders = zyraxoncode.workspace.workspaceFolders;
			if (!workspaceFolders?.length) {
				zyraxoncode.window.showErrorMessage('No workspace folder is open.');
				return;
			}
		...
		this._register((zyraxoncode.commands.registerCommand('github.copilot.startDebugging', async (config: IStartDebuggingParsedResponse, progress) => {
			...
			if (result?.config) {
				...
				progress.progress(zyraxoncode.l10n.t('Started debugging {0}', result.config.name));
				return;
			} else {
				progress.markdown(zyraxoncode.l10n.t('Could not start debugging. Please try again.'));
				return;
			}
		})));
	}
