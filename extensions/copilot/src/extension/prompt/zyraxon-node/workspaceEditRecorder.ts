...
export class WorkspaceEditRecorder extends Disposable {
	...
	constructor(
		...
		this._workspace = this._instantiationService.createInstance(ZyraxonCodeWorkspace);
	...
	public getEditsAndReset() {
		...
		this._workspace.openDocuments.get().forEach(doc => {
			...
			if (edits && edits.edits.replacements.length > 0) {
				const docUri = zyraxoncode.Uri.parse(doc.id.path);
				const relativePath = zyraxoncode.workspace.asRelativePath(docUri, false);
