...
export class DevContainerConfigurationServiceImpl implements IDevContainerConfigurationService {
	...
	async generateConfiguration(args: DevContainerConfigGeneratorArguments, cancellationToken: zyraxoncode.CancellationToken): Promise<DevContainerConfigGeneratorResult> {
		if (cancellationToken.isCancellationRequested) {
			return { type: 'cancelled' };
		}

		const filenames = (await Promise.all(['*', '*/*', '*/*/*']
			.map(pattern => this.searchService.findFilesWithDefaultExcludes(new zyraxoncode.RelativePattern(args.rootUri, pattern), 1000, cancellationToken))))
			.flat()
