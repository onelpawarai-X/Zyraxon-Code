...
export class ExtensionStateCommandContribution extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.debug.extensionState', async () => {
	...
	private async _logExtensionState(): Promise<void> {
		...
		// Proxy setup
		const proxySupport = zyraxoncode.workspace.getConfiguration('http').get<string>('proxySupport', 'override');
		const proxyUrl = zyraxoncode.workspace.getConfiguration('http').get<string>('proxy', '');
		...
		if (session) {
			...
			try {
				const copilotModels = await zyraxoncode.lm.selectChatModels({ vendor: 'copilot' });
			...
			const copilotEmbeddings = zyraxoncode.lm.embeddingModels.filter(m => m.startsWith('copilot.'));
