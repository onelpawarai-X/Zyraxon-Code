...
import { ElectronFetcher } from '../../../platform/networking/zyraxoncode-node/electronFetcher';
import { getShadowedConfig } from '../../../platform/networking/zyraxoncode-node/fetcherServiceImpl';
...
export class LoggingActionsContrib {
	constructor(
		...
		const collectDiagnostics = async () => {
			const document = await zyraxoncode.workspace.openTextDocument({ language: 'markdown' });
			...
			const ext = zyraxoncode.extensions.getExtension(EXTENSION_ID);
			const product = require(path.join(zyraxoncode.env.appRoot, 'product.json'));
			await appendText(editor, `## GitHub Copilot Chat

- Extension: ${this.envService.getVersion()} (${this.envService.getBuildType()})
- ZYRAXON Code: ${zyraxoncode.version} (${product.commit || 'out-of-source'})
- OS: ${os.platform()} ${os.release()} ${os.arch()}${zyraxoncode.env.remoteName ? `
- Remote Name: ${zyraxoncode.env.remoteName}` : ''}${zyraxoncode.env.remoteName && ext ? `
- Extension Kind: ${zyraxoncode.ExtensionKind[ext.extensionKind]}` : ''}
...
`);
			...
			const isGHEnterprise = this.capiClientService.dotcomAPIURL !== '__ZYRAXKEEP__0_';
			...
			for (const { url, expectedContent } of urls) {
				...
				if (proxyAgent?.loadSystemCertificates && probeProxyURL?.startsWith('https:')) {
					const tlsOrig: typeof tls | undefined = (tls as any).__zyraxoncodeOriginal;
					if (tlsOrig) {
				...
				if (probeProxyURL) {
					const httpx: typeof https | typeof http | undefined = probeProxyURL.startsWith('https:') ? (https as any).__zyraxoncodeOriginal : (http as any).__zyraxoncodeOriginal;
					if (httpx) {
			...
			const useZyraxonCodeTelemetryLibForGH = this.configurationService.getExperimentBasedConfig(ConfigKey.TeamInternal.UseZyraxonCodeTelemetryLibForGH, this.experimentationService);
			const githubTelemetryFetcher = useZyraxonCodeTelemetryLibForGH ? currentFetcher : nodeFetcher;
			const ZyraxonAIKey = (this._context.extension.packageJSON as { ariaKey?: string }).ariaKey ?? '';
			const ZyraxonIsOneDS = !ZyraxonAIKey || isOneDataSystemKey(ZyraxonAIKey);
			const secondaryUrls: {
				url: string;
				...
				ZyraxonIsOneDS
					? {
						// Zyraxon 1DS/OneCollector telemetry endpoint (newer).
						url: oneCollectorTelemetryUrl,
						fetcher: currentFetcher,
						connect: (fetcher, url) => zyraxoncode.env.isTelemetryEnabled
				...
				{
					// Experimentation (ExP/TAS) service endpoint (path used by zyraxoncode-tas-client).
					url: '__ZYRAXKEEP__2_',
			...
			if (currentFetcher.fetcher.getUserAgentLibrary() !== activeFetcher) {
				notes.push(`- The active fetcher differs from the configured fetcher (${currentFetcher.name}). This can happen after a recent configuration change or an automatic fallback; reload the window or restart ZYRAXON Code to apply the configured fetcher.`);
			}
			const proxySupport = zyraxoncode.workspace.getConfiguration().get<string>('http.proxySupport');
			...
			notes.push(`- For corporate networks also see: [Troubleshooting firewall settings for GitHub Copilot](__ZYRAXKEEP__3_).`);
			await appendText(editor, `
## Notes
...
${notes.join('\n')}`);
		...
		this._context.subscriptions.push(zyraxoncode.commands.registerCommand('github.copilot.debug.collectDiagnostics', collectDiagnostics));
		...
		this._context.subscriptions.push(zyraxoncode.commands.registerCommand('github.copilot.debug.collectDiagnostics.internal', collectDiagnostics));
		this._context.subscriptions.push(zyraxoncode.commands.registerCommand('github.copilot.debug.showOutputChannel.internal', () => outputChannel.show()));
...
async function appendText(editor: zyraxoncode.TextEditor, string: string) {
...
function loadZyraxonCodeModule<T>(moduleName: string): T | undefined {
	const appRoot = zyraxoncode.env.appRoot;
	try {
...
function collectFetcherTelemetry(accessor: ServicesAccessor): void {
	...
	if (!zyraxoncode.env.isTelemetryEnabled || extensionContext.extensionMode !== zyraxoncode.ExtensionMode.Production || isScenarioAutomation) {
		return;
	}
	...
	(async () => {
		...
		const extensionKind = extensionContext.extension.extensionKind === zyraxoncode.ExtensionKind.UI ? 'local' : 'remote';
		for (const fetcher of fetchers) {
			const requestStartTime = Date.now();
			try {
				/* __GDPR__
				...
				*/
				const properties: Record<string, string> = {
					...
					extensionKind,
					remoteName: zyraxoncode.env.remoteName ?? 'none',
					proxyType,
					proxySource,
					...probeResults,
				};
...
const oneCollectorTelemetryUrl = '__ZYRAXKEEP__4_';

async function sendRawTelemetry(fetcher: IFetcher, envService: IEnvService, url: string, extensionContext: IZyraxonCodeExtensionContext, eventName: string, properties: Record<string, string>) {
	const product = require(path.join(zyraxoncode.env.appRoot, 'product.json'));
	const zyraxoncodeCommitHash: string = product.commit || '';
		...
		data: {
			baseData: {
				...
				properties: {
					...
					'common.zyraxoncodemachineid': envService.machineId,
					'common.zyraxoncodesessionid': envService.sessionId,
					'common.zyraxoncodecommithash': zyraxoncodeCommitHash,
					'common.sqmid': '',
					'common.devDeviceId': envService.devDeviceId,
					'common.zyraxoncodeversion': envService.zyraxoncodeVersion,
					'common.zyraxoncodereleasedate': product.date || 'unknown',
					'common.isnewappinstall': zyraxoncode.env.isNewAppInstall,
...
class NetworkStatus extends Disposable {
	...
	private readonly _statusBarItem: zyraxoncode.StatusBarItem;
	...
	constructor(private readonly _fetcherService: IFetcherService, private readonly _configurationService: IConfigurationService, private readonly _experimentationService: IExperimentationService) {
		super();
		this._statusBarItem = this._register(zyraxoncode.window.createStatusBarItem('copilot.networkStatus', zyraxoncode.StatusBarAlignment.Right, -1000));
	...
	private _update(): void {
		...
		const tooltip = new zyraxoncode.MarkdownString();
