...
const showHtmlCommand = 'zyraxoncode.copilot.chat.showRequestHtmlItem';
...
function serializeMcpServers(servers: readonly zyraxoncode.McpServerDefinition[]): object[] {
	return servers.map(server => {
		if (server instanceof zyraxoncode.McpStdioServerDefinition) {
			return {
				type: 'stdio',
				label: server.label,
...
export class RequestLogTree extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(zyraxoncode.window.registerTreeDataProvider('copilot-chat', this.chatRequestProvider));
		...
		this._register(zyraxoncode.commands.registerCommand(showHtmlCommand, async (elementId: string) => {
			if (!server) {
			...
			await zyraxoncode.commands.executeCommand('simpleBrowser.show', address);
		}));

		this._register(zyraxoncode.commands.registerCommand(exportLogItemCommand, async (treeItem: TreeItem) => {
			...
			if (treeItem instanceof ChatPromptItem) {
				...
				zyraxoncode.window.showWarningMessage('Cannot export chat prompt item. Please select a specific request, tool call, or element.');
				return;
				...
				zyraxoncode.window.showErrorMessage('Unable to determine log entry ID for this item.');
				return;
			}

			// Check if this entry type supports markdown export
			if (logEntry.kind === LoggedInfoKind.Element) {
				zyraxoncode.window.showWarningMessage('Element entries cannot be exported as markdown. They contain HTML content that can be viewed in the browser.');
				return;
			}
			...
			// Show save dialog
			const saveUri = await zyraxoncode.window.showSaveDialog({
				defaultUri: zyraxoncode.Uri.file(path.join(os.homedir(), defaultFilename)),
				filters: {
			...
			try {
				...
				const virtualUri = zyraxoncode.Uri.parse(ChatRequestScheme.buildUri({ kind: 'request', id: logEntry.id }));
				const document = await zyraxoncode.workspace.openTextDocument(virtualUri);
				const content = document.getText();

				// Write to the selected file
				await zyraxoncode.workspace.fs.writeFile(saveUri, Buffer.from(content, 'utf8'));
				...
				if (result === openAction) {
					await zyraxoncode.commands.executeCommand('zyraxoncode.open', saveUri);
				}
			} catch (error) {
				zyraxoncode.window.showErrorMessage(`Failed to export log entry: ${error}`);
			}
		}));

		// Save the currently opened chat log (ccreq:*.copilotmd) to a file
		this._register(zyraxoncode.commands.registerCommand(saveCurrentMarkdownCommand, async (...args: any[]) => {
			// Accept resource from menu invocation (editor/title passes the resource)
			let resource: zyraxoncode.Uri | undefined;
			const first = args?.[0];
			if (first instanceof zyraxoncode.Uri) {
				resource = first;
			} else if (first && typeof first === 'object') {
				// Some menu invocations pass { resource: Uri }
				const candidate = (first as { resource?: zyraxoncode.Uri }).resource;
				if (candidate instanceof zyraxoncode.Uri) {
					resource = candidate;
				}
			}
			...
			if (!resource) {
				zyraxoncode.window.showWarningMessage('No document is active to save.');
				return;
			}

			if (resource.scheme !== ChatRequestScheme.chatRequestScheme) {
				zyraxoncode.window.showWarningMessage('This command only works for Copilot request documents.');
				return;
			}
			...
			try {
				...
				const doc = await zyraxoncode.workspace.openTextDocument(resource);
				await zyraxoncode.workspace.fs.writeFile(saveUri, Buffer.from(doc.getText(), 'utf8'));
				...
				zyraxoncode.window.showErrorMessage(`Failed to save markdown: ${error}`);
			}
		}));

		this._register(zyraxoncode.commands.registerCommand(exportPromptArchiveCommand, async (treeItem: ChatPromptItem) => {
			const logEntries = getExportableLogEntries(treeItem);

			if (logEntries.length === 0) {
				zyraxoncode.window.showInformationMessage('No exportable entries found in this prompt.');
				return;
			}
			...
			try {
				...
				const tempDir = path.join(os.tmpdir(), `zyraxoncode-copilot-export-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`);
				await zyraxoncode.workspace.fs.createDirectory(zyraxoncode.Uri.file(tempDir));
				...
				for (const logEntry of logEntries) {
					...
					await zyraxoncode.workspace.fs.writeFile(zyraxoncode.Uri.file(tempFilePath), Buffer.from(content, 'utf8'));
				...
				if (filesToArchive.length > 0) {
					...
					for (const filePath of filesToArchive) {
						await zyraxoncode.workspace.fs.delete(zyraxoncode.Uri.file(filePath));
					}
					await zyraxoncode.workspace.fs.delete(zyraxoncode.Uri.file(tempDir));
					...
					if (result === revealAction) {
						await zyraxoncode.commands.executeCommand('revealFileInOS', saveUri);
					}
				} else {
					zyraxoncode.window.showWarningMessage('No valid entries could be exported.');
				}
			} catch (error) {
				zyraxoncode.window.showErrorMessage(`Failed to export prompt archive: ${error}`);
			}
		}));

		this._register(zyraxoncode.commands.registerCommand(exportPromptLogsAsJsonCommand, async (treeItem: ChatPromptItem) => {
			const promptObject = await preparePromptLogsAsJson(treeItem);
			if (!promptObject) {
				zyraxoncode.window.showWarningMessage('No exportable entries found for this prompt.');
				return;
			}
			...
			try {
				...
				// Write to the selected file
				await zyraxoncode.workspace.fs.writeFile(saveUri, Buffer.from(finalContent, 'utf8'));
				...
				zyraxoncode.window.showErrorMessage(`Failed to export prompt logs as JSON: ${error}`);
			}
		}));

		this._register(zyraxoncode.commands.registerCommand(exportAllPromptLogsAsJsonCommand, async (savePath?: string) => {
			...
			if (!allTreeItems || allTreeItems.length === 0) {
				zyraxoncode.window.showInformationMessage('No chat prompts found to export.');
				return;
			}
			...
			let saveUri: zyraxoncode.Uri;

			if (savePath && typeof savePath === 'string') {
				// Use provided path
				saveUri = zyraxoncode.Uri.file(savePath);
			} else {
				...
				// Show save dialog
				const dialogResult = await zyraxoncode.window.showSaveDialog({
					defaultUri: zyraxoncode.Uri.file(path.join(os.homedir(), defaultFilename)),
					filters: {
						'JSON': ['json'],
						'All Files': ['*']
			...
			try {
				...
				const exportData = assembleChatLogExport(
					allPromptsContent,
					serializeMcpServers(zyraxoncode.lm.mcpServerDefinitions ?? [])
				);
				...
				zyraxoncode.window.showErrorMessage(`Failed to export all prompt logs as JSON: ${error}`);
			}
		}));

		this._register(zyraxoncode.commands.registerCommand(showRawRequestBodyCommand, async (arg?: ChatPromptItem) => {
			const requestId = arg?.id;
			if (!requestId) {
				return;
			}

			await zyraxoncode.commands.executeCommand('zyraxoncode.open', zyraxoncode.Uri.parse(ChatRequestScheme.buildUri({ kind: 'request', id: requestId }, 'rawrequest')));
		}));

		this._register(zyraxoncode.commands.registerCommand('github.copilot.debug.showOutputChannel', async () => {
			outputChannel.show();
		}));
	}
}
...
class RequestServer extends Disposable {
	...
	async addRouter(info: ILoggedElementInfo) {
		...
		const router = info.trace.serveRouter(`__ZYRAXKEEP__0_{port}`);
...
class ChatRequestProvider extends Disposable implements zyraxoncode.TreeDataProvider<TreeItem> {
	...
	private readonly _onDidChangeTreeData = new zyraxoncode.EventEmitter<TreeItem | undefined | void>();
	onDidChangeTreeData = this._onDidChangeTreeData.event;

	getTreeItem(element: TreeItem): zyraxoncode.TreeItem | Thenable<zyraxoncode.TreeItem> {
		return element;
	}

	getChildren(element?: TreeItem | undefined): zyraxoncode.ProviderResult<TreeItem[]> {
...
class ChatPromptItem extends zyraxoncode.TreeItem {
	...
	protected constructor(public readonly token: CapturingToken) {
		super(token.label, zyraxoncode.TreeItemCollapsibleState.Expanded);
		if (token.icon) {
			this.iconPath = new zyraxoncode.ThemeIcon(token.icon);
		}
	}
	...
	public setMainEntry(info: ILoggedRequestInfo): void {
		...
		this.iconPath = resolvedIcon !== undefined ? new zyraxoncode.ThemeIcon(resolvedIcon.id) : undefined;
		this.command = {
			command: 'zyraxoncode.open',
			title: '',
			arguments: [zyraxoncode.Uri.parse(ChatRequestScheme.buildUri({ kind: 'request', id: info.id }))]
		};
	}
	...
	public withFilteredChildren(filter: (child: TreeChildItem) => boolean): ChatPromptItem {
		...
		item.collapsibleState = item.children.length > 0
			? zyraxoncode.TreeItemCollapsibleState.Expanded
			: zyraxoncode.TreeItemCollapsibleState.None;
...
class ToolCallItem extends zyraxoncode.TreeItem {
	...
	constructor(
		...
		super(info.name, zyraxoncode.TreeItemCollapsibleState.None);
		...
		this.iconPath = new zyraxoncode.ThemeIcon('tools');
	}
}

class ChatElementItem extends zyraxoncode.TreeItem {
	...
	constructor(
		...
		this.iconPath = new zyraxoncode.ThemeIcon('code');
	}
}

class ChatRequestItem extends zyraxoncode.TreeItem {
	public override id: string;
	override readonly contextValue = 'request';
	constructor(
		readonly info: ILoggedRequestInfo
	) {
		super(info.entry.debugName, zyraxoncode.TreeItemCollapsibleState.None);
		this.id = info.id;

		if (info.entry.type === LoggedRequestKind.MarkdownContentRequest) {
			const resolvedIcon = resolveMarkdownIcon(info.entry);
			this.iconPath = resolvedIcon === undefined ? undefined : new zyraxoncode.ThemeIcon(resolvedIcon.id);
			...
			this.iconPath = info.entry.type === LoggedRequestKind.ChatMLSuccess ? undefined : new zyraxoncode.ThemeIcon('error');
	...
	${startTimeStr}`;
		...
		this.iconPath ??= new zyraxoncode.ThemeIcon('copilot');
	}
}
...
class LogTreeFilters extends Disposable {
	...
	private readonly _onDidChangeFilters = new zyraxoncode.EventEmitter<void>();
	readonly onDidChangeFilters = this._onDidChangeFilters.event;

	constructor(
		@IZyraxonCodeExtensionContext private readonly zyraxoncodeExtensionContext: IZyraxonCodeExtensionContext,
	) {
		super();

		this.setElementsShown(!zyraxoncodeExtensionContext.workspaceState.get(this.getStorageKey('elements')));
		this.setToolsShown(!zyraxoncodeExtensionContext.workspaceState.get(this.getStorageKey('tools')));
		this.setNesRequestsShown(!zyraxoncodeExtensionContext.workspaceState.get(this.getStorageKey('nesRequests')));
		this.setGhostRequestsShown(!zyraxoncodeExtensionContext.workspaceState.get(this.getStorageKey('ghostRequests')));
	}
	...
	private setShown(name: string, value: boolean): void {
		zyraxoncode.commands.executeCommand('setContext', `github.copilot.chat.debug.${name}Hidden`, !value);
		this.zyraxoncodeExtensionContext.workspaceState.update(this.getStorageKey(name), !value);
...
class LogTreeFilterCommands extends Disposable {
	constructor(filters: LogTreeFilters) {
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.showElements', () => filters.setElementsShown(true)));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.hideElements', () => filters.setElementsShown(false)));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.showTools', () => filters.setToolsShown(true)));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.hideTools', () => filters.setToolsShown(false)));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.showNesRequests', () => filters.setNesRequestsShown(true)));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.hideNesRequests', () => filters.setNesRequestsShown(false)));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.showGhostRequests', () => filters.setGhostRequestsShown(true)));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.debug.hideGhostRequests', () => filters.setGhostRequestsShown(false)));
	}
}
