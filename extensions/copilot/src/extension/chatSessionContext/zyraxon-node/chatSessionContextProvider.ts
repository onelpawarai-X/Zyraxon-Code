...
import { LanguageModelChatMessage, LanguageModelTextPart } from '../../../zyraxoncodeTypes';
...
class ContextResolver implements Copilot.ContextResolver<Copilot.SupportedContextItem> {
	...
	private async generateSummary(conversation: Conversation): Promise<string | undefined> {
		try {
			...
			const models = await zyraxoncode.lm.selectChatModels({ family: 'gpt-4o-mini', vendor: 'copilot' });
			if (models.length === 0) {
				// Fallback to any available model
				const allModels = await zyraxoncode.lm.selectChatModels({ vendor: 'copilot' });
