...
export function registerInlineChatCommands(accessor: ServicesAccessor): IDisposable {
	...
	const extensionContext = accessor.get(IZyraxonCodeExtensionContext);
	...
	const doExplain = async (arg0: any, fromPalette?: true) => {
		...
		if (typeof arg0 === 'string' && arg0) {
			...
			if (emptySelection) {
				const severeDiagnostics = zyraxoncode.languages.getDiagnostics(emptySelection.activeDocument.uri);
		...
		zyraxoncode.commands.executeCommand('workbench.action.chat.open', { query: message });
	};
	const doApplyReview = async (commentThread: zyraxoncode.CommentThread, revealNext = false) => {
	...
	const doContinueInInlineChat = async (commentThread: zyraxoncode.CommentThread) => {
		const comment = reviewService.findReviewComment(commentThread);
		if (!comment) {
			return;
		}
		const totalComments = reviewService.getReviewComments().length;
		const message = comment.body instanceof zyraxoncode.MarkdownString ? comment.body.value : comment.body;
		reviewService.removeReviewComments([comment]);
	...
	const doContinueInChat = async (thread: zyraxoncode.CommentThread) => {
		...
		const totalComments = reviewService.getReviewComments().length;
		const message = comment.body instanceof zyraxoncode.MarkdownString ? comment.body.value : comment.body;
		await zyraxoncode.commands.executeCommand('workbench.action.chat.open', {
...
${message}`,
	...
	const doDiscardReview = async (commentThread: zyraxoncode.CommentThread, revealNext = false) => {
		if (revealNext) {
			goToNextReview(commentThread, +1);
		}
	...
	const markReviewHelpful = async (comment: zyraxoncode.Comment) => {
	...
	const markReviewUnhelpful = async (comment: zyraxoncode.Comment) => {
		const reviewComment = reviewService.findReviewComment(comment);
		if (reviewComment) {
			const commentThread = reviewService.findCommentThread(reviewComment);
			if (commentThread) {
	...
	if (typeof extensionMode === 'number' && (extensionMode !== zyraxoncode.ExtensionMode.Test || isScenarioAutomation)) {
		reviewService.updateContextValues();
	}
	const goToNextReview = (currentThread: zyraxoncode.CommentThread | undefined, direction: number) => {
		...
		if (direction !== 0) {
			(newThread as unknown as zyraxoncode.CommentThread2).reveal();
		}
		instaService.invokeFunction(fetchSuggestion, newThread);
	};
	const doGenerate = () => {
		return zyraxoncode.commands.executeCommand('zyraxoncode.editorChat.start', { message: '/generate ' });
	};
	const doFix = () => {
		const activeDocument = zyraxoncode.window.activeTextEditor;
		if (!activeDocument) {
			return;
		}
		const activeSelection = activeDocument.selection;
		const diagnostics = zyraxoncode.languages.getDiagnostics(activeDocument.document.uri).filter(diagnostic => {
			return !!activeSelection.intersection(diagnostic.range);
		}).map(d => d.message).join(', ');
		return zyraxoncode.commands.executeCommand('zyraxoncode.editorChat.start', { message: `/${Intent.Fix} ${diagnostics}`, autoSend: true, initialRange: zyraxoncode.window.activeTextEditor?.selection });
	};
	...
	const doGenerateAltText = async (arg: unknown) => {
		if (arg && typeof arg === 'object' && 'isUrl' in arg && 'resolvedImagePath' in arg && typeof arg.resolvedImagePath === 'string' && 'type' in arg) {
			...
			return zyraxoncode.commands.executeCommand('zyraxoncode.editorChat.start', { message: fullQuery, attachments: [uri], autoSend: true, initialRange: zyraxoncode.window.activeTextEditor?.selection });
		}
	};

	// register commands
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.explain', doExplain));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.explain.palette', () => doExplain(undefined, true)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review', () => instaService.createInstance(ReviewSession).review('selection', zyraxoncode.ProgressLocation.Notification)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.stagedChanges', () => instaService.createInstance(ReviewSession).review('index', zyraxoncode.ProgressLocation.Notification)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.unstagedChanges', () => instaService.createInstance(ReviewSession).review('workingTree', zyraxoncode.ProgressLocation.Notification)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.changes', () => instaService.createInstance(ReviewSession).review('all', zyraxoncode.ProgressLocation.Notification)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.stagedFileChange', (resource: zyraxoncode.SourceControlResourceState) => {
		return instaService.createInstance(ReviewSession).review({ group: 'index', file: resource.resourceUri }, zyraxoncode.ProgressLocation.Notification);
	}));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.unstagedFileChange', (resource: zyraxoncode.SourceControlResourceState) => {
		return instaService.createInstance(ReviewSession).review({ group: 'workingTree', file: resource.resourceUri }, zyraxoncode.ProgressLocation.Notification);
	}));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.codeReview.run', (input: CodeReviewInput) => {
		return instaService.invokeFunction(reviewFileChanges, input);
	}));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.apply', doApplyReview));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.applyAndNext', (commentThread: zyraxoncode.CommentThread) => doApplyReview(commentThread, true)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.applyShort', (commentThread: zyraxoncode.CommentThread) => doApplyReview(commentThread, true)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.continueInInlineChat', doContinueInInlineChat));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.continueInChat', doContinueInChat));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.discard', doDiscardReview));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.discardAndNext', (commentThread: zyraxoncode.CommentThread) => doDiscardReview(commentThread, true)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.discardShort', (commentThread: zyraxoncode.CommentThread) => doDiscardReview(commentThread, true)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.discardAll', doDiscardAllReview));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.markHelpful', markReviewHelpful));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.markUnhelpful', markReviewUnhelpful));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.previous', thread => goToNextReview(thread, -1)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.next', thread => goToNextReview(thread, +1)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.review.current', thread => goToNextReview(thread, 0)));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.generate', doGenerate));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.fix', doFix));
	disposables.add(zyraxoncode.commands.registerCommand('github.copilot.chat.generateAltText', doGenerateAltText));
	// register code actions
	disposables.add(zyraxoncode.languages.registerCodeActionsProvider('*', instaService.createInstance(QuickFixesProvider), {
		providedCodeActionKinds: QuickFixesProvider.providedCodeActionKinds,
	}));
	disposables.add(zyraxoncode.languages.registerCodeActionsProvider('*', instaService.createInstance(RefactorsProvider), {
		providedCodeActionKinds: RefactorsProvider.providedCodeActionKinds,
	}));
	disposables.add(zyraxoncode.notebooks.registerNotebookCellStatusBarItemProvider(
		'jupyter-notebook',
...
function fetchSuggestion(accessor: ServicesAccessor, thread: zyraxoncode.CommentThread) {
	const logService = accessor.get(ILogService);
	const reviewService = accessor.get(IReviewService);
	...
	comment.suggestion = (async () => {
		...
		const selection = new zyraxoncode.Selection(comment.range.start, comment.range.end);
		const textEditor = zyraxoncode.window.visibleTextEditors.find(editor => editor.document.uri.toString() === document.uri.toString()) ??
			zyraxoncode.window.activeTextEditor ??
			await zyraxoncode.window.showTextDocument(document.document, { preserveFocus: true, preview: false });

		const command = Intent.Fix;
		const prompt = message;
		const request: zyraxoncode.ChatRequest = {
			location: zyraxoncode.ChatLocation.Editor,
			location2: new zyraxoncode.ChatRequestEditorData(textEditor, document.document, selection, selection),
			command,
			prompt,
			references: [],
			attempt: 0,
