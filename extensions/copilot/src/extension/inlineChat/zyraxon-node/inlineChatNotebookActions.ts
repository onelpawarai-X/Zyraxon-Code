...
export class NotebookExectionStatusBarItemProvider implements zyraxoncode.NotebookCellStatusBarItemProvider {
	constructor(@ILogService private readonly logService: ILogService) { }

	async provideCellStatusBarItems(
		cell: zyraxoncode.NotebookCell,
		token: zyraxoncode.CancellationToken
	): Promise<zyraxoncode.NotebookCellStatusBarItem[]> {
		// check if quickfix contributions should come from core instead of copilot
		const coreQuickFix = zyraxoncode.workspace.getConfiguration('notebook').get<boolean>('cellFailureDiagnostics');
		if (coreQuickFix) {
			return [];
		}
		...
		if (cell.kind === zyraxoncode.NotebookCellKind.Markup) {
			...
			const enabled = zyraxoncode.workspace.getConfiguration('notebook.experimental').get<boolean>('cellChat');
			...
			return [
				{
					text: `$(sparkle)`,
					alignment: zyraxoncode.NotebookCellStatusBarAlignment.Left,
