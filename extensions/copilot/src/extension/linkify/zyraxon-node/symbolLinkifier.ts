...
import { SymbolInformation, Uri } from '../../../zyraxoncodeTypes';
...
interface ResolvedSymbolLinkMatch {
	...
	readonly initialLocation: zyraxoncode.Location | undefined;
}
...
export class SymbolLinkifier implements IContributedLinkifier {
	...
	private createSymbolAnchor(symbolText: string, resolvedUri: Uri, initialLocation: zyraxoncode.Location | undefined): LinkifySymbolAnchor {
		const info: SymbolInformation = {
			name: symbolText,
			containerName: '',
			kind: zyraxoncode.SymbolKind.Variable,
			location: initialLocation ?? new zyraxoncode.Location(resolvedUri, new zyraxoncode.Position(0, 0))
		};
		...
		return new LinkifySymbolAnchor(info, async (token) => {
			...
			try {
				symbols = await zyraxoncode.commands.executeCommand<Array<zyraxoncode.SymbolInformation | zyraxoncode.DocumentSymbol> | undefined>('zyraxoncode.executeDocumentSymbolProvider', resolvedUri);
			} catch {
				// noop
			}
			...
			if (symbols?.length) {
				...
				if (matchingSymbol) {
					...
					// Not a real instance of 'zyraxoncode.DocumentSymbol' so use cast to check
					if ((matchingSymbol as zyraxoncode.DocumentSymbol).children) {
						const symbol = matchingSymbol as zyraxoncode.DocumentSymbol;
						info.location = new zyraxoncode.Location(resolvedUri, collapseRangeToStart(symbol.selectionRange));
					} else {
						const symbol = matchingSymbol as zyraxoncode.SymbolInformation;
						info.location = new zyraxoncode.Location(symbol.location.uri, collapseRangeToStart(symbol.location.range));
					}
				}
			}
			return info;
