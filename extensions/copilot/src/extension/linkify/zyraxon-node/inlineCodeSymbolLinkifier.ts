...
import { SymbolInformation } from '../../../zyraxoncodeTypes';
...
export class InlineCodeSymbolLinkifier implements IContributedLinkifier {
	...
	async linkify(text: string, context: LinkifierContext, token: CancellationToken): Promise<LinkifiedText | undefined> {
		if (!context.references.length || zyraxoncode.version.startsWith('1.94')) {
			return;
		}
		...
		const resolvedMap = new Map<string, readonly zyraxoncode.Location[] | undefined>();
			...
			if (loc?.length) {
				...
				out.push(new LinkifySymbolAnchor(info, async (token) => {
					...
					if (dest) {
						...
						info.location = new zyraxoncode.Location(dest.loc.targetUri, collapseRangeToStart(selectionRange));
	...
	private async tryResolveSymbol(symbolText: string, context: LinkifierContext, token: CancellationToken): Promise<zyraxoncode.Location[] | undefined> {
		if (/^https?:\/\__ZYRAXKEEP__0_(symbolText)) {
			return;
		}
