...
async function doGetFileSymbols(parserService: IParserService, uri: zyraxoncode.Uri, token: CancellationToken): Promise<FileSymbols> {
...
function toFileSymbols(uri: zyraxoncode.Uri, doc: SimpleTextDocument, symbols: readonly TreeSitterExpressionInfo[]): readonly FileSymbol[] {
	return symbols.map(symbol => ({
		identifier: symbol.identifier,
		location: new zyraxoncode.Location(uri, doc.positionAt(symbol.startIndex))
	}));
}
		...
		await Promise.all(references.map(ref =>
			limiter.queue(async () => {
				...
				if (isUriComponents(ref.anchor)) {
					loc = await findWordInDoc(parserService, word, ref.anchor, new zyraxoncode.Range(0, 0, Number.MAX_SAFE_INTEGER, 0), options, token, documentCache);
					...
					loc = await findWordInDoc(parserService, word, ref.anchor.value, new zyraxoncode.Range(0, 0, Number.MAX_SAFE_INTEGER, 0), options, token, documentCache);
				}
...
async function findWordInDoc(parserService: IParserService, word: string, uri: zyraxoncode.Uri, range: zyraxoncode.Range, options: FindWordOptions, token: zyraxoncode.CancellationToken, documentCache?: Map<string, Promise<SimpleTextDocument | undefined>>): Promise<ResolvedWordLocation | undefined> {
	...
	for (const symbol of symbols) {
		if (symbol.identifier === word) {
			...
			return { type: ResolvedWordLocationType.SymbolMatch, location: new zyraxoncode.Location(uri, pos) };
		}
	}
	...
	for (const match of text.matchAll(new RegExp(escapeRegExpCharacters(word), 'g'))) {
		if (match.index) {
			...
			if ('getWordRangeAtPosition' in doc) {
				const wordInDoc = doc.getText((doc as zyraxoncode.TextDocument).getWordRangeAtPosition(wordPos));
				if (word === wordInDoc) {
					return { type: ResolvedWordLocationType.TextualMatch, location: new zyraxoncode.Location(uri, wordPos) };
				}
			} else {
				const wordInDoc = doc.getText(new zyraxoncode.Range(wordPos, doc.positionAt(doc.offsetAt(wordPos) + word.length)));
				if (word === wordInDoc) {
					return { type: ResolvedWordLocationType.TextualMatch, location: new zyraxoncode.Location(uri, wordPos) };
				}
			}
...
interface SimpleTextDocument {
	...
	offsetAt(position: zyraxoncode.Position): number;

	positionAt(offset: number): zyraxoncode.Position;
}


async function openDocument(uri: zyraxoncode.Uri, documentCache?: Map<string, Promise<SimpleTextDocument | undefined>>): Promise<SimpleTextDocument | undefined> {
	const vsCodeDoc = zyraxoncode.workspace.textDocuments.find(doc => doc.uri.toString() === uri.toString());
	if (vsCodeDoc) {
		return vsCodeDoc;
	}
...
async function doOpenDocument(uri: zyraxoncode.Uri): Promise<SimpleTextDocument | undefined> {
	try {
		const contents = await zyraxoncode.workspace.fs.readFile(uri);
		const languageId = getLanguageForResource(uri).languageId;
...
async function getSymbolsInRange(parserService: IParserService, doc: SimpleTextDocument, range: zyraxoncode.Range, token: zyraxoncode.CancellationToken): Promise<TreeSitterExpressionInfo[]> {
...
export class ReferencesSymbolResolver {
	...
	private readonly cache = new Map<string, Promise<zyraxoncode.Location[] | undefined>>();
	...
	async resolve(codeText: string, references: readonly PromptReference[], token: CancellationToken): Promise<zyraxoncode.Location[] | undefined> {
	...
	private async doResolve(codeText: string, references: readonly PromptReference[], token: CancellationToken): Promise<zyraxoncode.Location[] | undefined> {
		// Prefer exact match
