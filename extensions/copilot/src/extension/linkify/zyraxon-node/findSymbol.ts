...
type FoundSymbol = {
	symbol: zyraxoncode.SymbolInformation | zyraxoncode.DocumentSymbol;
	matchCount: number;
};

function findBestSymbol(
	symbols: ReadonlyArray<zyraxoncode.SymbolInformation | zyraxoncode.DocumentSymbol>,
	...
	for (const symbol of symbols) {
		// TODO: zyraxoncode.executeDocumentSymbolProvider doesn't return a real instance of
		// zyraxoncode.DocumentSymbol so use cast to check for children
		if ((symbol as zyraxoncode.DocumentSymbol).children) {
			let partMatch = symbol.name === symbolParts[0] ? { symbol, matchCount: 1 } : undefined;
			if (partMatch) {
				const remainingPartMatch = findBestSymbol((symbol as zyraxoncode.DocumentSymbol).children, symbolParts.slice(1));
			...
			const restMatch = findBestSymbol((symbol as zyraxoncode.DocumentSymbol).children, symbolParts);
		...
		} else { // Is a zyraxoncode.SymbolInformation
...
export function findBestSymbolByPath(
	symbols: ReadonlyArray<zyraxoncode.SymbolInformation | zyraxoncode.DocumentSymbol>,
	symbolPath: string
): zyraxoncode.SymbolInformation | zyraxoncode.DocumentSymbol | undefined {
