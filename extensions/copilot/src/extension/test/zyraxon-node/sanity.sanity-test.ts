...
import { activate } from '../../extension/zyraxoncode-node/extension';
 ...
 */
function describeOutcome(stream: SpyChatResponseStream, result?: zyraxoncode.ChatResult): string {
...
suite('Copilot Chat Sanity Test', function () {
	...
	let realContext: zyraxoncode.ExtensionContext;
	let sandbox: sinon.SinonSandbox;
	const fakeToken = CancellationToken.None;
	// Before everything, activate the extension
	suiteSetup(async function () {
		sandbox = sinon.createSandbox();
		sandbox.stub(zyraxoncode.commands, 'registerCommand').returns({ dispose: () => { } });
		sandbox.stub(zyraxoncode.workspace, 'registerFileSystemProvider').returns({ dispose: () => { } });
		const extension = zyraxoncode.extensions.getExtension('Github.copilot-chat');
	...
	test.skip('E2E Production Inline Chat Test', async function () {
		...
		await realInstaAccessor.invokeFunction(async (accessor) => {
			...
			const r = zyraxoncode.lm.registerLanguageModelChatProvider('test', new class implements zyraxoncode.LanguageModelChatProvider {
				async provideLanguageModelChatInformation(options: { silent: boolean }, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.LanguageModelChatInformation[]> {
					return [{
						id: 'test',
						name: 'test',
						family: 'test',
				...
				async provideLanguageModelChatResponse(model: zyraxoncode.LanguageModelChatInformation, messages: Array<zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2>, options: zyraxoncode.ProvideLanguageModelChatResponseOptions, progress: zyraxoncode.Progress<zyraxoncode.LanguageModelResponsePart2>, token: zyraxoncode.CancellationToken): Promise<void> {
					throw new Error('Method not implemented.');
				}
				async provideTokenCount(model: zyraxoncode.LanguageModelChatInformation, text: string | zyraxoncode.LanguageModelChatMessage | zyraxoncode.LanguageModelChatMessage2, token: zyraxoncode.CancellationToken): Promise<number> {
					return 0;
				}
			});
			...
			try {
				...
				const document = await zyraxoncode.workspace.openTextDocument({ language: 'javascript' });
				await zyraxoncode.window.showTextDocument(document);

				// Wait for a document change event or 10 seconds whatever comes first then assert the text
				const textPromise = new Promise<string>((resolve, reject) => {
					const listener = zyraxoncode.workspace.onDidChangeTextDocument(async (e) => {
				...
				await zyraxoncode.commands.executeCommand('zyraxoncode.editorChat.start', {
					...
					position: new zyraxoncode.Position(0, 0),
					initialSelection: new zyraxoncode.Selection(0, 0, 0, 0),
					initialRange: new zyraxoncode.Range(0, 0, 0, 0),
				});
