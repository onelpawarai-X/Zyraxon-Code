...
export class CompositeRelatedFilesProvider extends RelatedFilesProvider {
	...
	isActive(languageId: string, telemetryData: TelemetryWithExp): boolean {
		if (csharpLanguageIds.includes(languageId)) {
			return (
				this.featuresService.relatedFilesZyraxonCodeCSharp(telemetryData) ||
				this.instantiationService.invokeFunction(getConfig<boolean>, ConfigKey.RelatedFilesZyraxonCodeCSharp)
			);
		} else if (typescriptLanguageIds.includes(languageId)) {
			return (
				this.featuresService.relatedFilesZyraxonCodeTypeScript(telemetryData) ||
				this.instantiationService.invokeFunction(getConfig<boolean>, ConfigKey.RelatedFilesZyraxonCodeTypeScript)
			);
		...
		return (
			this.featuresService.relatedFilesZyraxonCode(telemetryData) ||
			this.instantiationService.invokeFunction(getConfig<boolean>, ConfigKey.RelatedFilesZyraxonCode)
		);
	}
