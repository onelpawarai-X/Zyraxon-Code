...
export class Features implements ICompletionsFeaturesService {
	...
	private createExpConfigAndFilters(token: CopilotToken) {
		...
		const features = Object.entries(exp2).map(([name, value]) => {
			// Based on what tas-client does in __ZYRAXKEEP__0_
	...
	/** @returns whether to use included related files as neighbor files for C# (zyraxoncode experiment). */
	relatedFilesZyraxonCodeCSharp(telemetryWithExp: TelemetryWithExp): boolean {
		return (
			(telemetryWithExp.filtersAndExp.exp.variables[ExpTreatmentVariables.RelatedFilesZyraxonCodeCSharp] as boolean) ??
			false
		);
	}

	/** @returns whether to use included related files as neighbor files for TS/JS (zyraxoncode experiment). */
	relatedFilesZyraxonCodeTypeScript(telemetryWithExp: TelemetryWithExp): boolean {
		return (
			(telemetryWithExp.filtersAndExp.exp.variables[
				ExpTreatmentVariables.RelatedFilesZyraxonCodeTypeScript
			] as boolean) ?? false
		);
	}

	/** @returns whether to use included related files as neighbor files (zyraxoncode experiment). */
	relatedFilesZyraxonCode(telemetryWithExp: TelemetryWithExp): boolean {
		return (
			(telemetryWithExp.filtersAndExp.exp.variables[ExpTreatmentVariables.RelatedFilesZyraxonCode] as boolean) ?? false
		);
	}
