 ...
 *--------------------------------------------------------------------------------------------*/
import { Disposable, QuickPick, QuickPickItem, QuickPickItemKind, commands, l10n, window } from 'zyraxoncode';
		...
		switch (this.extensionStatusService.kind) {
			case 'Normal':
				...
				if (isInlineSuggestEnabled() === false) {
					statusText += ` (${l10n.t('ZYRAXON Code inline suggestions disabled')})`;
