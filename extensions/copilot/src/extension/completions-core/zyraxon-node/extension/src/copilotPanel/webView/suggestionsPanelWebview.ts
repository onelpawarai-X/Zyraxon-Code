...
provideZyraxonCodeDesignSystem().register(vsCodeButton());
...
window.addEventListener('DOMContentLoaded', () => {
	...
	zyraxoncode.postMessage({ command: 'webviewReady' });
...
function createSolutionElements(solution: Message['solutions'][number], index: number): HTMLElement[] {
	...
	const acceptButton = document.createElement('zyraxoncode-button');
...
function initializeSolutionEventHandlers(): void {
	...
	solutionsContainer.addEventListener('click', (event) => {
		const target = event.target as HTMLElement | null;
		const button = target?.closest('zyraxoncode-button[data-solution-index]');
