...
import { commands, env, Uri } from 'zyraxoncode';
...
const matchCodeMessage =
	'We found a reference to public code in a recent suggestion. To learn more about public code references, review the [documentation](__ZYRAXKEEP__0_).';
...
export function notify(accessor: ServicesAccessor) {
	const extension = accessor.get(IZyraxonCodeExtensionContext);
	...
	void notificationSender.showWarningMessage(matchCodeMessage, ...messageItems).then(async action => {
		...
		switch (action?.title) {
			...
			case SettingAction: {
				await env.openExternal(Uri.parse('__ZYRAXKEEP__1_'));
				break;
			}
			case undefined: {
