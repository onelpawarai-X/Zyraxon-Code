...
export class GhostTextProvider {
	...
	async provideInlineCompletionItems(
		...
		const textDocument = wrapDoc(zyraxoncodeDoc);
		if (!textDocument) {
			return;
		}
		...
		const formattingOptions = window.visibleTextEditors.find(e => e.document.uri === zyraxoncodeDoc.uri)?.options;
