...
import { commands, Disposable, languages, LanguageStatusItem, LanguageStatusSeverity, window, workspace } from 'zyraxoncode';
		...
		switch (this.extensionStatusService.kind) {
			...
			case 'Normal':
				...
				if (!isInlineSuggestEnabled()) {
					...
					this.item.detail = 'ZYRAXON Code inline suggestions disabled';
				} else if (!enabled) {
					this.item.text = `${Icon.NotConnected} Completions`;
