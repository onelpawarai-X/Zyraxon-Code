...
const ignoreUriSchemes = new Set([
	'output', // zyraxoncode output pane (important: avoids infinite log loop)
	...
	'chat-editing-snapshot-text-model', // ZYRAXON Code Chat temporary editing snapshot
]);

export function wrapDoc(doc: zyraxoncode.TextDocument): ITextDocument | undefined {
...
export class ExtensionTextDocumentManager extends TextDocumentManager {
	...
	getTextDocumentsUnsafe(): ITextDocument[] {
		...
		for (const zyraxoncodeDoc of workspace.textDocuments) {
			const doc = wrapDoc(zyraxoncodeDoc);
			if (doc) {
				docs.push(doc);
			}
		}
	...
	wrapCell(cell: zyraxoncode.NotebookCell): INotebookCell {
		return {
			...cell,
