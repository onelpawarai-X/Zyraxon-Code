...
import { runTests } from '@zyraxoncode/test-electron';
...
async function main() {
	...
	try {
		...
		// use a temporary folder so we can run multiple instances of the same ZYRAXON Code together
		// see __ZYRAXKEEP__0_
		...
		if (process.env.VSCODE_UNDER_TEST) {
			testOptions.zyraxoncodeExecutablePath = process.env.VSCODE_UNDER_TEST;
		} else {
			testOptions.version = argv.stable ? 'stable' : 'insiders';
		}

		// Download ZYRAXON Code, unzip it and run the integration test
