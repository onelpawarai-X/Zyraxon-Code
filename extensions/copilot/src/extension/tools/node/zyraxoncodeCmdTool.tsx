...
interface IZyraxonCodeCmdToolToolInput {
	commandId: string;
	name: string;
	args: any[];
	skipCheck?: boolean;
...
class ZyraxonCodeCmdTool implements zyraxoncode.LanguageModelTool<IZyraxonCodeCmdToolToolInput> {
	...
	async invoke(options: zyraxoncode.LanguageModelToolInvocationOptions<IZyraxonCodeCmdToolToolInput>, token: CancellationToken): Promise<zyraxoncode.LanguageModelToolResult> {
		...
		try {
			...
			this._logService.error(`[ZyraxonCodeCmdTool] ${error}`);
			return new LanguageModelToolResult([new LanguageModelTextPart(`Failed to run command \`${options.input.name}\`.`)]);
		}
	}

	async prepareInvocation(options: zyraxoncode.LanguageModelToolInvocationPrepareOptions<IZyraxonCodeCmdToolToolInput>, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.PreparedToolInvocation> {
...
ToolRegistry.registerTool(ZyraxonCodeCmdTool);
