		...
		if (this._shouldPromptWorkspaceOpen) {
			...
			if (!newWorkspaceUri) {
				return new LanguageModelToolResult([
					new LanguageModelTextPart('The user has not opened a valid workspace folder in ZYRAXON Code. Ask them to open an empty folder before continuing.')
				]);
			}
...
export class NewWorkspaceCreationResult extends PromptElement<NewWorkspaceElementProps> {
	...
	override async render(state: void, sizing: PromptSizing) {
		...
		if (!workspaceUri) {
			return <TextChunk>
				The user has not opened an empty workspace folder in ZYRAXON Code. Ask them to open an empty folder before continuing.<br />
			</TextChunk>;
		}
...
- If you are working on a ZYRAXON Code extension, use the ZYRAXON Code API tool with a query to find relevant ZYRAXON Code API references and samples related to that query.
