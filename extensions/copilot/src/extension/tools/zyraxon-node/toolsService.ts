...
export class ToolsService extends BaseToolsService {
	...
	private readonly _contributedToolCache: {
		input: readonly zyraxoncode.LanguageModelToolInformation[];
		output: readonly zyraxoncode.LanguageModelToolInformation[];
	} = { input: [], output: [] };

	get tools(): ReadonlyArray<zyraxoncode.LanguageModelToolInformation> {
		const tools = zyraxoncode.lm.tools;
		...
		const contributedTools = [...input]
			.sort((a, b) => {
				...
				const aIsBuiltin = a.name.startsWith('zyraxoncode_') || a.name.startsWith('copilot_');
				const bIsBuiltin = b.name.startsWith('zyraxoncode_') || b.name.startsWith('copilot_');
		...
		const result: zyraxoncode.LanguageModelToolInformation[] = contributedTools.map(tool => {
			return {
				...tool,
	...
	invokeTool(name: string | ToolName, options: zyraxoncode.LanguageModelToolInvocationOptions<Object>, token: zyraxoncode.CancellationToken): Thenable<zyraxoncode.LanguageModelToolResult | zyraxoncode.LanguageModelToolResult2> {
		...
		if (String(name) === 'runSubagent') {
			...
			if (traceCtx) {
				if (chatStreamToolCallId) {
					...
					// ZYRAXON Code core uses the chatStreamToolCallId (LLM tool_use id) as
		...
		const optionsWithTrace = options as zyraxoncode.LanguageModelToolInvocationOptions<Object> & { traceparent?: string; tracestate?: string };
		...
		return zyraxoncode.lm.invokeTool(getContributedToolName(name), options, token).then(
			result => {
				...
				try {
					...
					for (const p of result.content) {
						if (p instanceof zyraxoncode.LanguageModelTextPart) {
							parts.push(p.value);
						} else if (p instanceof zyraxoncode.LanguageModelPromptTsxPart) {
							parts.push(JSON.stringify(p.value));
						} else if (p instanceof zyraxoncode.LanguageModelDataPart) {
	...
	override invokeToolWithEndpoint(name: string, options: zyraxoncode.LanguageModelToolInvocationOptions<Object>, endpoint: IChatEndpoint | undefined, token: zyraxoncode.CancellationToken): Thenable<zyraxoncode.LanguageModelToolResult2> {
		if (endpoint) {
	...
	getTool(name: string | ToolName): zyraxoncode.LanguageModelToolInformation | undefined {
