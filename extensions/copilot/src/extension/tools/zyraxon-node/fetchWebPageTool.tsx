 ...
 *--------------------------------------------------------------------------------------------*/
import { BasePromptElementProps, Chunk, PromptElement, PromptSizing, TextChunk, useKeepWith } from '@zyraxoncode/prompt-tsx';
import { CancellationToken, LanguageModelDataPart, LanguageModelPromptTsxPart, LanguageModelTextPart, LanguageModelToolInvocationOptions, LanguageModelToolInvocationPrepareOptions, LanguageModelToolResult, lm, PreparedToolInvocation, ProviderResult } from 'zyraxoncode';
 ...
 */
const internalToolName = 'zyraxoncode_fetchWebPage_internal';
...
class FetchWebPageTool implements ICopilotTool<IFetchWebPageParams> {
	...
	async invoke(options: LanguageModelToolInvocationOptions<IFetchWebPageParams>, token: CancellationToken): Promise<LanguageModelToolResult> {
		...
		if (filesAndTheirChunks) {
			...
			// already-completed fetch (see __ZYRAXKEEP__0_).
