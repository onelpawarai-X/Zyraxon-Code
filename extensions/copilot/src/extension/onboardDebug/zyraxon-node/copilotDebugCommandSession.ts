...
export const handleDebugSession = (
	...
	workspaceFolder: zyraxoncode.WorkspaceFolder | undefined,
	...
	let gotRoot = false;
	const sessions = new Set<zyraxoncode.DebugSession>();
	...
	handle.ended.then(() => {
		if (!store.isDisposed) {
			sessions.forEach(s => zyraxoncode.debug.stopDebugging(s));
		}
	});

	store.add(zyraxoncode.debug.registerDebugAdapterTrackerFactory('*', {
	...
	zyraxoncode.debug.startDebugging(workspaceFolder, { ...config, [TRACKED_SESSION_KEY]: trackedId }).then(ok => {
		if (!ok) {
			// error will be displayed to user by zyraxoncode
			ended(1);
		}
	});
};
