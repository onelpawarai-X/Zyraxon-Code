...
import { ChatSessionsUriHandler, CustomUriHandler } from '../../chatSessions/zyraxoncode/chatSessionsUriHandler';
...
export class CopilotDebugCommandContribution extends Disposable implements zyraxoncode.UriHandler {
	...
	constructor(
		...
		this._register(zyraxoncode.window.registerUriHandler(this));
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.startCopilotDebugCommand', async () => {
			const term = zyraxoncode.window.createTerminal();
			term.show(false);
		...
		const globPattern = new zyraxoncode.RelativePattern(this.context.globalStorageUri, '.pendingSession');
		const fileWatcher = zyraxoncode.workspace.createFileSystemWatcher(globPattern);
	...
	private async ensureTask(workspaceFolder: URI | undefined, def: zyraxoncode.TaskDefinition, handle: CopilotDebugCommandHandle): Promise<boolean> {
		if (!workspaceFolder) {
		...
		*/
		...
		const socket = connect(pipePath, () => {
			...
			const factory = this.instantiationService.createInstance(CopilotDebugCommandSessionFactory, {
				ensureTask: (wf, def) => this.ensureTask(wf || zyraxoncode.workspace.workspaceFolders?.[0].uri, def, handle),
				...
				switch (result.kind) {
					...
					case StartResultKind.Ok:
						if (opts.printOnly) {
							...
							handle.confirm(l10n.t('Configuration saved, debug now?'), true).then(debug => {
								if (debug) {
									zyraxoncode.debug.startDebugging(result.folder && zyraxoncode.workspace.getWorkspaceFolder(result.folder), result.config);
								}
								exit(0);
							});
						} else {
							handleDebugSession(
								launchConfigService,
								result.folder && zyraxoncode.workspace.getWorkspaceFolder(result.folder),
								{
									...result.config,
					...
					case StartResultKind.NeedExtension:
						handle.confirm(l10n.t`We generated a "${result.debugType}" debug configuration, but you don't have an extension installed for that. Do you want to look for one?`, true).then(search => {
							if (search) {
								zyraxoncode.commands.executeCommand('workbench.extensions.search', `@category:debuggers ${result.debugType}`);
							}
							exit(0);
						});
						break;
	...
	private getVersionNonce() {
		if (this.context.extensionMode !== zyraxoncode.ExtensionMode.Production) {
			return String(Date.now());
		}

		const extensionInfo = zyraxoncode.extensions.getExtension(EXTENSION_ID);
		return (extensionInfo?.packageJSON.version ?? String(Date.now())) + '/' + zyraxoncode.env.remoteName;
	}
	...
	private async fillStoragePath(storagePath: string) {
		const callbackUri = zyraxoncode.Uri.from({
			scheme: zyraxoncode.env.uriScheme,
			authority: EXTENSION_ID,
		});

		let remoteCommand = '';
		if (zyraxoncode.env.remoteName) {
			remoteCommand = (zyraxoncode.env.appName.includes('Insider') ? 'code-insiders' : 'code') + ' --openExternal ';
		}
...
const makeShellScript = (remoteCommand: string, dir: string, callbackUri: zyraxoncode.Uri) => `#!/bin/sh
unset NODE_OPTIONS
