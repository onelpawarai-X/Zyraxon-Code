...
export class LaunchConfigService implements ILaunchConfigService {
	...
	/** @inheritdoc */
	async add(workspaceFolder: URI | undefined, toAdd: { configurations: zyraxoncode.DebugConfiguration[]; inputs?: unknown[] }): Promise<void> {
		const config = zyraxoncode.workspace.getConfiguration('launch', workspaceFolder);

		const existingConfigs = config.get<zyraxoncode.DebugConfiguration[]>('configurations');
	...
	async show(workspaceFolder: URI, showConfigName?: string): Promise<void> {
		const fileUri = URI.joinPath(workspaceFolder, '.zyraxoncode', 'launch.json');
		let document: zyraxoncode.TextDocument | undefined;
		try {
			document = await zyraxoncode.workspace.openTextDocument(fileUri);
		} catch {
			return;
		}
				...
				JSONC.visit(text, {
					...
					onObjectEnd(endOffset, length) {
						...
						if (didFind) {
							...
							range = new zyraxoncode.Range(start.lineNumber - 1, start.column - 1, end.lineNumber - 1, end.column - 1);
						}
					},
		...
		await zyraxoncode.window.showTextDocument(document, { selection: range });
	}

	/** @inheritdoc */
	async launch(config: zyraxoncode.DebugConfiguration | ILaunchJSON): Promise<void> {
		const debugConfig: zyraxoncode.DebugConfiguration | undefined = 'configurations' in config && config.configurations.length ? config.configurations[0] : config;
		if (!debugConfig) {
			return;
		}
		await zyraxoncode.debug.startDebugging(undefined, debugConfig);
	}
	...
	async resolveConfigurationInputs(launchJson: ILaunchJSON, defaults?: Map<string, string>, interactor?: ICommandInteractor) {
		if (!interactor) {
			interactor = {
				...
				prompt: async (text: string, defaultValue?: string) => {
					return await zyraxoncode.window.showInputBox({
						prompt: text,
						value: defaultValue,
						ignoreFocusOut: true,
					});
		...
		const config: zyraxoncode.DebugConfiguration = cloneAndChange(launchJson.configurations[0], orig => {
