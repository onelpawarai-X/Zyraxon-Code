...
export class NotebookFollowCommands extends Disposable {
	...
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.notebook.enableFollowCellExecution', () => {
			this.updateFollowContext(true);
		}));

		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.notebook.disableFollowCellExecution', () => {
			this.updateFollowContext(false);
		}));
	}

	private updateFollowContext(value: boolean): void {
		zyraxoncode.commands.executeCommand('setContext', NOTEBOOK_FOLLOW_IN_SESSION_KEY, value);
