...
class TelemetrySender {
	...
	public sendRequestTelemetry(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: RequestContext, data: ContextItemSummary, timeTaken: number, cacheState: { before: CacheState; after: CacheState } | undefined, cacheRequest: string | undefined): void {
...
type RequestInfo = {
	...
	readonly position: zyraxoncode.Position;
...
class RunnableResultManager implements zyraxoncode.Disposable {
	...
	private readonly withInRangeRunnableResults: { resultId: protocol.ContextRunnableResultId; range: zyraxoncode.Range }[];
	private readonly outsideRangeRunnableResults: { resultId: protocol.ContextRunnableResultId; ranges: zyraxoncode.Range[] }[] = [];
	...
	constructor() {
		...
		this.disposables.add(zyraxoncode.workspace.onDidChangeTextDocument((event: zyraxoncode.TextDocumentChangeEvent) => {
		...
		this.disposables.add(zyraxoncode.workspace.onDidCloseTextDocument((document: zyraxoncode.TextDocument) => {
			if (this.requestInfo?.document === document.uri.toString()) {
				this.clear();
			}
		}));
		this.disposables.add(zyraxoncode.window.onDidChangeActiveTextEditor(() => {
			this.clear();
		}));
		this.disposables.add(zyraxoncode.window.tabGroups.onDidChangeTabs((event: zyraxoncode.TabChangeEvent) => {
	...
	public update(document: zyraxoncode.TextDocument, version: number, position: zyraxoncode.Position, context: RequestContext, body: protocol.ComputeContextResponse.OK, requestState: ContextRequestState | undefined): ManagerUpdateResult {
		...
		const updateRunnableResult = (resultItem: protocol.ContextRunnableResultTypes): ResolvedRunnableResult | undefined => {
			...
			if (result.cache !== undefined) {
				if (result.cache.scope.kind === protocol.CacheScopeKind.WithinRange) {
					...
					const range = new zyraxoncode.Range(scopeRange.start.line, scopeRange.start.character, scopeRange.end.line, scopeRange.end.character);
					...
					for (const scopeRange of result.cache.scope.ranges) {
						ranges.push(new zyraxoncode.Range(scopeRange.start.line, scopeRange.start.character, scopeRange.end.line, scopeRange.end.character));
					}
	...
	public getCachedRunnableResults(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, emitMode?: protocol.EmitMode): ResolvedRunnableResult[] {
		const results: ResolvedRunnableResult[] = [];
		if (this.requestInfo?.document !== document.uri.toString()) {
			return results;
		}
		...
		for (const item of this.results.values()) {
			...
			if (scope === undefined || scope.kind !== protocol.CacheScopeKind.WithinRange) {
				...
				const r = scope.range;
				const range = new zyraxoncode.Range(r.start.line, r.start.character, r.end.line, r.end.character);
	...
	public getContextRequestState(document: zyraxoncode.TextDocument, position: zyraxoncode.Position): ContextRequestState | undefined {
		if (this.requestInfo?.document !== document.uri.toString()) {
			return undefined;
		}
		if (this.cacheInfo.version !== document.version || this.cacheInfo.state === CacheState.NotPopulated || this.requestInfo.path.length === 0 || this.requestInfo.path[0] === 0) {
	...
	private isCacheFullyUpToDate(document: zyraxoncode.TextDocument, position: zyraxoncode.Position): boolean {
		...
		const range = position.isBefore(this.requestInfo.position) ? new zyraxoncode.Range(position, this.requestInfo.position) : new zyraxoncode.Range(this.requestInfo.position, position);
	...
	private affectsTypeScript(event: zyraxoncode.TextDocumentChangeEvent): boolean {
	...
	private applyTextContentChangeEventToWithinRange(event: zyraxoncode.TextDocumentContentChangeEvent, range: zyraxoncode.Range): zyraxoncode.Range {
		...
		return new zyraxoncode.Range(range.start, new zyraxoncode.Position(endLine, endCharacter));
	}

	private applyTextContentChangeEventToOutsideRanges(event: zyraxoncode.TextDocumentContentChangeEvent, ranges: zyraxoncode.Range[]): zyraxoncode.Range[] | undefined {
		if (ranges.length === 0) {
			return ranges;
		}
		...
		const adjustedRanges: zyraxoncode.Range[] = [];
		...
		for (const range of ranges) {
			if (range.end.isBefore(changeRange.start)) {
				...
				if (linesDelta === 0) {
					...
					adjustedRanges.push(new zyraxoncode.Range(
						new zyraxoncode.Position(range.start.line + linesDelta, range.start.character),
						new zyraxoncode.Position(range.end.line + linesDelta, range.end.character)
					));
				}
			} else {
...
namespace TextDocuments {
	export function consider(document: zyraxoncode.TextDocument): boolean {
		return document.uri.scheme === 'file' && (document.languageId === 'typescript' || document.languageId === 'typescriptreact');
	}
}

class NeighborFileModel implements zyraxoncode.Disposable {
	...
	constructor() {
		...
		this.disposables.add(zyraxoncode.window.onDidChangeActiveTextEditor((editor: zyraxoncode.TextEditor | undefined) => {
			if (editor === undefined) {
				return;
			}
		...
		this.disposables.add(zyraxoncode.window.tabGroups.onDidChangeTabs((e: zyraxoncode.TabChangeEvent) => {
		...
		const textDocumentsToConsider: Map<string, zyraxoncode.Uri> = new Map();
		for (const document of zyraxoncode.workspace.textDocuments) {
			if (TextDocuments.consider(document)) {
				textDocumentsToConsider.set(document.uri.toString(), document.uri);
			}
		}
		for (const group of zyraxoncode.window.tabGroups.all) {
			for (const tab of group.tabs) {
				const uri = tab.input instanceof zyraxoncode.TabInputText ? tab.input.uri : undefined;
	...
	public getNeighborFiles(currentDocument: zyraxoncode.TextDocument): string[] {
		const result: string[] = [];
...
namespace ComputeContextRequestArgs {
	export function create(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: RequestContext, startTime: number, timeBudget: number, willLogRequestTelemetry: boolean, neighborFiles: readonly string[] | undefined, clientSideRunnableResults: readonly protocol.CachedContextRunnableResult[] | undefined, includeDocumentation: boolean): ComputeContextRequestArgs {
		return {
...
class PendingRequestInfo {
	...
	constructor(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: RequestContext) {
...
class InflightRequestInfo {
	...
	private readonly tokenSource: zyraxoncode.CancellationTokenSource;

	constructor(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: RequestContext, tokenSource: zyraxoncode.CancellationTokenSource, serverPromise: Thenable<protocol.ComputeContextResponse>) {
		this.document = document.uri.toString();
		this.position = position;
	...
	public matches(document: zyraxoncode.TextDocument, position: zyraxoncode.Position): boolean {
		return this.document === document.uri.toString() && this.position.isEqual(position);
	}

	public matchesDocument(document: zyraxoncode.TextDocument): boolean {
...
class OnTimeoutData {
	...
	private readonly position: zyraxoncode.Position;

	public readonly runnableResults: ResolvedRunnableResult[] = [];
	public resultBuilder: ContextItemResultBuilder | undefined;

	constructor(document: zyraxoncode.TextDocument, position: zyraxoncode.Position) {
		this.document = document.uri.toString();
		this.version = document.version;
		this.position = position;
	}
	...
	matches(document: zyraxoncode.TextDocument, position: zyraxoncode.Position): boolean {
...
export class LanguageContextServiceImpl implements ILanguageContextService, zyraxoncode.Disposable {
	...
	private _onCachePopulated: zyraxoncode.EventEmitter<OnCachePopulatedEvent>;
	public readonly onCachePopulated: zyraxoncode.Event<OnCachePopulatedEvent>;

	private _onContextComputed: zyraxoncode.EventEmitter<OnContextComputedEvent>;
	public readonly onContextComputed: zyraxoncode.Event<OnContextComputedEvent>;

	private _onContextComputedOnTimeout: zyraxoncode.EventEmitter<OnContextComputedOnTimeoutEvent>;
	public readonly onContextComputedOnTimeout: zyraxoncode.Event<OnContextComputedOnTimeoutEvent>;
	...
	constructor(
		...
		this._onCachePopulated = this.disposables.add(new zyraxoncode.EventEmitter<OnCachePopulatedEvent>());
		this.onCachePopulated = this._onCachePopulated.event;

		this._onContextComputed = this.disposables.add(new zyraxoncode.EventEmitter<OnContextComputedEvent>());
		this.onContextComputed = this._onContextComputed.event;

		this._onContextComputedOnTimeout = this.disposables.add(new zyraxoncode.EventEmitter<OnContextComputedOnTimeoutEvent>());
	...
	async isActivated(documentOrLanguageId: zyraxoncode.TextDocument | string): Promise<boolean> {
	...
	private async doIsTypeScriptActivated(languageId: string): Promise<boolean> {
		...
		try {
			...
			const response: protocol.PingResponse | undefined = await zyraxoncode.commands.executeCommand('typescript.tsserverRequest', '_.copilot.ping', LanguageContextServiceImpl.ExecConfig, CancellationToken.None);
	...
	async populateCache(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: RequestContext): Promise<void> {
		...
		try {
			...
			try {
				const promise: Thenable<protocol.ComputeContextResponse> = zyraxoncode.commands.executeCommand('typescript.tsserverRequest', '_.copilot.context', args, LanguageContextServiceImpl.ExecConfig, token);
		...
		if (this.pendingRequest !== undefined) {
			...
			this.pendingRequest = undefined;
			const textEditor = zyraxoncode.window.activeTextEditor;
	...
	public async *getContext(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: RequestContext, token: zyraxoncode.CancellationToken): AsyncIterable<ContextItem> {
		this.onTimeoutData = undefined;
		if (document.languageId !== 'typescript' && document.languageId !== 'typescriptreact') {
			return;
		}
	...
	private getRunnables(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, cachePopulationInflight: boolean): { mandatory: readonly ResolvedRunnableResult[]; optional: readonly ResolvedRunnableResult[]; onTimeout: readonly ResolvedRunnableResult[] } {
	...
	public getContextOnTimeout(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: RequestContext): readonly ContextItem[] | undefined {
		try {
			if (this.onTimeoutData === undefined) {
				return [];
			}
	...
	private getCharacterBudget(context: RequestContext, document: zyraxoncode.TextDocument): CharacterBudget {
...
interface TokenBudgetProvider {
	getTokenBudget(document: zyraxoncode.TextDocument): number;
}

class CachePopulationTrigger implements zyraxoncode.Disposable {
	...
	constructor(languageContextService: ILanguageContextService, tokenBudgetProvider: TokenBudgetProvider) {
		...
		this.disposables.add(zyraxoncode.workspace.onDidChangeTextDocument((event) => {
			// console.log(`Text document change ${Date.now()}`);
			this.didChangeTextDocument(event);
		}));

		this.disposables.add(zyraxoncode.window.onDidChangeActiveTextEditor((editor) => {
			this.didChangeActiveTextEditor(editor);
		}));

		this.disposables.add(zyraxoncode.window.onDidChangeTextEditorSelection(async (event) => {
			// console.log(`Selection ${Date.now()}`);
			this.didChangeTextEditorSelection(event);
		}));
		this.disposables.add(zyraxoncode.languages.registerInlineCompletionItemProvider([{ scheme: 'file', language: 'typescript' }, { scheme: 'file', language: 'typescriptreact' }], {
	...
	private didChangeTextDocument(event: zyraxoncode.TextDocumentChangeEvent): void {
		const time = Date.now();
		this.lastDocumentChange = undefined;
	...
	private didChangeActiveTextEditor(editor: zyraxoncode.TextEditor | undefined): void {
	...
	private didChangeTextEditorSelection(event: zyraxoncode.TextEditorSelectionChangeEvent): void {
		...
		try {
			if (event.kind === zyraxoncode.TextEditorSelectionChangeKind.Command || event.kind === zyraxoncode.TextEditorSelectionChangeKind.Mouse) {
				this.selectionChangeDebouncer.cancel();
	...
	private onInlineCompletion(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, context: zyraxoncode.InlineCompletionContext): void {
		const tokenBudget = this.tokenBudgetProvider.getTokenBudget(document);
		if (tokenBudget <= 0) {
			// There is no token budget left, so we don't want to trigger the cache population.
			return;
	...
	private getPosition(event: zyraxoncode.TextEditorSelectionChangeEvent): zyraxoncode.Position | undefined {
		const time = Date.now();
		const activeEditor = zyraxoncode.window.activeTextEditor;
	...
	private populateCache(document: zyraxoncode.TextDocument, position: zyraxoncode.Position, tokenBudget: number, requestId: string | undefined, trigger: TriggerKind, check: boolean): void {
		if (check) {
...
export class InlineCompletionContribution implements zyraxoncode.Disposable, TokenBudgetProvider {
	...
	constructor(
		...
		if (languageContextService instanceof LanguageContextServiceImpl) {
			this.disposables.add(zyraxoncode.commands.registerCommand('github.copilot.debug.showContextInspectorView', async () => {
				await zyraxoncode.commands.executeCommand('setContext', showContextInspectorViewContextKey, true);
				await zyraxoncode.commands.executeCommand('context-inspector.focus');
			}));
			this.disposables.add(zyraxoncode.window.registerTreeDataProvider('context-inspector', new InspectorDataProvider(languageContextService)));
		}

		// Check if there are any TypeScript files open in the workspace.
		const open = zyraxoncode.workspace.textDocuments.some((document) => document.languageId === 'typescript' || document.languageId === 'typescriptreact');
		if (open) {
			this.typeScriptFileOpen();
		} else {
			const disposable = zyraxoncode.workspace.onDidOpenTextDocument((document) => {
				if (document.languageId === 'typescript' || document.languageId === 'typescriptreact') {
					disposable.dispose();
					this.typeScriptFileOpen();
				}
	...
	private async register(): Promise<void> {
		...
		try {
			...
			const resolver: Copilot.ContextResolver<Copilot.SupportedContextItem> = {
				resolve(request: Copilot.ResolveRequest, token: zyraxoncode.CancellationToken): Promise<Copilot.SupportedContextItem[]> | AsyncIterable<Copilot.SupportedContextItem> {
	...
	private getDocumentAndPosition(request: Copilot.ResolveRequest, token?: zyraxoncode.CancellationToken): [zyraxoncode.TextDocument | undefined, zyraxoncode.Position | undefined] {
		let document: zyraxoncode.TextDocument | undefined;
		if (zyraxoncode.window.activeTextEditor?.document.uri.toString() === request.documentContext.uri) {
			document = zyraxoncode.window.activeTextEditor.document;
		} else {
			document = zyraxoncode.workspace.textDocuments.find((doc) => doc.uri.toString() === request.documentContext.uri);
		}
		...
		const position = requestPos !== undefined ? new zyraxoncode.Position(requestPos.line, requestPos.character) : document.positionAt(request.documentContext.offset);
	...
	private async getCopilotApi(): Promise<Copilot.ContextProviderApiV1 | undefined> {
		const copilotExtension = zyraxoncode.extensions.getExtension('GitHub.copilot');
	...
	public getTokenBudget(document: zyraxoncode.TextDocument): number {
