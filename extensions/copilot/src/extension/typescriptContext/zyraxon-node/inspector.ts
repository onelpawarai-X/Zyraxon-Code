...
class TreePropertyItem {
	...
	public toTreeItem(): zyraxoncode.TreeItem {
		const item = new zyraxoncode.TreeItem(`${this.name} = ${this.value}`, zyraxoncode.TreeItemCollapsibleState.None);
		item.tooltip = this.createTooltip();
		item.id = this.id;
		return item;
	}

	protected createTooltip(): zyraxoncode.MarkdownString {
		const markdown = new zyraxoncode.MarkdownString(`${this.value}`);
		return markdown;
	}
...
abstract class TreeContextItem {
	...
	protected createTooltip(): zyraxoncode.MarkdownString {
		const markdown = new zyraxoncode.MarkdownString(`**${this.getLabel()}**\n\n`);
...
class TreeTrait extends TreeContextItem {
	...
	public toTreeItem(): zyraxoncode.TreeItem {
		...
		const item = new zyraxoncode.TreeItem(label, zyraxoncode.TreeItemCollapsibleState.Collapsed);
		item.tooltip = this.createTooltip();
		item.id = this.id;
		return item;
	}
...
class TreeCacheInfo {
	...
	public toTreeItem(): zyraxoncode.TreeItem {
		const item = new zyraxoncode.TreeItem(this.getLabel());
		item.collapsibleState = this.from.scope.kind === protocol.CacheScopeKind.OutsideRange || this.from.scope.kind === protocol.CacheScopeKind.WithinRange ? zyraxoncode.TreeItemCollapsibleState.Collapsed : zyraxoncode.TreeItemCollapsibleState.None;
		return item;
	}
...
class TreeRunnableResult {
	...
	private createTooltip(): zyraxoncode.MarkdownString {
		let id = this.from.id;
		if (id.startsWith('_')) {
			id = id.substring(1);
		}
		const markdown = new zyraxoncode.MarkdownString(`**${id}** - ${this.items.length} items\n\n`);
		markdown.appendCodeblock(JSON.stringify(this.from, undefined, 2), 'json');
		return markdown;
	}
}
...
class TreeYieldedSnippet {
	...
	public toTreeItem(): zyraxoncode.TreeItem {
		const item = new zyraxoncode.TreeItem(`${this.getLabel()}: ${this.from.value}`, zyraxoncode.TreeItemCollapsibleState.Collapsed);
		item.tooltip = this.createTooltip();
		return item;
	}
...
abstract class TreeContextRequest {
	...
	protected readonly position: zyraxoncode.Position;
	...
	public toTreeItem(): zyraxoncode.TreeItem {
		const item = new zyraxoncode.TreeItem(this.label, zyraxoncode.TreeItemCollapsibleState.Collapsed);
		item.tooltip = this.createTooltip();
		return item;
	}

	private createTooltip(): zyraxoncode.MarkdownString {
		const markdown = new zyraxoncode.MarkdownString(`**${this.label}**\n\n`);
...
class TreeYieldContextRequest extends TreeContextRequest {
	...
	public override toTreeItem(): zyraxoncode.TreeItem {
		const item = new zyraxoncode.TreeItem(this.label, zyraxoncode.TreeItemCollapsibleState.Collapsed);
		item.id = this.id;
		return item;
	}
...
export class InspectorDataProvider implements zyraxoncode.TreeDataProvider<InspectorItems> {

	private readonly languageContextService: IInternalLanguageContextService;

	private readonly _onDidChangeTreeData: zyraxoncode.EventEmitter<InspectorItems | InspectorItems[] | undefined | null | void>;
	public readonly onDidChangeTreeData: zyraxoncode.Event<InspectorItems | InspectorItems[] | undefined | null | void>;
	...
	constructor(languageContextService: IInternalLanguageContextService) {
		...
		this._onDidChangeTreeData = new zyraxoncode.EventEmitter<InspectorItems | InspectorItems[] | undefined | null | void>();
	...
	public getTreeItem(element: InspectorItems): zyraxoncode.TreeItem | Thenable<zyraxoncode.TreeItem> {
