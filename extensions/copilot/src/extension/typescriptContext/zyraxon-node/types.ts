...
export interface IInternalLanguageContextService extends ILanguageContextService {
	onCachePopulated: zyraxoncode.Event<OnCachePopulatedEvent>;
	onContextComputed: zyraxoncode.Event<OnContextComputedEvent>;
	onContextComputedOnTimeout: zyraxoncode.Event<OnContextComputedOnTimeoutEvent>;
}
...
export class ContextItemResultBuilder implements ContextItemSummary {
	...
	public updateResponse(result: protocol.ContextRequestResult, token: zyraxoncode.CancellationToken): void {
		...
		switch (item.kind) {
			case protocol.ContextKind.Snippet:
				return {
					...
					id: id,
					priority: priority,
					uri: zyraxoncode.Uri.file(item.fileName),
					additionalUris: item.additionalFileNames?.map(uri => zyraxoncode.Uri.file(uri)),
					value: item.value
				};
			case protocol.ContextKind.Trait:
				return {
