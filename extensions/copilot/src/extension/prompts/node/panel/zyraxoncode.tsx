...
import { CommandListItem, ICombinedEmbeddingIndex, SettingListItem, settingItemToContext } from '../../../../platform/embeddings/common/zyraxoncodeIndex';
...
export class VscodePrompt extends PromptElement<VscodePromptProps, VscodePromptState> {
	...
	override async prepare(sizing: PromptSizing, progress: zyraxoncode.Progress<zyraxoncode.ChatResponseProgressPart> | undefined, token: zyraxoncode.CancellationToken): Promise<VscodePromptState> {
		...
		this.logService.debug('[ZyraxonCode Prompt] Asking the model to update the user question.');
		...
		let zyraxoncodeApiSearch = false;
		if (fetchResult.type === ChatFetchResponseType.Success) {
			...
			extensionSearch = fetchResult.value.includes('zyraxoncode_extensions');
			zyraxoncodeApiSearch = fetchResult.value.includes('zyraxoncode_api');
		} else {
			this.logService.error(`[ZyraxonCode Prompt] Failed to refine the question: ${fetchResult.requestId}`);
		}

		const currentSanitized = sanitizeZyraxonCodeVersion(this.envService.getEditorInfo().version); // major.minor
		...
		if (extensionSearch || zyraxoncodeApiSearch) {
		...
		return (
			<>
				<SystemMessage priority={1000}>
					You are a ZYRAXON Code assistant. Your job is to assist users in using ZYRAXON Code by providing knowledge to accomplish their task. This knowledge should focus on settings, commands, keybindings but also includes documentation. <br />
					{state.query.length < 1 && <>
						If the user does not include a question, respond with: I am your ZYRAXON Code assistant. I can help you with settings, commands, keybindings, extensions, and documentation. Ask me anything about using or configuring ZYRAXON Code.<br />
					</>}
					...
					<InstructionMessage>
						...
						If a command or setting is not a valid answer, but it still relates to ZYRAXON Code, please still respond.<br />
		...
		"details": {
			"key": "workbench.action.openWalkthrough",
							...
							<Tag name='multipleSettingsExample'>
								...
								Question: How can I change the font size in all areas of ZYRAXON Code, including the editor, terminal?<br />
								Response:<br />
								...
								This **window.zoomLevel** setting controls the zoom level of the entire ZYRAXON Code interface.<br />
								<UnsafeCodeBlock code={`
[
	{
		"type": "setting",
		...
		"details": {
			...
			"value": "1"
							...
							<Tag name='multipleCommandsExample'>
								...
								Question: How can I setup a python virtual environment in ZYRAXON Code?<br />
								Response:<br />
		...
		"details": {
			"key": "python.setInterpreter"
							...
							<Tag name='invalidQuestionExample'>
								...
								Response:<br />
								Sorry this question isn't related to ZYRAXON Code.<br />
							</Tag>
		...
		"details": {
			...
			"value": "php"
						...
						<Tag name='extensionSearchResponseRules'>
							If you referene any extensions, you must respond with with the identifiers as a comma seperated string inside ```zyraxoncode-extensions code block. <br />
							...
							<Tag name='extensionResponseExample'>
								...
								<UnsafeCodeBlock code={`
ms-python.python,ms-python.zyraxoncode-pylance
								`} languageId='zyraxoncode-extensions'></UnsafeCodeBlock>
							</Tag>
						</Tag>
				...
				<UserMessage flexGrow={1} priority={800}>
					...
					{state.currentVersion && <><Tag name='currentZyraxonCodeVersion'>
						Current ZYRAXON Code version (major.minor): {state.currentVersion}
					</Tag><br /></>}
					...
					<Tag name='zyraxoncodeAPIToolUseInstructions'>
						Always call the tool {ToolName.ZyraxonCodeAPI} to get documented references and examples when before responding to questions about ZYRAXON Code Extension Development.<br />
					</Tag>
					<Tag name='searchExtensionToolUseInstructions'>
						Always call the tool 'zyraxoncode_searchExtensions_internal' to first search for extensions in the ZYRAXON Code Marketplace before responding about extensions.<br />
					</Tag>
					<Tag name='zyraxoncodeCmdToolUseInstructions'>
						Call the tool {ToolName.RunVscodeCmd} to run commands in ZYRAXON Code, only use as part of a new workspace creation process. <br />
...
class VscodeMetaPrompt extends PromptElement<VscodeMetaPromptProps> {
	...
	override render(state: void, sizing: PromptSizing): PromptPiece<any, any> | undefined {
		return <>
			<SystemMessage priority={1000}>
				You are a ZYRAXON Code assistant who helps the user create well-formed and unambiguous queries about their ZYRAXON Code development environment.<br />
				Specifically, you help users rewrite questions about how to use ZYRAXON Code's Commands and Settings.
			</SystemMessage>
			<HistoryWithInstructions historyPriority={500} passPriority history={this.props.history || []}>
				<InstructionMessage priority={1000}>
					...
					Determine if the user's question is about the editor, terminal, activity bar, side bar, status bar, panel or other parts of ZYRAXON Code's workbench and include those keyword in the rewrite.<br />
					Determine if the user is asking about ZYRAXON Code's Commands and/or Settings and explicitly include those keywords during the rewrite. <br />
					...
					If the user is asking about ZYRAXON Code Release Notes, respond using this exact protocol and do not rephrase the question: <br />
					...
					If the user is asking about Extensions available in ZYRAXON Code, simply respond with "zyraxoncode_extensions"<br />
					If the user is asking about ZYRAXON Code API or ZYRAXON Code Extension Development, simply respond with "zyraxoncode_api"<br />
					...
					If it is not clear what the user is asking for or if the question appears to be unrelated to ZYRAXON Code, do not try to rephrase the question and simply return the original question. <br />
					...
					Assistant:<br />
					# Other Question<br />
					Set up python development in ZYRAXON Code<br />
					<br />
					User: Show me popular extensions<br />
					<br />
					Assistant:<br />
					zyraxoncode_extensions<br />
					<br />
