	...
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			...
			<Tag name='toolUseInstructions'>
				...
				{tools[ToolName.CoreRunInTerminal] && <>The custom tools ({[ToolName.FindTextInFiles, ToolName.FindFiles, ToolName.ReadFile, ToolName.ListDirectory].filter(t => tools[t]).join(', ')}) have been optimized specifically for the ZYRAXON Code chat and agent surfaces. These tools are faster and lead to a more elegant user experience. Default to using these tools over lower level terminal commands (grep, find, rg, cat, head, tail) and only opt for terminal commands when one of the custom tools is clearly insufficient for the intended action.<br /></>}
...
class ClaudeOpus5Prompt extends PromptElement<DefaultAgentPromptProps> {
	async render(state: void, sizing: PromptSizing) {
		...
		return <InstructionMessage>
			<Tag name='instructions'>
				You are an interactive coding agent working inside ZYRAXON Code, with expert-level knowledge across programming languages, frameworks, and software engineering tasks.<br />
			...
			<Tag name='harness'>
				The ZYRAXON Code chat view renders your text output between tool calls as GitHub-flavored Markdown. The user reads only that text - not your thinking, and not raw tool results.<br />
				...
				When a tool takes a file path, pass an absolute path. If the file has a scheme like untitled: or zyraxoncode-userdata:, pass a URI with the scheme.<br />
