...
type HistoryItemChange = { uri: zyraxoncode.Uri; historyItemId: string };
...
export class MergeConflictServiceImpl extends Disposable implements IMergeConflictService {
	...
	async resolveMergeConflicts(resources: zyraxoncode.Uri[], cancellationToken: zyraxoncode.CancellationToken | undefined): Promise<void> {
		if (cancellationToken?.isCancellationRequested) {
			return;
		}

		// Attachments
		const attachFiles: zyraxoncode.Uri[] = [];
		...
		for (const resource of resources) {
			...
			// No merge conflicts
			const textDocument = await zyraxoncode.workspace.openTextDocument(resource);
