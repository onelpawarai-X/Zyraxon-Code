...
export class MergeConflictParser {
	...
	static scanDocument(document: zyraxoncode.TextDocument): IDocumentMergeConflictDescriptor[] {
		...
		for (let i = 0; i < document.lineCount; i++) {
			...
			// Are we within a conflict block and is this a footer? >>>>>>>
				...
				const completeDescriptor = MergeConflictParser.scanItemToMergeConflictDescriptor(document, currentConflict);
	...
	private static scanItemToMergeConflictDescriptor(document: zyraxoncode.TextDocument, scanned: IScanMergedConflict): IDocumentMergeConflictDescriptor | null {
