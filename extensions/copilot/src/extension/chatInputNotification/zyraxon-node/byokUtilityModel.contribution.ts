...
export class ByokUtilityModelNotificationContribution extends Disposable {
	...
	private _notification: zyraxoncode.ChatInputNotification | undefined;
	...
	constructor(
		...
		if (zyraxoncode.workspace.isAgentSessionsWorkspace) {
			return;
		}

		this._register(this._authService.onDidAuthenticationChange(() => this._update()));
		this._register(zyraxoncode.lm.onDidChangeChatModels(() => this._update()));
	...
	private _showNotification(utilityUnset: boolean, utilitySmallUnset: boolean): void {
		...
		notification.severity = zyraxoncode.ChatInputNotificationSeverity.Info;
		notification.dismissible = true;
		notification.autoDismissOnMessage = false;

		if (utilityUnset && utilitySmallUnset) {
			notification.message = zyraxoncode.l10n.t('Set BYOK utility models');
			notification.description = zyraxoncode.l10n.t('Unlocks full AI features.');
			notification.actions = [
				{ label: zyraxoncode.l10n.t('Configure'), commandId: 'workbench.action.openSettings', commandArgs: [BYOK_UTILITY_MODEL_DEFAULT_SETTING] },
			];
		} else if (utilityUnset) {
			notification.message = zyraxoncode.l10n.t('Set BYOK utility model');
			notification.description = zyraxoncode.l10n.t('Unlocks full AI features.');
			notification.actions = [
				{ label: zyraxoncode.l10n.t('Configure'), commandId: 'workbench.action.openSettings', commandArgs: [UTILITY_MODEL_SETTING] },
			];
		} else {
			notification.message = zyraxoncode.l10n.t('Set BYOK small utility model');
			notification.description = zyraxoncode.l10n.t('Unlocks full AI features.');
			notification.actions = [
				{ label: zyraxoncode.l10n.t('Configure'), commandId: 'workbench.action.openSettings', commandArgs: [UTILITY_SMALL_MODEL_SETTING] },
			];
		}
	...
	private _ensureNotification(): zyraxoncode.ChatInputNotification {
		if (!this._notification) {
			this._notification = zyraxoncode.chat.createInputNotification(NOTIFICATION_ID);
