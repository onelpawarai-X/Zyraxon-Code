...
export const GHPR_EXTENSION_ID = 'GitHub.zyraxoncode-pull-request-github';
...
export type CustomUriHandler = zyraxoncode.UriHandler & { canHandleUri(uri: zyraxoncode.Uri): boolean };
...
export class ChatSessionsUriHandler extends Disposable implements CustomUriHandler {
	...
	async handleUri(uri: zyraxoncode.Uri): Promise<void> {
		switch (uri.path) {
			case UriHandlerPaths.OpenSession:
				{
					...
					*/
					this._telemetryService.sendTelemetryEvent('copilot.codingAgent.deeplink', { Zyraxon: true, github: false }, {
	...
	private async _openGitHubSession(type: string, id: string, url: string | null, branch: string | null): Promise<void> {
		...
		if (gitAPI && url && branch) {
			...
			const pendingSessionUri = zyraxoncode.Uri.joinPath(this._extensionContext.globalStorageUri, '.pendingSession');
			try {
			...
			const uri = zyraxoncode.Uri.parse(url);
			const cachedWorkspaces: zyraxoncode.Uri[] | null = await gitAPI.getRepositoryWorkspace(uri);

			let folderToOpen: zyraxoncode.Uri | null = null;
			if (!cachedWorkspaces || (cachedWorkspaces && cachedWorkspaces.length > 1)) {
				const selectFolderItem: zyraxoncode.QuickPickItem & { uri?: zyraxoncode.Uri } = {
					label: 'Select Directory...',
					description: 'Choose a directory to open',
					uri: undefined
				};
				const cloneRepoItem: zyraxoncode.QuickPickItem & { uri?: zyraxoncode.Uri } = {
					label: 'Clone Repository and Open',
					description: 'Clone the repository to a new local folder and open it',
					uri: undefined
				};

				const items: (zyraxoncode.QuickPickItem & { uri?: zyraxoncode.Uri })[] = [selectFolderItem];
				items.push({
					label: '',
				...
				if (selected) {
					if (selected === selectFolderItem) {
						const selectedFolder = await zyraxoncode.window.showOpenDialog({
						...
						folderToOpen = await gitAPI.clone(zyraxoncode.Uri.parse(url), { postCloneAction: 'none', ref: branch });
					}
				}
			} else {
			...
			const forceReuseWindow = ((zyraxoncode.workspace.workspaceFile === undefined) && (zyraxoncode.workspace.workspaceFolders === undefined));
			zyraxoncode.commands.executeCommand('zyraxoncode.openFolder', folderToOpen, { forceReuseWindow });
			return;
		}
	...
	public canHandleUri(uri: zyraxoncode.Uri): boolean {
	...
	public async openPendingSession(details?: {
		...
		const uri = zyraxoncode.Uri.from({ scheme: 'copilot-cloud-agent', path: '/' + pullRequest.number.toString() });
		await this._extensionContext.globalState.update(PENDING_CHAT_SESSION_STORAGE_KEY, undefined);
		await zyraxoncode.commands.executeCommand('zyraxoncode.open', uri);
