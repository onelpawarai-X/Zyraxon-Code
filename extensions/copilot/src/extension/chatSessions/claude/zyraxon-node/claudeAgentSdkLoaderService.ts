...
class SdkExtensionNotInstalledError extends Error {
	constructor() {
		super('The ms-zyraxoncode.zyraxoncode-claude-sdk extension is not installed.');
...
export class VsCodeClaudeAgentSdkLoaderService implements IClaudeAgentSdkLoaderService {
	...
	get isAvailable(): boolean {
		return zyraxoncode.extensions.getExtension(CLAUDE_SDK_EXTENSION_ID) !== undefined;
	}

	async install(token: zyraxoncode.CancellationToken): Promise<boolean> {
		if (token.isCancellationRequested) {
			return false;
		}

		try {
			await zyraxoncode.commands.executeCommand('workbench.extensions.installExtension', CLAUDE_SDK_EXTENSION_ID);
		} catch (err) {
	...
	private async _doLoad(): Promise<typeof AgentSdk> {
		const ext = zyraxoncode.extensions.getExtension<SDKActivation>(CLAUDE_SDK_EXTENSION_ID);
		if (!ext) {
