...
export interface IClaudeSlashCommandService {
	 ...
	 *
	 * Checks `request.command` first (ZYRAXON Code slash command), then falls back to
...
export class ClaudeSlashCommandService extends Disposable implements IClaudeSlashCommandService {
	...
	constructor(
		...
		super();
		// Initialize eagerly to register ZYRAXON Code commands at startup
	...
	async tryHandleCommand(
		...
		// 1. Check request.command (ZYRAXON Code slash command selected via UI)
		if (request.command) {
	...
	private _ensureInitialized(): void {
		...
		for (const ctor of ctors) {
			...
			// Register ZYRAXON Code command if commandId is provided
			if (handler.commandId) {
				this._register(zyraxoncode.commands.registerCommand(handler.commandId, () => {
