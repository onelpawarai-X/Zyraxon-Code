...
export class MemorySlashCommand implements IClaudeSlashCommandHandler {
	...
	async handle(
		...
		stream?.markdown(zyraxoncode.l10n.t('Opening memory file picker...'));
		...
		this._runPicker().catch(error => {
			...
			zyraxoncode.window.showErrorMessage(
				zyraxoncode.l10n.t('Error opening memory file: {0}', error instanceof Error ? error.message : String(error))
			);
		});
	...
	private async _runPicker(): Promise<void> {
		...
		const selected = await zyraxoncode.window.showQuickPick(items, {
			placeHolder: zyraxoncode.l10n.t('Select memory file to edit'),
			ignoreFocusOut: true,
		});
	...
	private _getMemoryLocations(): MemoryLocation[] {
		...
		locations.push({
			type: 'user',
			label: zyraxoncode.l10n.t('User memory'),
		...
		for (const folder of workspaceFolders) {
			...
			locations.push({
				type: 'project',
				label: isMultiRoot
					? zyraxoncode.l10n.t('Project memory - {0}', folderName)
					: zyraxoncode.l10n.t('Project memory'),
			...
			locations.push({
				type: 'local',
				label: isMultiRoot
					? zyraxoncode.l10n.t('Project memory (local) - {0}', folderName)
					: zyraxoncode.l10n.t('Project memory (local)'),
	...
	private async _buildQuickPickItems(locations: MemoryLocation[]): Promise<(zyraxoncode.QuickPickItem & { location: MemoryLocation })[]> {
		const items: (zyraxoncode.QuickPickItem & { location: MemoryLocation })[] = [];
		...
		for (const location of locations) {
			...
			items.push({
				...
				description: exists ? location.description : zyraxoncode.l10n.t('{0} (will be created)', location.description),
				location,
			});
		}
	...
	private async _openOrCreateMemoryFile(location: MemoryLocation): Promise<void> {
		...
		// Open in editor
		const doc = await zyraxoncode.workspace.openTextDocument(zyraxoncode.Uri.file(location.path.fsPath));
