...
import { CancellationToken } from 'zyraxoncode';
...
export interface IClaudeSlashCommandHandler {
	...
	/**
	 * Optional ZYRAXON Code command ID for Command Palette registration.
	 * If provided, the command will be registered as a ZYRAXON Code command
 ...
 *    - Under `contributes.commands`: Add the ZYRAXON Code command (e.g., `copilot.claude.mycommand`)
 ...
 *
 *     async handle(args: string, stream: zyraxoncode.ChatResponseStream, token: CancellationToken) {
