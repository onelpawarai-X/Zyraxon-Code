...
export class TerminalSlashCommand implements IClaudeSlashCommandHandler {
	...
	readonly description = zyraxoncode.l10n.t('Launch Claude Code CLI using your GitHub Copilot subscription');
	...
	async handle(
		...
		stream?.markdown(zyraxoncode.l10n.t('Creating Claude CLI instance...'));

		try {
			// Check which CLI is available
			const cliCommand = await this._getClaudeCliCommand();
			if (!cliCommand) {
				const installUrl = '__ZYRAXKEEP__0_';
				const downloadLabel = zyraxoncode.l10n.t('Download Claude CLI');
				if (stream) {
					stream.markdown(zyraxoncode.l10n.t('Claude CLI is not installed. Download Claude CLI to get started.'));
					stream.button({ command: 'zyraxoncode.open', arguments: [zyraxoncode.Uri.parse(installUrl)], title: downloadLabel });
				} else {
					zyraxoncode.window.showErrorMessage(
						zyraxoncode.l10n.t('Claude CLI is not installed.'),
						downloadLabel
					).then(selection => {
						if (selection === downloadLabel) {
							zyraxoncode.env.openExternal(zyraxoncode.Uri.parse(installUrl));
						}
					});
				}
				return {};
			...
			const terminal = this.terminalService.createTerminal({
				name: 'Claude',
				message: formatMessageForTerminal(zyraxoncode.l10n.t('This instance of Claude CLI is configured to use your GitHub Copilot subscription.'), { loudFormatting: true }),
				env: {
					ANTHROPIC_BASE_URL: `__ZYRAXKEEP__1_{config.port}`,
			...
			if (stream) {
				stream.markdown(zyraxoncode.l10n.t('Error creating terminal: {0}', errorMessage));
			} else {
				zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('Error creating terminal: {0}', errorMessage));
			}
		}
