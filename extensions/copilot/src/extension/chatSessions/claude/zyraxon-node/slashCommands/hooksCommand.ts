...
const HOOK_EVENTS = [
	...
	{
		id: 'PreToolUse',
		label: zyraxoncode.l10n.t('Before tool execution'),
		needsMatcher: true,
		inputDescription: zyraxoncode.l10n.t('Exit 0: allow, Exit 2: block with stderr to model.'),
		jsonSchema: '{ "tool_name": string, "tool_input": object }',
	},
	{
		id: 'PostToolUse',
		label: zyraxoncode.l10n.t('After tool execution'),
		needsMatcher: true,
		inputDescription: zyraxoncode.l10n.t('Runs after tool completes successfully.'),
		jsonSchema: '{ "tool_name": string, "tool_input": object, "tool_response": string }',
	},
	{
		id: 'PostToolUseFailure',
		label: zyraxoncode.l10n.t('After tool execution fails'),
		needsMatcher: true,
		inputDescription: zyraxoncode.l10n.t('Runs when a tool fails or is interrupted.'),
		jsonSchema: '{ "tool_name": string, "tool_input": object, "error": string, "is_interrupt": boolean }',
	},
	{
		id: 'PermissionRequest',
		label: zyraxoncode.l10n.t('When permission dialog would be displayed'),
		needsMatcher: true,
		inputDescription: zyraxoncode.l10n.t('Custom permission handling. Exit 0: allow, Exit 2: deny.'),
	...
	{
		id: 'UserPromptSubmit',
		label: zyraxoncode.l10n.t('When the user submits a prompt'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Exit 0: allow, Exit 2: block with stderr to model.'),
	...
	{
		id: 'Stop',
		label: zyraxoncode.l10n.t('When agent execution stops'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Use to save state or clean up resources.'),
		jsonSchema: '{ "stop_hook_active": boolean }',
	},
	{
		id: 'SubagentStart',
		label: zyraxoncode.l10n.t('When a subagent is initialized'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Track parallel task spawning.'),
		jsonSchema: '{ "agent_id": string, "agent_type": string }',
	},
	{
		id: 'SubagentStop',
		label: zyraxoncode.l10n.t('When a subagent completes'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Aggregate results from parallel tasks.'),
		jsonSchema: '{ "agent_id": string, "agent_transcript_path": string, "stop_hook_active": boolean }',
	},
	{
		id: 'PreCompact',
		label: zyraxoncode.l10n.t('Before conversation compaction'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Archive transcript before summarizing.'),
		jsonSchema: '{ "trigger": "manual" | "auto", "custom_instructions": string }',
	},
	{
		id: 'SessionStart',
		label: zyraxoncode.l10n.t('When a session is initialized'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Initialize logging and telemetry.'),
		jsonSchema: '{ "source": "startup" | "resume" | "clear" | "compact" }',
	},
	{
		id: 'SessionEnd',
		label: zyraxoncode.l10n.t('When a session terminates'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Clean up temporary resources.'),
		jsonSchema: '{ "reason": "clear" | "logout" | "prompt_input_exit" | "other" }',
	},
	{
		id: 'Notification',
		label: zyraxoncode.l10n.t('When agent status messages are sent'),
		needsMatcher: false,
		inputDescription: zyraxoncode.l10n.t('Send updates to Slack or dashboards.'),
...
export class HooksSlashCommand implements IClaudeSlashCommandHandler {
	...
	async handle(
		...
		stream?.markdown(zyraxoncode.l10n.t('Opening hooks configuration...'));
		...
		this._runWizard().catch(error => {
			...
			zyraxoncode.window.showErrorMessage(
				zyraxoncode.l10n.t('Error configuring hook: {0}', error instanceof Error ? error.message : String(error))
			);
		});
	...
	private async _openFileAtHook(location: SettingsLocation, command: string): Promise<void> {
		try {
			const document = await zyraxoncode.workspace.openTextDocument(zyraxoncode.Uri.file(location.settingsPath.fsPath));
			...
			let position = new zyraxoncode.Position(0, 0);
			if (match) {
				const beforeMatch = text.substring(0, match.index);
				const lineNumber = (beforeMatch.match(/\n/g) || []).length;
				const lastNewline = beforeMatch.lastIndexOf('\n');
				const column = match.index - lastNewline - 1 + match[0].indexOf(command);
				position = new zyraxoncode.Position(lineNumber, column);
			}
			...
			editor.revealRange(new zyraxoncode.Range(position, position), zyraxoncode.TextEditorRevealType.InCenter);
		} catch (error) {
	...
	private async _selectHookEvent(): Promise<HookEvent | undefined> {
		...
		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Configure Hook'),
			placeHolder: zyraxoncode.l10n.t('Which hook would you like to configure?'),
			matchOnDetail: true,
			ignoreFocusOut: true
		});
	...
	private async _selectOrCreateMatcher(eventConfig: HookEvent): Promise<{
		...
		type MatcherItem = zyraxoncode.QuickPickItem & {
			isAddNew: boolean;
			matcher?: string;
			location?: SettingsLocation;
		};
		...
		const addNewItem: MatcherItem = {
			label: '$(add) ' + zyraxoncode.l10n.t('Add new matcher...'),
			isAddNew: true,
		};

		// Group matchers by location label and create items with separators
		const items: (MatcherItem | zyraxoncode.QuickPickItem)[] = [addNewItem];
		...
		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Configure Hook: {0}', eventConfig.id),
			placeHolder: zyraxoncode.l10n.t('Which tool should trigger this hook?'),
			ignoreFocusOut: true,
		...
		if (selected.isAddNew) {
			...
			const newMatcher = await zyraxoncode.window.showInputBox({
				title: zyraxoncode.l10n.t('Configure Hook: {0}', eventConfig.id),
				prompt: zyraxoncode.l10n.t('Enter a tool name or pattern (e.g., "Bash", "Edit", or "*" for all)'),
				placeHolder: zyraxoncode.l10n.t('Which tool should trigger this hook?'),
				ignoreFocusOut: true,
			});
	...
	private async _selectOrAddHookForEdit(
		...
		type HookItem = zyraxoncode.QuickPickItem & {
			isAddNew: boolean;
			command?: string;
		};

		const addNewItem: HookItem = {
			label: '$(add) ' + zyraxoncode.l10n.t('Add new hook...'),
			isAddNew: true,
		};

		// Build items with location label separator
		const items: (HookItem | zyraxoncode.QuickPickItem)[] = [addNewItem];
	...
	private async _selectOrAddHookFromList(
		...
		const editedCommand = await zyraxoncode.window.showInputBox({
			title: zyraxoncode.l10n.t('Edit Hook: {0}', eventConfig.id),
			value: selected.command,
			prompt: zyraxoncode.l10n.t('Modifying {0}. Stdin Input: {1}', selected.location!.label, eventConfig.jsonSchema),
			placeHolder: './my-hook-script.sh',
			ignoreFocusOut: true,
		});
	...
	private async _enterCommand(eventConfig: HookEvent, matcher: string, locationLabel?: string): Promise<string | undefined> {
		const promptText = locationLabel
			? zyraxoncode.l10n.t('Modifying {0}. Stdin Input: {1}', locationLabel, eventConfig.jsonSchema)
			: zyraxoncode.l10n.t('What shell command should run? Stdin Input: {0}', eventConfig.jsonSchema);
	...
	private async _selectSaveLocation(): Promise<SettingsLocation | undefined> {
		type LocationItem = zyraxoncode.QuickPickItem & {
			location: SettingsLocation;
		};
		...
		for (const folderUri of workspaceFolders) {
			...
			items.push({
				label: workspaceFolders.length > 1
					? zyraxoncode.l10n.t('Workspace (local) - {0}', folderName)
					: zyraxoncode.l10n.t('Workspace (local)'),
				description: `${folderName}/.claude/settings.local.json`,
				location: {
					type: 'local',
					label: workspaceFolders.length > 1 ? zyraxoncode.l10n.t('Workspace (local) - {0}', folderName) : zyraxoncode.l10n.t('Workspace (local)'),
			...
			items.push({
				label: workspaceFolders.length > 1
					? zyraxoncode.l10n.t('Workspace - {0}', folderName)
					: zyraxoncode.l10n.t('Workspace'),
				description: `${folderName}/.claude/settings.json`,
				location: {
					type: 'shared',
					label: workspaceFolders.length > 1 ? zyraxoncode.l10n.t('Workspace - {0}', folderName) : zyraxoncode.l10n.t('Workspace'),
					workspaceFolder: folderUri,
		...
		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Save Hook Configuration'),
			placeHolder: zyraxoncode.l10n.t('Where should this hook be saved?'),
			ignoreFocusOut: true,
		});
