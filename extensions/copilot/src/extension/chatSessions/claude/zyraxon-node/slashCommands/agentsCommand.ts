...
export class AgentsSlashCommand implements IClaudeSlashCommandHandler {
	...
	async handle(
		_args: string,
		stream: zyraxoncode.ChatResponseStream | undefined,
		_token: CancellationToken
	): Promise<zyraxoncode.ChatResult> {
		stream?.markdown(zyraxoncode.l10n.t('Opening agents configuration...'));
		...
		this._runWizard().catch(error => {
			...
			zyraxoncode.window.showErrorMessage(
				zyraxoncode.l10n.t('Error configuring agent: {0}', error instanceof Error ? error.message : String(error))
			);
		});
	...
	private async _showMainMenu(): Promise<{ action: 'create' | 'select'; agent?: AgentWithSource } | undefined> {
		...
		type AgentMenuItem = zyraxoncode.QuickPickItem & {
			action: 'create' | 'select';
			agent?: AgentWithSource;
		};

		const items: (AgentMenuItem | zyraxoncode.QuickPickItem)[] = [];

		// Create new agent option
		items.push({
			label: '$(add) ' + zyraxoncode.l10n.t('Create new agent'),
			action: 'create',
		});
		...
		if (projectAgents.length > 0) {
			items.push({
				label: zyraxoncode.l10n.t('Project agents'),
		...
		if (userAgents.length > 0) {
			items.push({
				label: zyraxoncode.l10n.t('Personal agents'),
				kind: zyraxoncode.QuickPickItemKind.Separator,
			});
		...
		const placeholderText = projectAgents.length === 0 && userAgents.length === 0
			? zyraxoncode.l10n.t('No agents found. Create specialized subagents that Claude can delegate to.')
			: zyraxoncode.l10n.t('Select an agent to view, edit, or delete');

		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Agents'),
	...
	private async _selectLocation(): Promise<AgentLocation | undefined> {
		type LocationItem = zyraxoncode.QuickPickItem & { location: AgentLocation };
		...
		if (workspaceFolders.length > 0) {
			...
			items.push({
				label: zyraxoncode.l10n.t('1. Project (.claude/agents/)'),
				location: {
					type: 'project',
					label: zyraxoncode.l10n.t('Project'),
		...
		// Personal location
		items.push({
			label: zyraxoncode.l10n.t('2. Personal (~/.claude/agents/)'),
			location: {
				type: 'user',
				label: zyraxoncode.l10n.t('Personal'),
				agentsDir: URI.joinPath(this.envService.userHome, '.claude', 'agents'),
			},
		});

		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Create new agent'),
			placeHolder: zyraxoncode.l10n.t('Choose location'),
			ignoreFocusOut: true,
		});
	...
	private async _selectCreationMethod(): Promise<'generate' | 'manual' | undefined> {
		const items: (zyraxoncode.QuickPickItem & { method: 'generate' | 'manual' })[] = [
			{
				label: zyraxoncode.l10n.t('1. Generate with Claude (recommended)'),
				method: 'generate',
			},
			{
				label: zyraxoncode.l10n.t('2. Manual configuration'),
				method: 'manual',
			},
		];
		...
		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Create new agent'),
			placeHolder: zyraxoncode.l10n.t('Creation method'),
			ignoreFocusOut: true,
		});
	...
	private async _runGenerateFlow(location: AgentLocation): Promise<void> {
		...
		const description = await zyraxoncode.window.showInputBox({
			title: zyraxoncode.l10n.t('Create new agent'),
			prompt: zyraxoncode.l10n.t('Describe what this agent should do and when it should be used (be comprehensive for best results)'),
			placeHolder: zyraxoncode.l10n.t('e.g., Help me write unit tests for my code...'),
			ignoreFocusOut: true,
		});
		...
		const generated = await zyraxoncode.window.withProgress({
			location: zyraxoncode.ProgressLocation.Notification,
			title: zyraxoncode.l10n.t('Generating agent from description...'),
			cancellable: true,
	...
	private async _runManualFlow(location: AgentLocation): Promise<void> {
		...
		const name = await zyraxoncode.window.showInputBox({
			title: zyraxoncode.l10n.t('Create new agent'),
			prompt: zyraxoncode.l10n.t('Enter a unique identifier for your agent:'),
			placeHolder: zyraxoncode.l10n.t('e.g., test-runner, tech-lead, etc'),
			ignoreFocusOut: true,
			validateInput: value => {
				if (!value) {
					return zyraxoncode.l10n.t('Agent name is required');
				}
				if (!/^[a-z0-9-]+$/.test(value)) {
					return zyraxoncode.l10n.t('Use lowercase letters, numbers, and hyphens only');
				}
				return null;
			},
		});
		...
		const systemPrompt = await zyraxoncode.window.showInputBox({
			title: zyraxoncode.l10n.t('Create new agent'),
			prompt: zyraxoncode.l10n.t('Enter the system prompt for your agent:') + '\n' + zyraxoncode.l10n.t('Be comprehensive for best results'),
			placeHolder: zyraxoncode.l10n.t('You are a helpful code reviewer who...'),
			ignoreFocusOut: true,
		});
		...
		const description = await zyraxoncode.window.showInputBox({
			title: zyraxoncode.l10n.t('Create new agent'),
			prompt: zyraxoncode.l10n.t('When should Claude use this agent?'),
			placeHolder: zyraxoncode.l10n.t("e.g., use this agent after you're done writing code..."),
			ignoreFocusOut: true,
		});
...
Respond ONLY with the JSON object, no markdown code blocks or other text.`;
			...
			let models = await zyraxoncode.lm.selectChatModels({ family: 'claude-sonnet-4.5', vendor: 'copilot' });
			if (models.length === 0) {
				// Fallback to any available model
				models = await zyraxoncode.lm.selectChatModels({ vendor: 'copilot' });
				// Get latest claude-sonnet- model
				models = models
					.filter(model => model.family.startsWith('claude-sonnet-'))
					.sort((a, b) => b.family.localeCompare(a.family));
				if (models.length === 0) {
					zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('No language model available for agent generation'));
					return undefined;
				}
			}

			const response = await models[0].sendRequest(
				[zyraxoncode.LanguageModelChatMessage.User(prompt)],
				{},
				token
			);
			...
			for await (const chunk of response.stream) {
				if (chunk instanceof zyraxoncode.LanguageModelTextPart) {
			...
			zyraxoncode.window.showErrorMessage(
				zyraxoncode.l10n.t('Failed to generate agent: {0}', error instanceof Error ? error.message : String(error))
			);
			return undefined;
		}
	}
	...
	private async _selectTools(): Promise<string[] | undefined> {
		type ToolPickItem = zyraxoncode.QuickPickItem & {
			categoryId?: string;
			toolId?: string;
		};

		// Toggle button for advanced options
		const showAdvancedButton: zyraxoncode.QuickInputButton = {
			iconPath: new zyraxoncode.ThemeIcon('chevron-down'),
			tooltip: zyraxoncode.l10n.t('Show advanced options'),
		};
		const hideAdvancedButton: zyraxoncode.QuickInputButton = {
			iconPath: new zyraxoncode.ThemeIcon('chevron-up'),
			tooltip: zyraxoncode.l10n.t('Hide advanced options'),
		};
		...
		return new Promise<string[] | undefined>((resolve) => {
			...
			const quickPick = zyraxoncode.window.createQuickPick<ToolPickItem>();
			disposables.add(quickPick);
			quickPick.title = zyraxoncode.l10n.t('Create new agent');
			quickPick.placeholder = zyraxoncode.l10n.t('Select tools');
			...
			const updateItems = () => {
				...
				if (showAdvanced) {
					items.push({
						label: zyraxoncode.l10n.t('Individual Tools'),
						kind: zyraxoncode.QuickPickItemKind.Separator,
					});
	...
	private async _selectModel(): Promise<string | undefined> {
		...
		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Create new agent'),
			placeHolder: zyraxoncode.l10n.t('Select model') + '\n' + zyraxoncode.l10n.t("Model determines the agent's reasoning capabilities and speed."),
			ignoreFocusOut: true,
		});
	...
	private async _runAgentActionMenu(agent: AgentWithSource): Promise<void> {
		type ActionItem = zyraxoncode.QuickPickItem & { action: 'view' | 'edit' | 'delete' | 'back' };

		const items: ActionItem[] = [
			{ label: zyraxoncode.l10n.t('1. View agent'), action: 'view' },
			{ label: zyraxoncode.l10n.t('2. Edit agent'), action: 'edit' },
			{ label: zyraxoncode.l10n.t('3. Delete agent'), action: 'delete' },
			{ label: zyraxoncode.l10n.t('4. Back'), action: 'back' },
		];

		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: agent.config.name,
			placeHolder: zyraxoncode.l10n.t('Choose an action'),
			ignoreFocusOut: true,
		});
	...
	private async _runEditMenu(agent: AgentWithSource): Promise<void> {
		type EditItem = zyraxoncode.QuickPickItem & { action: 'open' | 'tools' | 'model' };

		const items: EditItem[] = [
			{ label: '$(edit) ' + zyraxoncode.l10n.t('Open in editor'), action: 'open' },
			{ label: '$(tools) ' + zyraxoncode.l10n.t('Edit tools'), action: 'tools' },
			{ label: '$(symbol-misc) ' + zyraxoncode.l10n.t('Edit model'), action: 'model' },
		];

		const selected = await zyraxoncode.window.showQuickPick(items, {
			title: zyraxoncode.l10n.t('Edit agent: {0}', agent.config.name),
			placeHolder: zyraxoncode.l10n.t('Source: {0}', agent.location.label),
			ignoreFocusOut: true,
		});
	...
	private async _deleteAgent(agent: AgentWithSource): Promise<void> {
		const confirm = await zyraxoncode.window.showWarningMessage(
			zyraxoncode.l10n.t('Are you sure you want to delete the agent "{0}"?', agent.config.name),
			{ modal: true },
			zyraxoncode.l10n.t('Delete')
		);

		if (confirm === zyraxoncode.l10n.t('Delete')) {
			await this.fileSystemService.delete(agent.filePath);
			zyraxoncode.window.showInformationMessage(zyraxoncode.l10n.t('Agent "{0}" deleted', agent.config.name));
			// Return to main menu
			await this._runWizard();
		}
	}
	...
	private async _loadAgentsFromDirectory(dir: URI, location: AgentLocation): Promise<AgentWithSource[]> {
		...
		try {
			...
			for (const [name, type] of entries) {
				if (type === zyraxoncode.FileType.File && name.endsWith('.md')) {
	...
	private async _openAgentFile(filePath: URI): Promise<void> {
		const doc = await zyraxoncode.workspace.openTextDocument(zyraxoncode.Uri.file(filePath.fsPath));
