...
This folder contains the Claude Code integration for ZYRAXON Code Chat. It enables users to open a new Chat window and interact with a Claude Code instance directly within ZYRAXON Code. **ZYRAXON Code provides the UI, Claude Code provides the smarts.**
...
The Claude Code integration allows ZYRAXON Code's chat interface to communicate with Claude Code, Anthropic's agentic coding assistant. When a user sends a message in a ZYRAXON Code Chat window using this integration, the message is routed to a Claude Code session that can:
...
All interactions are displayed through ZYRAXON Code's native chat UI, providing a seamless experience.

## Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                         ZYRAXON Code Chat UI                          │
└─────────────────────────┬───────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────┐
...
**ClaudeAgentManager**
- Entry point for handling chat requests from ZYRAXON Code
...
- Resolves prompts by replacing ZYRAXON Code references (files, locations) with actual paths

**ClaudeCodeSession**
- Represents a single Claude Code conversation session
- Manages a queue of incoming requests from ZYRAXON Code Chat
...
- Handles tool confirmation dialogs via ZYRAXON Code's chat API
...
Formats tool invocations for display in ZYRAXON Code's chat UI:
...
1. **User sends message** in ZYRAXON Code Chat
...
4. **Claude Code SDK** returns streaming messages:
   ...
   - Tool use requests → shown as progress, then confirmed via ZYRAXON Code's confirmation API
...
ZYRAXON Code calls `getChatSessionInputState` to get a `ChatSessionInputState` object whose `.groups` array drives the UI. Rather than computing groups once and returning them, the pipeline keeps `groups` live: shared observables push changes into each state object whenever relevant configuration changes.
...
When ZYRAXON Code discards a `ChatSessionInputState`, the `onDidDispose` event fires and deterministically cleans up all autoruns for that state. The `onDidDispose` subscription is itself registered on the pipeline store, so it is cleaned up as part of disposal.
...
- **`context.previousInputState` path** — ZYRAXON Code already has a state for this session and is asking for a fresh one; seed from the old groups.
...
- Commands can optionally register with ZYRAXON Code Command Palette
...
interface IClaudeSlashCommandHandler {
	...
	readonly commandId?: string;          // Optional ZYRAXON Code command ID
...
- All other tools show a confirmation dialog via ZYRAXON Code's chat API
...
The integration respects ZYRAXON Code settings:
