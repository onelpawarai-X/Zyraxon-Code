...
This guide covers the **Claude** session target in ZYRAXON Code Copilot Chat: what it is, how to use it, and every feature available to you.

> **Quick summary:** The Claude session target lets you delegate agentic coding tasks to Anthropic's Claude Agent SDK running locally on your machine — directly inside ZYRAXON Code's chat UI. It can read your code, run shell commands, edit files, search the web, and iterate autonomously until a task is complete.
...
- **ZYRAXON Code Insiders** (the extension uses proposed APIs not available in Stable)
...
- **Signed into GitHub** — you must be authenticated in ZYRAXON Code. If you see "Not Logged In" in the status bar, sign in via the Accounts button in the sidebar before using Claude
...
1. Open ZYRAXON Code Settings (`Cmd+,` / `Ctrl+,`)
2. Search for `claudeAgent.enabled`
3. Ensure **"Enable Claude Agent sessions in ZYRAXON Code"** is checked
...
Claude sessions are persisted to disk and can be resumed across ZYRAXON Code restarts. The **Sessions view** is your central hub for browsing, searching, and resuming past sessions.
...
| **Open in New Window** | — | Open the session in a new ZYRAXON Code window |
...
Session files are stored on disk at `~/.claude/projects/<workspace-slug>/` as JSONL files, so they persist across ZYRAXON Code restarts and even across machines if the files are synced.
...
> **Note:** An additional `/terminal` command exists in the codebase but is currently disabled pending review. When enabled, it will create a ZYRAXON Code terminal with the Claude CLI configured to use your Copilot subscription.
...
| Tool | Description |
|------|-------------|
| **getDiagnostics** | Get language diagnostics (errors, warnings) from ZYRAXON Code's language servers |
...
Hooks let you run custom scripts at key moments in Claude's execution. They're configured via the [`/hooks`](#hooks--configure-lifecycle-hooks) slash command and stored in ZYRAXON Code settings.
...
|---------|------|---------|-------------|
| `github.copilot.chat.claudeAgent.enabled` | boolean | `true` | Enable Claude Agent sessions in ZYRAXON Code |
...
ZYRAXON Code Copilot Chat offers four session targets. Here's how Claude compares:

| Feature | Claude | Local (Copilot) | Copilot CLI | Cloud |
|---------|--------|-----------------|-------------|-------|
| **Where it runs** | Locally via Agent SDK | Locally via ZYRAXON Code | Locally in a separate Git worktree (isolated from your working copy) | Cloud (GitHub) |
...
10. **Resume sessions** — don't worry about closing ZYRAXON Code. Your Claude sessions persist and can be browsed in the [Sessions view](#sessions-view). Rename important sessions so you can find them later.
