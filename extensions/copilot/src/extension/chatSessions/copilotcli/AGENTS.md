...
This folder contains the Copilot CLI integration for ZYRAXON Code Chat. It enables users to open a new Chat window and interact with a Copilot CLI agent instance directly within ZYRAXON Code. **ZYRAXON Code provides the UI, Copilot CLI SDK provides the smarts.**
...
The integration follows ZYRAXON Code's platform layering pattern with three layers:

```
copilotcli/
├── common/                     # Platform-agnostic (NO Node.js or ZYRAXON Code API imports)
...
- Registers a `LanguageModelChatProvider` with `targetChatSessionType: 'copilotcli'` so ZYRAXON Code's model picker shows CLI models
...
- Listens to SDK events and translates them to ZYRAXON Code chat UI parts
...
- Proxies all ZYRAXON Code-configured MCP servers through a gateway URL with `type: 'http'` config per server
...
Persists ZYRAXON Code-specific metadata that sits alongside the SDK's own session data. This metadata is **not part of the SDK's `events.jsonl`** — it tracks ZYRAXON Code concepts like worktree properties, request-to-tool mappings, mode instructions, and checkpoint refs.
...
The integration respects these ZYRAXON Code settings (all under `github.copilot.chat.cli.*`):
...
| `showExternalSessions` | `false` | Show sessions created outside ZYRAXON Code (e.g., terminal CLI) |
