...
export class LockFileHandle {
	...
	private readonly serverUri: zyraxoncode.Uri;
	private readonly headers: Record<string, string>;
	private readonly timestamp: number;
	private readonly logger: ILogger;

	constructor(lockFilePath: string, serverUri: zyraxoncode.Uri, headers: Record<string, string>, timestamp: number, logger: ILogger) {
	...
	async update(): Promise<void> {
		try {
			const workspaceFolders = zyraxoncode.workspace.workspaceFolders?.map(f => f.uri.fsPath) || [];
			...
			const lockInfo: LockFileInfo = {
				...
				pid: process.pid,
				ideName: zyraxoncode.env.appName,
				timestamp: this.timestamp,
				workspaceFolders: workspaceFolders,
				isTrusted: zyraxoncode.workspace.isTrusted,
			};
...
export async function createLockFile(serverUri: zyraxoncode.Uri, headers: Record<string, string>, logger: ILogger): Promise<LockFileHandle> {
