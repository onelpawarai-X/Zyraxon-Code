...
export interface ICopilotCLIChatSessionInitializer {
	...
	initializeWorkingDirectory(
		chatResource: zyraxoncode.Uri | undefined,
		options: SessionInitOptions,
...
export class CopilotCLIChatSessionInitializer implements ICopilotCLIChatSessionInitializer {
	...
	async initializeWorkingDirectory(
		...
		const { stream } = options;
		let folder: undefined | zyraxoncode.Uri = options?.folder;
	 ...
	 */
	async resolveModel(request: zyraxoncode.ChatRequest | undefined, token: zyraxoncode.CancellationToken): Promise<{ model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined> {
	 ...
	 */
	async resolveAgent(request: zyraxoncode.ChatRequest | undefined, token: zyraxoncode.CancellationToken): Promise<SweCustomAgent | undefined> {
