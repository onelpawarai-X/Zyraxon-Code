...
export class CopilotCLICustomizationProvider extends Disposable implements zyraxoncode.ChatSessionCustomizationProvider {

	private readonly _onDidChange = this._register(new Emitter<void>());
	readonly onDidChange = this._onDidChange.event;

	static get metadata(): zyraxoncode.ChatSessionCustomizationProviderMetadata {
		return {
			label: 'Copilot CLI',
			iconId: 'copilot',
			supportedTypes: [
				zyraxoncode.ChatSessionCustomizationType.Agent,
				zyraxoncode.ChatSessionCustomizationType.Skill,
				zyraxoncode.ChatSessionCustomizationType.Instructions,
				zyraxoncode.ChatSessionCustomizationType.Hook,
				zyraxoncode.ChatSessionCustomizationType.Plugins,
			].filter((t): t is zyraxoncode.ChatSessionCustomizationType => t !== undefined),
		};
	}
	...
	async provideSourceFolders(_sessionResource: zyraxoncode.Uri, type: zyraxoncode.ChatSessionCustomizationType, _token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionCustomizationSourceFolder[]> {
		const folders: zyraxoncode.ChatSessionCustomizationSourceFolder[] = [];
	 ...
	 */
	private async getAgentItems(_token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		const agentInfos = await this.copilotCLIAgents.getAgents();
		return agentInfos.map(({ agent, sourceUri, pluginUri, extensionId, source }) => ({
			uri: sourceUri,
			type: zyraxoncode.ChatSessionCustomizationType.Agent,
	 ...
	 */
	private async getInstructionItems(token: CancellationToken): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		...
		for (const uri of agentInstructionUriList) {
			...
			items.push({
				uri,
				type: zyraxoncode.ChatSessionCustomizationType.Instructions,
				name: basename(uri),
	 ...
	 */
	private async getSkillItems(token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		return (await this.promptsService.getSkills(token)).filter(isEnabledForCopilotCLI).map(s => ({
			uri: s.uri,
			type: zyraxoncode.ChatSessionCustomizationType.Skill,
			name: s.name,
	 ...
	 */
	private async getHookItems(token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		return (await this.promptsService.getHooks(token)).filter(isEnabledForCopilotCLI).map(h => ({
			uri: h.uri,
			type: zyraxoncode.ChatSessionCustomizationType.Hook,
	 ...
	 */
	private async getPluginItems(token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		return (await this.promptsService.getPlugins(token)).filter(isEnabledForCopilotCLI).map(p => ({
			uri: p.uri,
			type: zyraxoncode.ChatSessionCustomizationType.Plugins,
			name: basename(p.uri),
			description: undefined,
			extensionId: undefined,
			pluginUri: undefined,
...
function getSearchRoots() {
	return {
		workspace: [
			{ path: ['.github', 'agents'], type: zyraxoncode.ChatSessionCustomizationType.Agent },
			{ path: ['.agents', 'agents'], type: zyraxoncode.ChatSessionCustomizationType.Agent },
			{ path: ['.claude', 'agents'], type: zyraxoncode.ChatSessionCustomizationType.Agent },
			{ path: ['.github', 'skills'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Skill },
			{ path: ['.agents', 'skills'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Skill },
			{ path: ['.claude', 'skills'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Skill },
			{ path: ['.github', 'instructions'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Instructions },
			{ path: ['.github', 'hooks'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Hook },
		],
		user: [
			{ path: ['.copilot', 'agents'], type: zyraxoncode.ChatSessionCustomizationType.Agent },
			{ path: ['.agents', 'skills'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Skill },
			{ path: ['.copilot', 'instructions'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Instructions },
			{ path: ['.copilot', 'hooks'], recursive: true, type: zyraxoncode.ChatSessionCustomizationType.Hook },
		],
	};
}
