...
export function registerGetDiagnosticsTool(server: McpServer, logger: ILogger): void {
	...
	server.registerTool(
		'get_diagnostics',
		{
			description: 'Gets language diagnostics (errors, warnings, hints) from ZYRAXON Code',
			inputSchema: schema,
		},
		...
		async (args: { uri?: string }) => {
			...
			let diagnostics: [zyraxoncode.Uri, readonly zyraxoncode.Diagnostic[]][];

			if (uri) {
				const fileUri = zyraxoncode.Uri.parse(uri);
				const fileDiagnostics = zyraxoncode.languages.getDiagnostics(fileUri);
				diagnostics = [[fileUri, fileDiagnostics]];
			} else {
				diagnostics = zyraxoncode.languages.getDiagnostics();
			}
			...
			const result = diagnostics.map(([fileUri, fileDiagnostics]) => ({
				...
				diagnostics: fileDiagnostics.map(d => ({
					message: d.message,
					severity: zyraxoncode.DiagnosticSeverity[d.severity].toLowerCase(),
					range: {
