	...
	server.registerTool(
		...
		async (args: { original_file_path: string; new_file_contents: string; tab_name: string }) => {
			...
			try {
				...
				logger.info('[DIFF] Calling zyraxoncode.diff command');
				await zyraxoncode.commands.executeCommand('zyraxoncode.diff', originalUri, newUri, title, {
					preview: false,
					preserveFocus: true,
				});
				logger.info('[DIFF] zyraxoncode.diff command completed');
				...
				const result = await new Promise<{ status: 'SAVED' | 'REJECTED'; trigger: string }>(resolve => {
							...
							for (const tab of group.tabs) {
								if (tab.input instanceof zyraxoncode.TabInputTextDiff) {
									...
									if (tabModifiedUri === newUri.toString()) {
										...
										try {
											await zyraxoncode.window.tabGroups.close(tab);
					...
					// Watch for tab close
					disposables.push(
						zyraxoncode.window.tabGroups.onDidChangeTabs(event => {
							...
							for (const closedTab of event.closed) {
								if (closedTab.input instanceof zyraxoncode.TabInputTextDiff) {
