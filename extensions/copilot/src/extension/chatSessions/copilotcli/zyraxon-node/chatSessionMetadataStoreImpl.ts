...
const REQUEST_MAPPING_FILENAME = 'zyraxoncode.requests.metadata.json';

/**
 * Maximum number of sessions kept in the shared bulk metadata cache file
 * (`~/.copilot/zyraxoncode.session.metadata.cache.json`). Older entries (by `modified`)
 * are evicted from the file but remain available via the per-session metadata files
 * (`~/.copilot/session-state/{id}/zyraxoncode.metadata.json`) and the JSONL worktree index.
 */
...
export class ChatSessionMetadataStore extends Disposable implements IChatSessionMetadataStore {
	...
	public getSessionIdsForFolder(folder: zyraxoncode.Uri): string[] {
		return Array.from(this._folderToSessions.get(folder.fsPath) ?? []);
	}

	public getWorktreeSessions(folder: zyraxoncode.Uri): string[] {
	...
	private async initializeStorage(): Promise<void> {
		...
		// In case user closed zyraxoncode early or we couldn't save the session information for some reason.
	...
	public getMetadataFileUri(sessionId: string): zyraxoncode.Uri {
		return Uri.joinPath(Uri.file(getCopilotCLISessionDir(sessionId)), 'zyraxoncode.metadata.json');
	}

	private getRequestMappingFileUri(sessionId: string): zyraxoncode.Uri {
	...
	async deleteSessionMetadata(sessionId: string): Promise<void> {
		...
		this._requestDetailsCacheTimers.deleteAndDispose(sessionId);
	...
	async getSessionWorkspaceFolder(sessionId: string): Promise<zyraxoncode.Uri | undefined> {
		const metadata = await this.getSessionMetadata(sessionId);
		if (!metadata) {
			return undefined;
		}
	...
	async getRequestDetails(sessionId: string): Promise<RequestDetails[]> {
		const details = this._requestDetailsCache.get(sessionId);
		if (details) {
			this.cacheRequestDetails(sessionId, details);
			return details;
		}
		return this._requestMappingReadSequencer.queue(sessionId, async () => {
			await this._ready;
	...
	async updateRequestDetails(sessionId: string, details: (Partial<RequestDetails> & { zyraxoncodeRequestId: string })[]): Promise<void> {
		...
		await this._requestMappingWriteSequencer.queue(sessionId, async () => {
			...
			for (const item of details) {
				const existingDetails = existing.find(e => e.zyraxoncodeRequestId === item.zyraxoncodeRequestId);
				if (existingDetails) {
	...
	public async setSessionOrigin(sessionId: string): Promise<void> {
		await this._ready;
		await this.updateMetadataFields(sessionId, { origin: 'zyraxoncode' });
	}

	public async getSessionOrigin(sessionId: string): Promise<'zyraxoncode' | 'other'> {
