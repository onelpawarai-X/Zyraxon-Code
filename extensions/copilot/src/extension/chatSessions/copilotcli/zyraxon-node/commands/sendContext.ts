 ...
 */
function validateScheme(logger: ILogger, uri: zyraxoncode.Uri): boolean {
	if (ALLOWED_SCHEMES.has(uri.scheme)) {
		return true;
	}
	logger.debug(`Unsupported URI scheme: ${uri.scheme}`);
	zyraxoncode.window.showWarningMessage(l10n.t('Cannot send virtual files to Copilot CLI.'));
	return false;
}
...
export async function sendEditorContextToSession(
	...
	if (!editor) {
		...
		zyraxoncode.window.showWarningMessage(l10n.t('No active editor. Open a file to add a reference.'));
		return;
	}
