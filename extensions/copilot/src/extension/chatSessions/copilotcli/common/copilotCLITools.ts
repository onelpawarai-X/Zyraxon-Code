 ...
 * ZYRAXON Code {@link McpToolInvocationContentData}[] objects for rendering in the chat UI.
 *
 ...
 * content shapes into a common binary+MIME-type representation that the ZYRAXON Code chat
...
export function createCopilotCLIToolInvocation(data: {
	...
	if (toolCall.toolName === 'show_file') {
		...
		// Its a WIP, no clear way to render in CLI either, hence decided to hide in ZYRAXON Code.
		return undefined;
	}
