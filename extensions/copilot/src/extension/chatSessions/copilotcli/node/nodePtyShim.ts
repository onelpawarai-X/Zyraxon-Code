...
/**
 * Copies the node-pty files from ZYRAXON Code's installation into a @github/copilot location.
 *
 ...
 * Code's app tree). For built-in installs `require('node-pty')` walks up into ZYRAXON Code's
 * own `node_modules` and this shim is a no-op (the `shims.txt` placeholder skips it). For
 * marketplace installs we copy ZYRAXON Code's pre-built `pty.node` into the SDK's expected
