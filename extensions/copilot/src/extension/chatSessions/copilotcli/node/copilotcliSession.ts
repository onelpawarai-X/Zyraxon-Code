...
export class CopilotCLISession extends DisposableStore implements ICopilotCLISession {
	...
	private async _handleRequestImplInner(
		...
		// and therefore never reach the tools service). MCP/ZYRAXON Code tools already emit `execute_tool`
	 ...
	 * chat debug logs view. MCP/ZYRAXON Code tools (those carrying an `mcpServerName`) already emit spans
	 ...
	 * status. No-op for tools that were not synthesized (e.g. MCP/ZYRAXON Code tools).
	 */
