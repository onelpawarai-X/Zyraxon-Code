...
export class ClaudeCustomizationProvider extends Disposable implements zyraxoncode.ChatSessionCustomizationProvider {
	...
	async provideChatSessionCustomizations(_sessionResource: zyraxoncode.Uri, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		...
		const agentItems = items.filter(i => i.type === zyraxoncode.ChatSessionCustomizationType.Agent);
		...
		const skillItems: zyraxoncode.ChatSessionCustomizationItem[] = [];
		for (const skill of await this.promptsService.getSkills(token)) {
			if (this.isClaudePath(skill.uri)) {
				const item: zyraxoncode.ChatSessionCustomizationItem = {
					uri: skill.uri,
	...
	private async discoverInstructions(): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		const items: zyraxoncode.ChatSessionCustomizationItem[] = [];
		const candidates: { uri: URI; source: zyraxoncode.ChatResourceSource }[] = [];
	...
	private async discoverHooks(): Promise<zyraxoncode.ChatSessionCustomizationItem[]> {
		const items: zyraxoncode.ChatSessionCustomizationItem[] = [];
	...
	private getSettingsFilePaths(): { uri: URI; source: zyraxoncode.ChatResourceSource }[] {
		const paths: { uri: URI; source: zyraxoncode.ChatResourceSource }[] = [];
