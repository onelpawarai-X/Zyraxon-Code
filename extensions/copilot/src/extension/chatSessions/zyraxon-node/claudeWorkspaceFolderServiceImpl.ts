...
export class ClaudeWorkspaceFolderService extends Disposable implements IClaudeWorkspaceFolderService {
	...
	private readonly _cache = new Map<string, zyraxoncode.ChatSessionChangedFile[]>();
	private readonly _inflight = new Map<string, Promise<zyraxoncode.ChatSessionChangedFile[]>>();
	...
	private async _computeAndCacheChanges(
		...
		const changes = result.changes.map(change => new zyraxoncode.ChatSessionChangedFile(
	...
	private async computeRepositoryChanges(
		...
		const repository = await this._gitService.getRepository(zyraxoncode.Uri.file(repositoryPath));
		...
		if (hasUntrackedChanges) {
			...
			const tmpDirName = `zyraxoncode-sessions-${generateUuid()}`;
			...
			try {
				...
				await this._fileSystemService.createDirectory(zyraxoncode.Uri.file(path.dirname(diffIndexFile)));
				...
				await this._fileSystemService.writeFile(zyraxoncode.Uri.file(pathspecFile), new TextEncoder().encode(uncommittedFilePaths.join('\n')));
				...
				try {
					await this._fileSystemService.delete(zyraxoncode.Uri.file(path.dirname(diffIndexFile)), { recursive: true });
				} catch (error) {
