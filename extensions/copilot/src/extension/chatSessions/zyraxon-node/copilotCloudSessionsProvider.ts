...
import { CONTINUE_TRUNCATION, extractTitle, getAuthorDisplayName, getRepoId, isActiveTaskState, SessionIdForPr, SessionIdForTask, toOpenPullRequestWebviewUri, truncatePrompt } from '../zyraxoncode/copilotCodingAgentUtils';
import { CloudAgentBackend, CloudSessionData, PullArtifactRef, TaskCloudAgentBackend, TaskContent } from '../zyraxoncode/cloudAgentBackend';
...
interface ConfirmationMetadata {
	prompt: string;
	references?: readonly zyraxoncode.ChatPromptReference[];
	chatContext: zyraxoncode.ChatContext;
}

type InitialSessionOption = {
	readonly optionId: string;
	readonly value: string | zyraxoncode.ChatSessionProviderOptionItem;
};
...
function isOptionItemValue(value: unknown): value is zyraxoncode.ChatSessionProviderOptionItem {
...
export function normalizeInitialSessionOptions(initialOptions: unknown, logService?: ILogService, chatResource?: zyraxoncode.Uri): readonly InitialSessionOption[] {
	if (!initialOptions) {
		return [];
	}
...
/**
 * Map a Task API lifecycle state (v2) directly to a {@link zyraxoncode.ChatSessionStatus}. Unlike v1's
...
export function taskStateToChatSessionStatus(state: AgentTaskState): zyraxoncode.ChatSessionStatus {
	switch (state) {
		...
		case 'waiting_for_user':
			return zyraxoncode.ChatSessionStatus.NeedsInput;
		case 'idle':
		case 'completed':
			return zyraxoncode.ChatSessionStatus.Completed;
		case 'failed':
		case 'timed_out':
		case 'cancelled':
			return zyraxoncode.ChatSessionStatus.Failed;
		default:
...
const DEFAULT_CUSTOM_AGENT_ID = '___zyraxoncode_default___';
const DEFAULT_MODEL_ID = 'auto';
const DEFAULT_PARTNER_AGENT_ID = '___zyraxoncode_partner_agent_default___';
const DEFAULT_REPOSITORY_ID = '___zyraxoncode_repository_default___';
...
export class CopilotCloudSessionsProvider extends Disposable implements zyraxoncode.ChatSessionContentProvider, zyraxoncode.ChatSessionItemProvider {
	public static readonly TYPE = 'copilot-cloud-agent';
	private readonly _onDidChangeChatSessionItems = this._register(new zyraxoncode.EventEmitter<void>());
	public readonly onDidChangeChatSessionItems = this._onDidChangeChatSessionItems.event;
	private readonly _onDidCommitChatSessionItem = this._register(new zyraxoncode.EventEmitter<{ original: zyraxoncode.ChatSessionItem; modified: zyraxoncode.ChatSessionItem }>());
	public readonly onDidCommitChatSessionItem = this._onDidCommitChatSessionItem.event;
	private readonly _onDidChangeChatSessionProviderOptions = this._register(new zyraxoncode.EventEmitter<void>());
	public readonly onDidChangeChatSessionProviderOptions = this._onDidChangeChatSessionProviderOptions.event;
	private readonly _onDidChangeChatSessionOptions = this._register(new zyraxoncode.EventEmitter<zyraxoncode.ChatSessionOptionChangeEvent>());
	...
	private chatSessionItemsPromise: Promise<zyraxoncode.ChatSessionItem[]> | undefined;
	...
	private readonly sessionReferencesMap = new ResourceMap<readonly zyraxoncode.ChatPromptReference[]>();
	public chatParticipant = zyraxoncode.chat.createChatParticipant(CopilotCloudSessionsProvider.TYPE, async (request, context, stream, token) => {
	...
	private cachedSessionItems: (zyraxoncode.ChatSessionItem & {
	...
	private _optionsCache = new SingleSlotTtlCache<zyraxoncode.ChatSessionProviderOptions>(OPTIONS_CACHE_TTL_MS);

	// Title
	private TITLE = zyraxoncode.l10n.t('Delegate to cloud agent');

	// Buttons (used for matching, be careful changing!)
	private readonly AUTHORIZE = zyraxoncode.l10n.t('Authorize');
	private readonly COMMIT = zyraxoncode.l10n.t('Commit Changes');
	private readonly PUSH_BRANCH = zyraxoncode.l10n.t('Push Branch');
	private readonly DELEGATE = zyraxoncode.l10n.t('Delegate');
	private readonly CANCEL = zyraxoncode.l10n.t('Cancel');

	// Messages
	private readonly BASE_MESSAGE = zyraxoncode.l10n.t('Cloud agent works asynchronously to create a pull request with your requested changes. This chat\'s history will be summarized and appended to the pull request as context.');
	private readonly AUTHORIZE_MESSAGE = zyraxoncode.l10n.t('Cloud agent requires elevated GitHub access to proceed.');
	private readonly COMMIT_MESSAGE = zyraxoncode.l10n.t('This workspace has uncommitted changes. Should these changes be pushed and included in cloud agent\'s work?');
	private readonly PUSH_BRANCH_MESSAGE = (baseRef: string, defaultBranch: string) => zyraxoncode.l10n.t('Push your currently checked out branch `{0}`, or start from the default branch `{1}`?', baseRef, defaultBranch);
	...
	constructor(
		...
		getRepoId(this._gitService).then(async repoIds => {
			const telemetryObj: {
				...
				isEmptyWindow: !zyraxoncode.workspace.workspaceFolders || zyraxoncode.workspace.workspaceFolders.length === 0
			};
			if (repoIds && repoIds.length > 0) {
				let intervalMs: number;
				let hasHistoricalSessions: boolean;
				try {
					const sessionList = await this._backend.fetchSessionList(repoIds, zyraxoncode.workspace.isAgentSessionsWorkspace, false);
				...
				const schedulerCallback = async () => {
					try {
						const sessionList = await this._backend.fetchSessionList(repoIds, zyraxoncode.workspace.isAgentSessionsWorkspace, true);
				...
				scheduler.schedule();
				this._register(zyraxoncode.window.onDidChangeWindowState((e) => {
					if (!e.active) {
						scheduler.cancel();
			...
			this.telemetry.sendTelemetryEvent('copilotCloudSessions.refreshInterval', { Zyraxon: true, github: false }, telemetryObj);
		});
	}

	private registerCommands() {
		const executePullRequestActionWithExtensionInstall = async (
			sessionItemOrResource: zyraxoncode.ChatSessionItem | zyraxoncode.Uri | number | undefined,
			options: {
				actionLabel: string;
			...
			if (typeof sessionItemOrResource === 'number') {
				...
				const resource = sessionItemOrResource instanceof zyraxoncode.Uri
			...
			if (!repoIds || repoIds.length === 0) {
				zyraxoncode.window.showErrorMessage(options.noRepoErrorMessage);
				return;
			}

			const extensionId = 'github.zyraxoncode-pull-request-github';
			const isExtensionInstalled = zyraxoncode.extensions.getExtension(extensionId) !== undefined;

			if (!isExtensionInstalled) {
				const result = await zyraxoncode.window.showInformationMessage(
				...
				await zyraxoncode.commands.executeCommand('workbench.extensions.installExtension', extensionId, { enable: true });
			}

			await options.executeAction(repoIds[0], pullRequestNumber);
		};

		const checkoutPullRequestReroute = (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) =>
			executePullRequestActionWithExtensionInstall(sessionItemOrResource, {
				...
				executeAction: async (repoId, pullRequestNumber) => {
					await zyraxoncode.commands.executeCommand('pr.checkoutFromDescription', { owner: repoId.org, repo: repoId.repo, number: pullRequestNumber });
				},
			});
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.checkoutPullRequestReroute', checkoutPullRequestReroute));

		const openPullRequestReroute = (sessionItemOrResource?: zyraxoncode.ChatSessionItem | number | zyraxoncode.Uri) =>
			executePullRequestActionWithExtensionInstall(sessionItemOrResource, {
				...
				executeAction: async (repoId, pullRequestNumber) => {
					await zyraxoncode.commands.executeCommand('pr.openDescription', {
						pullRequestDetails: {
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.openPullRequestReroute', openPullRequestReroute));

		// Command for browsing repositories in the repository picker
		const openRepositoryCommand = async (sessionItemResource?: zyraxoncode.Uri): Promise<string | undefined> => {
			const quickPick = zyraxoncode.window.createQuickPick();
					...
					if (selected && sessionItemResource) {
						...
						this._onDidChangeChatSessionOptions.fire({
							...
							updates: [{
								...
								value: { id: selected.label, name: selected.label, icon: new zyraxoncode.ThemeIcon('repo') }
							}]
						});
					}
		...
		this._register(zyraxoncode.commands.registerCommand(OPEN_REPOSITORY_COMMAND_ID, openRepositoryCommand));

		this._register(zyraxoncode.commands.registerCommand(CLEAR_CACHES_COMMAND_ID, () => {
		...
		this._register(zyraxoncode.commands.registerCommand(CREATE_PULL_REQUEST_FOR_TASK_COMMAND_ID, (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => this.handleCreatePullRequestForTaskCommand(sessionItemOrResource)));
		this._register(zyraxoncode.commands.registerCommand(OPEN_PULL_REQUEST_FOR_TASK_COMMAND_ID, (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => this.handleOpenPullRequestForTaskCommand(sessionItemOrResource)));
	}
	...
	private async checkCCAEnabled(owner: string, repo: string): Promise<CCAEnabledResult> {
		...
		this.telemetry.sendTelemetryEvent('copilot.codingAgent.CCAIsEnabledCheck', { Zyraxon: true, github: false }, {
		...
		if (result.statusCode !== undefined && !CCA_KNOWN_STATUS_CODES.has(result.statusCode)) {
			...
			*/
			this.telemetry.sendTelemetryErrorEvent('copilot.codingAgent.CCAIsEnabledUnexpectedStatus', { Zyraxon: true, github: false }, {
	...
	private getCCADisabledMessage(result: CCAEnabledResult, host: string = 'github.com'): string {
		if (result.statusCode === 422) {
			return zyraxoncode.l10n.t('Cloud agent is unable to create pull requests in this repository. Please verify repository rules allow this operation.');
		}
		if (result.statusCode === 401) {
			return zyraxoncode.l10n.t('Cloud agent is not authorized to run on this repository. This may be because the Copilot coding agent is disabled for your organization, or your active GitHub account does not have push access to the target repository.');
		}
		// Default to 403 'disabled' message
		const settingsUrl = `__ZYRAXKEEP__0_{host}/settings/copilot/coding_agent`;
		return zyraxoncode.l10n.t('Cloud agent is not enabled for this repository. You may need to enable it in [GitHub settings]({0}) or contact your organization administrator.', settingsUrl);
	}
	...
	private async getLocalCustomAgentFiles(remoteAgents: { name: string }[]): Promise<{
		...
		const workspaceFolders = zyraxoncode.workspace.workspaceFolders;
		...
		try {
			...
			for (const [name, type] of entries) {
				...
				if (remoteAgentNames.has(agentName.toLowerCase())) {
					...
					localOnly.push({
						name: agentName,
						path: zyraxoncode.workspace.asRelativePath(fileUri)
					});
				}
			}
		} catch (error) {
			if (error instanceof zyraxoncode.FileSystemError && error.code === 'FileNotFound') {
				return { matches, localOnly };
			}
	...
	async provideChatSessionProviderOptions(token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionProviderOptions> {
		...
		const optionGroups: zyraxoncode.ChatSessionProviderOptionGroup[] = [];
		try {
			...
			try {
				...
				if (items.length !== 1) {
					optionGroups.push({
						...
						name: zyraxoncode.l10n.t('Repository'),
						description: zyraxoncode.l10n.t('Select repository'),
						icon: new zyraxoncode.ThemeIcon('repo'),
						items,
						commands: [{
							command: OPEN_REPOSITORY_COMMAND_ID,
							title: zyraxoncode.l10n.t('Browse repositories...'),
						}]
					});
				}
			...
			if (partnerAgents.status === 'fulfilled' && partnerAgents.value.length > 1) {
				const partnerAgentItems: zyraxoncode.ChatSessionProviderOptionItem[] = partnerAgents.value.map(agent => ({
					id: agent.id,
					name: agent.name,
					...(agent.id === DEFAULT_PARTNER_AGENT_ID && { default: true }),
					icon: agent.codiconId ? new zyraxoncode.ThemeIcon(agent.codiconId) : undefined
				}));
				optionGroups.push({
					id: PARTNER_AGENTS_OPTION_GROUP_ID,
					name: zyraxoncode.l10n.t('Partner Agents'),
					description: zyraxoncode.l10n.t('Select which partner agent to use'),
			...
			if ((customAgents.status === 'fulfilled' && customAgents.value.length > 0) || (repoIds?.length === 1 && localOnly.length > 0)) {
				const agentItems: zyraxoncode.ChatSessionProviderOptionItem[] = [
					{
						id: DEFAULT_CUSTOM_AGENT_ID,
						default: true,
						name: zyraxoncode.l10n.t('Agent'),
						icon: new zyraxoncode.ThemeIcon('agent')
					},
					...
					...localOnly.map(localAgent => ({
						...
						description: zyraxoncode.l10n.t('Missing from {0}', repoId ? `${repoId.org}/${repoId.repo}` : 'remote repository'),
						locked: true,
						icon: new zyraxoncode.ThemeIcon('warning')
					}) satisfies zyraxoncode.ChatSessionProviderOptionItem)
				];
				optionGroups.push({
					id: CUSTOM_AGENTS_OPTION_GROUP_ID,
					name: zyraxoncode.l10n.t('Custom Agents'),
					description: zyraxoncode.l10n.t('Select which custom agent to use'),
					items: agentItems,
			...
			if (models.status === 'fulfilled' && models.value.length > 0) {
				...
				const modelItems: zyraxoncode.ChatSessionProviderOptionItem[] = models.value.map(model => {
				...
				if (!models.value.find(m => m.id === DEFAULT_MODEL_ID)) {
					modelItems.unshift({
						id: DEFAULT_MODEL_ID,
						name: zyraxoncode.l10n.t('Auto'),
						description: zyraxoncode.l10n.t('Automatically select the best model'),
						tooltip: zyraxoncode.l10n.t('Automatically select the best model'),
						modelMetadata: {
							name: zyraxoncode.l10n.t('Auto'),
							id: DEFAULT_MODEL_ID,
							tooltip: zyraxoncode.l10n.t('Automatically select the best model'),
				...
				optionGroups.push({
					...
					name: zyraxoncode.l10n.t('Model'),
					description: zyraxoncode.l10n.t('Select which model to use'),
					items: modelItems,
					when: `!chatSessionOption.partnerAgents || chatSessionOption.partnerAgents == ${DEFAULT_PARTNER_AGENT_ID}`
				});
			}

			const result: zyraxoncode.ChatSessionProviderOptions = { optionGroups };
	...
	private async getRepositoriesOptionItems(repoIds?: GithubRepoId[], fetchAll: boolean = false): Promise<zyraxoncode.ChatSessionProviderOptionItem[]> {
	...
	private async fetchAllRepositoriesFromGitHub(query?: string): Promise<zyraxoncode.ChatSessionProviderOptionItem[]> {
		try {
	...
	provideHandleOptionsChange(resource: Uri, updates: ReadonlyArray<zyraxoncode.ChatSessionOptionUpdate>, token: zyraxoncode.CancellationToken): void {
	...
	async provideChatSessionItems(token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionItem[]> {
		...
		this.chatSessionItemsPromise = (async () => {
			const repoIds = await getRepoId(this._gitService);
			this.logService.debug(`copilotCloudSessionsProvider#provideChatSessionItems: repoIds=${JSON.stringify(repoIds?.map(r => ({ org: r.org, repo: r.repo, host: r.host })))}, isAgentSessionsWorkspace=${zyraxoncode.workspace.isAgentSessionsWorkspace}`);
			// Make sure if it's not a github repo we don't show any sessions
			// (unless we're in an agent sessions workspace)
			if (!zyraxoncode.workspace.isAgentSessionsWorkspace && !this.isGitHubRepoOrEmpty(repoIds)) {
			...
			const sessionItems = await Promise.all(sessionList.map(async entry => {
				...
				if (!pr) {
					...
					return {
						resource: zyraxoncode.Uri.from({ scheme: CopilotCloudSessionsProvider.TYPE, path: '/' + SessionIdForTask.getId(taskId) }),
					...
					} satisfies zyraxoncode.ChatSessionItem;
				}

				const multiDiffPart = await this._prFileChangesService.getFileChangesMultiDiffPart(pr);
				const changes = multiDiffPart?.value?.map(change => new zyraxoncode.ChatSessionChangedFile(
					change.goToFileUri!,
					change.originalUri,
					change.modifiedUri,
					change.added ?? 0,
			...
			zyraxoncode.commands.executeCommand('setContext', 'github.copilot.chat.cloudSessionsEmpty', filteredSessions.length === 0);
	...
	private isGitHubRepoOrEmpty(repoIds: GithubRepoId[] | undefined) {
		const hasOpenedFolder = zyraxoncode.workspace.workspaceFolders && zyraxoncode.workspace.workspaceFolders.length > 0;
		if (!hasOpenedFolder) {
			return true;
		}
	...
	async provideChatSessionContent(resource: Uri, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSession> {
		...
		const summaryReference = new DeferredPromise<zyraxoncode.ChatPromptReference | undefined>();
	 ...
	 */
	private async provideTaskChatSessionContent(resource: Uri, taskId: string, _token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSession> {
		const backend = this._backend;
		...
		const storedReferences: Promise<zyraxoncode.ChatPromptReference[]> = Promise.resolve([...(this.sessionReferencesMap.get(resource) ?? [])]);
	 ...
	 */
	private _createTaskStreamCallback(taskId: string, baseline: StreamBaseline): (stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken) => Thenable<void> {
	...
	async openSessionInBrowser(chatSessionItem: zyraxoncode.ChatSessionItem): Promise<void> {
		...
		if (taskParsed) {
			...
			if (!url) {
				zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('Could not find task {0}', taskParsed.taskId));
				this.logService.error(`Could not find task ${taskParsed.taskId}`);
				return;
			}
			await zyraxoncode.env.openExternal(zyraxoncode.Uri.parse(url));
			return;
		}
		...
		if (typeof prNumber === 'undefined' || isNaN(prNumber)) {
			...
			if (isNaN(prNumber)) {
				zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('Invalid pull request number: {0}', '' + chatSessionItem.resource));
		...
		if (!pr) {
			zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('Could not find pull request #{0}', prNumber));
			this.logService.error(`Could not find pull request #${prNumber}`);
			return;
		}

		await zyraxoncode.env.openExternal(zyraxoncode.Uri.parse(pr.url));
	}

	private findActiveResponseCallback(
		sessions: SessionInfo[],
		pr: PullRequestSearchItem
	): ((stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken) => Thenable<void>) | undefined {
	...
	private createActiveResponseCallback(pr: PullRequestSearchItem, sessionId: string): (stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken) => Thenable<void> {
		return async (stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken) => {
			const backend = this._backend;
			if (backend.kind !== 'pr') {
				return;
			}
	 ...
	 */
	private isUntitledChatSession(context: zyraxoncode.ChatContext): boolean {
	...
	private createEmptySession(resource: Uri): zyraxoncode.ChatSession {
	...
	private getSessionStatusFromSession(session: SessionInfo): zyraxoncode.ChatSessionStatus {
	...
	private getPullRequestBadge(repoIds: GithubRepoId[] | undefined, pr: PullRequestSearchItem): zyraxoncode.MarkdownString | undefined {
		if (
			zyraxoncode.workspace.workspaceFolders === undefined || // empty window
			zyraxoncode.workspace.isAgentSessionsWorkspace ||       // agent sessions workspace
			(repoIds && repoIds.length > 1)                    // multiple repositories
		) {
			const badgeLabel = `${pr.repository.owner.login}/${pr.repository.name}`;
			const badge = new zyraxoncode.MarkdownString(`$(repo) ${badgeLabel}`, true);
	...
	private createPullRequestTooltip(pr: PullRequestSearchItem): zyraxoncode.MarkdownString {
		const markdown = new zyraxoncode.MarkdownString(undefined, true);
	...
	private hasHistoryToSummarize(history: readonly (zyraxoncode.ChatRequestTurn | zyraxoncode.ChatResponseTurn)[]): boolean {
		if (!history || history.length === 0) {
			return false;
		}
		const allResponsesEmpty = history.every(turn => {
			if (turn instanceof zyraxoncode.ChatResponseTurn) {
	...
	async delegate(
		...
		if (!base_ref || repoOwner !== selectedRepoOwner || repoName !== selectedRepoName) {
			if (selectedRepoOwner && selectedRepoName) {
				...
				if (!repoId) {
					throw new Error(zyraxoncode.l10n.t('Open a GitHub repository to use the cloud agent.'));
				}
				repoOwner = repoId.org;
				repoName = repoId.repo;
			}
		...
		if (invokeResult.kind === 'task') {
			...
			const sessionUri = zyraxoncode.Uri.from({ scheme: CopilotCloudSessionsProvider.TYPE, path: '/' + SessionIdForTask.getId(invokeResult.taskId) });
			...
			if (this.isUntitledChatSession(metadata.chatContext)) {
				...
				stream.markdown(zyraxoncode.l10n.t('A cloud agent has begun working on your request. Follow its progress in the sessions list.'));
			}
			return {
				uri: zyraxoncode.Uri.parse(invokeResult.taskUrl),
				command: {
					title: invokeResult.title,
					command: 'zyraxoncode.open',
					arguments: [zyraxoncode.Uri.parse(invokeResult.taskUrl)],
				},
				title: invokeResult.title,
				description: '',
				author: '',
		...
		const sessionUri = zyraxoncode.Uri.from({ scheme: CopilotCloudSessionsProvider.TYPE, path: '/' + number });
		...
		stream.progress(zyraxoncode.l10n.t('Fetching pull request details'));
		...
		if (this.isUntitledChatSession(metadata.chatContext)) {
			...
			// Delegated flow
			// NOTE: ZYRAXON Code will now close the parent/source chat in most cases.
			stream.markdown(zyraxoncode.l10n.t('A cloud agent has begun working on your request. Follow its progress in the sessions list and associated pull request.'));
		}
		...
		return {
			uri, // PR uri,
			command: {
				title: zyraxoncode.l10n.t('View Pull Request #{0}', pullRequest.number),
	...
	private async handleConfirmationData(request: zyraxoncode.ChatRequest, stream: zyraxoncode.ChatResponseStream, context: zyraxoncode.ChatContext, token: zyraxoncode.CancellationToken) {
		...
		if (!selection || selection === this.CANCEL.toUpperCase() || token.isCancellationRequested) {
			...
			*/
			...
			stream.markdown(zyraxoncode.l10n.t('Cloud agent cancelled'));
			return {};
		}

		if (selection.includes(this.AUTHORIZE.toUpperCase())) {
			stream.progress(zyraxoncode.l10n.t('Authorizing'));
			try {
				...
				throw new Error(zyraxoncode.l10n.t('Authorization failed. Please sign into GitHub and try again.'));
		...
		if (selection.includes(this.COMMIT.toUpperCase())) {
			try {
				stream.progress(zyraxoncode.l10n.t('Committing and pushing local changes'));
				head_ref = await this.gitOperationsManager.commitAndPushChanges();
				stream.markdown(zyraxoncode.l10n.t('Local changes pushed to remote branch `{0}`.', head_ref));
			} catch (error) {
				this.logService.error(`Commit and push failed: ${error}`);
				throw zyraxoncode.l10n.t('{0}. Commit or stash your changes and try again.', (error instanceof Error ? error.message : String(error)) ?? zyraxoncode.l10n.t('Failed to commit and push changes.'));
			}
		} else if (selection.includes(this.PUSH_BRANCH.toUpperCase())) {
			try {
				stream.progress(zyraxoncode.l10n.t('Pushing base branch to remote'));
				const baseBranch = await this.gitOperationsManager.pushBaseRefToRemote();
				stream.markdown(zyraxoncode.l10n.t('Base branch `{0}` pushed to remote.', baseBranch));
			} catch (error) {
				this.logService.error(`Push branch failed: ${error}`);
				throw zyraxoncode.l10n.t('{0}. Push the current branch to remote and try again.', (error instanceof Error ? error.message : String(error)) ?? zyraxoncode.l10n.t('Failed to push current branch.'));
			}
		}
		...
		const base_ref: string = await (async () => {
			...
			if (!res) {
				// Unexpected
				throw new Error(zyraxoncode.l10n.t('Repo base branch is not detected on remote. Push your branch and try again.'));
			}
			return (res?.missingOnRemote || !res?.baseRef) ? res.repoDefaultBranch : res?.baseRef;
		})();
		stream.progress(zyraxoncode.l10n.t('Validating branch base branch exists on remote'));
		...
		try {
			...
			throw new Error(zyraxoncode.l10n.t('{0}', (error instanceof Error ? error.message : String(error))));
		}
	}
	 ...
	 */
	private async handleCreatePullRequestForTaskCommand(sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri): Promise<void> {
		const backend = this._backend;
		if (backend.kind !== 'task') {
			zyraxoncode.window.showWarningMessage(zyraxoncode.l10n.t('Creating a pull request from a task is only supported on the v2 cloud agent backend.'));
			return;
		}
		const resource = sessionItemOrResource instanceof zyraxoncode.Uri ? sessionItemOrResource : sessionItemOrResource?.resource;
		...
		try {
			await zyraxoncode.window.withProgress(
				{ location: zyraxoncode.ProgressLocation.Notification, title: zyraxoncode.l10n.t('Creating pull request') },
				async () => {
					...
					const taskContent = await backend.fetchTaskContent(taskId);
					if (!taskContent) {
						throw new Error(zyraxoncode.l10n.t('Unable to load the task to create a pull request.'));
					}
	...
	private setCanCreatePullRequestContext(canCreate: boolean): void {
		void zyraxoncode.commands.executeCommand('setContext', CAN_CREATE_PULL_REQUEST_CONTEXT_KEY, canCreate);
	}
	...
	private setCanOpenPullRequestContext(canOpen: boolean): void {
		void zyraxoncode.commands.executeCommand('setContext', CAN_OPEN_PULL_REQUEST_CONTEXT_KEY, canOpen);
	}
	 ...
	 */
	private async resolveTaskIdFromSessionResource(resource: zyraxoncode.Uri | undefined): Promise<string | undefined> {
		if (!resource) {
			return undefined;
		}
	 ...
	 */
	private async handleOpenPullRequestForTaskCommand(sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri): Promise<void> {
		const backend = this._backend;
		if (backend.kind !== 'task') {
			return;
		}
		...
		const taskContent = await backend.fetchTaskContent(taskId);
		if (!taskContent) {
			zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('Unable to load the task to open its pull request.'));
			return;
		}
		...
		if (!url) {
			zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('Could not determine the pull request URL for this task.'));
			return;
		}
		await zyraxoncode.env.openExternal(zyraxoncode.Uri.parse(url));
	}
	 ...
	 * */
	private async buildConfirmation(context: zyraxoncode.ChatContext): Promise<{ title: string; message: string; buttons: string[] } | undefined> {
		...
		if (needsPermissiveAuth && hasUncommittedChanges) {
			...
			message += '\n\n' + this.COMMIT_MESSAGE;
			buttons.unshift(
				zyraxoncode.l10n.t('{0} and {1}', this.AUTHORIZE, this.COMMIT),
				this.AUTHORIZE,
			);
			...
			buttons.unshift(
				zyraxoncode.l10n.t('{0} and {1}', this.AUTHORIZE, this.PUSH_BRANCH),
				this.AUTHORIZE,
			);
			...
			message += '\n\n' + this.COMMIT_MESSAGE;
			buttons.unshift(
				zyraxoncode.l10n.t('{0} and {1}', this.COMMIT, this.DELEGATE),
				this.DELEGATE,
			);
			...
			message += '\n\n' + this.PUSH_BRANCH_MESSAGE(baseRef, repoDefaultBranch);
			buttons.unshift(
				zyraxoncode.l10n.t('{0} and {1}', this.PUSH_BRANCH, this.DELEGATE),
				this.DELEGATE,
			);
		}
	...
	private async chatParticipantImpl(request: zyraxoncode.ChatRequest, context: zyraxoncode.ChatContext, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken) {
		if (token.isCancellationRequested) {
			stream.warning(zyraxoncode.l10n.t('Cloud session cancelled.'));
			return {};
		}
	...
	private async handleTaskFollowUp(taskId: string, prompt: string, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken, context: zyraxoncode.ChatContext): Promise<{}> {
		const backend = this._backend;
		if (backend.kind !== 'task') {
			stream.warning(zyraxoncode.l10n.t('Task follow-up is not available on the current backend.'));
			return {};
		}
		...
		if (this._activeTaskStreams.has(taskId)) {
			stream.progress(zyraxoncode.l10n.t('Steering'));
			const steerResult = await backend.sendFollowUpToTask(taskId, prompt);
			if (!steerResult) {
				stream.markdown(zyraxoncode.l10n.t('Failed to send follow-up to the task.'));
			}
			return {};
		}
		...
		if (!before) {
			stream.warning(zyraxoncode.l10n.t('Could not find the task for this chat session.'));
			return {};
		}
		...
		stream.progress(zyraxoncode.l10n.t('Delegating'));
		...
		const yieldCts = new zyraxoncode.CancellationTokenSource();
	...
	private async handleFollowUp(request: zyraxoncode.ChatRequest, context: zyraxoncode.ChatContext, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken) {
		if (!context.chatSessionContext || this.isUntitledChatSession(context)) {
			return {};
		}
		const { prompt } = request;
		if (!prompt || prompt.trim().length === 0) {
			stream.markdown(zyraxoncode.l10n.t('Please provide a message for the cloud agent.'));
			return {};
		}

		stream.progress(zyraxoncode.l10n.t('Preparing'));
		...
		if (!pullRequest) {
			stream.warning(zyraxoncode.l10n.t('Could not find the associated pull request {0} for this chat session.', '' + context.chatSessionContext.chatSessionItem.resource));
			return {};
		}
		...
		if (!result) {
			stream.markdown(zyraxoncode.l10n.t('Failed to add follow-up comment to the pull request.'));
			return {};
		}
		...
		stream.progress(zyraxoncode.l10n.t('Attaching to session'));
		...
		stream.markdown(zyraxoncode.l10n.t('Cloud agent has begun work on your request'));
		stream.markdown('\n\n');
	 ...
	 */
	private async extractReferences(references: readonly zyraxoncode.ChatPromptReference[] | undefined, pushedInProgressBranch: boolean): Promise<{ result: string; processedReferences: readonly zyraxoncode.ChatPromptReference[] }> {
		// '__ZYRAXKEEP__1_'  -> '.github/workflows/build-vsix.yml'
		const fileRefs: string[] = [];
		const fullFileParts: string[] = [];
		const processedReferences: zyraxoncode.ChatPromptReference[] = [];
		const git = this._gitExtensionService.getExtensionApi();
		for (const ref of references || []) {
			if (ref.value instanceof zyraxoncode.Uri && ref.value.scheme === 'file') {
			...
			} else if (ref.value instanceof zyraxoncode.Uri && ref.value.scheme === 'github-remote-file') {
				// Virtual filesystem for cloud repos in the sessions window.
				// URI format: __ZYRAXKEEP__2_{owner}/{repo}/{ref}/{path...}
			...
			} else if (ref.value instanceof zyraxoncode.Uri && ref.value.scheme === 'untitled') {
				// Get full content of untitled file
				try {
					const document = await zyraxoncode.workspace.openTextDocument(ref.value);
	...
	private async streamSessionLogs(stream: zyraxoncode.ChatResponseStream, pullRequest: PullRequestSearchItem, sessionId: string, token: zyraxoncode.CancellationToken): Promise<void> {
		const backend = this._backend;
		if (backend.kind !== 'pr') {
			return;
		}
	...
	private async streamNewLogContent(pullRequest: PullRequestSearchItem, stream: zyraxoncode.ChatResponseStream, newLogContent: string): Promise<{ hasStreamedContent: boolean; hasSetupStepProgress: boolean }> {
		try {
						...
						if (choice.finish_reason === 'tool_calls' && toolCalls?.length && (toolCalls[0].function.name === 'run_custom_setup_step' || toolCalls[0].function.name === 'run_setup')) {
							...
							if (delta.content && delta.content.trim()) {
								...
								if (toolPart) {
									...
									if (toolPart instanceof zyraxoncode.ChatResponseThinkingProgressPart) {
										stream.push(new zyraxoncode.ChatResponseThinkingProgressPart('', '', { zyraxoncodeReasoningDone: true }));
									}
								}
							} else {
	...
	private async waitForNewSession(
		...
		while (Date.now() - startTime < maxWaitTime && !token.isCancellationRequested) {
			...
			if (currentSessions.length > initialSessionCount) {
				...
				if (!inProgressSession) {
					stream.markdown(zyraxoncode.l10n.t('Timed out waiting for cloud agent to begin work. Please try again shortly.'));
					return;
				}
				this.refresh();
		...
		stream.markdown(zyraxoncode.l10n.t('Timed out waiting for the cloud agent to respond. The agent may still be processing your request.'));
		return;
	}
	...
	private async invokeRemoteAgent(prompt: string, problemContext: string, token: zyraxoncode.CancellationToken, stream: zyraxoncode.ChatResponseStream, base_ref: string, head_ref?: string, customAgentName?: string, modelName?: string, partnerAgentName?: string, selectedRepository?: string): Promise<
	...
	> {
		...
		if (selectedRepository && selectedRepository !== DEFAULT_REPOSITORY_ID) {
			...
			const repoId = repoIds?.[0];
			if (!repoId) {
				throw new Error(zyraxoncode.l10n.t('Unable to determine repository information. Please ensure you are working within a Git repository.'));
			}
			repoOwner = repoId.org;
			repoName = repoId.repo;
		...
		if (isTruncated) {
			stream.progress(zyraxoncode.l10n.t('Truncating context'));
			const truncationResult = await zyraxoncode.window.showWarningMessage(
				zyraxoncode.l10n.t('Prompt size exceeded'), { modal: true, detail: zyraxoncode.l10n.t('Your prompt will be truncated to fit within cloud agent\'s context window. This may affect the quality of the response.') }, CONTINUE_TRUNCATION);
			...
			*/
			this.telemetry.sendTelemetryEvent('copilot.codingAgent.truncation', { Zyraxon: true, github: false }, {
				isCancelled: String(userCancelled),
			});
			if (userCancelled) {
				throw new Error(zyraxoncode.l10n.t('User cancelled due to truncation.'));
			}
		}
