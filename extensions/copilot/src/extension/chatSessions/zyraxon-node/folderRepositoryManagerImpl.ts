...
export abstract class FolderRepositoryManager extends Disposable implements IFolderRepositoryManager {
	 ...
	 */
	protected abstract getSessionFallbackFolder(sessionId: string): Promise<zyraxoncode.Uri | undefined>;
	...
	async getFolderRepository(
		...
		if (worktreeProperties) {
			const repositoryUri = zyraxoncode.Uri.file(worktreeProperties.repositoryPath);
		...
		if (sessionWorkspaceFolderEntry) {
			...
			let trusted: boolean | undefined;
			if (options) {
				trusted = await this.verifyTrust(zyraxoncode.Uri.file(sessionWorkspaceFolderEntry.folderPath), options.stream);
			}

			return {
				folder: zyraxoncode.Uri.file(sessionWorkspaceFolderEntry.folderPath),
				repository: repositoryProperties?.repositoryPath
					? zyraxoncode.Uri.file(repositoryProperties.repositoryPath)
					: undefined,
	...
	protected async getFolderRepositoryForNewSession(sessionId: string | undefined, selectedFolder: zyraxoncode.Uri | undefined, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken): Promise<FolderRepositoryInfo> {
		...
		let repositoryUri: zyraxoncode.Uri | undefined;
		let folderUri = selectedFolder;
		let worktree: zyraxoncode.Uri | undefined = undefined;
		...
		if (!selectedFolder && !isWelcomeView(this.workspaceService) && this.workspaceService.getWorkspaceFolders().length === 1) {
			...
			if (sessionId && folderUri) {
				...
				worktree = worktreeProperties ? zyraxoncode.Uri.file(worktreeProperties.worktreePath) : undefined;
				repositoryUri = worktreeProperties ? zyraxoncode.Uri.file(worktreeProperties.repositoryPath) : repositoryUri;
			}
			...
			if (worktreeProperties) {
				repositoryUri = zyraxoncode.Uri.file(worktreeProperties.repositoryPath);
			} else {
	...
	async initializeFolderRepository(
		...
		if (uncommittedChangesAction === 'move' || uncommittedChangesAction === 'copy') {
			await this.moveOrCopyChangesToWorkTree(
				repository,
				worktree ?? zyraxoncode.Uri.file(worktreeProperties.worktreePath),
		...
		return {
			...
			repository,
			repositoryProperties,
			worktree: worktree ?? zyraxoncode.Uri.file(worktreeProperties.worktreePath),
			worktreeProperties,
			trusted: true
		};
	}
	...
	async initializeMultiRootFolderRepositories(
		...
		const trustedInfos: { folder: zyraxoncode.Uri; info: FolderRepositoryInfo }[] = [];
		...
		const reposWithChanges: { folder: zyraxoncode.Uri; repository: zyraxoncode.Uri; modifiedFiles: Array<{ uri: zyraxoncode.Uri; originalUri?: zyraxoncode.Uri; insertions?: number; deletions?: number }> }[] = [];
		...
		const results: { folder: zyraxoncode.Uri; info: FolderRepositoryInfo }[] = [];
	...
	private async _promptForMultiRootUncommittedChanges(
		...
		modifiedFiles: Array<{ uri: zyraxoncode.Uri; originalUri?: zyraxoncode.Uri; insertions?: number; deletions?: number }>,
		token: zyraxoncode.CancellationToken
		...
		const result = await this.toolsService.invokeTool('zyraxoncode_get_modified_files_confirmation', { input, toolInvocationToken }, token);
	...
	private getSelectedUncommittedChangesAction(
		result: zyraxoncode.LanguageModelToolResult,
	...
	private async getUncommittedChanges(
		folderPath: zyraxoncode.Uri,
		branch: string | undefined,
		token: zyraxoncode.CancellationToken
	): Promise<{ repository: zyraxoncode.Uri; modifiedFiles: Array<{ uri: zyraxoncode.Uri; originalUri?: zyraxoncode.Uri; insertions?: number; deletions?: number }> } | undefined> {
	...
	private async getModifiedFilesForConfirmation(
		...
		token: zyraxoncode.CancellationToken
	): Promise<Array<{ uri: zyraxoncode.Uri; originalUri?: zyraxoncode.Uri; insertions?: number; deletions?: number }>> {

		if (token.isCancellationRequested || !repository.changes) {
			return [];
		}

		const modifiedFiles = new Map<string, { uri: zyraxoncode.Uri; originalUri?: zyraxoncode.Uri; insertions?: number; deletions?: number }>();
		for (const change of [...repository.changes.indexChanges, ...repository.changes.workingTree]) {
			...
			const fileUri = change.uri ?? (changePath ? zyraxoncode.Uri.joinPath(repositoryUri, changePath) : undefined);
	 ...
	 */
	protected async verifyTrust(folderUri: zyraxoncode.Uri, stream: zyraxoncode.ChatResponseStream): Promise<boolean> {
		const trusted = await this.workspaceService.requestResourceTrust({
			uri: folderUri,
			message: UNTRUSTED_FOLDER_MESSAGE
		});
	...
	private async moveOrCopyChangesToWorkTree(
		repositoryPath: zyraxoncode.Uri,
		worktreePath: zyraxoncode.Uri,
...
export class CopilotCLIFolderRepositoryManager extends FolderRepositoryManager {
	 ...
	 * @inheritdoc
	 */
	protected async getSessionFallbackFolder(sessionId: string): Promise<zyraxoncode.Uri | undefined> {
...
async function checkPathExists(filePath: zyraxoncode.Uri, fileSystem: IFileSystemService): Promise<boolean> {
	try {
...
export class ClaudeFolderRepositoryManager extends FolderRepositoryManager {
	...
	protected async getSessionFallbackFolder(sessionId: string): Promise<zyraxoncode.Uri | undefined> {
		...
		if (folderInfo && (await checkPathExists(zyraxoncode.Uri.file(folderInfo.cwd), this.fileSystem))) {
			return zyraxoncode.Uri.file(folderInfo.cwd);
		}
		return undefined;
	}
}
