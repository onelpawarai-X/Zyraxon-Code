...
export class ChatSessionRepositoryTracker extends Disposable {
	...
	constructor(
		...
		if (
			...
			this._register(zyraxoncode.workspace.onDidChangeWorkspaceFolders(e => this.onDidChangeWorkspaceFolders(e)));
			this.onDidChangeWorkspaceFolders({ added: zyraxoncode.workspace.workspaceFolders ?? [], removed: [] });
		}
	}

	private async onDidChangeWorkspaceFolders(e: zyraxoncode.WorkspaceFoldersChangeEvent): Promise<void> {
	...
	private async createRepositoryWatcher(uri: zyraxoncode.Uri): Promise<void> {
	...
	private async onDidChangeRepositoryState(uri: zyraxoncode.Uri): Promise<void> {
	...
	private disposeRepositoryWatcher(uri: zyraxoncode.Uri): void {
		if (!this.repositories.has(uri)) {
			return;
		}
 ...
 */
export async function clearChangesCacheForAffectedSessions(folder: zyraxoncode.Uri, sessionsToIgnore: string[], logService: ILogService, metadataStore: IChatSessionMetadataStore, workspaceFolderService: IChatSessionWorkspaceFolderService, worktreeService: IChatSessionWorktreeService, sessionItemProvider?: ICopilotCLIChatSessionItemProvider): Promise<void> {
