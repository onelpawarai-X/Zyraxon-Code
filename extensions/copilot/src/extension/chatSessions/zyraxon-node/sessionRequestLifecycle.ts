...
export interface ISessionRequestLifecycle {
	 ...
	 */
	startRequest(sessionId: string, request: zyraxoncode.ChatRequest, isFirstRequest: boolean, workspace: IWorkspaceInfo, agentName?: string): Promise<void>;
	 ...
	 * steering).
	 */
	endRequest(sessionId: string, request: zyraxoncode.ChatRequest, session: SessionCompletionInfo, token: zyraxoncode.CancellationToken): Promise<void>;
}
...
export class SessionRequestLifecycle extends Disposable implements ISessionRequestLifecycle {
	...
	async startRequest(sessionId: string, request: zyraxoncode.ChatRequest, isFirstRequest: boolean, workspace: IWorkspaceInfo, agentName?: string): Promise<void> {
		if (isFirstRequest) {
		...
		this.metadataStore.updateRequestDetails(sessionId, [{ zyraxoncodeRequestId: request.id, agentId: agentName ?? '', modeInstructions }]).catch(ex => this.logService.error(ex, 'Failed to update request details'));

		const requests = this.pendingRequestBySession.get(sessionId) ?? new Set<zyraxoncode.ChatRequest>();
	...
	async endRequest(sessionId: string, request: zyraxoncode.ChatRequest, session: SessionCompletionInfo, token: zyraxoncode.CancellationToken): Promise<void> {
	...
	private untrackRequest(sessionId: string, request: zyraxoncode.ChatRequest): void {
