...
import { extractTitle, SessionIdForPr, SessionIdForTask } from '../zyraxoncode/copilotCodingAgentUtils';
 ...
 * (Copilot CLI / ZYRAXON Code / JetBrains). The owning surface is identified by the agent integration
...
/**
 * Parse `task.html_url` (e.g. `__ZYRAXKEEP__0_<owner>/<repo>/agents/tasks/<id>`) to
...
export class TaskApiBackend implements TaskCloudAgentBackend {
	...
	async createSession(params: CreateCloudSessionParams, _stream: zyraxoncode.ChatResponseStream, _token: zyraxoncode.CancellationToken): Promise<CloudDelegationResult> {
	...
	async waitForTaskUpdate(taskId: string, since: { turnCount: number; updatedAt?: string }, token?: zyraxoncode.CancellationToken): Promise<TaskContent | undefined> {
