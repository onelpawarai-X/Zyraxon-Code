...
import { getAuthorDisplayName, isFailedTaskState } from '../zyraxoncode/copilotCodingAgentUtils';
...
export function formatTaskStoppedMessage(state: AgentTaskState, detail: string | undefined): string {
	...
	return zyraxoncode.l10n.t('Copilot stopped: {0}', reason);
}

function defaultStopReasonForState(state: AgentTaskState): string {
	switch (state) {
		case 'cancelled':
			return zyraxoncode.l10n.t('cancelled');
		case 'timed_out':
			return zyraxoncode.l10n.t('timed out');
		default:
			return zyraxoncode.l10n.t('an error occurred');
	}
}
...
export class ChatSessionContentBuilder {
	...
	public async buildSessionHistory(
		...
		initialReferences: Promise<zyraxoncode.ChatPromptReference[]>,
		...
		const sessionResults = await Promise.all(
			sessions.map(async (session, sessionIndex) => {
				...
				if (sessionIndex === 0 && pullRequest.author && zyraxoncode.workspace.workspaceFolders && zyraxoncode.workspace.workspaceFolders.length > 0) {
					const plaintextBody = pullRequest.body;

					const card = new zyraxoncode.ChatResponsePullRequestPart({ command: 'github.copilot.chat.openPullRequestReroute', title: zyraxoncode.l10n.t('View Pull Request {0}', `#${pullRequest.number}`), arguments: [pullRequest.number] }, pullRequest.title, plaintextBody, getAuthorDisplayName(pullRequest.author), `#${pullRequest.number}`);
					const cardTurn = new zyraxoncode.ChatResponseTurn2([card], {}, this.type);
					turns.push(cardTurn);
				}
	...
	public buildTaskHistory(
		...
		return (async () => {
			...
			turns.forEach((turn, index) => {
				...
				if (index === 0 && pullRequest && pullRequest.author && zyraxoncode.workspace.workspaceFolders && zyraxoncode.workspace.workspaceFolders.length > 0) {
					const card = new zyraxoncode.ChatResponsePullRequestPart(
						{ command: 'github.copilot.chat.openPullRequestReroute', title: zyraxoncode.l10n.t('View Pull Request {0}', `#${pullRequest.number}`), arguments: [pullRequest.number] },
						pullRequest.title,
						pullRequest.body,
						getAuthorDisplayName(pullRequest.author),
						`#${pullRequest.number}`,
					);
					history.push(new zyraxoncode.ChatResponseTurn2([card], {}, this.type));
				}
	...
	private buildTaskResponseTurn(events: readonly AgentTaskSessionEvent[], state: AgentTaskSession['state'] | undefined, failureMessage?: string): ChatResponseTurn2[] {
		...
		if (parts.length === 0) {
			if (failureMessage !== undefined) {
				...
				parts.push(new ChatResponseProgressPart(zyraxoncode.l10n.t('Session is in progress...')));
			}
		}
	 ...
	 */
	public createToolPartFromRequest(req: { name?: string; toolCallId?: string; arguments?: unknown }): zyraxoncode.ChatToolInvocationPart | zyraxoncode.ChatResponseThinkingProgressPart | undefined {
				...
				if (delta.content && delta.content.trim()) {
					...
					if (toolPart) {
						...
						if (toolPart instanceof ChatResponseThinkingProgressPart) {
							responseParts.push(new ChatResponseThinkingProgressPart('', '', { zyraxoncodeReasoningDone: true }));
						}
					}
				}
				...
				if (toolCalls) {
					...
					if (trimmedContent) {
						...
						if (choice.finish_reason === 'stop') {
							...
							responseParts.push(new ChatResponseThinkingProgressPart(trimmedContent, '', { zyraxoncodeReasoningDone: true }));
						}
						currentResponseContent = '';
					}
				}
