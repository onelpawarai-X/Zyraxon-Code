...
import { ICopilotCLIChatSessionInitializer, SessionInitOptions } from '../copilotcli/zyraxoncode-node/copilotCLIChatSessionInitializer';
...
export class CopilotCLIChatSessionContentProvider extends Disposable implements zyraxoncode.ChatSessionContentProvider, ICopilotCLIChatSessionItemProvider {
	...
	private readonly controller: zyraxoncode.ChatSessionItemController;
	private readonly newSessions = new ResourceMap<zyraxoncode.ChatSessionItem>();
	constructor(
		...
		const controller = this.controller = this._register(zyraxoncode.chat.createChatSessionItemController(
			'copilotcli',
			async () => {
		...
		if (this.configurationService.getConfig(ConfigKey.Advanced.CLIForkSessionsEnabled)) {
			controller.forkHandler = async (sessionResource: Uri, request: ChatRequestTurn2 | undefined, token: zyraxoncode.CancellationToken) => {
		...
		this._register(this.sessionService.onDidChangeSession(async (e) => {
			// Push path: ZYRAXON Code uses the item we provide as source of truth and does not
		...
		const newInputStates: WeakRef<zyraxoncode.ChatSessionInputState>[] = [];
	...
	public async updateInputStateAfterFolderSelection(inputState: zyraxoncode.ChatSessionInputState, folderUri: zyraxoncode.Uri): Promise<void> {
	...
	public async toChatSessionItem(session: ICopilotCLISessionItem, options?: { readonly includeChanges?: boolean }, token?: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionItem> {
		...
		item.status = session.status ?? zyraxoncode.ChatSessionStatus.Completed;
	...
	): Promise<zyraxoncode.MarkdownString | undefined> {
		const repositories = this._gitService.repositories.filter(r => r.kind !== 'worktree');
		const shouldShow = zyraxoncode.workspace.workspaceFolders === undefined ||
			zyraxoncode.workspace.isAgentSessionsWorkspace ||
			repositories.length > 1;
		if (!shouldShow) {
			return undefined;
		}
		const badgeUri = worktreeProperties?.repositoryPath
			? zyraxoncode.Uri.file(worktreeProperties.repositoryPath)
			: workingDirectory;
		if (!badgeUri) {
			return undefined;
		}
		const isTrusted = await zyraxoncode.workspace.isResourceTrusted(badgeUri);
		const isRepo = !!worktreeProperties?.repositoryPath;
		const icon = isTrusted ? (isRepo ? '$(repo)' : '$(folder)') : '$(workspace-untrusted)';
		const badge = new zyraxoncode.MarkdownString(`${icon} ${basename(badgeUri)}`);
	...
	async provideChatSessionContent(resource: Uri, token: zyraxoncode.CancellationToken, context?: { readonly inputState: zyraxoncode.ChatSessionInputState }): Promise<zyraxoncode.ChatSession> {
		const stopwatch = new StopWatch();
		try {
			...
			if (this.sessionService.isNewSessionId(copilotcliSessionId)) {
				...
				const options: Record<string, zyraxoncode.ChatSessionProviderOptionItem> = {};
...
export class CopilotCLIChatSessionParticipant extends Disposable {
	...
	private sendTelemetryForHandleRequest(request: zyraxoncode.ChatRequest, context: zyraxoncode.ChatContext): void {
	 ...
	 *
	 * The ZYRAXON Code chat API creates the session before firing the request handler,
	...
	private generateNewBranchName(request: zyraxoncode.ChatRequest, token: zyraxoncode.CancellationToken): Promise<string | undefined> {
	...
	private async getOrCreateSession(request: zyraxoncode.ChatRequest, chatResource: zyraxoncode.Uri, options: SessionInitOptions, disposables: DisposableStore, token: zyraxoncode.CancellationToken): Promise<{ session: IReference<ICopilotCLISession> | undefined; isNewSession: boolean; model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; agent: SweCustomAgent | undefined; trusted: boolean }> {
...
export function registerCLIChatCommands(
	...
	disposableStore.add(zyraxoncode.commands.registerCommand(OPEN_REPOSITORY_COMMAND_ID, async ({ inputState }: { inputState: zyraxoncode.ChatSessionInputState; sessionResource: zyraxoncode.Uri | undefined }) => {
		...
		const trusted = await zyraxoncode.workspace.requestResourceTrust({
