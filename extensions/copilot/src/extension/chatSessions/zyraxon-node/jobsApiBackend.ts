...
import {
	...
	FollowUpResult,
	PrCloudAgentBackend,
} from '../zyraxoncode/cloudAgentBackend';
import { body_suffix, formatBodyPlaceholder, JOBS_API_VERSION, SessionIdForPr } from '../zyraxoncode/copilotCodingAgentUtils';
...
export class JobsApiBackend implements PrCloudAgentBackend {
	...
	parseSessionId(resource: zyraxoncode.Uri): CloudSessionIdentity | undefined {
	...
	async fetchPullRequestContent(repoOwner: string, repoName: string, sessions: SessionInfo[]): Promise<CloudSessionContent> {
		...
		const jobInfo = await this._octoKitService.getJobBySessionId(repoOwner, repoName, sessions[0].id, 'zyraxoncode-copilot-chat', CLOUD_SESSIONS_AUTH_OPTIONS);
	...
	async createSession(params: CreateCloudSessionParams, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken): Promise<CloudDelegationResult> {
		...
		stream?.progress(zyraxoncode.l10n.t('Delegating to cloud agent'));
		...
		stream.progress(zyraxoncode.l10n.t('Creating pull request'));
		...
		if (!jobInfo || !jobInfo.pull_request) {
			throw new Error(zyraxoncode.l10n.t('Failed to retrieve pull request information from job'));
		}

		const { number } = jobInfo.pull_request;
		if (!number || isNaN(number)) {
			throw new Error(zyraxoncode.l10n.t('Invalid pull request number received from cloud agent'));
		}
	...
	private invalidRemoteAgentJobError(response: unknown, params: CreateCloudSessionParams): Error {
		...
		switch (statusCode) {
			case 401:
				return new JobsApiError(zyraxoncode.l10n.t('Cloud agent is not authorized to run on this repository. This may be because the Copilot coding agent is disabled for your organization, or your active GitHub account does not have push access to the target repository.'), statusCode);
			case 403:
				return new JobsApiError(zyraxoncode.l10n.t('Cloud agent is not enabled for this repository. You may need to enable it in [GitHub settings]({0}) or contact your organization administrator.', `__ZYRAXKEEP__0_{params.host}/settings/copilot/coding_agent`), statusCode);
			case 404:
				return new JobsApiError(zyraxoncode.l10n.t('The repository `{0}/{1}` was not found or you do not have access to it.', params.owner, params.repo), statusCode);
			case 422:
				return new JobsApiError(zyraxoncode.l10n.t('Cloud agent was unable to create a pull request with the specified base branch `{0}`. Please push the branch to the remote and verify repository rules allow this operation. For empty repos, push an initial commit and try again.', params.baseRef), statusCode);
			case 500:
				return new JobsApiError(zyraxoncode.l10n.t('Cloud agent service encountered an internal error. Please try again later.'), statusCode);
			default:
				return new JobsApiError(zyraxoncode.l10n.t('Received invalid response {0} from cloud agent.', statusCode ? statusCode : ''), statusCode);
		}
	}

	async waitForSessionReady(sessionId: string, token?: zyraxoncode.CancellationToken): Promise<SessionInfo | undefined> {
	...
	private async waitForJobWithPullRequest(
		...
		while (Date.now() - startTime < maxWaitTime && (!token || !token.isCancellationRequested)) {
			const jobInfo = await this._octoKitService.getJobByJobId(owner, repo, jobId, 'zyraxoncode-copilot-chat', CLOUD_SESSIONS_AUTH_OPTIONS);
