...
import { DiffChange } from '../../../platform/git/zyraxoncode/git';
...
export class ChatSessionWorktreeService extends Disposable implements IChatSessionWorktreeService {
	...
	private readonly _onDidChangeWorktreeChanges = this._register(new zyraxoncode.EventEmitter<{ sessionId: string }>());
	...
	async createWorktree(repositoryPath: zyraxoncode.Uri, stream?: zyraxoncode.ChatResponseStream, baseBranch?: string, branchName?: string): Promise<ChatSessionWorktreeProperties | undefined> {
		if (!stream) {
	...
	private async _createWorktree(repositoryPath: zyraxoncode.Uri, progress?: zyraxoncode.Progress<zyraxoncode.ChatResponsePart>, baseBranch?: string, branchName?: string): Promise<ChatSessionWorktreeProperties | undefined> {
		try {
			const activeRepository = await this.gitService.getRepository(repositoryPath);
			if (!activeRepository) {
				progress?.report(new zyraxoncode.ChatResponseWarningPart(zyraxoncode.l10n.t('Failed to create worktree for isolation, using default workspace directory')));
			...
			progress?.report(new zyraxoncode.ChatResponseWarningPart(zyraxoncode.l10n.t('Error creating worktree for isolation: {0}', error instanceof Error ? error.message : String(error))));
	...
	private async generateBranchName(preferredName: string | undefined, repository: RepoContext) {
		const branchPrefixConfig = zyraxoncode.workspace.getConfiguration('git').get<string>('branchPrefix') ?? '';
	...
	async updateWorktreeProperties(sessionId: string, properties: Partial<ChatSessionWorktreeProperties>): Promise<void> {
		await this._updatePropertiesSequencer.queue(sessionId, async () => {
			const currentProperties = await this.getWorktreeProperties(sessionId);
			if (!currentProperties || typeof currentProperties === 'string') {
				return;
			}

			await this.setWorktreeProperties(sessionId, { ...currentProperties, ...properties } as ChatSessionWorktreeProperties);
		});
	}
	...
	async getWorktreeRepository(sessionId: string): Promise<RepoContext | undefined> {
		...
		return this.gitService.getRepository(zyraxoncode.Uri.file(worktreeProperties.repositoryPath));
	}

	async getWorktreePath(sessionId: string): Promise<zyraxoncode.Uri | undefined> {
		const worktreeProperties = await this.getWorktreeProperties(sessionId);
		if (!worktreeProperties) {
			return undefined;
		} else if (typeof worktreeProperties === 'string') {
			// Legacy worktree path
			return zyraxoncode.Uri.file(worktreeProperties);
		} else {
			// Worktree properties v1
			return zyraxoncode.Uri.file(worktreeProperties.worktreePath);
		}
	}
	...
	async applyWorktreeChanges(sessionId: string): Promise<void> {
		...
		if (worktreeProperties === undefined || (worktreeProperties.version === 1 && worktreeProperties.autoCommit === false)) {
			...
			const activeRepository = worktreeProperties?.repositoryPath
				? await this.gitService.getRepository(zyraxoncode.Uri.file(worktreeProperties.repositoryPath))
		...
		const patch = await this.gitService.diffBetweenPatch(
			zyraxoncode.Uri.file(worktreeProperties.worktreePath),
		...
		const patchFileUri = zyraxoncode.Uri.file(patchFilePath);
		await zyraxoncode.workspace.fs.writeFile(patchFileUri, encoder.encode(patch));

		try {
			// Apply patch
			await this.gitService.applyPatch(zyraxoncode.Uri.file(worktreeProperties.repositoryPath), patchFilePath);
		} catch (error) {
			...
			await zyraxoncode.workspace.fs.delete(patchFileUri);
		}

		// Update base commit for the worktree after applying the changes
		const ref = await this.gitService.getRefs(zyraxoncode.Uri.file(worktreeProperties.repositoryPath), {
	...
	async refreshWorktreeChanges(sessionId: string): Promise<void> {
		...
		// Clear the cache
		await this.updateWorktreeProperties(sessionId, { changes: undefined });

		// Populate the cache
		await this.getWorktreeChanges(sessionId);
	}

	async getWorktreeChanges(sessionId: string): Promise<readonly zyraxoncode.ChatSessionChangedFile[] | undefined> {
		const worktreeProperties = await this.getWorktreeProperties(sessionId);
		if (!worktreeProperties || typeof worktreeProperties === 'string') {
			return undefined;
		}
		...
		try {
			...
			// states are initialized in the zyraxoncode.git extension. This is needed as these
			...
			if (worktreeProperties.version === 2 && worktreeProperties.autoCommit === true) {
				const properties = zyraxoncode.workspace.isAgentSessionsWorkspace
	...
	async handleRequestCompleted(sessionId: string): Promise<void> {
		...
		const repository = await this.gitCommitMessageService.getRepository(zyraxoncode.Uri.file(worktreePath));
		if (!repository) {
		...
		// Commit the changes
		await this.gitService.commit(zyraxoncode.Uri.file(worktreePath), message, { all: true, noVerify: true, signCommit: false });
	...
	async cleanupWorktreeOnArchive(sessionId: string): Promise<{ cleaned: boolean; reason?: string }> {
		...
		// Delete the worktree
		try {
			const parentRepository = await this.gitService.getRepository(zyraxoncode.Uri.file(worktreeProperties.repositoryPath), true);
			if (!parentRepository) {
	...
	private async _getWorktreeChangesFromIndex(worktreeProperties: ChatSessionWorktreeProperties): Promise<readonly ChatSessionWorktreeFile[] | undefined> {
		const worktreePath = zyraxoncode.Uri.file(worktreeProperties.worktreePath);
	...
	private async _getWorktreeChangesFromCommits(worktreeProperties: ChatSessionWorktreePropertiesV2): Promise<{ changes: readonly ChatSessionWorktreeFile[] } | undefined> {
		...
		const repository = await this.gitService.getRepository(zyraxoncode.Uri.file(worktreeProperties.repositoryPath));
	...
	private async _getWorktreeChanges(sessionId: string, worktreeProperties: ChatSessionWorktreeProperties): Promise<{
		...
		const worktreeRepository = await this.gitService.getRepository(zyraxoncode.Uri.file(worktreeProperties.worktreePath));
		...
		if (hasUntrackedChanges) {
			...
			const tmpDirName = `zyraxoncode-sessions-${sessionId}-${generateUuid()}`;
	...
	private _toChatSessionChangedFile2(sessionId: string, change: ChatSessionWorktreeFile, worktreeProperties: ChatSessionWorktreeProperties): zyraxoncode.ChatSessionChangedFile {
		let originalFileRef: string, modifiedFileRef: string | undefined;
		if (worktreeProperties.version === 2) {
			// Commit | Working tree
			originalFileRef = zyraxoncode.workspace.isAgentSessionsWorkspace
				? worktreeProperties.mergeBaseCommit ?? worktreeProperties.baseCommit
				: worktreeProperties.baseCommit;
			modifiedFileRef = zyraxoncode.workspace.isAgentSessionsWorkspace
				? undefined
				: worktreeProperties.branchName;
		} else {
			// Legacy
		...
		return new zyraxoncode.ChatSessionChangedFile(
			...
			change.originalFilePath
				? toGitUri(zyraxoncode.Uri.file(change.originalFilePath), originalFileRef)
				: undefined,
			change.modifiedFilePath
				? modifiedFileRef
					? toGitUri(zyraxoncode.Uri.file(change.modifiedFilePath), modifiedFileRef)
	...
	async setAdditionalWorktreeProperties(sessionId: string, properties: ChatSessionWorktreeProperties[]): Promise<void> {
		const workspaces = properties.map(props => ({
			folder: undefined,
			repository: zyraxoncode.Uri.file(props.repositoryPath),
			worktree: zyraxoncode.Uri.file(props.worktreePath),
