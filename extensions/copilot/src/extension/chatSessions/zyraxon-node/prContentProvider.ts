 ...
 */
export function fromPRContentUri(uri: zyraxoncode.Uri): PRContentUriParams | undefined {
 ...
 */
export class PRContentProvider extends Disposable implements zyraxoncode.TextDocumentContentProvider {
	private static readonly ID = 'PRContentProvider';
	private _onDidChange = this._register(new zyraxoncode.EventEmitter<zyraxoncode.Uri>());
	...
	constructor(
		...
		this._register(
			zyraxoncode.workspace.registerTextDocumentContentProvider(
				PR_SCHEME,
				this
			)
		);
	...
	async provideTextDocumentContent(uri: zyraxoncode.Uri): Promise<string> {
