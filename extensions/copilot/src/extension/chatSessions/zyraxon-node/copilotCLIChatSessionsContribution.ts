...
import { ICopilotCLISessionTracker } from '../copilotcli/zyraxoncode-node/copilotCLISessionTracker';
...
import { convertReferenceToVariable } from '../copilotcli/zyraxoncode-node/copilotCLIPromptReferences';
...
namespace SessionIdForCLI {
	export function getResource(sessionId: string): zyraxoncode.Uri {
		return zyraxoncode.Uri.from({
			scheme: 'copilotcli', path: `/${sessionId}`,
		});
	}

	export function parse(resource: zyraxoncode.Uri): string {
		return resource.path.slice(1);
	}

	export function isCLIResource(resource: zyraxoncode.Uri): boolean {
...
function getIssueRuntimeInfo(): { readonly platform: string; readonly zyraxoncodeInfo: string; readonly extensionVersion: string } {
	const extensionVersion = zyraxoncode.extensions.getExtension(EXTENSION_ID)?.packageJSON?.version;

	return {
		platform: `${process.platform}-${process.arch}`,
		zyraxoncodeInfo: `${zyraxoncode.env.appName} ${zyraxoncode.version}`,
...
function getSessionLoadFailureIssueInfo(invalidSessionMessage: string): { readonly issueBody: string; readonly issueUrl: string } {
	...
	const issueBody = `## Description\n\nFailed to load a Copilot CLI chat session.\n\n## Environment\n\n- Platform: ${runtimeInfo.platform}\n- ZYRAXON Code: ${runtimeInfo.zyraxoncodeInfo}\n- Chat Extension Version: ${runtimeInfo.extensionVersion}\n\n## Error\n\n\`\`\`\n${invalidSessionMessage}\n\`\`\``;
	const issueUrl = `__ZYRAXKEEP__0_{encodeURIComponent(issueTitle)}&body=${encodeURIComponent(issueBody)}`;
...
export class CopilotCLIChatSessionItemProvider extends Disposable implements zyraxoncode.ChatSessionItemProvider, ICopilotCLIChatSessionItemProvider {
	 ...
	 * When ZYRAXON Code asks for the session, always return the old untitled session Uri.
	 */
	...
	private readonly _onDidCommitChatSessionItem = this._register(new Emitter<{ original: zyraxoncode.ChatSessionItem; modified: zyraxoncode.ChatSessionItem }>());
	public readonly onDidCommitChatSessionItem: Event<{ original: zyraxoncode.ChatSessionItem; modified: zyraxoncode.ChatSessionItem }> = this._onDidCommitChatSessionItem.event;
	/**
	...
	public resolveChatSessionItem?: (item: zyraxoncode.ChatSessionItem, token: zyraxoncode.CancellationToken) => Promise<zyraxoncode.ChatSessionItem | undefined>;
	...
	constructor(
		...
		if (configurationService.getConfig(ConfigKey.Advanced.CLIChatLazyLoadSessionItem)) {
			this.resolveChatSessionItem = async (item: zyraxoncode.ChatSessionItem, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionItem | undefined> => {
	...
	public notifySessionsChange(): void {
		...
		// (e.g. another ZYRAXON Code window editing the same session) become visible
	...
	public swap(original: zyraxoncode.ChatSessionItem, modified: zyraxoncode.ChatSessionItem): void {
		this._onDidCommitChatSessionItem.fire({ original, modified });
	}

	public async provideChatSessionItems(token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionItem[]> {
	...
	private shouldShowBadge(): boolean {
		...
		return zyraxoncode.workspace.workspaceFolders === undefined || // empty window
			zyraxoncode.workspace.isAgentSessionsWorkspace ||          // agent sessions workspace
			repositories.length > 1;                              // multiple repositories
	}

	public async toChatSessionItem(session: ICopilotCLISessionItem, options?: { readonly includeChanges?: boolean }, token: zyraxoncode.CancellationToken = CancellationToken.None): Promise<zyraxoncode.ChatSessionItem> {
		...
		const workingDirectory = worktreeProperties?.worktreePath ? zyraxoncode.Uri.file(worktreeProperties.worktreePath)
		...
		// Badge
		let badge: zyraxoncode.MarkdownString | undefined;
		if (this.shouldShowBadge() && !token.isCancellationRequested) {
			if (worktreeProperties?.repositoryPath) {
				// Worktree
				const repositoryPathUri = zyraxoncode.Uri.file(worktreeProperties.repositoryPath);
				const isTrusted = await zyraxoncode.workspace.isResourceTrusted(repositoryPathUri);
				const badgeIcon = isTrusted ? '$(repo)' : '$(workspace-untrusted)';

				badge = new zyraxoncode.MarkdownString(`${badgeIcon} ${basename(repositoryPathUri)}`);
				badge.supportThemeIcons = true;
			} else if (workingDirectory) {
				// Workspace
				const isTrusted = await zyraxoncode.workspace.isResourceTrusted(workingDirectory);
				const badgeIcon = isTrusted ? '$(folder)' : '$(workspace-untrusted)';

				badge = new zyraxoncode.MarkdownString(`${badgeIcon} ${basename(workingDirectory)}`);
				badge.supportThemeIcons = true;
			}
		}
		...
		let changes: zyraxoncode.ChatSessionChangedFile[] | undefined;
		...
		// Status
		const status = session.status ?? zyraxoncode.ChatSessionStatus.Completed;
	...
	private async buildChanges(
		...
		workingDirectory: zyraxoncode.Uri | undefined,
		token: zyraxoncode.CancellationToken
	): Promise<zyraxoncode.ChatSessionChangedFile[]> {
		const changes: zyraxoncode.ChatSessionChangedFile[] = [];
		if (worktreeProperties?.repositoryPath && await zyraxoncode.workspace.isResourceTrusted(zyraxoncode.Uri.file(worktreeProperties.repositoryPath))) {
			changes.push(...(await raceCancellation(this.worktreeManager.getWorktreeChanges(sessionId), token) ?? []));
		} else if (workingDirectory && await zyraxoncode.workspace.isResourceTrusted(workingDirectory)) {
			...
			changes.push(...workspaceChanges.map(change => {
				...
				return new zyraxoncode.ChatSessionChangedFile(
					zyraxoncode.Uri.file(change.filePath),
					change.originalFilePath
						? toGitUri(zyraxoncode.Uri.file(change.originalFilePath), originalRef)
						: undefined,
					change.modifiedFilePath
						? zyraxoncode.Uri.file(change.modifiedFilePath)
						: undefined,
	...
	public async resumeCopilotCLISessionInTerminal(sessionItem: zyraxoncode.ChatSessionItem): Promise<void> {
		...
		const token = new zyraxoncode.CancellationTokenSource();
		try {
...
export class CopilotCLIChatSessionContentProvider extends Disposable implements zyraxoncode.ChatSessionContentProvider {
	private readonly _onDidChangeChatSessionOptions = this._register(new Emitter<zyraxoncode.ChatSessionOptionChangeEvent>());
	...
	public notifySessionOptionsChange(resource: zyraxoncode.Uri, updates: ReadonlyArray<{ optionId: string; value: string | zyraxoncode.ChatSessionProviderOptionItem }>): void {
	...
	private async getDefaultUntitledSessionRepositoryOption(copilotcliSessionId: string | undefined, token: zyraxoncode.CancellationToken) {
	...
	public trackLastUsedFolderInWelcomeView(folderUri: zyraxoncode.Uri): void {
	...
	async provideChatSessionContentForUntitledSession(resource: Uri, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSession> {
		const copilotcliSessionId = SessionIdForCLI.parse(resource);
		...
		const options: Record<string, string | zyraxoncode.ChatSessionProviderOptionItem> = {};
		...
		if (defaultRepo) {
			...
			const defaultRepoIsTrusted = await zyraxoncode.workspace.isResourceTrusted(defaultRepo);
			...
			if (defaultRepoIsTrusted) {
				...
				if (repoInfo.repository && isIsolationOptionFeatureEnabled(this.configurationService)) {
					...
					options[ISOLATION_OPTION_ID] = {
						...
						icon: new zyraxoncode.ThemeIcon(isolationMode === IsolationMode.Worktree ? 'worktree' : 'folder')
					};
				}
				...
				if (branchItems.length > 0 && shouldShowBranch) {
					...
					options[BRANCH_OPTION_ID] = {
						...
						icon: new zyraxoncode.ThemeIcon('git-branch')
					};
				}
			} else {
	...
	async provideChatSessionContentForExistingSession(resource: Uri, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSession> {
		const copilotcliSessionId = SessionIdForCLI.parse(resource);
		this._currentSessionId = copilotcliSessionId;
		...
		if (folderInfo.repository) {
			...
			options[REPOSITORY_OPTION_ID] = {
				id: '',
				name: folderName,
				icon: new zyraxoncode.ThemeIcon('folder'),
				locked: true
			};
		}
		...
		if (worktreeProperties?.repositoryPath) {
			...
			const repoUri = zyraxoncode.Uri.file(worktreeProperties.repositoryPath);
		...
		if (isIsolationOptionFeatureEnabled(this.configurationService)) {
			...
			options[ISOLATION_OPTION_ID] = {
				...
				icon: new zyraxoncode.ThemeIcon(isWorktree ? 'worktree' : 'folder'),
				locked: true
			};
		}
	...
	private async forkSession(sessionId: string, requestId: string | undefined, token: CancellationToken): Promise<zyraxoncode.ChatSessionItem> {
	...
	private async getSessionHistory(sessionId: string, workspaceInfo: IWorkspaceInfo, token: zyraxoncode.CancellationToken) {
		try {
	...
	async provideChatSessionProviderOptions(): Promise<zyraxoncode.ChatSessionProviderOptions> {
		...
		if (this._selectedRepoForBranches && isIsolationOptionFeatureEnabled(this.configurationService)) {
			optionGroups.push({
				...
				items: [
					{ id: IsolationMode.Workspace, name: l10n.t('Workspace'), icon: new zyraxoncode.ThemeIcon('folder') },
					{ id: IsolationMode.Worktree, name: l10n.t('Worktree'), icon: new zyraxoncode.ThemeIcon('worktree') },
				]
			});
		}
		...
		if (this.isUntitledWorkspace()) {
			...
			const commands: zyraxoncode.Command[] = [];
			commands.push({
	...
	private _branchRepositoryOptions?: { repoUri: Uri; items: Promise<zyraxoncode.ChatSessionProviderOptionItem[]> };
	private async getBranchOptionItems(overrideListBranches = false): Promise<zyraxoncode.ChatSessionProviderOptionItem[]> {
	...
	private async getBranchOptionItemsForRepository(repoUri: Uri, headBranchName: string | undefined): Promise<zyraxoncode.ChatSessionProviderOptionItem[]> {
		...
		return this._getBranchOptionItemsForRepositorySequencer.queue(key, async () => {
			...
			let headItem: zyraxoncode.ChatSessionProviderOptionItem | undefined;

			for (const ref of localBranches) {
				const isHead = ref.name === headBranchName;
				const item: zyraxoncode.ChatSessionProviderOptionItem = {
					id: ref.name!,
					name: ref.name!,
					icon: new zyraxoncode.ThemeIcon('git-branch'),
					// default: isHead
	...
	async provideHandleOptionsChange(resource: Uri, updates: ReadonlyArray<zyraxoncode.ChatSessionOptionUpdate>, token: zyraxoncode.CancellationToken): Promise<void> {
		...
		for (const update of updates) {
			if (update.optionId === PERMISSION_LEVEL_OPTION_ID) {
				...
				const folder = zyraxoncode.Uri.file(update.value);
				...
				if ((await checkPathExists(folder, this.fileSystem))) {
					...
					if (this._selectedRepoForBranches && updates.length === 1) {
						const sessionChanges: { optionId: string; value: string | zyraxoncode.ChatSessionProviderOptionItem }[] = [];
					...
					zyraxoncode.window.showErrorMessage(l10n.t('Path does not exist'), { modal: true, detail: message });
...
function toRepositoryOptionItem(repository: RepoContext | Uri, isDefault: boolean = false): ChatSessionProviderOptionItem {
	...
	return {
		...
		name: repositoryName,
		icon: new zyraxoncode.ThemeIcon(repositoryIcon),
		default: isDefault
...
export class CopilotCLIChatSessionParticipant extends Disposable {
	 ...
	 */
	private readonly pendingRequestsForUntitledSessions = new Map<string, Map<string, Promise<zyraxoncode.ChatResult | void>>>();
	 ...
	 */
	private readonly pendingRequestBySession = new Map<string, Set<zyraxoncode.ChatRequest>>();
	 ...
	 * 2. ZYRAXON Code signals this by setting `context.yieldRequested = true` on the
	 ...
	 */
	private async handleRequest(request: zyraxoncode.ChatRequest, context: zyraxoncode.ChatContext, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatResult | void> {
		...
		try {
			...
			if (context.chatSessionContext) {
				...
				if (isUntitled) {
					const promises = this.pendingRequestsForUntitledSessions.get(id) ?? new Map<string, Promise<zyraxoncode.ChatResult | void>>();
	...
	private sendTelemetryForHandleRequest(request: zyraxoncode.ChatRequest, chatSessionContext: zyraxoncode.ChatSessionContext | undefined): void {
	...
	private async handleRequestImpl(request: zyraxoncode.ChatRequest, context: zyraxoncode.ChatContext, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatResult | void> {
		...
		try {
				...
				if (initialOptions.length > 0) {
					...
					for (const opt of initialOptions) {
						...
						if (opt.optionId === REPOSITORY_OPTION_ID && value && this.sessionItemProvider.isNewSession(sessionId)) {
							this.contentProvider.trackLastUsedFolderInWelcomeView(zyraxoncode.Uri.file(value));
							this.folderRepositoryManager.setNewSessionFolder(sessionId, zyraxoncode.Uri.file(value));
			...
			if (!chatSessionContext && SessionIdForCLI.isCLIResource(request.sessionResource)) {
				...
				if (this.contextForRequest.has(id)) {
					...
					} satisfies zyraxoncode.ChatContext;
				}
			}
			...
			if (invalidSessionMessage) {
				...
				const warningMessage = new zyraxoncode.MarkdownString();
			...
			const fakeContext: zyraxoncode.ChatContext = {
				history: [requestTurn],
				yieldRequested: false,
			};
			...
			this.chatSessionMetadataStore.updateRequestDetails(sessionId, [{ zyraxoncodeRequestId: request.id, agentId: agent?.name ?? '', modeInstructions }]).catch(ex => this.logService.error(ex, 'Failed to update request details'));
			...
			const requestsForSession = this.pendingRequestBySession.get(session.object.sessionId) ?? new Set<zyraxoncode.ChatRequest>();
	...
	private scheduleUntitledSessionSwap(untitledSessionId: string, requestId: string, requestPrompt: string, sdkSessionId: string, chatSessionItem: zyraxoncode.ChatSessionItem): void {
	...
	private async lockRepoOptionForSession(context: zyraxoncode.ChatContext, token: zyraxoncode.CancellationToken) {
		const { chatSessionContext } = context;
		...
		if (folderInfo.folder) {
			...
			const changes: { optionId: string; value: string | zyraxoncode.ChatSessionProviderOptionItem }[] = [
				...
				changes.push({
					...
					value: {
						...
						name: selectedIsolation === IsolationMode.Worktree
							? l10n.t('Worktree')
							: l10n.t('Workspace'),
						icon: new zyraxoncode.ThemeIcon(selectedIsolation === IsolationMode.Worktree ? 'worktree' : 'folder'),
						locked: true
					}
				});
			}
	...
	private async unlockRepoOptionForSession(context: zyraxoncode.ChatContext, token: zyraxoncode.CancellationToken) {
		const { chatSessionContext } = context;
		if (!chatSessionContext?.isUntitled) {
			return;
		}
	...
	private async commitWorktreeChangesIfNeeded(request: zyraxoncode.ChatRequest, session: ICopilotCLISession, token: zyraxoncode.CancellationToken): Promise<void> {
		...
		try {
			if (session.status === zyraxoncode.ChatSessionStatus.Completed) {
	 ...
	 */
	private async getAgent(sessionId: string | undefined, request: zyraxoncode.ChatRequest | undefined, token: zyraxoncode.CancellationToken): Promise<SweCustomAgent | undefined> {
	...
	private async getPromptInfoFromRequest(request: zyraxoncode.ChatRequest, token: zyraxoncode.CancellationToken): Promise<ParsedPromptFile | undefined> {
	...
	private async getOrCreateSession(request: zyraxoncode.ChatRequest, chatSessionContext: zyraxoncode.ChatSessionContext, stream: zyraxoncode.ChatResponseStream, options: { model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; agent: SweCustomAgent | undefined; newBranch?: Promise<string | undefined>; sessionParentId?: string; permissionLevel?: string }, disposables: DisposableStore, token: zyraxoncode.CancellationToken): Promise<{ session: IReference<ICopilotCLISession> | undefined; trusted: boolean }> {
		const { resource } = chatSessionContext.chatSessionItem;
		const parsedId = SessionIdForCLI.parse(resource);
		const existingSessionId = this.sessionItemProvider.untitledSessionIdMapping.get(parsedId);
		const id = existingSessionId ?? parsedId;
	...
	private async getModelId(request: zyraxoncode.ChatRequest | undefined, token: zyraxoncode.CancellationToken): Promise<{ model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined> {
	...
	private async handleDelegationToCloud(session: ICopilotCLISession, request: zyraxoncode.ChatRequest, context: zyraxoncode.ChatContext, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken) {
	...
	private async getOrInitializeWorkingDirectory(
		chatSessionContext: zyraxoncode.ChatSessionContext | undefined,
		stream: zyraxoncode.ChatResponseStream,
		toolInvocationToken: zyraxoncode.ChatParticipantToolToken,
		token: zyraxoncode.CancellationToken,
	...
	private createModeInstructions(request: zyraxoncode.ChatRequest): StoredModeInstructions | undefined {
	...
	private async handleDelegationFromAnotherChat(
		...
		otherReferences: readonly zyraxoncode.ChatPromptReference[] | undefined,
		...
		const modeInstructions = this.createModeInstructions(request);
		this.chatSessionMetadataStore.updateRequestDetails(session.object.sessionId, [{ zyraxoncodeRequestId: request.id, agentId: agent?.name ?? '', modeInstructions }]).catch(ex => this.logService.error(ex, 'Failed to update request details'));
		if (summary) {
		...
		void zyraxoncode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.copilotcli', {
	...
	private async recordPushToSession(
		...
		userPrompt: string,
		prInfo: zyraxoncode.ChatResponsePullRequestPart
	): Promise<void> {
...
export function registerCLIChatCommands(
	...
	async function shouldKeepWorktreeForOtherSessions(sessionId: string, worktreePath: zyraxoncode.Uri): Promise<boolean> {
	...
	async function deleteSessionById(sessionId: string, options?: { keepWorktree?: boolean; sessionLabel?: string }): Promise<void> {
		...
		if (worktreePath && !options?.keepWorktree) {
			...
			if (worktreeExists) {
				try {
					const repository = worktree ? await gitService.getRepository(zyraxoncode.Uri.file(worktree.repositoryPath), true) : undefined;
					if (!repository) {
					...
					zyraxoncode.window.showErrorMessage(l10n.t('Failed to delete worktree: {0}', error instanceof Error ? error.message : String(error)));
				}
			}
		}
	}
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.delete', async (sessionItem?: zyraxoncode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			...
			const result = await zyraxoncode.window.showWarningMessage(
				confirmMessage,
				{ modal: true },
				deleteLabel
			);
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('agents.github.copilot.cli.deleteSessions', async (sessionItems?: zyraxoncode.ChatSessionItem[], options?: { skipConfirmation?: boolean }) => {
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.resumeInTerminal', async (sessionItem?: zyraxoncode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			await copilotcliSessionItemProvider.resumeCopilotCLISessionInTerminal(sessionItem);
		}
	}));
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.rename', async (sessionItem?: zyraxoncode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		const newTitle = await zyraxoncode.window.showInputBox({
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.setTitle', async (sessionItem?: zyraxoncode.ChatSessionItem, title?: string) => {
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.newSession', async () => {
		await copilotcliSessionItemProvider.createCopilotCLITerminal('editor', l10n.t('Copilot CLI'));
	}));
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.newSessionToSide', async () => {
		await copilotcliSessionItemProvider.createCopilotCLITerminal('editorBeside', l10n.t('Copilot CLI'));
	}));
	disposableStore.add(zyraxoncode.commands.registerCommand(OPEN_IN_COPILOT_CLI_COMMAND_ID, async (sourceControlContext?: unknown) => {
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.openWorktreeInNewWindow', async (sessionItem?: zyraxoncode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		if (folder) {
			await zyraxoncode.commands.executeCommand('zyraxoncode.openFolder', folder, { forceNewWindow: true });
		}
	}));
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.openWorktreeInTerminal', async (sessionItem?: zyraxoncode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		const folder = folderInfo.worktree ?? folderInfo.repository ?? folderInfo.folder;
		if (folder) {
			zyraxoncode.window.createTerminal({ cwd: folder }).show();
		}
	}));
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.copyWorktreeBranchName', async (sessionItem?: zyraxoncode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		if (worktreeProperties?.branchName) {
			await zyraxoncode.env.clipboard.writeText(worktreeProperties.branchName);
		}
	}));
	async function selectFolder() {
		// Open folder picker dialog
		const folderUris = await zyraxoncode.window.showOpenDialog({
	...
	function getSourceControlRootUri(sourceControlContext?: unknown): zyraxoncode.Uri | undefined {
	...
	disposableStore.add(zyraxoncode.commands.registerCommand(OPEN_REPOSITORY_COMMAND_ID, async (sessionItemResource?: zyraxoncode.Uri) => {
		...
		if (mruItems.length === 0) {
			...
			type RecentFolderQuickPickItem = zyraxoncode.QuickPickItem & ({ folderUri: zyraxoncode.Uri; openFolder: false } | { folderUri: undefined; openFolder: true });
			...
			items.unshift({
				...
				iconPath: new zyraxoncode.ThemeIcon('folder-opened'),
				folderUri: undefined,
				openFolder: true
			}, {
			...
			const disposables = new DisposableStore();
			const quickPick = disposables.add(zyraxoncode.window.createQuickPick<RecentFolderQuickPickItem>());
			...
			disposables.add(quickPick.onDidAccept(async () => {
				if (quickPick.selectedItems.length === 0 && !quickPick.value) {
					...
					const fileOrFolder = zyraxoncode.Uri.file(quickPick.value);
					try {
						const stat = await zyraxoncode.workspace.fs.stat(fileOrFolder);
						let directory: Uri | undefined = undefined;
						if (stat.type & zyraxoncode.FileType.Directory) {
							quickPick.hide();
							directory = fileOrFolder;
						} else if (stat.type & zyraxoncode.FileType.File) {
							directory = dirname(fileOrFolder);
						}
						if (directory) {
							// Possible user selected a folder thats inside an existing workspace folder.
							selectedFolder.complete(zyraxoncode.workspace.getWorkspaceFolder(directory)?.uri || directory);
							quickPick.hide();
						}
					} catch {
						// ignore
		...
		// Notify ZYRAXON Code that the option changed
	...
	const applyChanges = async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		...
		try {
			...
			if (worktreePath) {
				...
				const multiDiffTabToClose = zyraxoncode.window.tabGroups.all.flatMap(g => g.tabs)
					.filter(({ input }) => input instanceof zyraxoncode.TabInputTextMultiDiff && input.textDiffs.some(input =>
						extUri.isEqualOrParent(zyraxoncode.Uri.file(input.original.fsPath), worktreePath, true) ||
						extUri.isEqualOrParent(zyraxoncode.Uri.file(input.modified.fsPath), worktreePath, true)));

				if (multiDiffTabToClose.length > 0) {
					// Close the tabs
					await zyraxoncode.window.tabGroups.close(multiDiffTabToClose, true);
				}
			}
			...
			zyraxoncode.window.showErrorMessage(l10n.t('Failed to apply changes to the current workspace. Please stage or commit your changes in the current workspace and try again.'), { modal: true });
		}
	};

	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.chat.applyCopilotCLIAgentSessionChanges', applyChanges));
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.chat.applyCopilotCLIAgentSessionChanges.apply', applyChanges));

	const mergeChanges = async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri, syncWithRemote: boolean = false) => {
		const resource = sessionItemOrResource instanceof zyraxoncode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			...
			const worktreeProperties = await copilotCLIWorktreeManagerService.getWorktreeProperties(sessionId);
			if (!worktreeProperties || worktreeProperties.version !== 2) {
				zyraxoncode.window.showErrorMessage(l10n.t('Merging changes is only supported for worktree-based sessions.'));
				return;
			}
		...
		await zyraxoncode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.copilotcli', {
			resource,
			prompt,
			attachedContext: [{
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.chat.mergeCopilotCLIAgentSessionChanges.merge', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		await mergeChanges(sessionItemOrResource);
	}));

	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.chat.mergeCopilotCLIAgentSessionChanges.mergeAndSync', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		await mergeChanges(sessionItemOrResource, true);
	}));

	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.sessions.refreshChanges', async (resource?: zyraxoncode.Uri) => {
		if (!resource) {
			return;
		}
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.sessions.initializeRepository', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		const resource = sessionItemOrResource instanceof zyraxoncode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	const setHasGitOperationInProgress = async (sessionId: string, inProgress: boolean, commandId = '') => {
		if (inProgress) {
			...
			await zyraxoncode.commands.executeCommand('setContext', 'sessions.hasGitOperationInProgress', inProgress);
			await zyraxoncode.commands.executeCommand('setContext', 'sessions.gitOperationInProgress', `${sessionId};${commandId}`);
		}
		...
		if (worktreeProperties) {
			...
			if (!inProgress) {
				...
				await zyraxoncode.commands.executeCommand('setContext', 'sessions.hasGitOperationInProgress', inProgress);
				await zyraxoncode.commands.executeCommand('setContext', 'sessions.gitOperationInProgress', `${sessionId};`);
			}
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.sessions.commit', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		const resource = sessionItemOrResource instanceof zyraxoncode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.sessions.commitAndSync', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		const resource = sessionItemOrResource instanceof zyraxoncode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.sessions.sync', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		const resource = sessionItemOrResource instanceof zyraxoncode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
	...
	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.sessions.discardChanges', async (sessionResource: zyraxoncode.Uri, ref: string, ...resources: zyraxoncode.Uri[]) => {
		...
		const choice = await zyraxoncode.window.showWarningMessage(message, { modal: true }, confirmAction);
	...
	const createPullRequest = async (sessionItemOrResource: zyraxoncode.ChatSessionItem | zyraxoncode.Uri | undefined, isDraft: boolean) => {
		const resource = sessionItemOrResource instanceof zyraxoncode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		if (!worktreeProperties || worktreeProperties.version !== 2) {
			zyraxoncode.window.showErrorMessage(l10n.t('Creating a pull request is only supported for worktree-based sessions.'));
			return;
		}
		...
		try {
			...
			const worktreeUri = zyraxoncode.Uri.file(worktreeProperties.worktreePath);
			...
			zyraxoncode.window.showErrorMessage(l10n.t('Failed to create pull request: {0}', errorMessage));
		} finally {
			await setHasGitOperationInProgress(sessionId, false);
		}
	};

	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.chat.createPullRequestCopilotCLIAgentSession.createPR', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		await createPullRequest(sessionItemOrResource, false);
	}));

	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.chat.createDraftPullRequestCopilotCLIAgentSession.createDraftPR', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
		await createPullRequest(sessionItemOrResource, true);
	}));

	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.commitToWorktree', async (args?: { worktreeUri?: zyraxoncode.Uri; fileUri?: zyraxoncode.Uri }) => {
		...
		const worktreeUri = zyraxoncode.Uri.from(args.worktreeUri);
		const fileUri = zyraxoncode.Uri.from(args.fileUri);
		try {
			...
			zyraxoncode.window.showErrorMessage(l10n.t('Failed to commit: {0}', error instanceof Error ? error.message : String(error)));
		}
	}));

	disposableStore.add(zyraxoncode.commands.registerCommand('github.copilot.cli.sessions.commitToRepository', async (args?: { repositoryUri?: zyraxoncode.Uri; fileUri?: zyraxoncode.Uri }) => {
		...
		const repositoryUri = zyraxoncode.Uri.from(args.repositoryUri);
		const fileUri = zyraxoncode.Uri.from(args.fileUri);
		try {
			const fileName = basename(fileUri);
 ...
 */
async function checkPathExists(filePath: zyraxoncode.Uri, fileSystemService: IFileSystemService): Promise<boolean> {
	try {
