...
export class ChatSessionWorkspaceFolderService extends Disposable implements IChatSessionWorkspaceFolderService {
	...
	private readonly _onDidChangeWorkspaceFolderChanges = this._register(new zyraxoncode.EventEmitter<{ sessionId: string }>());
	...
	async deleteTrackedWorkspaceFolder(sessionId: string): Promise<void> {
		...
		if (entry?.folderPath) {
			const folderUri = zyraxoncode.Uri.file(entry.folderPath);
	...
	async trackSessionWorkspaceFolder(sessionId: string, workspaceFolderUri: string, repositoryProperties?: RepositoryProperties): Promise<void> {
		...
		const folderUri = zyraxoncode.Uri.file(workspaceFolderUri);
	...
	async getSessionWorkspaceFolder(sessionId: string): Promise<zyraxoncode.Uri | undefined> {
		const entry = this.workspaceState.get(sessionId);
		if (entry?.folderPath) {
			return zyraxoncode.Uri.file(entry.folderPath);
		}
	...
	private async computeWorkspaceChanges(repositoryProperties: RepositoryProperties, sessionId: string): Promise<{
		...
		const repository = await this.gitService.getRepository(zyraxoncode.Uri.file(repositoryProperties.repositoryPath));
		if (repository) {
	...
	clearWorkspaceChanges(sessionIdOrFolderUri: string | zyraxoncode.Uri): string[] {
