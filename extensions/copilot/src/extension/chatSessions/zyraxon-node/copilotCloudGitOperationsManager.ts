...
import { getRepoId } from '../zyraxoncode/copilotCodingAgentUtils';
...
export class CopilotCloudGitOperationsManager {
	...
	async repoInfo(): Promise<GitRepoInfo> {
		...
		if (!repoIds || repoIds.length === 0) {
			throw new Error(zyraxoncode.l10n.t('Repository information is not available. Open a GitHub repository to continue with cloud agent.'));
		}
		const repoId = repoIds[0];
		const currentRepository = this.gitService.activeRepository.get();
		if (!currentRepository) {
			throw new Error(zyraxoncode.l10n.t('No active repository found. Open a GitHub repository to continue with cloud agent.'));
		}
	...
	async pushBaseRefToRemote(): Promise<string> {
		try {
			...
			throw new Error(zyraxoncode.l10n.t('Failed to push base branch to remote. Please push the branch manually and try again.'));
		}
	}
	...
	async commitAndPushChanges(): Promise<string> {
		...
		const commitMessage = zyraxoncode.l10n.t('Checkpoint from ZYRAXON Code for cloud agent session');
		try {
			...
			throw new Error(zyraxoncode.l10n.t('Failed to automatically commit and push your changes. Please commit or stash your changes manually and try again.'));
		}
	}
	...
	private async performCommit(asyncBranch: string, repository: Repository, commitMessage: string): Promise<void> {
		try {
			...
			if (repository.state.HEAD?.name !== asyncBranch || repository.state.workingTreeChanges.length > 0 || repository.state.indexChanges.length > 0) {
				throw new Error(zyraxoncode.l10n.t('Uncommitted changes still detected.'));
			}
		} catch (error) {
			...
			if (!commitSuccessful) {
				throw new Error(zyraxoncode.l10n.t('Failed to commit changes. Please commit or stash your changes manually before using the cloud agent.'));
			}
		}
	}

	private async handleInteractiveCommit(repository: Repository): Promise<boolean> {
		const COMMIT_YOUR_CHANGES = zyraxoncode.l10n.t('Commit your changes to continue cloud agent session. Close integrated terminal to cancel.');
		return zyraxoncode.window.withProgress({
			...
			return new Promise<boolean>((resolve) => {
				...
				const terminal = zyraxoncode.window.createTerminal({
				...
				let stateListener: zyraxoncode.Disposable | undefined = undefined;
				let disposalListener: zyraxoncode.Disposable | undefined = undefined;
				let cancellationListener: zyraxoncode.Disposable | undefined = undefined;
				const cleanup = () => {
					if (disposed) {
						return;
					}
				...
				disposalListener = zyraxoncode.window.onDidCloseTerminal((closedTerminal) => {
	...
	private async generateRandomBranchName(repository: Repository, prefix: string): Promise<string> {
		for (let index = 0; index < 5; index++) {
			const randomName = `${prefix}/zyraxoncode-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
			try {
		...
		return `${prefix}/zyraxoncode-${Date.now().toString(36)}`;
	}
}
