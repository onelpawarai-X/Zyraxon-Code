...
/**
 *
 * We keep parsing in two phases to mirror ZYRAXON Code's shape:
 ...
 * workspace root. ZYRAXON Code's built-in detector cannot resolve that context.
 */
export class CopilotCLITerminalLinkProvider implements TerminalLinkProvider<CopilotCLITerminalLink> {
	...
	async provideTerminalLinks(context: TerminalLinkContext, token: CancellationToken): Promise<CopilotCLITerminalLink[]> {
		...
		// Match ZYRAXON Code's built-in MaxLineLength limit (terminalLocalLinkDetector.ts).
		...
		for (const candidate of this._detectLinkCandidates(line)) {
			// Match ZYRAXON Code's built-in MaxResolvedLinksInLine (terminalLocalLinkDetector.ts).
			...
			// Mirrors ZYRAXON Code's specialEndCharRegex (terminalLocalLinkDetector.ts).
			let trimmed = 0;
	 ...
	 * files may still be on disk). See __ZYRAXKEEP__0_
	 */
