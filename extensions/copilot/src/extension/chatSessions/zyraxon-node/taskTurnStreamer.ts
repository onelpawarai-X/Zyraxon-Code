...
import { TaskCloudAgentBackend } from '../zyraxoncode/cloudAgentBackend';
import { isActiveTaskState, isFailedTaskState } from '../zyraxoncode/copilotCodingAgentUtils';
...
/**
 * Streams the currently active (or upcoming) Task turn into a `zyraxoncode.ChatResponseStream`.
 *
...
export class TaskTurnStreamer {
	...
	private _renderToolRequest(req: ToolRequest): zyraxoncode.ChatToolInvocationPart | zyraxoncode.ChatResponseThinkingProgressPart | undefined {
...
class StreamSink<TToolCall> {
	...
	flush(): void {
		...
		for (let i = this._emittedPartsCount; i < parts.length; i++) {
			try {
				this._stream.push(parts[i] as unknown as zyraxoncode.ChatResponsePart);
			} catch (e) {
...
function delay(ms: number, token: zyraxoncode.CancellationToken): Promise<void> {
