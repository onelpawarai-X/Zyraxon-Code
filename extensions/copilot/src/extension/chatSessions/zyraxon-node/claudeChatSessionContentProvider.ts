...
import { IClaudeSlashCommandService } from '../claude/zyraxoncode-node/claudeSlashCommandService';
...
import '../claude/zyraxoncode-node/toolPermissionHandlers/index';
...
function getSessionResource(sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri): zyraxoncode.Uri | undefined {
	return sessionItemOrResource instanceof zyraxoncode.Uri
		? sessionItemOrResource
		: sessionItemOrResource?.resource;
}

// Import the MCP server contributors to trigger self-registration
import '../claude/zyraxoncode-node/mcpServers/index';
...
interface InputStateReactivePipeline {
	...
	readonly folderItems: ISettableObservable<readonly zyraxoncode.ChatSessionProviderOptionItem[]>;
	readonly isSessionStarted: ISettableObservable<boolean>;
	readonly store: DisposableStore;
}

function getSelectedFolderUri(inputState: zyraxoncode.ChatSessionInputState | undefined): URI | undefined {
...
export class ClaudeChatSessionContentProvider extends Disposable implements zyraxoncode.ChatSessionContentProvider {
	...
	provideHandleOptionsChange(resource: zyraxoncode.Uri, updates: ReadonlyArray<zyraxoncode.ChatSessionOptionUpdate>, _token: zyraxoncode.CancellationToken): void {
	...
	createHandler(): ChatExtendedRequestHandler {
		return async (request: zyraxoncode.ChatRequest, context: zyraxoncode.ChatContext, stream: zyraxoncode.ChatResponseStream, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatResult | void> => {
			const { chatSessionContext } = context;
			if (!chatSessionContext) {
				/* Via @claude */
				// TODO: Think about how this should work
				stream.markdown(zyraxoncode.l10n.t("Start a new Claude Agent session"));
				stream.button({ command: `workbench.action.chat.openNewSessionEditor.${ClaudeSessionUri.scheme}`, title: zyraxoncode.l10n.t("Start Session") });
				return {};
			}
			...
			if (!this._sdkLoader.isAvailable) {
				stream.markdown(zyraxoncode.l10n.t("Installing the Claude Agent SDK..."));
				...
				if (!installed) {
					...
					stream.button({
						...
						title: zyraxoncode.l10n.t("Install Claude Agent SDK"),
						arguments: [CLAUDE_SDK_EXTENSION_ID],
					});
					return {
						errorDetails: {
							message: zyraxoncode.l10n.t("Failed to install {0}", CLAUDE_SDK_EXTENSION_ID)
						}
					};
				}
				stream.markdown(zyraxoncode.l10n.t("Done") + '\n\n');
			}
			...
			if (untrustedDirectory) {
				return {
					errorDetails: {
						message: zyraxoncode.l10n.t("Workspace '{0}' is not trusted", untrustedDirectory.fsPath)
					}
				};
			}
			...
			let result: zyraxoncode.ChatResult;
			let creditsUsed: number | undefined;
			try {
				const prompt = request.prompt;
				await this._controller.updateItemStatus(effectiveSessionId, zyraxoncode.ChatSessionStatus.InProgress, prompt);
				result = await this.claudeAgentManager.handleRequest(effectiveSessionId, request, stream, token, isNewSession, yieldRequested);
				await this._controller.updateItemStatus(effectiveSessionId, zyraxoncode.ChatSessionStatus.Completed, prompt);
			} finally {
	...
	async provideChatSessionContent(sessionResource: zyraxoncode.Uri, token: zyraxoncode.CancellationToken, context?: { readonly inputState: zyraxoncode.ChatSessionInputState }): Promise<zyraxoncode.ChatSession> {
...
export class ClaudeChatSessionItemController extends Disposable {
	private readonly _controller: zyraxoncode.ChatSessionItemController;
	private readonly _optionBuilder: ClaudeSessionOptionBuilder;
	private readonly _inProgressItems = new Map<string, zyraxoncode.ChatSessionItem>();
	...
	constructor(
		...
		this._controller = this._register(zyraxoncode.chat.createChatSessionItemController(
		...
		this._controller.newChatSessionItemHandler = async (context, _token) => {
			...
			item.iconPath = new zyraxoncode.ThemeIcon('claude');
		...
		this._controller.forkHandler = async (sessionResource: zyraxoncode.Uri, request: zyraxoncode.ChatRequestTurn2 | undefined, token: CancellationToken): Promise<zyraxoncode.ChatSessionItem> => {
			const item = this._controller.items.get(sessionResource);
			const title = zyraxoncode.l10n.t('Forked: {0}', item?.label ?? request?.prompt ?? 'Claude Session');
			...
			newItem.iconPath = new zyraxoncode.ThemeIcon('claude');
	...
	private _createInputStateReactivePipeline(
		state: zyraxoncode.ChatSessionInputState,
		...
		const folderItems = observableValue<readonly zyraxoncode.ChatSessionProviderOptionItem[]>(this, seed.folderItems);
		...
		const folderGroup = derived<zyraxoncode.ChatSessionProviderOptionGroup | undefined>(reader => {
			...
			return {
				id: FOLDER_OPTION_ID,
				name: zyraxoncode.l10n.t('Folder'),
				description: zyraxoncode.l10n.t('Pick Folder'),
				items: lockedItems,
		...
		const allGroups = derived(reader => {
			...
			const groups: zyraxoncode.ChatSessionProviderOptionGroup[] = [];
	...
	private _setupInputState(): void {
		this._controller.getChatSessionInputState = async (sessionResource, context, token) => {
			let state: zyraxoncode.ChatSessionInputState;
	...
	private _computeSeedValues(groups: readonly zyraxoncode.ChatSessionProviderOptionGroup[]): {
		...
		readonly folderItems: readonly zyraxoncode.ChatSessionProviderOptionItem[];
		...
		let folderItems: readonly zyraxoncode.ChatSessionProviderOptionItem[] = [];
	...
	private async _buildExistingSessionGroups(sessionResource: zyraxoncode.Uri): Promise<zyraxoncode.ChatSessionProviderOptionGroup[]> {
		const sessionId = ClaudeSessionUri.getSessionId(sessionResource);
	...
	async updateItemStatus(sessionId: string, status: zyraxoncode.ChatSessionStatus, newItemLabel: string): Promise<void> {
		const resource = ClaudeSessionUri.forSessionId(sessionId);
		...
		item.status = status;
		if (status === zyraxoncode.ChatSessionStatus.InProgress) {
			...
			if (status === zyraxoncode.ChatSessionStatus.Completed) {
				if (!item.timing) {
					item.timing = {
						created: Date.now(),
						lastRequestEnded: Date.now()
	...
	private async _refreshItems(token: zyraxoncode.CancellationToken): Promise<void> {
		...
		const items: zyraxoncode.ChatSessionItem[] = [];
	...
	private async _createClaudeChatSessionItem(session: IClaudeCodeSessionInfo): Promise<zyraxoncode.ChatSessionItem> {
		let badge: zyraxoncode.MarkdownString | undefined;
		if (session.folderName && this._showBadge) {
			badge = new zyraxoncode.MarkdownString(`$(folder) ${session.folderName}`);
	...
	private _registerPromptCommand(commandId: string, prompt: string): void {
		this._register(zyraxoncode.commands.registerCommand(commandId, async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
			const resource = getSessionResource(sessionItemOrResource);
			if (!resource) {
				return;
			}
			await zyraxoncode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.claude-code', {
				resource,
				prompt,
			});
		}));
	...
	private _registerCommands(): void {
		this._register(zyraxoncode.commands.registerCommand('github.copilot.claude.sessions.rename', async (sessionItem?: zyraxoncode.ChatSessionItem) => {
			...
			const newTitle = await zyraxoncode.window.showInputBox({
				prompt: zyraxoncode.l10n.t('New agent session title'),
				value: sessionItem.label,
				validateInput: value => {
					if (!value.trim()) {
						return zyraxoncode.l10n.t('Title cannot be empty');
					}
					return undefined;
				}
			});
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.claude.sessions.initializeRepository', async (sessionItemOrResource?: zyraxoncode.ChatSessionItem | zyraxoncode.Uri) => {
			const resource = getSessionResource(sessionItemOrResource);
			if (!resource) {
				return;
			}
