...
import { prExtensionInstalledContextKey } from '../../contextKeys/zyraxoncode-node/contextKeys.contribution';
...
import { RoutingClaudeAgentSdkLoaderService } from '../claude/zyraxoncode-node/routingClaudeAgentSdkLoaderService';
...
import { ClaudeSlashCommandService, IClaudeSlashCommandService } from '../claude/zyraxoncode-node/claudeSlashCommandService';
...
import { CopilotCLIContrib, getServices } from '../copilotcli/zyraxoncode-node/contribution';
import { CopilotCLIFolderMruService } from '../copilotcli/zyraxoncode-node/copilotCLIFolderMru';
import { ICopilotCLISessionTracker } from '../copilotcli/zyraxoncode-node/copilotCLISessionTracker';
import { CustomSessionTitleService } from '../copilotcli/zyraxoncode-node/customSessionTitleServiceImpl';
import { GHPR_EXTENSION_ID } from '../zyraxoncode/chatSessionsUriHandler';
import { AgentSessionsWorkspace } from './agentSessionsWorkspace';
import { UserQuestionHandler } from '../copilotcli/zyraxoncode-node/askUserQuestionHandler';
import { ChatSessionMetadataStore } from '../copilotcli/zyraxoncode-node/chatSessionMetadataStoreImpl';
...
import { CopilotCLIChatSessionInitializer, ICopilotCLIChatSessionInitializer } from '../copilotcli/zyraxoncode-node/copilotCLIChatSessionInitializer';
...
import { CopilotCLICustomizationProvider } from '../copilotcli/zyraxoncode-node/copilotCLICustomizationProvider';
...
export class ChatSessionsContrib extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(zyraxoncode.workspace.registerTextDocumentContentProvider(delegationSummary.scheme, {
			provideTextDocumentContent: (uri: zyraxoncode.Uri): string | undefined => delegationSummary.provideTextDocumentContent(uri)
		}));
		...
		const configKey = zyraxoncode.workspace.isAgentSessionsWorkspace
		...
		claudeModels.registerLanguageModelChatProvider(zyraxoncode.lm);
		...
		const chatParticipant = zyraxoncode.chat.createChatParticipant(ClaudeSessionUri.scheme, chatSessionContentProvider.createHandler());
		chatParticipant.iconPath = new zyraxoncode.ThemeIcon('claude');
		this._register(zyraxoncode.chat.registerChatSessionContentProvider(ClaudeSessionUri.scheme, chatSessionContentProvider, chatParticipant));
		const claudeCustomizationProvider = this._register(claudeAgentInstaService.createInstance(ClaudeCustomizationProvider));
		this._register(zyraxoncode.chat.registerChatSessionCustomizationProvider(ClaudeSessionUri.scheme, ClaudeCustomizationProvider.metadata, claudeCustomizationProvider));
	...
	private registerCopilotCLIServices(instantiationService: IInstantiationService, delegationSummary: IChatDelegationSummaryService, logService: ILogService) {
		...
		copilotModels.registerLanguageModelChatProvider(zyraxoncode.lm);

		const copilotcliParticipant = zyraxoncode.chat.createChatParticipant(this.copilotcliSessionType, copilotcliChatSessionParticipant.createHandler());
		this._register(zyraxoncode.chat.registerChatSessionContentProvider(this.copilotcliSessionType, copilotcliChatSessionContentProvider, copilotcliParticipant));
		...
		this._register(zyraxoncode.chat.registerChatSessionCustomizationProvider(this.copilotcliSessionType, CopilotCLICustomizationProvider.metadata, copilotcliCustomizationProvider));
	...
	private registerCopilotCLIServicesV1(instantiationService: IInstantiationService, delegationSummary: IChatDelegationSummaryService, logService: ILogService) {
		...
		const providerRegistration = zyraxoncode.chat.registerChatSessionItemProvider(this.copilotcliSessionType, copilotcliSessionItemProvider);
		...
		const onDidChangeChatSessionItemState = (providerRegistration as { onDidChangeChatSessionItemState?: zyraxoncode.Event<zyraxoncode.ChatSessionItem> }).onDidChangeChatSessionItemState;
	...
	private registerCopilotCloudAgent() {
		...
		this.copilotCloudRegistrations.add(
			zyraxoncode.chat.registerChatSessionItemProvider(CopilotCloudSessionsProvider.TYPE, cloudSessionsProvider)
		);
		this.copilotCloudRegistrations.add(
			zyraxoncode.chat.registerChatSessionContentProvider(
		...
		this.copilotCloudRegistrations.add(
			zyraxoncode.commands.registerCommand('github.copilot.cloud.resetWorkspaceConfirmations', () => {
				cloudSessionsProvider.resetWorkspaceContext();
			})
		);
		this.copilotCloudRegistrations.add(
			zyraxoncode.commands.registerCommand('github.copilot.cloud.sessions.openInBrowser', async (chatSessionItem: zyraxoncode.ChatSessionItem) => {
				cloudSessionsProvider.openSessionInBrowser(chatSessionItem);
			})
		);
		this.copilotCloudRegistrations.add(
			zyraxoncode.commands.registerCommand(CLOSE_SESSION_PR_CMD, async (ctx: CrossChatSessionWithPR) => {
				try {
		...
		this.copilotCloudRegistrations.add(
			zyraxoncode.commands.registerCommand('github.copilot.cloud.sessions.installPRExtension', async () => {
	...
	private isPullRequestExtensionInstalled(): boolean {
		return zyraxoncode.extensions.getExtension(GHPR_EXTENSION_ID) !== undefined;
	}
	...
	private async installPullRequestExtension(): Promise<void> {
		...
		try {
			...
			const installOptions = { enable: true, installPreReleaseVersion: isInsiders, justification: zyraxoncode.l10n.t('Enable additional pull request features, such as checking out and applying changes.') };
			await zyraxoncode.commands.executeCommand('workbench.extensions.installExtension', GHPR_EXTENSION_ID, installOptions);
			const maxWaitTime = 10_000; // 10 seconds
			const pollInterval = 100; // 100ms
			let elapsed = 0;
			while (elapsed < maxWaitTime) {
				if (this.isPullRequestExtensionInstalled()) {
					zyraxoncode.window.showInformationMessage(zyraxoncode.l10n.t('GitHub Pull Request extension installed successfully.'));
					break;
				}
			...
			if (!this.isPullRequestExtensionInstalled()) {
				zyraxoncode.window.showWarningMessage(zyraxoncode.l10n.t('GitHub Pull Request extension is taking longer than expected to install.'));
			}
			await zyraxoncode.commands.executeCommand('setContext', prExtensionInstalledContextKey, true);
		} catch (error) {
			zyraxoncode.window.showErrorMessage(zyraxoncode.l10n.t('Failed to install GitHub Pull Request extension: {0}', error instanceof Error ? error.message : String(error)));
		}
	}
}
