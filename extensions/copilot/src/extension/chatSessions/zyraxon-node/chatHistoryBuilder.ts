...
interface ToolContext {
	...
	pendingToolInvocations: Map<string, zyraxoncode.ChatToolInvocationPart>;
}
 ...
 */
function extractImageReferences(contents: readonly (string | ContentBlock[])[]): zyraxoncode.ChatPromptReference[] {
	const references: zyraxoncode.ChatPromptReference[] = [];
	let imageIndex = 0;
	for (const content of contents) {
		if (typeof content === 'string') {
			continue;
 ...
 */
function extractUserRequest(contents: readonly (string | ContentBlock[])[], messageId: string, modelId: string | undefined): zyraxoncode.ChatRequestTurn2 | undefined {
 ...
 */
function extractAssistantParts(messages: readonly AssistantMessageContent[], toolContext: ToolContext): (zyraxoncode.ChatResponseMarkdownPart | zyraxoncode.ChatResponseThinkingProgressPart | zyraxoncode.ChatToolInvocationPart)[] {
	const allParts: (zyraxoncode.ChatResponseMarkdownPart | zyraxoncode.ChatResponseThinkingProgressPart | zyraxoncode.ChatToolInvocationPart)[] = [];

	for (const message of messages) {
		const parts = coalesce(message.content.map(block => {
			if (isTextBlock(block)) {
				return new zyraxoncode.ChatResponseMarkdownPart(new zyraxoncode.MarkdownString(block.text));
			} else if (isThinkingBlock(block)) {
				return new zyraxoncode.ChatResponseThinkingProgressPart(block.thinking);
...
): zyraxoncode.ChatToolInvocationPart[] {
	const toolContext: ToolContext = {
		unprocessedToolCalls: new Map(),
		pendingToolInvocations: new Map()
	};
	const parts: zyraxoncode.ChatToolInvocationPart[] = [];
...
/**
 * Converts a Claude Code session into ZYRAXON Code chat history turns.
 *
 ...
 * appears as alternating assistant/user messages in the JSONL. ZYRAXON Code's chat
 ...
 */
export function buildChatHistory(session: IClaudeCodeSession, getModelDetails?: (modelId: string) => string | undefined): (zyraxoncode.ChatRequestTurn2 | zyraxoncode.ChatResponseTurn2)[] {
	const result: (zyraxoncode.ChatRequestTurn2 | zyraxoncode.ChatResponseTurn2)[] = [];
	const toolContext: ToolContext = {
		unprocessedToolCalls: new Map(),
		pendingToolInvocations: new Map()
	};
	...
	let pendingResponseParts: (zyraxoncode.ChatResponseMarkdownPart | zyraxoncode.ChatResponseThinkingProgressPart | zyraxoncode.ChatToolInvocationPart)[] = [];
	...
	const makeResponseResult = (modelId: string | undefined): zyraxoncode.ChatResult => {
		...
		if (currentType === 'user') {
			...
			if (commandInfo) {
				...
				if (commandInfo.stdout) {
					result.push(new zyraxoncode.ChatResponseTurn2(
						[new zyraxoncode.ChatResponseMarkdownPart(new zyraxoncode.MarkdownString(commandInfo.stdout))],
						{},
						''
					));
				}
			...
			// as a separate ChatResponseTurn2 because the ZYRAXON Code chat widget
			...
			if (msg.message.role === 'system') {
				...
				pendingResponseParts.push(
					new zyraxoncode.ChatResponseMarkdownPart(new zyraxoncode.MarkdownString(`\n\n---\n\n*${content}*`))
				);
			}
			i++;
		} else {
