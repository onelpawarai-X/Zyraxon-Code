...
import { Terminal, TerminalLocation, TerminalOptions, TerminalProfile, ThemeIcon, Uri, ViewColumn, window, workspace } from 'zyraxoncode';
...
ELECTRON_RUN_AS_NODE=1 "${process.execPath}" "${path.join(storageLocation, COPILOT_CLI_SHIM_JS)}" "$@"`;
	...
	private async getShellInfo(cliArgs: string[]): Promise<IShellInfo | undefined> {
		...
		// zyraxoncode.env.shell already resolves to the user's configured default terminal profile path.
