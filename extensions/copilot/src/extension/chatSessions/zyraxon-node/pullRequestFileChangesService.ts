...
export interface IPullRequestFileChangesService {
	...
	getFileChangesMultiDiffPart(pullRequest: PullRequestSearchItem): Promise<zyraxoncode.ChatResponseMultiDiffPart | undefined>;
	/**
	 ...
	 */
	getComparisonChangedFiles(params: { owner: string; repo: string; baseRef: string; headRef: string }): Promise<zyraxoncode.ChatSessionChangedFile[] | undefined>;
}
...
export class PullRequestFileChangesService implements IPullRequestFileChangesService {
	...
	async getFileChangesMultiDiffPart(pullRequest: PullRequestSearchItem): Promise<zyraxoncode.ChatResponseMultiDiffPart | undefined> {
		try {
			...
			const diffEntries: zyraxoncode.ChatResponseDiffEntry[] = files.map(file => {
			...
			return new zyraxoncode.ChatResponseMultiDiffPart(diffEntries, title, false);
		} catch (error) {
	 ...
	 */
	async getComparisonChangedFiles(params: { owner: string; repo: string; baseRef: string; headRef: string }): Promise<zyraxoncode.ChatSessionChangedFile[] | undefined> {
		...
		try {
			...
			return comparison.files.map(file => {
				...
				return new zyraxoncode.ChatSessionChangedFile(goToFileUri, originalUri, modifiedUri, file.additions, file.deletions);
			});
		} catch (error) {
	...
	): { goToFileUri: zyraxoncode.Uri; originalUri?: zyraxoncode.Uri; modifiedUri?: zyraxoncode.Uri } {
