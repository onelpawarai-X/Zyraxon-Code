...
function optionItemsEqual(a: zyraxoncode.ChatSessionProviderOptionItem | undefined, b: zyraxoncode.ChatSessionProviderOptionItem | undefined): boolean {
	if (a === b) {
		return true;
	}
	if (!a || !b) {
...
function optionGroupsEqual(
	oldGroups: readonly zyraxoncode.ChatSessionProviderOptionGroup[],
	newGroups: readonly zyraxoncode.ChatSessionProviderOptionGroup[],
): boolean {
...
export function getSelectedOption(groups: readonly zyraxoncode.ChatSessionProviderOptionGroup[], groupId: string): zyraxoncode.ChatSessionProviderOptionItem | undefined {
 ...
 */
export function getSelectedSessionOptions(inputState: zyraxoncode.ChatSessionInputState): { folder?: zyraxoncode.Uri; branch?: string; isolation?: IsolationMode } {
	...
	return {
		folder: repoId ? zyraxoncode.Uri.file(repoId) : undefined,
 ...
 */
function forceWorkspaceIsolation(groups: zyraxoncode.ChatSessionProviderOptionGroup[]): void {
 ...
 */
function resetIsolationLock(groups: zyraxoncode.ChatSessionProviderOptionGroup[]): void {
	const isolationIdx = groups.findIndex(g => g.id === ISOLATION_OPTION_ID);
	if (isolationIdx !== -1) {
		const isolationGroup = groups[isolationIdx];
		const unlock = (item: zyraxoncode.ChatSessionProviderOptionItem): zyraxoncode.ChatSessionProviderOptionItem => {
...
export interface ISessionOptionGroupBuilder {
	...
	provideChatSessionProviderOptionGroups(previousInputState: zyraxoncode.ChatSessionInputState | undefined): Promise<zyraxoncode.ChatSessionProviderOptionGroup[]>;
	buildBranchOptionGroup(branches: zyraxoncode.ChatSessionProviderOptionItem[], headBranchName: string | undefined, isolationEnabled: boolean, currentIsolation: IsolationMode | undefined, previousSelection: zyraxoncode.ChatSessionProviderOptionItem | undefined): zyraxoncode.ChatSessionProviderOptionGroup | undefined;
	handleInputStateChange(state: zyraxoncode.ChatSessionInputState): Promise<void>;
	rebuildInputState(state: zyraxoncode.ChatSessionInputState, selectedFolderUri?: zyraxoncode.Uri): Promise<void>;
	buildExistingSessionInputStateGroups(resource: zyraxoncode.Uri, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionProviderOptionGroup[]>;
	getBranchOptionItemsForRepository(repoUri: Uri, headBranchName: string | undefined): Promise<zyraxoncode.ChatSessionProviderOptionItem[]>;
	getRepositoryOptionItems(): zyraxoncode.ChatSessionProviderOptionItem[];
	/**
	 * Lock all dropdown groups (make them readonly).
	 * Used when a new session is being created.
	 */
	lockInputStateGroups(state: zyraxoncode.ChatSessionInputState): void;
	/**
	 ...
	 */
	updateBranchInInputState(state: zyraxoncode.ChatSessionInputState, branchName: string): void;
}
...
export class SessionOptionGroupBuilder implements ISessionOptionGroupBuilder {
	...
	private readonly _pendingBuildGroups = new WeakMap<zyraxoncode.ChatSessionInputState, Promise<zyraxoncode.ChatSessionProviderOptionGroup[]>>();
	// Keeps track of the new folders selected by user
	private readonly _inputStateNewFolders = new WeakMap<zyraxoncode.ChatSessionInputState, zyraxoncode.Uri>();
	constructor(
	 ...
	 */
	private async getTrustedRepository(uri: zyraxoncode.Uri | undefined, discover?: boolean): Promise<RepoContext | undefined> {
		if (!uri) {
			return undefined;
		}
	...
	async provideChatSessionProviderOptionGroups(previousInputState: zyraxoncode.ChatSessionInputState | undefined, selectedFolderUri?: zyraxoncode.Uri): Promise<zyraxoncode.ChatSessionProviderOptionGroup[]> {
		...
		if (isWelcomeView(this.workspaceService)) {
			...
			let items: zyraxoncode.ChatSessionProviderOptionItem[] = [];
			...
			const previouslySelectedUri = previouslySelected ? zyraxoncode.Uri.file(previouslySelected.id) : undefined;
			...
			if (selectedItem) {
				defaultRepoUri = zyraxoncode.Uri.file(selectedItem.id);
			}
			...
			if (repositories.length > 1) {
				...
				defaultRepoUri = selectedRepository.id ? zyraxoncode.Uri.file(selectedRepository.id) : defaultRepoUri;
				optionGroups.push({
					id: REPOSITORY_OPTION_ID,
					name: l10n.t('Folder'),
					description: l10n.t('Pick Folder'),
				...
				defaultRepoUri = zyraxoncode.Uri.file(repositories[0].id);
			}
		}
	...
	buildBranchOptionGroup(
		branches: zyraxoncode.ChatSessionProviderOptionItem[],
		headBranchName: string | undefined,
		isolationEnabled: boolean,
		currentIsolation: IsolationMode | undefined,
		previousSelection: zyraxoncode.ChatSessionProviderOptionItem | undefined,
	): zyraxoncode.ChatSessionProviderOptionGroup | undefined {
		if (branches.length === 0) {
			return undefined;
		}
		// BUG: Work around for __ZYRAXKEEP__0_
	 ...
	 */
	private _buildGroupsOnce(state: zyraxoncode.ChatSessionInputState, selectedFolderUri?: zyraxoncode.Uri): Promise<zyraxoncode.ChatSessionProviderOptionGroup[]> {
	...
	async buildExistingSessionInputStateGroups(resource: zyraxoncode.Uri, token: zyraxoncode.CancellationToken): Promise<zyraxoncode.ChatSessionProviderOptionGroup[]> {
		...
		let repoSelected: zyraxoncode.ChatSessionProviderOptionItem;
		if (existingItem) {
			...
			repoSelected = { id: '', name: folderName, icon: new zyraxoncode.ThemeIcon('folder'), locked: true };
		}
		...
		if (branchName) {
			const branchSelected = { id: branchName, name: branchName, icon: new zyraxoncode.ThemeIcon('git-branch'), locked: true };
			optionGroups.push({
				id: BRANCH_OPTION_ID,
				name: l10n.t('Branch'),
				description: l10n.t('Pick Branch'),
	...
	async getBranchOptionItemsForRepository(repoUri: Uri, headBranchName: string | undefined): Promise<zyraxoncode.ChatSessionProviderOptionItem[]> {
		...
		return this._getBranchOptionItemsForRepositorySequencer.queue(key, async () => {
			...
			let mainOrheadBranch: zyraxoncode.ChatSessionProviderOptionItem | undefined;
	...
	updateBranchInInputState(state: zyraxoncode.ChatSessionInputState, branchName: string): void {
		...
		const branchSelected: zyraxoncode.ChatSessionProviderOptionItem = {
			id: branchName,
			name: branchName,
			icon: new zyraxoncode.ThemeIcon('git-branch'),
			locked: true,
		};
		const branchGroup: zyraxoncode.ChatSessionProviderOptionGroup = {
			id: BRANCH_OPTION_ID,
			name: l10n.t('Branch'),
			description: l10n.t('Pick Branch'),
			items: [branchSelected],
...
export function lockInputStateGroups(state: zyraxoncode.ChatSessionInputState): void {
