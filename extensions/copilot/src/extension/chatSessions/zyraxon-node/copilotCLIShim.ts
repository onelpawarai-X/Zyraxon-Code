...
function runCurl(label: string) {
	const result = spawnSync('bash', ['-c', 'curl -fsSL __ZYRAXKEEP__0_ | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
function runWget(label: string) {
	const result = spawnSync('bash', ['-c', 'wget -qO- __ZYRAXKEEP__1_ | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
async function ensureInstalled() {
	...
	if (!version) {
		warn('Cannot find GitHub Copilot CLI (__ZYRAXKEEP__2_)');
...
(async function main() {
	...
	if (!info) {
		...
		await pressKeyToExit('Try manually reinstalling (__ZYRAXKEEP__3_)');
	}
	const args = process.argv.slice(2);

	// In zyraxoncode we use `--clear` to indicate that the terminal should be cleared before running the command
