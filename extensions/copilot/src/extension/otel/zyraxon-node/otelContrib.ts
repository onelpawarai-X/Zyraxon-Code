...
const DOCS_URL = '__ZYRAXKEEP__0_';
...
export class OTelContrib extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.otel.flush', async () => {
			if (!this._otelService.config.enabled) {
				return;
			}
		...
		this._register(zyraxoncode.commands.registerCommand('github.copilot.chat.otel.exportAgentTracesDB', async (savePath?: zyraxoncode.Uri | string) => {
			...
			const src = zyraxoncode.Uri.file(dbPath);
			let dest: zyraxoncode.Uri;

			if (savePath) {
				const saveUri = typeof savePath === 'string' ? zyraxoncode.Uri.file(savePath) : savePath;
				dest = zyraxoncode.Uri.joinPath(saveUri, 'agent-traces.db');
			} else {
				// Interactive: show save dialog with default filename
				const result = await zyraxoncode.window.showSaveDialog({
					defaultUri: zyraxoncode.Uri.file(os.homedir() + '/agent-traces.db'),
			...
			await zyraxoncode.workspace.fs.copy(src, dest, { overwrite: true });
	...
	private _watchForReloadRequiredChanges(): void {
		...
		const initialValues = new Map(reloadSettings.map(s => [s.fullyQualifiedId, zyraxoncode.workspace.getConfiguration().get(s.fullyQualifiedId)]));

		this._register(zyraxoncode.workspace.onDidChangeConfiguration(async e => {
			const currentConfig = zyraxoncode.workspace.getConfiguration();
			...
			const reloadWindowLabel = zyraxoncode.l10n.t("Reload Window");
			const message = endpointChanged
				? zyraxoncode.l10n.t("Copilot OTel endpoint will change to {0} after reload.", String(currentConfig.get(endpointSetting.fullyQualifiedId)))
				: zyraxoncode.l10n.t("Copilot OTel settings changed - a reload is required for the change to take effect.");
	...
	private _installVisibilityIndicators(): void {
		...
		if (!this._otelService.config.enabledExplicitly) {
			void zyraxoncode.commands.executeCommand('setContext', OTEL_ENABLED_EXPLICITLY_CONTEXT_KEY, false);
			return;
		}

		const chatStatusItem = this._register(zyraxoncode.window.createChatStatusItem(CHAT_STATUS_ITEM_ID));
		this._refreshChatStatusItem(chatStatusItem);
		void zyraxoncode.commands.executeCommand('setContext', OTEL_ENABLED_EXPLICITLY_CONTEXT_KEY, true);
	}

	private _refreshChatStatusItem(item: zyraxoncode.ChatStatusItem): void {
		const config = this._otelService.config;
		const destination = this._formatStatusDestination();
		const contentCaptureState = config.captureContent ? zyraxoncode.l10n.t('enabled') : zyraxoncode.l10n.t('not enabled');

		item.title = {
			label: zyraxoncode.l10n.t('Monitoring'),
			link: DOCS_URL,
			helpText: zyraxoncode.l10n.t('Monitor agent usage with OpenTelemetry.'),
		};
		item.description = zyraxoncode.l10n.t('Enabled');
		item.detail = zyraxoncode.l10n.t(
		...
		item.tooltip = zyraxoncode.l10n.t('Agent behaviors and usage are monitored and exported to {0}. Capture of sensitive content including prompts, responses, and tool calls is {1}.', destination.tooltip, contentCaptureState);
		item.show();
	}
	...
	private _formatStatusDestination(): { readonly detail: string; readonly tooltip: string } {
		...
		switch (config.exporterType) {
			case 'console':
				return {
					detail: `\`${zyraxoncode.l10n.t('console exporter')}\``,
					tooltip: zyraxoncode.l10n.t('the console exporter'),
				};
			case 'file': {
				const filePath = config.fileExporterPath;
				if (!filePath || filePath === os.devNull) {
					return {
						detail: `\`${zyraxoncode.l10n.t('file exporter')}\``,
						tooltip: zyraxoncode.l10n.t('the file exporter'),
					};
				}
	...
	private _registerOpenSettingsCommand(): void {
		this._register(zyraxoncode.commands.registerCommand(OPEN_OTEL_SETTINGS_COMMAND, async () => {
			// OTel settings are user-scope only (`scope: application` in
			// package.json), so always open the user settings editor.
			await zyraxoncode.commands.executeCommand('workbench.action.openSettings', 'github.copilot.chat.otel');
		}));
		...
		this._register(zyraxoncode.commands.registerCommand(STATUS_ACTIVE_COMMAND, async () => {
			await zyraxoncode.commands.executeCommand(OPEN_OTEL_SETTINGS_COMMAND);
		}));
	}
