...
export class NewWorkspaceInitializer extends Disposable {
	...
	private async _updateWorkspace(): Promise<void> {
		...
		if (!context.initialized) {
			...
			const result = await zyraxoncode.window.showInformationMessage(message, { modal: true, detail }, confirm);
			if (result === confirm) {
				zyraxoncode.commands.executeCommand('workbench.action.chat.open', { mode: 'agent', query: `${l10n.t('Continue with #new workspace setup')}` });
			} else {
		...
		if (!isEffectivelyEmpty) {
			...
			const result = await zyraxoncode.window.showInformationMessage(message, { modal: true, detail }, confirm);
			if (result === confirm) {
				zyraxoncode.commands.executeCommand('workbench.action.chat.open', { mode: 'agent', query: context.userPrompt });
			} else {
				newWorkspaceContextsList.splice(exactIndex, 1);
				this._extensionContext.globalState.update(NEW_WORKSPACE_STORAGE_KEY, newWorkspaceContextsList);
			}
