	...
	private async register(): Promise<IDisposable> {
		...
		try {
			...
			const resolver: Copilot.ContextResolver<Copilot.SupportedContextItem> = {
				async resolve(request: Copilot.ResolveRequest, token: zyraxoncode.CancellationToken): Promise<Copilot.SupportedContextItem[]> {
			...
			case PromptFileLangageId.agent: {
				...
				return [
					...
					},
					{
						...
						value: `zyraxoncode, github-copilot`,
					},
					{
					...
					},
					{
						...
						value: [
							...
							'```md',
							`---`,
							`description: This custom agent researches and plans new features for ZYRAXON Code extensions.`,
							`model: GPT-4.1`,
							`tools: [${toolNamesList}]`,
							`handoffs:`,
							`  - label: Start Implementation`,
