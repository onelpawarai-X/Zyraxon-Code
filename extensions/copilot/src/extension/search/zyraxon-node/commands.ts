...
export class SearchPanelCommands extends Disposable {
	constructor(
		...
		super();
		this._register(zyraxoncode.commands.registerCommand('github.copilot.search.markHelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Helpful);
		}));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.search.markUnhelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Unhelpful);
		}));
		this._register(zyraxoncode.commands.registerCommand('github.copilot.search.feedback', () => {
			this.sendFeedback(SearchFeedbackKind.Feedback);
			zyraxoncode.commands.executeCommand('github.copilot.report', `Copilot search feedback: "${SemanticSearchTextSearchProvider.latestQuery}"`);
		}));
	}

	private sendFeedback(kind: SearchFeedbackKind) {
		this.feedbackReporter.reportSearch(kind);
		zyraxoncode.commands.executeCommand('setContext', SemanticSearchTextSearchProvider.feedBackSentKey, true);
	}
}
