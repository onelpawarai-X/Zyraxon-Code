...
function getFirstChangedLine(lineChanges) {
    let firstLine;
...
class MarkdownPreview extends dispose_1.Disposable {
    ...
    */
    ...
    #updateWebviewContent(html, reloadPage, lineChanges, diffScrollSync) {
        if (this.#disposed) {
            return;
        }
    ...
    #getLocalResourceRoots() {
        return (0, resources_1.getMarkdownLocalResourceRoots)(this.#resource, this.#contributionProvider.contributions.previewResourceRoots);
    }
...
class StaticMarkdownPreview extends dispose_1.Disposable {
    ...
    static revive(resource, webview, contentProvider, previewConfigurations, topmostLineMonitor, logger, contributionProvider, opener, scrollLine, getLineChanges, getDiffScrollSync) {
    ...
    #webviewPanel;
    #previewConfigurations;
    constructor(webviewPanel, resource, contentProvider, previewConfigurations, topmostLineMonitor, logger, contributionProvider, opener, scrollLine, getLineChanges, getDiffScrollSync) {
        super();
        this.#webviewPanel = webviewPanel;
        this.#previewConfigurations = previewConfigurations;
        const topScrollLocation = typeof scrollLine === 'number' ? new scrolling_1.StartingScrollLine(scrollLine) : undefined;
