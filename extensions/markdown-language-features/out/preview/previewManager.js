...
const renderedDiffWarning_1 = require("./renderedDiffWarning");
...
class MarkdownPreviewManager extends dispose_1.Disposable {
    ...
    #opener;
    #renderedDiffWarning;
    constructor(contentProvider, logger, contributions, opener, workspaceState) {
        super();
        this.#contentProvider = contentProvider;
        this.#logger = logger;
        this.#contributions = contributions;
        this.#opener = opener;
        this.#renderedDiffWarning = this._register(new renderedDiffWarning_1.RenderedDiffWarningManager(workspaceState));
    ...
    openDynamicPreview(resource, settings) {
        const scrollLocation = resource.fragment ? new scrolling_1.StartingScrollFragment(resource.fragment) : this.#getActiveTextEditorScrollLocation(resource);
    ...
    async resolveCustomTextEditorInlineDiff(documents, webview) {
        const lineDiffProvider = new lineDiff_1.MarkdownPreviewLineDiffProvider(documents.original, documents.modified);
    ...
    async resolveCustomTextEditorSideBySideDiff(documents, webviewPanels) {
        const lineDiffProvider = new lineDiff_1.MarkdownPreviewLineDiffProvider(documents.original, documents.modified);
        const channelName = `md-diff-scroll-${(0, uuid_1.generateUuid)()}`;
        const originalPreview = this.#resolveCustomTextEditor(documents.original, webviewPanels.original, () => lineDiffProvider.getOriginalLineChanges(), async () => ({
            channelName,
            role: 'original',
            lineMappings: [...await lineDiffProvider.getModifiedToOriginalMappings()],
        }));
        const modifiedPreview = this.#resolveCustomTextEditor(documents.modified, webviewPanels.modified, () => lineDiffProvider.getModifiedLineChanges(), async () => ({
            channelName,
            role: 'modified',
    ...
    #resolveCustomTextEditor(document, webview, getLineChanges, getDiffScrollSync) {
        const lineNumber = this.#topmostLineMonitor.getPreviousTextEditorLineByUri(document.uri);
        const preview = preview_1.StaticMarkdownPreview.revive(document.uri, webview, this.#contentProvider, this.#previewConfigurations, this.#topmostLineMonitor, this.#logger, this.#contributions, this.#opener, lineNumber, getLineChanges, getDiffScrollSync);
        this.#registerStaticPreview(preview);
        this.#setActivePreview(preview);
        return preview;
    }
    #refreshPreviewWhenDocumentChanges(preview, document) {
    ...
    #createNewDynamicPreview(resource, previewSettings, scrollLocation) {
    ...
    #getActiveTextEditorScrollLocation(resource) {
        const editor = vscode.window.activeTextEditor;
        if (editor?.document.uri.toString() !== resource.toString()) {
            return undefined;
        }
        const line = (0, topmostLineMonitor_1.getVisibleLine)(editor);
        return typeof line === 'number' ? new scrolling_1.StartingScrollLine(line) : undefined;
    }
