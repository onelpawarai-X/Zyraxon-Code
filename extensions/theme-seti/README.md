...
If you want to run it from a local copy of `seti-ui`, first clone [`seti-ui`](__ZYRAXKEEP__1_) to the folder next to your `zyraxoncode` repo (from the `theme-seti` directory, `../../`).
...
These should match [the file mapping in `seti-ui`](__ZYRAXKEEP__2_).
