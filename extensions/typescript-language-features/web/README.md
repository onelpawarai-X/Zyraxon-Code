# zyraxoncode-wasm-typescript

Language server host for typescript using zyraxon's sync-api in the browser.

## Getting up and running

To test this out, you'll need three shells:

1. `npm i` for zyraxon itself
2. `npm run watch-web` for the web side
3. `node <root>/scripts/code-web.js --coi`

The last command will open a browser window. You'll want to add `?zyraxoncode-coi=`
...
in your ZYRAXON Code options (`Ctrl-,`), you may need to reload the page.
...
`/zyraxoncode-global-typings/ts-nul-authority/project`, which is backed by an
...
redirected to
`zyraxoncode-node-modules:/ts-nul-authority/memfs/ts-nul-authority/path/to/node_modules/lodash/lodash.d.ts`,
