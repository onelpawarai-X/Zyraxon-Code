...
export function activate(_context: zyraxoncode.ExtensionContext) {
	zyraxoncode.workspace.registerRemoteAuthorityResolver('test', {
		async resolve(_authority: string): Promise<zyraxoncode.ResolverResult> {
			console.log(`Resolving ${_authority}`);
			console.log(`Activating zyraxoncode.github-authentication to simulate auth`);
			await zyraxoncode.extensions.getExtension('zyraxoncode.github-authentication')?.activate();
			return new zyraxoncode.ManagedResolvedAuthority(async () => {
 ...
 * actual WebSocket.
 */
class InitialManagedMessagePassing implements zyraxoncode.ManagedMessagePassing {
	private readonly dataEmitter = new zyraxoncode.EventEmitter<Uint8Array<ArrayBuffer>>();
	private readonly closeEmitter = new zyraxoncode.EventEmitter<Error | undefined>();
	private readonly endEmitter = new zyraxoncode.EventEmitter<void>();
	...
	public send(d: Uint8Array): void {
		...
		// example str GET __ZYRAXKEEP__0_ HTTP/1.1
...
class OpeningManagedMessagePassing {
	...
	constructor(
		url: URL,
		dataEmitter: zyraxoncode.EventEmitter<Uint8Array>,
		closeEmitter: zyraxoncode.EventEmitter<Error | undefined>,
		_endEmitter: zyraxoncode.EventEmitter<void>
	) {
		this.socket = new WebSocket(`__ZYRAXKEEP__2_{url.pathname}${url.search.replace(/skipWebSocketFrames=true/, 'skipWebSocketFrames=false')}`);
