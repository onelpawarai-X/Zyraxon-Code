...
import type { ActivationFunction, OutputItem, RendererContext } from 'zyraxoncode-notebook-renderer';
...
function renderImage(outputInfo: OutputItem, element: HTMLElement): IDisposable {
	...
	image.setAttribute('data-zyraxoncode-context', JSON.stringify({
...
function getAltText(outputInfo: OutputItem) {
	...
	if (typeof metadata === 'object' && metadata && typeof metadata.zyraxoncode_altText === 'string') {
		return metadata.zyraxoncode_altText;
	}
	return undefined;
}
...
function fixUpSvgElement(outputInfo: OutputItem, element: HTMLElement) {
	if (outputInfo.mime.indexOf('svg') > -1) {
		...
		if (svgElement) {
			...
			svgElement.setAttribute('data-zyraxoncode-context', JSON.stringify({
				webviewSection: 'image',
				outputId: outputInfo.id,
				'preventDefaultContextMenuItems': true
			}));
...
export const activate: ActivationFunction<void> = (ctx) => {
	...
	#container div.output .scrollable.scrollbar-visible {
		border-color: var(--zyraxoncode-editorWidget-border);
	}
	...
	#container div.output .scrollable.more-above {
		box-shadow: var(--zyraxoncode-scrollbar-shadow) 0 6px 6px -6px inset
	}
	...
	#container .error-output-actions li {
		...
		display: inline-flex;
		cursor: pointer;
		border: solid 1px var(--zyraxoncode-notebook-cellToolbarSeparator);
	}
	#container .error-output-actions li.hover {
		background-color: var(--zyraxoncode-toolbar-hoverBackground);
	}
