...
function resolveSessionRepo(gitAPI, sessionMetadata, showErrors) {
    ...
    return { repository, remoteInfo, gitRemote: { name: gitRemote.name, fetchUrl: gitRemote.fetchUrl }, head: head };
}
async function createPullRequest(gitAPI, sessionResource, sessionMetadata) {
    if (!sessionResource) {
        return;
    }
    ...
    if (!head.upstream) {
        try {
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: vscode.l10n.t('Publishing branch to {0}...', gitRemote.name) }, async () => {
...
async function openPullRequest(gitAPI, _sessionResource, sessionMetadata) {
    const resolved = resolveSessionRepo(gitAPI, sessionMetadata, true);
    if (!resolved) {
        return;
    }
...
export function registerCommands(gitAPI) {
    ...
    disposables.add(vscode.commands.registerCommand('github.createPullRequest', async (sessionResource, sessionMetadata) => {
        return createPullRequest(gitAPI, sessionResource, sessionMetadata);
    }));
    disposables.add(vscode.commands.registerCommand('github.openPullRequest', async (sessionResource, sessionMetadata) => {
