...
export function isFileInRepo(repository, file) {
...
export function getRepositoryForFile(gitAPI, file) {
...
export function rangeString(range) {
    if (!range) {
        return '';
    }
...
export function notebookCellRangeString(index, range) {
    if (index === undefined) {
        return '';
    }
    if (!range) {
...
export function encodeURIComponentExceptSlashes(path) {
...
export async function getLink(gitAPI, useSelection, shouldEnsurePublished, hostPrefix, linkType = 'permalink', context, useRange) {
...
export function getAvatarLink(userId, size) {
    return `https://avatars.githubusercontent.com/u/${userId}?s=${size}`;
}
export function getBranchLink(url, branch, hostPrefix = 'https://github.com') {
...
export function getCommitLink(url, hash, hostPrefix = 'https://github.com') {
    const repo = getRepositoryFromUrl(url);
    if (!repo) {
        throw new Error('Invalid repository URL provided');
    }
...
export async function ensurePublished(repository, file) {
