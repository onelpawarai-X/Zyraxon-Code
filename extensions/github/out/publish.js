...
export async function publishRepository(gitAPI, repository) {
    ...
    const githubRepository = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, cancellable: false, title: 'Publish to GitHub' }, async (progress) => {
        ...
        let createdGithubRepository = undefined;
        if (isInCodespaces()) {
            createdGithubRepository = await vscode.commands.executeCommand('github.codespaces.publish', { name: repo, isPrivate });
        }
        else {
        ...
        if (createdGithubRepository) {
            ...
            const protocol = vscode.workspace.getConfiguration('github').get('gitProtocol');
