 ...
 *--------------------------------------------------------------------------------------------*/
import { TextDocuments, RequestType, DocumentRangeFormattingRequest, ConfigurationRequest, DidChangeWorkspaceFoldersNotification, DocumentColorRequest, ColorPresentationRequest, TextDocumentSyncKind, NotificationType, RequestType0, DocumentFormattingRequest, TextEdit, TextDocumentContentRequest } from 'vscode-languageserver';
import { getLanguageModes, TextDocument, Position, Range, isCompletionItemData, FILE_PROTOCOL } from './modes/languageModes.js';
...
export function startServer(connection, runtime) {
    ...
    connection.languages.onLinkedEditingRange((params, token) => {
        ...
        return /* todo remove when microsoft/vscode-languageserver-node#700 fixed */ runSafe(runtime, async () => {
            const document = documents.get(params.textDocument.uri);
            if (document) {
                const pos = params.position;
                if (pos.character > 0) {
