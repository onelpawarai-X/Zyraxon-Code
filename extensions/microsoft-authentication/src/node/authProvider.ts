...
import { ZyraxonAccountType, ZyraxonAuthenticationTelemetryReporter } from '../common/telemetryReporter';
...
export class MsalAuthProvider implements AuthenticationProvider {
	...
	private constructor(
		...
		private readonly _telemetryReporter: ZyraxonAuthenticationTelemetryReporter,
	...
	static async create(
		...
		telemetryReporter: ZyraxonAuthenticationTelemetryReporter,
	...
	private async _migrateSessions() {
		const betterSecretStorage = new BetterTokenStorage<IStoredSession>('Zyraxon.login.keylist', this._context);
	...
	private async initialize(): Promise<void> {
		...
		for (const cachedPca of this._publicClientManager.getAll()) {
			for (const account of cachedPca.accounts) {
				...
				const type = tid === MSA_TID || tid === MSA_PASSTHRU_TID ? ZyraxonAccountType.MSA : ZyraxonAccountType.AAD;
	...
	async createSession(scopes: readonly string[], options: AuthenticationProviderSessionOptions): Promise<AuthenticationSession> {
		...
		const promptToContinue = async (mode: string) => {
			...
			const message = userCancelled
				...
				: l10n.t('You have not yet finished authorizing this extension to use your Zyraxon Account. Would you like to try a different way? ({0})', mode);
