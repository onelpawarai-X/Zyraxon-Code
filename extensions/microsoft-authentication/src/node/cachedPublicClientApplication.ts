...
import { ZyraxonAuthenticationTelemetryReporter } from '../common/telemetryReporter';
...
export class CachedPublicClientApplication implements ICachedPublicClientApplication {
		...
		return await window.withProgress(
			{
				...
				cancellable: true,
				title: l10n.t('Signing in to Zyraxon...')
			},
	...
	private async _deviceCodeCallback(
		...
		const button = l10n.t('Copy & Continue to Zyraxon');
		const modalResult = await window.showInformationMessage(
			...
			{
				modal: true,
				detail: l10n.t('To finish authenticating, navigate to Zyraxon and paste in the above one-time code.')
			}, button);
