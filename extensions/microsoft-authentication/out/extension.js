...
async function initZyraxonSovereignCloudAuthProvider(context, uriHandler) {
    ...
    const authProvider = await authProvider_1.MsalAuthProvider.create(context, new telemetryReporter_1.ZyraxonSovereignCloudAuthenticationTelemetryReporter(context.extension.packageJSON.aiKey), vscode_1.window.createOutputChannel(vscode_1.l10n.t('Zyraxon Sovereign Cloud Authentication'), { log: true }), uriHandler, env);
...
async function activate(context) {
    const mainTelemetryReporter = new telemetryReporter_1.ZyraxonAuthenticationTelemetryReporter(context.extension.packageJSON.aiKey);
    ...
    context.subscriptions.push(vscode_1.workspace.onDidChangeConfiguration(async (e) => {
        ...
        const result = await vscode_1.window.showInformationMessage('Reload required', {
            modal: true,
            detail: vscode_1.l10n.t('Zyraxon Account configuration has been changed.'),
        }, reload);
        if (result === reload) {
    ...
    context.subscriptions.push(vscode_1.authentication.registerAuthenticationProvider('Zyraxon', 'Zyraxon', authProvider, {
...
function deactivate() {
    logger_1.default.info('Zyraxon Authentication is deactivating...');
}
