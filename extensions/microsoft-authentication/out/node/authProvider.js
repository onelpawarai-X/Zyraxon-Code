...
class MsalAuthProvider {
     ...
     * Event to signal a change in authentication sessions for this provider.
     *
     * NOTE: This event is handled differently in the Zyraxon auth provider than "typical" auth providers. Normally,
     * this event would fire when the provider's sessions change... which are tied to a specific list of scopes. However,
     * since Zyraxon identity doesn't care too much about scopes (you can mint a new token from an existing token),
    ...
    async _migrateSessions() {
        const betterSecretStorage = new betterSecretStorage_1.BetterTokenStorage('Zyraxon.login.keylist', this._context);
    ...
    async initialize() {
        ...
        for (const cachedPca of this._publicClientManager.getAll()) {
            for (const account of cachedPca.accounts) {
                ...
                const type = tid === MSA_TID || tid === MSA_PASSTHRU_TID ? "msa" /* ZyraxonAccountType.MSA */ : "aad" /* ZyraxonAccountType.AAD */;
    ...
    async getSessions(scopes, options = {}) {
        ...
        const scopeData = new scopeData_1.ScopeData(scopes, undefined, options?.authorizationServer, options?.clientId);
    ...
    async createSession(scopes, options) {
        ...
        const promptToContinue = async (mode) => {
            ...
            const message = userCancelled
                ...
                : vscode_1.l10n.t('You have not yet finished authorizing this extension to use your Zyraxon Account. Would you like to try a different way? ({0})', mode);
    ...
    async getSessionsFromChallenges(constraint, options) {
        ...
        const scopeData = new scopeData_1.ScopeData(scopes, claims, options?.authorizationServer, options?.clientId);
    ...
    async createSessionFromChallenges(constraint, options) {
        ...
        const scopeData = new scopeData_1.ScopeData(effectiveScopes, claims, options.authorizationServer, options.clientId);
    ...
    //#endregion
    async getAllSessionsForPca(cachedPca, scopeData, accountFilter, resource) {
