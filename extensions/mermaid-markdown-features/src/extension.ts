...
export function activate(context: zyraxoncode.ExtensionContext) {
	...
	context.subscriptions.push(
		zyraxoncode.commands.registerCommand('_mermaid-markdown.resetPanZoom', (ctx?: { mermaidWebviewId?: string }) => {
			webviewManager.resetPanZoom(ctx?.mermaidWebviewId);
		})
	);

	context.subscriptions.push(
		zyraxoncode.commands.registerCommand('_mermaid-markdown.copySource', (ctx?: MermaidCommandContext) => {
			if (typeof ctx?.mermaidSource === 'string') {
				void zyraxoncode.env.clipboard.writeText(ctx.mermaidSource);
				return;
			}

			const webviewInfo = ctx?.mermaidWebviewId ? webviewManager.getWebview(ctx.mermaidWebviewId) : webviewManager.activeWebview;
			if (webviewInfo) {
				void zyraxoncode.env.clipboard.writeText(webviewInfo.mermaidSource);
			}
		})
	);
	...
	context.subscriptions.push(zyraxoncode.workspace.onDidChangeConfiguration(e => {
		if (e.affectsConfiguration(`${configSection}.languages`)) {
			void zyraxoncode.commands.executeCommand('markdown.api.reloadPlugins');
		}
		if (e.affectsConfiguration(configSection) || e.affectsConfiguration('workbench.colorTheme')) {
			void zyraxoncode.commands.executeCommand('markdown.preview.refresh');
		}
	}));
	...
	return {
		extendMarkdownIt(md: MarkdownIt) {
			extendMarkdownItWithMermaid(md, {
				languageIds: () => zyraxoncode.workspace.getConfiguration(configSection).get<readonly string[]>('languages', ['mermaid'])
			});
