...
class PackageJSONContribution {
    ...
    getDocumentation(description, version, time, homepage, installedVersion) {
        ...
        if (installedVersion) {
            str.appendText('\n\n');
            str.appendText(vscode_1.l10n.t("Installed version: {0}", installedVersion));
        }
        if (homepage) {
            str.appendText('\n\n');
     ...
     */
    async runNpmCommand(npmCommandPath, args, resource) {
    ...
    async npmListInstalledVersion(npmCommandPath, pack, resource) {
