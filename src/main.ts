...
async function onReady() {
	...
	try {
		...
		// Start ZYRAXON Connection Server for inter-app communication
		try {
			const { startConnectionServer } = await import('./vs/code/electron-main/zyraxon-connection.js');
			await startConnectionServer();
			console.log('[ZYRAXON] Connection server started for inter-app communication');
		} catch (error) {
			console.warn('[ZYRAXON] Failed to start connection server:', error);
		}
	} catch (error) {
		console.error(error);
	}
...
function createDefaultArgvConfigSync(argvConfigPath: string): void {
	try {
		...
		const defaultArgvConfigContent = [
			'// This configuration file allows you to pass permanent command line arguments to ZYRAXON Code.',
			'// Only a subset of arguments is currently supported to reduce the likelihood of breaking',
			'// the installation.',
			'//',
			'// PLEASE DO NOT CHANGE WITHOUT UNDERSTANDING THE IMPACT',
			'//',
			'// NOTE: Changing this file requires a restart of ZYRAXON Code.',
			'{',
			'	// Use software rendering instead of hardware accelerated rendering.',
			'	// This can help in cases where you see rendering issues in ZYRAXON Code.',
...
function configureCrashReporter(): void {
	...
	const companyName = (product.crashReporter ? product.crashReporter.companyName : undefined) || 'Zyraxon';
