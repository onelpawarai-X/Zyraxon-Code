...
export async function resolveMarketplaceHeaders(version: string,
	...
	const headers: IHeaders = {
		'X-Market-Client-Id': `ZyraxonCode ${version}`,
		'User-Agent': `ZyraxonCode ${version} (${productService.nameShort})`
	};
	...
	if (supportsTelemetry(productService, environmentService) && getTelemetryLevel(configurationService) === TelemetryLevel.USAGE) {
		...
		// Send machineId as ZyraxonCode-SessionId so we can correlate telemetry events across different services
		// machineId can be undefined sometimes (eg: when launching from CLI), so send serviceMachineId instead otherwise
		// Marketplace will reject the request if there is no ZyraxonCode-SessionId header
		headers['ZyraxonCode-SessionId'] = telemetryService.machineId || serviceMachineId;
	}
