 ...
 * variables (`HTTPS_PROXY`/`HTTP_PROXY`/`ALL_PROXY`, with `NO_PROXY`). ZYRAXON Code's
 ...
 * proxy configured only in ZYRAXON Code is honored here and by the native model
...
async function doEnsure(overrideDir: string, platformKey: string, download: IFoundryLocalRuntimeDownload, token: CancellationToken, onProgress?: FoundryLocalRuntimeProgress): Promise<string> {
	...
	// machine running x64 ZYRAXON Code under emulation, then arm64 ZYRAXON Code). Verify
...
export async function provisionRuntime(overrideDir: string, platformKey: string, urlTemplate: string, version: string, token: CancellationToken): Promise<void> {
	...
	// The cache is shared by the utility processes of every open ZYRAXON Code window,
 ...
 * mirrors the cache layout). The tarball is published to ZYRAXON Code's CDN by
