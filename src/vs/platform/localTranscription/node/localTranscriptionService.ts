...
export class LocalTranscriptionService extends Disposable implements ILocalTranscriptionService {
	...
	async start(options: { cacheDir: string; model?: string; language?: string; proxyUrl?: string; noProxy?: string; proxyStrictSSL?: boolean; proxyAuthorization?: string; runtimeUrlTemplate?: string; runtimeVersion?: string }): Promise<void> {
		// Bridge ZYRAXON Code's proxy settings into this process's environment before any
		...
		// proxy, not ZYRAXON Code settings directly).
	...
	/**
	 * Apply ZYRAXON Code's proxy settings as environment variables for this process, so
	 * every download leg (our fetches and the native model download) honors a proxy
	 * configured only in ZYRAXON Code (not in the OS environment):
	...
	private async _ensureModel(cacheDir: string, modelId: string): Promise<IModel> {
		...
		this._modelPromise = (async () => {
			try {
				...
				// from ZYRAXON Code's CDN (per `product.dictationRuntime`) into a
				...
				if (!this._manager) {
					// Store downloaded model files under ZYRAXON Code's cache dir so
				...
				if (!model.isCached) {
					...
					// Bridge ZYRAXON Code cancellation to the AbortSignal the SDK expects.
