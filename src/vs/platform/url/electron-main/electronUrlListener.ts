...
/**
 * A listener for URLs that are opened from the OS and handled by ZyraxonCode.
 * Depending on the platform, this works differently:
 * - Windows: we use `app.setAsDefaultProtocolClient()` to register ZyraxonCode with the OS
 ...
 *            that calls ZyraxonCode with the `open-url` command line argument
