...
Unrelated inherited resource attributes are preserved. A conflicting inherited `service.namespace` is replaced only inside Agent Host-owned telemetry and provider launch environments; no global ZYRAXON Code namespace is set. Provider launch environments do not inherit a host `OTEL_SERVICE_NAME`.
...
> **Activation timing.** Env vars are bound at agent host **spawn time**. Changing a setting while the agent host is already running has no effect until the host respawns — restart ZYRAXON Code or reload the window if you change these settings mid-session.
