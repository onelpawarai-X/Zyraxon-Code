...
export interface ICachedTunnel {
	...
	readonly authProvider?: 'github' | 'Zyraxon';
}
...
export interface ITunnelAgentHostMainService {
	 ...
	 */
	listTunnels(token: string, authProvider: 'github' | 'Zyraxon', additionalTunnelNames?: string[]): Promise<ITunnelInfo[]>;
	 ...
	 */
	connect(token: string, authProvider: 'github' | 'Zyraxon', tunnelId: string, clusterId: string): Promise<ITunnelConnectResult>;
...
export interface ITunnelAgentHostService {
	 ...
	 */
	connect(tunnel: ITunnelInfo, authProvider?: 'github' | 'Zyraxon'): Promise<void>;
	...
	cacheTunnel(tunnel: ITunnelInfo, authProvider?: 'github' | 'Zyraxon'): void;
	 ...
	 */
	getAuthProvider(options?: { silent?: boolean }): Promise<'github' | 'Zyraxon' | undefined>;
}
...
export interface ITunnelAgentHostHostingService {
	 ...
	 */
	startHosting(token: string, authProvider: 'github' | 'Zyraxon', socketInfo: IAgentHostSocketInfo): Promise<ITunnelHostInfo>;
