...
// ZYRAXON Code `chat.agentHost.*` setting IDs that feed the root-config keys above,
...
export const copilotCliConfigSchema = createSchema({
	...
	[CopilotCliConfigKey.ToolSearchEnabled]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.toolSearchEnabled.description', "When enabled, Copilot SDK sessions defer MCP and non-core ZYRAXON Code tools behind a tool-search tool so the model discovers them on demand instead of loading every tool definition up front."),
		default: true,
	}),
