...
/** ZYRAXON Code setting forwarded into {@link AgentHostEditTelemetryEnabledConfigKey}. */
...
/** ZYRAXON Code setting forwarded into {@link AgentHostDisableRepoInfoTelemetryConfigKey}. */
export const DISABLE_REPO_INFO_TELEMETRY_SETTING_ID = 'chat.advanced.debug.disableRepoInfoTelemetry';

/**
 * Root config key forwarded from the renderer when ZYRAXON Code's
...
/**
 * The ZYRAXON Code setting ID for terminal auto approve enablement. Defined here so
...
/**
 * The ZYRAXON Code setting ID for global auto approve. Defined here so renderer-side
...
/**
 * Root config key forwarded from the renderer when ZYRAXON Code's `chat.autoReply`
...
/**
 * The ZYRAXON Code setting ID for auto-reply. Defined here so renderer-side
 * agent-host clients can forward it without importing from `workbench/contrib/chat`.
 */
 ...
 * `chat.agentHost.copilotAgent.multiRootEnabled` ZYRAXON Code setting.
 */
 ...
 * the `multipleWorkingDirectories` capability. Mirrors the hidden
 * `chat.agentHost.claudeAgent.multiRootEnabled` ZYRAXON Code setting.
 */
...
/**
 * The ZYRAXON Code setting IDs for terminal auto approve rules. Defined here so
...
/**
 * The ZYRAXON Code setting ID for session sync. Defined here so the platform
...
export const platformRootSchema = createSchema({
	...
	[AgentHostGlobalAutoApproveEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.globalAutoApproveEnabled.description', "Whether ZYRAXON Code's global auto-approve setting is enabled. When `true`, every tool call is auto-approved, equivalent to a session using Allow all."),
		default: false,
	}),
	[AgentHostAutoReplyEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.autoReplyEnabled.description', "Whether ZYRAXON Code's auto-reply setting is enabled. When `true`, `ask_user` questions are auto-answered instead of blocking on the user, mirroring autopilot mode."),
		default: false,
	}),
