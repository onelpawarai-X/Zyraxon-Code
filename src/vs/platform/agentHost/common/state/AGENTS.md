...
This directory contains the ZYRAXON Code-facing wrappers around the Agent Host
...
- `sessionState.ts`, `sessionActions.ts`, `sessionReducers.ts`, and
  `sessionProtocol.ts` are ZYRAXON Code-facing wrappers and re-exports.
- `protocol/**` is generated from the sibling `agent-host-protocol` repo by
  ...
  banner; update the source protocol repo and sync the copy into ZYRAXON Code.
...
- Session teardown uses `changeset/cleared` plus the corresponding
  ...
  action in the ZYRAXON Code protocol copy.

## Updating generated protocol types

1. Update the source files in the sibling `agent-host-protocol` repo.
2. Run `npx tsx scripts/sync-agent-host-protocol.ts` from the ZYRAXON Code repo.
3. If ZYRAXON Code consumers need short aliases or type guards, update the wrapper
...
source protocol repo, sync `protocol/**`, and add wrapper exports only when ZYRAXON Code code needs them.
...
4. Re-export the new action from `sessionActions.ts` when ZYRAXON Code callers need
   the type directly.
