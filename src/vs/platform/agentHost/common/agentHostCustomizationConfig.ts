...
export const enum AgentHostConfigKey {
	 ...
	 * by the local ZYRAXON Code client from the workbench `github-enterprise.uri`
...
export const agentHostCustomizationConfigSchema = createSchema({
	...
	[AgentHostConfigKey.DefaultShell]: schemaProperty<string>({
		...
		description: localize('agentHost.config.defaultShell.description', "Absolute path to the shell executable used by host-managed terminals. Normally pushed by the connected ZYRAXON Code client from `terminal.integrated.agentHostProfile.<os>` (falling back to `terminal.integrated.defaultProfile.<os>`); when unset, the agent host falls back to the system shell. Only the path is supported; `args` and `env` from the workbench profile are not piped through yet. The workbench only pushes this for the local agent host — remote agent host operators should set this directly in the remote machine's `agent-host-config.json`."),
	}),
	...
	[AgentHostConfigKey.CodexUsageSource]: schemaProperty<CodexUsageSource>({
		...
		description: localize('agentHost.config.codexUsageSource.description', "Choose whether Codex usage is routed through GitHub Copilot or uses an existing Codex OpenAI login. ZYRAXON Code does not provide the OpenAI sign-in flow; authenticate Codex separately before selecting OpenAI."),
		default: 'copilot',
