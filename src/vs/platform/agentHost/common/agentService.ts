...
// bundled with ZYRAXON Code; users either install codex themselves (typically via
...
// ZYRAXON Code instead of leaking via shell env.
//
...
export interface IAgentHostNetworkDiagnosticsInfo {
	...
	/** ZYRAXON Code `http.*` proxy settings observed by the agent host, keyed by setting id (only those that are set). */
...
export interface IActiveClient {
	...
	/** Human-readable client name (e.g. `"ZYRAXON Code"`), if provided. */
