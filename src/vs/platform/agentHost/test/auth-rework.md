...
The current authentication mechanism is imperative and ZYRAXON Code-specific:

1. The renderer discovers agents via `listAgents()` and checks `IAgentDescriptor.requiresAuth`.
2. It obtains a GitHub OAuth token from ZYRAXON Code's built-in authentication service.
...
### Client-side (ZYRAXON Code renderer)
...
2. **Phase B**: Update ZYRAXON Code renderer to use `authenticate` instead of `setAuthToken`. External clients can start using the new flow.
