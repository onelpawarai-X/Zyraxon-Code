...
1. **ZYRAXON Code DI constructor ordering:** `GetLeadingNonServiceArgs` strips
...
- Trust: `product.json` lives inside the signed application bundle
  ...
  to a Zyraxon-controlled CDN. The runtime no longer carries or
...
- **It's the host's own config plane.** The toggle is a property of *the
  ...
  ZYRAXON Code setting, no `VSCODE_AGENT_HOST_*` env var, no
