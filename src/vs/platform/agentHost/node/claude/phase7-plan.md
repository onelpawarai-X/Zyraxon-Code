...
- **Phase 10 (client tools / MCP)** replaces the `onElicitation: cancel` stub from §3.7 with a real translation to the protocol's input request / pending tool call. The `getClaudePermissionKind('mcp__*')` rule from §4 is the Phase-10 entry point for routing. **[Correction 2026-07-14: Phase 10 shipped rescoped to in-process client tools only and never carried this forward — the stub still stands at [claudeSdkOptions.ts:123](./claudeSdkOptions.ts). The real translation is now tracked as [Phase 10.6 in roadmap.md](./roadmap.md).]**
