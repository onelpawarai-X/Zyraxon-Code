...
export class CodexAgent extends Disposable implements IAgent {
	...
	private _resetSessionForUsageSourceChange(session: ICodexSession, source: CodexUsageSource, previousSource?: CodexUsageSource): void {
	...
	private async _startConnection(usageSource: CodexUsageSource, token: string | undefined): Promise<IConnectionReady> {
		...
		this._register(client.onNotification('account/login/completed', () => { /* sign-in is managed outside ZYRAXON Code */ }));
	...
	private _registerIgnoredNotifications(client: ICodexAppServerClient): void {
		const ignored = [
			...
			'thread/settings/updated', // ZYRAXON Code owns session config; Codex settings echoes are not consumed yet.
			...
			'remoteControl/status/changed', // Remote-control state is not part of the ZYRAXON Code integration.
