...
import { IAiProviderService, AiProviderAgentBridgeRegistry } from '../../aiProvider/index.js';
import { IAiProviderService as IAiProviderServiceInterface } from '../../aiProvider/aiProviderTypes.js';
...
export class AgentService extends Disposable implements IAgentService {
	...
	constructor(
		...
		@IAiProviderAgentBridgeRegistry private readonly _aiProviderAgentBridgeRegistry?: AiProviderAgentBridgeRegistry
	) {
		super();
		...
		// Create and register the AI Provider Agent Bridge Registry
		const aiProviderAgentBridgeRegistry = this._aiProviderAgentBridgeRegistry ?? instantiationService.createInstance(AiProviderAgentBridgeRegistry);
		services.set(IAiProviderAgentBridgeRegistry, aiProviderAgentBridgeRegistry);
	...
	private _createSessionServerToolAccessor(): ISessionServerToolAccessor {
		return {
			...
			getModels: () => {
				...
				// Also include models from the ZYRAXON AI provider system
				if (this._aiProviderAgentBridgeRegistry) {
					models.push(...this._aiProviderAgentBridgeRegistry.getAllModels());
				}
				return models;
			},
	...
	async disposeSession(session: URI): Promise<void> {
		...
		// Remove the ZYRAXON Code per-session data directory (metadata DB + checkpoints) to mirror the SDK-side cleanup
