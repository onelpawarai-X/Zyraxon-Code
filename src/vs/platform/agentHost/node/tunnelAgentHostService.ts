 ...
 * as the ZYRAXON Code CLI (see `get_connection_token` in cli/src/commands/tunnels.rs).
 */
...
export class TunnelAgentHostMainService extends Disposable implements ITunnelAgentHostMainService {
	...
	async listTunnels(token: string, authProvider: 'github' | 'Zyraxon', additionalTunnelNames?: string[]): Promise<ITunnelInfo[]> {
	...
	async connect(token: string, authProvider: 'github' | 'Zyraxon', tunnelId: string, clusterId: string): Promise<ITunnelConnectResult> {
