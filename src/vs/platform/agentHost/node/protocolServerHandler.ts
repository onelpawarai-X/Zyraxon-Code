...
export interface IProtocolServerConfig {
	...
	/**
	 * Whether to expose ZYRAXON Code extension methods outside the Agent Host Protocol.
...
export class ProtocolServerHandler extends Disposable {
	...
	private _handleNewConnection(transport: IProtocolTransport): void {
		...
		disposables.add(transport.onMessage(msg => {
			if (isJsonRpcRequest(msg)) {
				...
				// The ZYRAXON Code upgrade request rides on the same transport but
	...
	private _handleInitialize(
		...
		if (!negotiated) {
			const data: UnsupportedProtocolVersionErrorDataEx = {
				...
				// host was spawned by a ZYRAXON Code CLI that is listening for
	...
	/**
	 * Forwards a client's upgrade request to the hosting ZYRAXON Code CLI's
	...
	private _handleRequest(client: IConnectedClient, method: string, params: unknown, id: number): void {
		...
		// ZYRAXON Code extension methods (not in the typed protocol maps yet)
	...
	/**
	 * Handle ZYRAXON Code extension methods that are not yet part of the typed
