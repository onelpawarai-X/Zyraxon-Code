...
export type IAgentHostTurnFailedClassification = {
	...
	msg: { classification: 'CallstackOrException'; purpose: 'PerformanceAndHealth'; comment: 'The error message. ZYRAXON Code telemetry scrubs file paths and likely secrets before transmission.' };
	callstack: { classification: 'CallstackOrException'; purpose: 'PerformanceAndHealth'; comment: 'The error stack. ZYRAXON Code telemetry scrubs file paths and likely secrets before transmission.' };
	owner: 'roblourens';
...
export class AgentHostTelemetryReporter {
	 ...
	 * extension. ZYRAXON Code-only enrichment dims (code-block languages/counts) are not reconstructed here.
	 *
	 * @param session Session URI string; its id becomes `conversationId`.
	 ...
	 * `skillExtensionId` / `skillExtensionVersion` encode the contributing *ZYRAXON Code extension*, which
