...
- **The override** (`copilotAgentSession._createClientSdkTools`): the client's
  ...
  maps `tool_search_tool` back to `toolSearch` to execute the real ZYRAXON Code tool.
...
registration and ranking into ZYRAXON Code core so Agent Host no longer depends on
