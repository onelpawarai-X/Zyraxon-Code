...
export class CopilotAgent extends Disposable implements IAgent {
	...
	private async _ensureClient(): Promise<CopilotClient> {
		...
		const clientStarting = (async () => {
			...
			// Build a clean env for the CLI subprocess, stripping Electron/ZYRAXON Code vars
			...
			// ZYRAXON Code. Already inherited via `process.env` (the strip loop only
			...
			// Identify ZYRAXON Code's agent host traffic in CAPI
			...
			// Add ZYRAXON Code's built-in ripgrep to PATH so the CLI subprocess can find it.
