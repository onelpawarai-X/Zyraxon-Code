...
ZYRAXON Code's local agent host exposes the Agent Host Protocol (AHP) to other
...
`<userDataPath>` is the active ZYRAXON Code user data directory. Its value depends on
...
The file is optional. If ZYRAXON Code cannot prepare or publish the external
...
`connectionToken` in the standard ZYRAXON Code connection-token query parameter:
...
- On shutdown, ZYRAXON Code removes the metadata only if its PID and `instanceId`
