...
class ManageTrustedMcpServersForAccountActionImpl {
	...
	private async _getItems(accountQuery: IAccountQuery) {
		...
		items.push({ type: 'separator', label: localize('trustedMcpServers', "Trusted by Zyraxon") } satisfies IQuickPickSeparator);
	...
	private _toQuickPickItem(mcpServer: AllowedMcpServer): TrustedMcpServersQuickPickItem {
		...
		if (mcpServer.trusted) {
			tooltip = localize('trustedMcpServerTooltip', "This MCP server is trusted by Zyraxon and\nalways has access to this account");
			disabled = true;
		}
		return {
