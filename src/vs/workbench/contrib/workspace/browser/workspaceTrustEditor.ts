...
class WorkspaceTrustedUrisTable extends Disposable {
	...
	validateUri(path: string, item?: ITrustedUriItem): IMessage | null {
		...
		if (item.uri.scheme === 'zyraxoncode-vfs') {
...
export class WorkspaceTrustEditor extends EditorPane {
	...
	private async render() {
		...
		const headerDescriptionActionsText = localize({ key: 'workspaceTrustEditorHeaderActions', comment: ['Please ensure the markdown link syntax is not broken up with whitespace [text block](link block)'] }, "[Configure your settings]({0}) or [learn more](__ZYRAXKEEP__0_).", `command:workbench.trust.configure`);
