...
export function createFeedbackDialog(): FeedbackDialogComponent {
	...
	successIcon.style.cssText = 'font-size:20px;color:var(--zyraxoncode-charts-green);';
	const successText = dom.$('span');
	successText.style.cssText = `font-size:${FONT_SIZE.body};color:var(--zyraxoncode-foreground);font-weight:500;text-align:center;`;
	...
	headerIcon.style.color = 'var(--zyraxoncode-foreground)';
	const headerText = dom.$('span');
	headerText.style.cssText = `font-size:${FONT_SIZE.body};font-weight:600;color:var(--zyraxoncode-foreground);`;
	...
	textarea.style.cssText = `width:100%;box-sizing:border-box;background:var(--zyraxoncode-input-background);color:var(--zyraxoncode-input-foreground);border:1px solid var(--zyraxoncode-input-border, var(--zyraxoncode-editorWidget-border));border-radius:4px;padding:6px 8px;font-family:inherit;font-size:${FONT_SIZE.body};resize:vertical;min-height:48px;max-height:120px;outline:none;-webkit-app-region:no-drag;`;
	textarea.addEventListener('focus', () => { textarea.style.borderColor = 'var(--zyraxoncode-focusBorder)'; });
	textarea.addEventListener('blur', () => { textarea.style.borderColor = 'var(--zyraxoncode-input-border, var(--zyraxoncode-editorWidget-border))'; });

	const consent = dom.$('span');
	consent.style.cssText = `font-size:${FONT_SIZE.micro};color:var(--zyraxoncode-descriptionForeground);line-height:1.3;`;
	...
	errorEl.style.cssText = `font-size:${FONT_SIZE.micro};color:var(--zyraxoncode-errorForeground);`;

	const buttonRow = dom.$('div');
	buttonRow.style.cssText = 'display:flex;gap:6px;justify-content:flex-end;';

	const cancelBtn = document.createElement('button');
	cancelBtn.style.cssText = `-webkit-app-region:no-drag;background:transparent;border:1px solid var(--zyraxoncode-button-secondaryBackground, var(--zyraxoncode-editorWidget-border));color:var(--zyraxoncode-foreground);font-size:${FONT_SIZE.body};padding:3px 10px;border-radius:3px;cursor:pointer;`;
	cancelBtn.textContent = localize('agentsVoice.cancel', "Cancel");

	const submitBtn = document.createElement('button');
	submitBtn.style.cssText = `-webkit-app-region:no-drag;background:var(--zyraxoncode-button-background);border:none;color:var(--zyraxoncode-button-foreground);font-size:${FONT_SIZE.body};padding:3px 10px;border-radius:3px;cursor:pointer;font-weight:500;`;
	submitBtn.addEventListener('mouseenter', () => { if (!submitBtn.disabled) { submitBtn.style.background = 'var(--zyraxoncode-button-hoverBackground)'; } });
	submitBtn.addEventListener('mouseleave', () => { submitBtn.style.background = 'var(--zyraxoncode-button-background)'; });
