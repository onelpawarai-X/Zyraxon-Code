...
While ZYRAXON Code is running (even unfocused), pressing that combination runs the command. Rules:
...
- **`when` clauses are ignored** for the global trigger — an OS-global shortcut has no editor/UI
  context, so it is always active while ZYRAXON Code runs. The user is warned once per binding label.
...
sequenceDiagram
    ...
    participant W as window.ts zyraxon:runAction (renderer)
    ...
    G->>W: sendWhenReady('zyraxon:runAction', { id, from:'systemWideKeybinding', args })
...
**`src/vs/workbench/electron-browser/window.ts`** — handles the `zyraxon:runAction` IPC in the
...
- `onTrigger(accelerator)` — resolves the target window: the focused window if it owns the
  ...
  `zyraxon:runAction` via `target.sendWhenReady(...)`. It deliberately does **not** force-focus the
  routing window — a system-wide keybinding fires while ZYRAXON Code is typically unfocused, and pulling
  the routing window forward would flicker when the command opens/reveals a *different* window
  (e.g. `workbench.action.openAgentsWindow`). This matches every other `zyraxon:runAction` sender.
...
- Adding another `zyraxon:runAction` sender kind requires extending the `from` union in
