...
import { ZyraxonCodeOscProperty, ZyraxonCodeOscPt, ZyraxonCodeSequence } from '../../terminal/browser/terminalEscapeSequences.js';
...
import { serializeZyraxonCodeOscMessage } from '../../../../platform/terminal/common/xterm/shellIntegrationAddon.js';
...
export class TerminalTaskSystem extends Disposable implements ITaskSystem {
	...
	taskShellIntegrationStartSequence(cwd: string | URI | undefined): string {
		return (
			ZyraxonCodeSequence(ZyraxonCodeOscPt.Property, `${ZyraxonCodeOscProperty.HasRichCommandDetection}=True`) +
			ZyraxonCodeSequence(ZyraxonCodeOscPt.PromptStart) +
			ZyraxonCodeSequence(ZyraxonCodeOscPt.Property, `${ZyraxonCodeOscProperty.Task}=True`) +
			(cwd
				? ZyraxonCodeSequence(ZyraxonCodeOscPt.Property, `${ZyraxonCodeOscProperty.Cwd}=${typeof cwd === 'string' ? cwd : cwd.fsPath}`)
				: ''
			) +
			ZyraxonCodeSequence(ZyraxonCodeOscPt.CommandStart)
		);
	}
	getTaskShellIntegrationOutputSequence(commandLineInfo: { commandLine: string; nonce: string } | undefined): string {
		return (
			(commandLineInfo
				? ZyraxonCodeSequence(ZyraxonCodeOscPt.CommandLine, `${serializeZyraxonCodeOscMessage(commandLineInfo.commandLine)};${commandLineInfo.nonce}`)
				: ''
			) +
			ZyraxonCodeSequence(ZyraxonCodeOscPt.CommandExecuted)
		);
	}
...
function taskShellIntegrationWaitOnExitSequence(message: string): (exitCode: number) => string {
	return (exitCode) => {
		return `${ZyraxonCodeSequence(ZyraxonCodeOscPt.CommandFinished, exitCode.toString())}${message}`;
	};
}
