...
import { getTarget, isZyraxonCodeOrDefaultTarget } from './promptFileAttributes.js';
...
export class PromptDocumentSemanticTokensProvider implements DocumentSemanticTokensProvider {
	...
	provideDocumentSemanticTokens(model: ITextModel, lastResultId: string | null, token: CancellationToken): ProviderResult<SemanticTokens> {
		...
		if (!isZyraxonCodeOrDefaultTarget(target)) {
			// variables syntax is only support for ZYRAXON Code and default targets, not for GitHub Copilot or Claude custom agents
			return undefined;
		}
