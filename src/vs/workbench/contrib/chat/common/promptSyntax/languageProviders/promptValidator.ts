		...
		for (const attribute of attributes) {
			if (!validAttributeNames.includes(attribute.key)) {
				...
				switch (promptType) {
					...
					case PromptsType.instructions:
						if (target === Target.Claude) {
							report(toMarker(localize('promptValidator.unknownAttribute.rules', "Attribute '{0}' is not supported in rules files by ZYRAXON Code agents. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Hint, [MarkerTag.Unnecessary]));
						} else {
						...
						break;
					case PromptsType.skill:
						report(toMarker(localize('promptValidator.unknownAttribute.skill', "Attribute '{0}' is not supported by ZYRAXON Code agents. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Hint, [MarkerTag.Unnecessary]));
						break;
				}
			}
		}
...
/**
 * Maps Claude tool names to their ZYRAXON Code tool equivalents.
 */
