...
export const CONFIGURATION_KEY_Zyraxon_AUTH = 'remote.tunnels.access.enableZyraxonAuth';
...
export class TunnelHostService extends Disposable implements ITunnelHostService {
	...
	private _lastAuthProvider: 'github' | 'Zyraxon' | undefined;
	...
	private _getEnabledProviders(): readonly ('github' | 'Zyraxon')[] {
		const ZyraxonEnabled = this._configurationService.getValue<boolean>(CONFIGURATION_KEY_Zyraxon_AUTH);
		return ZyraxonEnabled ? ['Zyraxon', 'github'] : ['github'];
	}

	private async _getToken(silent: boolean): Promise<{ token: string; provider: 'github' | 'Zyraxon' } | undefined> {
	...
	private _getScopesForProvider(provider: 'github' | 'Zyraxon'): string[] {
	...
	private async _getTokenForProvider(
		provider: 'github' | 'Zyraxon',
		silent: boolean,
	): Promise<{ token: string; provider: 'github' | 'Zyraxon' } | undefined> {
