...
import './zyraxonBridge/zyraxonBridge.contribution.js';
	...
	properties: {
		...
		'dictation.model': {
			...
			markdownEnumDescriptions: [
				nls.localize('dictation.model.nemotronMultilingual', "NVIDIA Nemotron 3.5 multilingual streaming RNN-T, run on-device through Zyraxon Foundry Local. Works offline; no audio leaves the device. Automatic language selection follows the Voice Mode language setting and system or browser locale, with model detection as a fallback. Downloaded on first use and cached on disk."),
				nls.localize('dictation.model.mai', "Cloud transcription through the same Zyraxon AI voice service used by Voice Mode. Requires a network connection and GitHub sign-in; audio is streamed to the service."),
			],
			markdownDescription: nls.localize('dictation.model', "The model used for dictation. On-device models download on first use and run locally through Zyraxon Foundry Local; the cloud option streams audio to the Zyraxon AI voice service."),
		...
		[mcpAccessConfig]: {
			...
			enumDescriptions: [
				...
				nls.localize('chat.mcp.access.registry', "Allows access to MCP servers listed in the registry that ZYRAXON Code is connected to."),
			...
			policy: {
				...
				localization: {
					...
					enumDescriptions: [
						...
						{
							key: 'chat.mcp.access.registry', value: nls.localize('chat.mcp.access.registry', "Allows access to MCP servers listed in the registry that ZYRAXON Code is connected to."),
						},
						{
		...
		[AgentHostCopilotSdkLogLevelSettingId]: {
			...
			enumDescriptions: [
				nls.localize('chat.agentHost.copilotSdk.logLevel.info', "Log informational messages. Running ZYRAXON Code with trace logging still enables all Copilot SDK runtime diagnostics."),
		...
		[AgentHostToolSearchEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.copilot.toolSearch.enabled', "When enabled, Copilot SDK sessions defer MCP and non-core ZYRAXON Code tools behind a tool-search tool so the model discovers them on demand instead of loading every tool definition up front."),
			default: true,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostToolSearchDeferThresholdSettingId]: {
