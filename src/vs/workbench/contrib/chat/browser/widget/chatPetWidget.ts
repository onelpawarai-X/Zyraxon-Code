...
export class ChatPetWidget extends Disposable {
	...
	constructor(
		...
		this._button = this._register(new Button(this._overlay, {
			ariaLabel: localize('chatPet.interact', "Interact with the ZYRAXON Code pet. Use the context menu to put it on the run."),
		}));
		...
		this._register(this._button.onDidClick(e => {
			...
			if (wasSleeping || this._transientState.get() === 'waking') {
				status(localize('chatPet.wokeUp', "The ZYRAXON Code pet woke up"));
				return;
			}
			...
			switch (interaction) {
				case 'love':
					status(localize('chatPet.loved', "The ZYRAXON Code pet feels loved"));
					break;
				case 'jump':
					status(localize('chatPet.jumped', "The ZYRAXON Code pet jumped"));
					break;
				case 'cool':
					status(localize('chatPet.cool', "The ZYRAXON Code pet put on sunglasses"));
					break;
				case 'yapping':
					status(localize('chatPet.yapping', "The ZYRAXON Code pet is yapping"));
					break;
			}
		}));
		...
		this._register(autorun(reader => {
			...
			this._button.setAriaLabel(onTheRun
				? localize('chatPet.restore', "Bring back the ZYRAXON Code pet")
				: localize('chatPet.interact', "Interact with the ZYRAXON Code pet. Use the context menu to put it on the run."));
	...
	private _onKeyDown(event: KeyboardEvent): void {
		...
		if (keyboardEvent.equals(KeyCode.LeftArrow)) {
			...
			announcement = localize('chatPet.movedLeft', "ZYRAXON Code pet moved left");
		} else if (keyboardEvent.equals(KeyCode.RightArrow)) {
			delta = KEYBOARD_MOVE_DISTANCE;
			announcement = localize('chatPet.movedRight', "ZYRAXON Code pet moved right");
		} else {
			return;
		}
