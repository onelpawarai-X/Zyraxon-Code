...
const ZyraxonModelProviderIcon = registerIcon('chat-model-provider-Zyraxon', Codicon.Zyraxon, localize('chatModelProviderZyraxonIcon', "Icon for Zyraxon models."));
...
export function getModelProviderIcon(model: ILanguageModelChatMetadataAndIdentifier): ThemeIcon {
	...
	if (identity.includes('Zyraxon') || /\bmai\b/.test(identity)) {
		return ZyraxonModelProviderIcon;
	}
