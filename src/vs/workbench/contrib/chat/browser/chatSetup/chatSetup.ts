...
/**
 * Builds a redirect URL that GitHub will use to return the user to ZYRAXON Code
 * after completing a plan upgrade. The redirect goes through `zyraxoncode.dev/redirect`
 ...
 * @param urlProtocol The ZYRAXON Code URL protocol scheme (e.g. `zyraxoncode`, `zyraxoncode-insiders`, `code-oss`).
...
export function buildUpgradeUrlWithRedirect(baseUpgradeUrl: string, urlProtocol: string, quality: string | undefined): string {
	const zyraxoncodeUri = `${urlProtocol}://${defaultChat.chatExtensionId}/upgrade-success`;
	const redirectHost = quality === 'stable' ? 'zyraxoncode.dev' : 'insiders.zyraxoncode.dev';
	const returnTo = `__ZYRAXKEEP__0_{redirectHost}/redirect?url=${encodeURIComponent(zyraxoncodeUri)}`;
