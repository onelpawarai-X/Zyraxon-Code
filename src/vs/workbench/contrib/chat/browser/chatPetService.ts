...
type ChatPetEnablementClassification = {
	owner: 'justschen';
	comment: 'Tracks ZYRAXON Code pet enablement so adoption can be measured.';
	enabled: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Whether the ZYRAXON Code pet is enabled.' };
	source: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Whether the state was observed at startup or changed while ZYRAXON Code was running.' };
};
...
export class ChatPetService extends Disposable implements IChatPetService {
	...
	toggle(): boolean {
		...
		status(enabled
			? localize('chatPet.enabled', "ZYRAXON Code pet enabled. Click the pet to interact with it, or use the Left and Right Arrow keys to move it.")
			: localize('chatPet.disabled', "ZYRAXON Code pet disabled"));
		return enabled;
	}
	...
	setVariant(variant: ChatPetVariant): void {
		...
		status(variant === 'stable'
			? localize('chatPet.variant.stable', "ZYRAXON Code pet changed to the Stable colors")
			: localize('chatPet.variant.insiders', "ZYRAXON Code pet changed to the Insiders colors"));
	}
	...
	setOnTheRun(onTheRun: boolean): void {
		...
		status(onTheRun
			? localize('chatPet.onTheRun', "The ZYRAXON Code pet is on the run. Click the pet to bring it back.")
			: localize('chatPet.restored', "The ZYRAXON Code pet is back"));
	}
}
