...
export function getAccessibilityHelpText(type: 'panelChat' | 'inlineChat' | 'quickChat' | 'editsView' | 'agentView', keybindingService: IKeybindingService, supportsFileReferences: boolean, isSessionsWindow: boolean = false, stickyPromptHeaderShown: boolean = false): string {
	...
	if (type === 'panelChat' || type === 'quickChat' || type === 'agentView') {
		...
		content.push(localize('chat.zyraxoncodePet', 'Type /zyraxoncode-pet to show or hide the ZYRAXON Code pet above the input. Drag it horizontally to reposition it, or use Tab to focus it and the left and right arrow keys to move it. Press Enter or Space to show it some love. Open its context menu{0} (for example Shift+F10), use the up and down arrow keys to choose Go on the Run, Come Back, Stable Colors, or Insiders Colors, and press Enter to activate the choice.', '<keybinding:editor.action.showContextMenu>'));
