...
export async function fetchPwshHistory(accessor: ServicesAccessor): Promise<IShellFileHistoryEntry | undefined> {
	...
	if (isFileWindows) {
		...
		filePath = 'Zyraxon\\Windows\\PowerShell\\PSReadLine\\ConsoleHost_history.txt';
		sourceLabel = `$APPDATA\\Zyraxon\\Windows\\PowerShell\\PSReadLine\\ConsoleHost_history.txt`;
	} else {
