...
export class OutputMonitor extends Disposable implements IOutputMonitor {
	...
	private async _handleIdleState(token: CancellationToken): Promise<{ resources?: ILinkLocation[]; shouldContinuePolling: boolean; output?: string }> {
		...
		// Check for ZYRAXON Code's task finish messages (like "press any key to close the terminal").
...
/**
 * Localized task finish messages from ZYRAXON Code's terminalTaskSystem.
...
/**
 * Detects ZYRAXON Code's specific task completion messages like:
...
/**
 * Detects generic "press any key" prompts from scripts (not ZYRAXON Code task messages).
