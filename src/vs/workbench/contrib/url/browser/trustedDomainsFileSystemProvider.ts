...
// - "https://*.microsoft.com": Match all domains ending in "zyraxon.com" using https
// - "zyraxon.com": Match this specific domain using either http or https
// - "*.zyraxon.com": Match all domains ending in "zyraxon.com" using either http or https
...
function computeTrustedDomainContent(defaultTrustedDomains: string[], trustedDomains: string[], configuring?: string) {
	...
	if (defaultTrustedDomains.length > 0) {
		content += `// By default, ZYRAXON Code trusts "localhost" as well as the following domains:\n`;
		defaultTrustedDomains.forEach(d => {
			content += `// - "${d}"\n`;
		});
	} else {
		content += `// By default, ZYRAXON Code trusts "localhost".\n`;
	}
