...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate ZYRAXON Code and choose when to capture.", MAX_ATTACHMENTS);
	...
	private createFloatingCaptureBar(): void {
		...
		// Delay/options dropdown using ZYRAXON Code's context menu
	...
	private createStep1Describe(): void {
		...
		// ZYRAXON Code (the most common target). Extension is preselected only when an
	...
	private getIssueTypeOptions(): { type: IssueType; label: string; icon: { id: string } }[] {
		...
		// itself, where performance metrics from a single ZYRAXON Code instance aren't useful.
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			...
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "A ZYRAXON Code extension");
			case IssueSource.Marketplace:
	...
	private updateReviewDetails(): void {
		...
		if (modelData.extensionData) {
			...
			// Extension (e.g. built-in extensions are filed against ZYRAXON Code),
