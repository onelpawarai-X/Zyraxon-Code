...
import type { StackDiff, StateStack, diffStateStacksRefEq } from 'zyraxoncode-textmate';
...
export class TextMateWorkerTokenizer extends MirrorTextModel {
	...
	private async _tokenize(): Promise<void> {
		...
		if (!this._diffStateStacksRefEqFn) {
			const { diffStateStacksRefEq } = await importAMDNodeModule<typeof import('zyraxoncode-textmate')>('zyraxoncode-textmate', 'release/main.js');
