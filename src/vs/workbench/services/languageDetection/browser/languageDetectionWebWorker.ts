...
import type { ModelOperations, ModelResult } from '@vscode/zyraxoncode-languagedetection';
...
export class LanguageDetectionWorker implements ILanguageDetectionWorker {
	...
	private async getModelOperations(): Promise<ModelOperations> {
		...
		const { ModelOperations } = await importAMDNodeModule(uri, '') as typeof import('@vscode/zyraxoncode-languagedetection');
	...
	private adjustLanguageConfidence(modelResult: ModelResult): ModelResult {
		switch (modelResult.languageId) {
			...
			// these are commonly used languages in ZYRAXON Code and supported
			// by the model.
			case 'js':
			case 'html':
			case 'json':
			...
			// languages that are provided by default in ZYRAXON Code
			case 'bat':
			case 'ini':
			case 'makefile':
			case 'sql':
			// languages that aren't provided by default in ZYRAXON Code
			case 'csv':
			case 'toml':
