...
/**
 * Service that tracks whether the user is actively paying attention to ZYRAXON Code.
 *
 * This is determined by:
 * * ZYRAXON Code window has focus
...
export interface IUserAttentionService {
	 ...
	 * This includes keyboard typing and mouse movements/clicks while ZYRAXON Code is focused.
	 * The 500ms window prevents event spam from continuous mouse movement.
	 */
	readonly isUserActive: IObservable<boolean>;

	/**
	 * Observable that indicates whether the user is actively paying attention to ZYRAXON Code.
	 * This is true when:
	 * * ZYRAXON Code has focus, AND
	 * * There was user activity within the last minute
	 */
	readonly hasUserAttention: IObservable<boolean>;

	/**
	 * The total time in milliseconds that the user has been paying attention to ZYRAXON Code.
	 */
