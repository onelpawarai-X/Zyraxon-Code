...
export class ExtHostCodeMapper implements extHostProtocol.ExtHostCodeMapperShape {
	...
	private readonly providers = new Map<number, zyraxoncode.MappedEditsProvider2>();
	...
	async $mapCode(handle: number, internalRequest: extHostProtocol.ICodeMapperRequestDto, token: CancellationToken): Promise<ICodeMapperResult | null> {
		...
		const stream: zyraxoncode.MappedEditsResponseStream = {
			textEdit: (target: zyraxoncode.Uri, edits: zyraxoncode.TextEdit | zyraxoncode.TextEdit[]) => {
				edits = asArray(edits);
			...
			notebookEdit: (target: zyraxoncode.Uri, edits: zyraxoncode.NotebookEdit | zyraxoncode.NotebookEdit[]) => {
				edits = asArray(edits);
				this._proxy.$handleProgress(internalRequest.requestId, {
					uri: target,
		...
		const request: zyraxoncode.MappedEditsRequest = {
	...
	registerMappedEditsProvider(extension: IExtensionDescription, provider: zyraxoncode.MappedEditsProvider2): zyraxoncode.Disposable {
