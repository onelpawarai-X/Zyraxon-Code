...
function managedSettingsPipeline(rawLabel: string, raw: unknown | undefined, normalized: ManagedSettingsData, projected: ManagedSettingsData, rawUnavailableMessage?: string): string {
	...
	content += '**ZYRAXON Code policy projection**\n\n';
...
class PolicyDiagnosticsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		let content = '# ZYRAXON Code Policy Diagnostics\n\n';
		...
		try {
			...
			content += '### ZYRAXON Code Managed-Settings Schema\n\n';
			content += '*Only keys declared here can reach ZYRAXON Code policy callbacks. Runtime-owned keys may still be enforced by the Copilot runtime even when absent from the projections below.*\n\n';
			...
			content += '**Effective ZYRAXON Code policy bag**\n\n';
			...
			content += '*Resolved independently by each provider through its own SDK/runtime. This may include runtime-owned keys that ZYRAXON Code does not declare as configuration policies.*\n\n';
