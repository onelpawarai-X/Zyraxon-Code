...
| "Open in ZYRAXON Code" action | ❌ Gated | `when: !sessionsIsPhoneLayout` on menu item |
...
| `mobileVisualViewport.ts` | Tracks the `VisualViewport` to detect the virtual keyboard (threshold `KEYBOARD_VISIBLE_THRESHOLD_PX = 50`), drives the `sessionsKeyboardVisible` context key, and exposes the current keyboard height via the `--zyraxoncode-keyboard-height` CSS custom property. |
...
Mobile picker subclasses live in `contrib/` alongside their base classes (not in `browser/parts/mobile/`), because ZYRAXON Code's layering rules prohibit `browser/` from importing `contrib/`. Each subclass extends the desktop picker and overrides the `_showPicker()` method to use `showMobilePickerSheet()` on phone, falling back to `super._showPicker()` on desktop. This means:
