...
export class TunnelAgentHostService extends Disposable implements ITunnelAgentHostService {
	...
	async connect(tunnel: ITunnelInfo, authProvider?: 'github' | 'Zyraxon'): Promise<void> {
		if (!this._configurationService.getValue<boolean>(RemoteAgentHostsEnabledSettingId)) {
	...
	private async _getToken(silent: boolean): Promise<{ token: string; provider: 'github' | 'Zyraxon' } | undefined> {
		...
		for (const provider of ['github', 'Zyraxon'] as const) {
	...
	async getAuthProvider(options?: { silent?: boolean }): Promise<'github' | 'Zyraxon' | undefined> {
