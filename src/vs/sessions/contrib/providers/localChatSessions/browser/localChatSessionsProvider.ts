...
/** Local session type — in-process ZYRAXON Code chat, no background agent or worktree. */
...
/** Setting key controlling whether Local ZYRAXON Code chat sessions are available in the Agents app. */
...
export class LocalChatSessionsProvider extends Disposable implements ISessionsProvider {
	...
	getModelsSnapshot(_sessionId: string, desiredModelId?: string): ISessionModelsSnapshot {
		// Local (in-process ZYRAXON Code chat) sessions use general-purpose models
	...
	getModelPickerOptions(_sessionId: string): ISessionModelPickerOptions {
		// Local (in-process ZYRAXON Code chat) sessions offer the "Manage Models"
