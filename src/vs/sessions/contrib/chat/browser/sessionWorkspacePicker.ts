...
export class WorkspacePicker extends Disposable {
	 ...
	 * without source grouping. Own recents come first, followed by ZYRAXON Code
	 * recent folders.
	 */
	protected _buildItems(): IActionListItem<IWorkspacePickerItem>[] {
		...
		const tabFilter = this._isTabFiltered()
			...
			: undefined;
		// Own recents first, then ZYRAXON Code recents (merged and deduplicated by the service)
	...
	private _restoreSelectedWorkspace(): IResolvedFolderWorkspace | undefined {
		...
		// own recent history (not ZYRAXON Code's global recents) so restoration never
