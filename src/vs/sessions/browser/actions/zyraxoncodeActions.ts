...
import { resolveRemoteAuthority } from '../openInZyraxonCodeUtils.js';
import { OpenInZyraxonCodeTitleBarWidget } from '../widget/openInZyraxonCodeWidget.js';

export class OpenInZyraxonCodeAction extends Action2 {
	static readonly ID = 'sessions.openInZyraxonCode';

	constructor() {
		super({
			id: OpenInZyraxonCodeAction.ID,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (remoteAuthority) {
			await openerService.open(URI.from({
				scheme,
				authority: Schemas.zyraxoncodeRemote,
...
export class OpenInZyraxonCodeWidgetContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		this._register(actionViewItemService.register(Menus.TitleBarCenterRight, OpenInZyraxonCodeAction.ID, (action, options) => {
			return instantiationService.createInstance(OpenInZyraxonCodeTitleBarWidget, action, options, OpenInZyraxonCodeAction.ID);
		}, undefined));
	}
}
