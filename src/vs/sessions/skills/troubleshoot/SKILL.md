...
`events.jsonl` is the **primary** source and answers almost every question on its own. A *separate* log captures the **transport/protocol** between ZYRAXON Code and the agent host process — the JSON-RPC-style frames that drive sessions.
...
**It is written by ZYRAXON Code on the _client_ machine, so it is only reachable for a _local_ agent host** (ZYRAXON Code and the agent on the same machine). For a remote agent host it lives on the client, not the host this skill runs on — skip it there.

### Location

```
<ZYRAXON Code user-data dir>/logs/<session-timestamp>/ahp/ahp-<timestamp>-<connectionId>.jsonl
```

The ZYRAXON Code user-data dir depends on the build:
- Windows: `%APPDATA%\zyraxon-code` (Insiders: `ZYRAXON Code`; OSS/dev may use `ZYRAXON Code` or a custom `--user-data-dir`)
- macOS: `~/Library/Application Support/ZYRAXON Code`
- Linux: `~/.config/Code`

A new `logs/<timestamp>/` folder is created per ZYRAXON Code session — pick the **most recently modified** `ahp/ahp-*.jsonl`.
...
**If no `ahp/` folder exists, the wire log is disabled** (it is off by default). Tell the user to set `"chat.agentHost.ahpJsonlLoggingEnabled": true`, restart ZYRAXON Code, reproduce the issue, then re-run.
...
- `_ahpLog.dir` — direction: `c2s` = ZYRAXON Code → host (requests/notifications), `s2c` = host → ZYRAXON Code (results/errors/actions/notifications).
