...
export class StickyScrollWidget extends Disposable implements IOverlayWidget {
	...
	private _updateWidgetWidth(): void {
		...
		this._linesDomNodeScrollable.style.setProperty('--zyraxoncode-editorStickyScroll-scrollableWidth', `${this._editor.getScrollWidth() - layoutInfo.verticalScrollbarWidth}px`);
		this._rootDomNode.style.width = `${layoutInfo.width - layoutInfo.verticalScrollbarWidth}px`;
	}

	private _useFoldingOpacityTransition(requireTransitions: boolean) {
		this._lineNumbersDomNode.style.setProperty('--zyraxoncode-editorStickyScroll-foldingOpacityTransition', `opacity ${requireTransitions ? 0.5 : 0}s`);
	}
