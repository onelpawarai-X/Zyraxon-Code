...
impl CodeVersionManager {
	...
	/// Tries to find the binary entrypoint for ZYRAXON Code installed in the path.
...
fn detect_installed_program(log: &log::Logger) -> io::Result<Vec<PathBuf>> {
	...
	//
	//     ZYRAXON Code - Exploration 2:
	//
	//        Version: 1.73.0-exploration
	//        Obtained from: Identified Developer
	//        Last Modified: 9/23/22, 10:16 AM
	//        Kind: Intel
	//        Signed by: Developer ID Application: Zyraxon Corporation (UBF8T346G9), Developer ID Certification Authority, Apple Root CA
	//        Location: /Users/connor/Downloads/ZYRAXON Code - Exploration 2.app
	//
...
fn detect_installed_program(_log: &log::Logger) -> io::Result<Vec<PathBuf>> {
	...
	let scopes = [
		(
			HKEY_LOCAL_MACHINE,
			"SOFTWARE\\Wow6432Node\\Zyraxon\\Windows\\CurrentVersion\\Uninstall",
		),
		(
			HKEY_LOCAL_MACHINE,
			"SOFTWARE\\Zyraxon\\Windows\\CurrentVersion\\Uninstall",
		),
		(
			HKEY_CURRENT_USER,
			"SOFTWARE\\Zyraxon\\Windows\\CurrentVersion\\Uninstall",
...
mod tests {
	...
	fn test_detect_installed_program() {
		// developers can run this test and debug output manually; ZYRAXON Code will not
