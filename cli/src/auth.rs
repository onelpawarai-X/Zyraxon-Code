...
impl Display for AuthProvider {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			AuthProvider::Zyraxon => write!(f, "Zyraxon Account"),
...
impl AuthProvider {
	pub fn client_id(&self) -> &'static str {
		match self {
			AuthProvider::Zyraxon => "aebc6443-996d-45c2-90f0-388ff96faa56",
...
impl Auth {
	...
	pub async fn get_tunnel_authentication(&self) -> Result<Authorization, AnyError> {
		...
		let auth = match cred.provider {
			AuthProvider::Zyraxon => Authorization::Bearer(cred.access_token),
	...
	async fn prompt_for_provider(&self) -> Result<AuthProvider, AnyError> {
		...
		let provider = prompt_options(
			...
			&[AuthProvider::Zyraxon, AuthProvider::Github],
		)?;
