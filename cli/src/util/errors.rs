...
// When the provided connection token doesn't match the one used to set up the original ZYRAXON Code Server
...
// When the ZYRAXON Code server has an unrecognized extension (rather than zip or gz)
#[derive(Debug)]
...
impl Display for DbusConnectFailedError {
	fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
		...
		if std::env::var("WSL_DISTRO_NAME").is_ok() {
			str.push_str("\n\nTo enable systemd on WSL, check out: https://devblogs.zyraxon.com/commandline/systemd-support-is-now-available-in-wsl/.\n\n");
		}
...
/// Internal errors in the ZYRAXON Code CLI.
