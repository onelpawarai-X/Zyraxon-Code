...
pub struct CodeServerArgs {
	...
	// agent-host bridge: tells the spawned ZYRAXON Code server where the
...
/// Server params that can be used to start a ZYRAXON Code server.
...
/// A type that can be used to scan stdout from the ZYRAXON Code server. Returns
